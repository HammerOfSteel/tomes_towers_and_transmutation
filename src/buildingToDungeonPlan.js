 ...
 */
import { factionBuildingDna, FLOOR_HEIGHT, } from './world/buildings/BuildingDNA';
import { generatePlan } from './world/buildings/InteriorGenerator';
...
function resolveFloorType(style, kind) {
    const base = STYLE_FLOOR[style] ?? 'stone';
    switch (kind) {
...
function buildPerimeterWalls(bw, bd) {
    const tiles = [];
    for (let x = 0; x < bw; x++) {
 ...
 */
function tryConnect(rA, rB, bpA, bpB, grid, gridW) {
    // N-S: A above B
    if (rA.z + rA.d + 1 === rB.z) {
        ...
        for (let gx = xMin; gx <= xMax; gx++) {
            if (grid[gx + gridW * wallZ] !== TILE_FLOOR)
                continue;
            const lxA = gx - rA.x + 1;
            const lxB = gx - rB.x + 1;
    ...
    if (rA.x + rA.w + 1 === rB.x) {
        ...
        for (let gz = zMin; gz <= zMax; gz++) {
            if (grid[wallX + gridW * gz] !== TILE_FLOOR)
                continue;
            const lzA = gz - rA.z + 1;
            const lzB = gz - rB.z + 1;
 ...
 */
export function buildingToDungeonPlan(kind, faction, seed, size = 'medium', floors = 2) {
    const rooms = new Map();
    let startRoomId = '';
    ...
    for (let fi = 0; fi < floors; fi++) {
        ...
        for (let a = 0; a < plan.rooms.length; a++) {
            for (let b = a + 1; b < plan.rooms.length; b++) {
                tryConnect(plan.rooms[a], plan.rooms[b], rooms.get(ids[a]), rooms.get(ids[b]), plan.grid, plan.w);
            }
        }
    ...
    for (let fi = 0; fi < floors - 1; fi++) {
        ...
        if (!thisIds.length || !nextIds.length)
            continue;
