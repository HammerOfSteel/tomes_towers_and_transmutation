...
export const BackroomRegistry = {
    all() { return BACKROOM_DEFS; },
    get(id) { return BACKROOM_DEFS.find(b => b.id === id); },
    extractedOnly() { return BACKROOM_DEFS; }, // all available
};
...
export class BackroomPortal {
    ...
    _void;
    _t = 0;
    constructor(roomId, portalColor, 
    ...
    position) {
        ...
        this._ring = new THREE.Mesh(ringGeo, new THREE.MeshBasicMaterial({ color: portalColor, transparent: true, opacity: 0.9 }));
        this.group.add(this._ring);
        // Void interior
        const voidGeo = new THREE.CircleGeometry(0.9, 32);
        this._void = new THREE.Mesh(voidGeo, new THREE.MeshBasicMaterial({
            color: 0x000000,
            transparent: true,
            opacity: 0.85,
    ...
    update(dt) {
        ...
        this._void.material.opacity = 0.7 + Math.sin(this._t * 1.8) * 0.1;
    }
    dispose() {
...
export class BackroomManager {
    ...
    get isInBackroom() { return this._returnStack.length > 0; }
    get currentBackroomId() {
    ...
    spawnMainDoorPortals(scene, positions) {
    ...
    updatePortals(dt) {
        for (const p of this._activePortals)
            p.update(dt);
    }
    /** Enter a backroom from the current player position. */
    async enterBackroom(roomId, fromPos) {
        const def = BackroomRegistry.get(roomId);
        if (!def) {
            console.warn(`[Backrooms] Unknown room: ${roomId}`);
            return;
        }
    ...
    async _loadPersistedBackroom(roomId) {
        const key = `ttt_backroom_persist_${roomId}`;
