...
import { ALL_SPAWN_ITEMS } from './SpawnPalette';
// ── Constants ─────────────────────────────────────────────────────────────────
const GRID_SIZE = 2;
...
function makeGhost(root) {
    root.traverse(c => {
        ...
        c.material = mats.map(m => {
            const g = m.clone();
            g.transparent = true;
            g.opacity = GHOST_ALPHA;
            g.depthWrite = false;
            return g;
        });
    });
}
function highlightMesh(root, on) {
    root.traverse(c => {
        if (!(c instanceof THREE.Mesh))
            return;
        const mats = Array.isArray(c.material) ? c.material : [c.material];
        for (const m of mats) {
            m.emissive?.setHex(on ? 0x88ffcc : 0);
...
export class CreativePlacementSystem {
    ...
    _mouseScreen = new THREE.Vector2();
    ...
    // ── Lifecycle ───────────────────────────────────────────────────────────────
    activate() {
        const captureDown = (e) => this._onCaptureDown(e);
        const captureUp = (e) => this._onCaptureUp(e);
        const move = (e) => this._onMove(e);
        const contextmenu = (e) => { e.preventDefault(); e.stopImmediatePropagation(); };
        const auxclick = (e) => this._onAuxClick(e);
        const wheel = (e) => this._onWheel(e);
        const keydown = (e) => this._onKeyDown(e);
        const keyup = (e) => this._onKeyUp(e);
    ...
    _makeSpawnMarkerMesh(item, opacity = 1) {
        const col = item.color;
        const group = new THREE.Group();
        group.userData['spawnItem'] = item;
        // Vertical cylinder
        const cyl = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.3, 1.6, 8), new THREE.MeshBasicMaterial({ color: col, transparent: true, opacity, depthWrite: false }));
        cyl.position.y = 0.8;
        group.add(cyl);
        // Flat ring at base (zone indicator)
        const ring = new THREE.Mesh(new THREE.TorusGeometry(0.7, 0.05, 6, 16), new THREE.MeshBasicMaterial({ color: col, transparent: true, opacity: opacity * 1.3, depthWrite: false }));
    ...
    get placedCount() { return this._placed.length; }
    exportPlaced() {
    ...
    _onWheel(e) {
        ...
        if (path !== this._ghostPath)
            void this.holdAsset(path ?? null);
    ...
    _onKeyDown(e) {
        ...
        if (e.key === 'z' || e.key === 'Z') {
            if (this._dragging) {
                ...
                this._prevDragMouseY = window._lastMouseY ?? 0;
            }
            return;
        }
        if (e.key === 'g' || e.key === 'G') {
    ...
    _place() {
        ...
        if (this._ghostPath.startsWith('spawn:')) {
            ...
            this.scene.add(root);
            const obj = { id: `cp_${++_id}`, path: this._ghostPath, group: root, x: gx, y: this.ghostHeight, z: gz, ry: this._ghostRotY, scale: 1 };
        ...
        this._loader.loadAsync(this._ghostPath).then(gltf => {
            ...
            root.userData['creativeObject'] = true;
            this.scene.add(root);
            const obj = { id: `cp_${++_id}`, path: this._ghostPath, group: root, x: gx, y: this.ghostHeight, z: gz, ry: this._ghostRotY, scale: this._ghostScale };
            this._placed.push(obj);
            this._pushUndo({ type: 'place', obj });
        }).catch(() => { });
    }
    _destroy(obj) {
        ...
        if (this._hovered === obj)
            this._hovered = null;
        if (this._dragging === obj)
            this._dragging = null;
    }
    // ── Undo / Redo ───────────────────────────────────────────────────────────────
    _pushUndo(action) {
        this._undoStack.push(action);
        if (this._undoStack.length > CreativePlacementSystem.UNDO_MAX)
    ...
    // ── Save to game ──────────────────────────────────────────────────────────────
    async saveToGame(zoneId = 'creative_default') {
        const objects = this.exportPlaced();
        const doc = {
            schema: 1, type: 'overworld', id: zoneId,
    ...
    async restoreVersion(index, zoneId = 'creative_default') {
        const versions = this.listVersions(zoneId);
        const entry = versions[index];
        if (!entry) {
            this._toast('Version not found');
            return;
        }
     ...
     */
    async publish(zoneId = 'creative_default') {
        const hasBase = !!EditorVersioning.getBase('overworld', zoneId);
        if (hasBase) {
            const existing = this.listVersions(zoneId);
            const confirmed = window.confirm(`Publish "${zoneId}" as the canonical game version?\n\n` +
        ...
        if (ok)
            this._toast('✅ Published as canonical game version');
    }
    ...
    exportScenario(scenarioName = 'My Scenario') {
    ...
    downloadScenario(name = 'scenario') {
    ...
    async importScenario(json) {
        try {
            ...
            for (const raw of doc.spawns ?? []) {
                ...
                root.userData['creativeObject'] = true;
                this.scene.add(root);
                const obj = { id: `cp_${++_id}`, path: s.path, group: root, x: s.x, y: s.y ?? 0, z: s.z, ry: s.ry, scale: 1 };
                this._placed.push(obj);
                if (s.config)
                    this._spawnConfigs.set(obj.id, s.config);
            }
    ...
    _shatter(obj) {
        ...
        for (let i = 0; i < COUNT; i++) {
            ...
            mesh.position.copy(centre).add(new THREE.Vector3((Math.random() - 0.5) * size.x, (Math.random() - 0.5) * size.y, (Math.random() - 0.5) * size.z));
    ...
    _alignSelected(mode) {
        ...
        objs.forEach(o => {
            if (mode.endsWith('x')) {
                o.x = target;
                o.group.position.x = target;
            }
            else {
                o.z = target;
                o.group.position.z = target;
            }
        });
    ...
    _distributeSelected(axis) {
        ...
        if (objs.length < 3)
            return;
        const min = objs[0][axis];
        const max = objs[objs.length - 1][axis];
    ...
    _spawnConfigs = new Map();
    _openSpawnInspector(obj) {
        document.getElementById('spawn-inspector')?.remove();
        const item = obj.group.userData['spawnItem'];
        if (!item)
            return;
    ...
    `;
        const mkField = (label, key, type, opts) => {
            const val = cfg[key] ?? '';
    ...
    `;
        ...
        panel.querySelector('#si-edit-dialogue')?.addEventListener('click', () => {
            this._openDialogueEditor(cfg['dialogueId'] ?? '', cfg['npcName'] ?? 'NPC', (id) => {
                cfg['dialogueId'] = id;
                const el = panel.querySelector('[data-key="dialogueId"]');
                if (el)
                    el.value = id;
                this._spawnConfigs.set(obj.id, { ...cfg });
            });
    ...
    // ── NPC Dialogue Editor ───────────────────────────────────────────────────────
    _openDialogueEditor(existingId, npcName, onSave) {
    ...
    `;
        ...
        panel.querySelector('#de-save')?.addEventListener('click', async () => {
            const id = (panel.querySelector('#de-id')).value.trim() || autoId;
            const lines = (panel.querySelector('#de-lines')).value.split('\n').filter(Boolean);
    ...
    // ── Wave Designer ─────────────────────────────────────────────────────────────
    _openWaveDesigner(cfg, onSave) {
        document.getElementById('wave-designer')?.remove();
        const waves = cfg['waves'] ?? [{ enemyId: 'slime', count: 3, tier: '1' }];
        const panel = document.createElement('div');
    ...
    `;
      ...
      `;
            ...
            panel.querySelectorAll('[data-wi]').forEach(el => {
                el.addEventListener('input', () => {
                    const i = parseInt(el.dataset['wi']);
                    const f = el.dataset['wf'];
                    if (f === 'count')
                        waves[i].count = parseInt(el.value) || 1;
                    else
                        waves[i][f] = el.value;
                });
            });
            panel.querySelectorAll('[data-wdel]').forEach(el => {
        ...
        bar.querySelector('#ms-clone')?.addEventListener('click', () => {
            ...
            toClone.forEach(o => {
                this._loader.loadAsync(o.path).then(gltf => {
                    ...
                    root.userData['creativeObject'] = true;
                    this.scene.add(root);
                    const obj = { id: `cp_${++_id}`, path: o.path, group: root, x: o.x + 2, y: o.y, z: o.z, ry: o.ry, scale: o.scale };
                    this._placed.push(obj);
                    this._pushUndo({ type: 'place', obj });
                }).catch(() => { });
            });
    ...
    _updateHover() {
        ...
        if (hit !== this._hovered) {
            if (this._hovered)
                highlightMesh(this._hovered.group, false);
            this._hovered = hit;
            if (this._hovered && !this._dragging)
                highlightMesh(this._hovered.group, true);
        }
        // Tooltip
        if (hit) {
            if (!this._tooltipEl) {
    ...
    _updateMouseWorld(e) {
        ...
        this._mouseScreen.set(((e.clientX - rect.left) / rect.width) * 2 - 1, -((e.clientY - rect.top) / rect.height) * 2 + 1);
        this._raycaster.setFromCamera(this._mouseScreen, this.camera);
        const t = new THREE.Vector3();
        if (this._raycaster.ray.intersectPlane(this._floorPlane, t))
            this._mouseWorld.copy(t);
    }
    _pickAt(screen) {
        this._raycaster.setFromCamera(screen, this.camera);
        const meshes = [];
        for (const obj of this._placed)
            obj.group.traverse(c => { if (c instanceof THREE.Mesh)
                meshes.push(c); });
        const hits = this._raycaster.intersectObjects(meshes, true);
        if (!hits.length)
            return null;
        const h = hits[0].object;
        return this._placed.find(o => { let f = false; o.group.traverse(c => { if (c === h)
            f = true; }); return f; }) ?? null;
    }
    _clearGhost() {
        if (this._ghost) {
