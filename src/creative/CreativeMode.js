...
import { buildAssetShowcase, buildDungeonPrototype, buildSpellLab, buildCombatArena, buildEmptyRoom, buildBuildingLab } from './backroomScenes';
import { setCreativeActive, isCreativeActive, setGodMode, setFlyEnabled, setNoClip, getCreativeState, getSpeedMultiplier, setCurrentZone, setHotbarSlot, setCurrentSkin, } from './CreativeModeState';
...
export const CreativeMode = {
    ...
    enter(opts) {
        if (!_ctx) {
            console.warn('[CreativeMode] Not initialised — call CreativeMode.init() first.');
            return;
        }
        if (isCreativeActive())
            return;
        ...
        const orbit = _ctx.orbit ?? _ctx.orbitControls;
        if (orbit) {
        ...
        try {
            _ctx.sceneManager.loadRoomImmediate(chamberRoom);
        }
    ...
    get active() { return isCreativeActive(); },
    ...
    async enterBackroom(roomId) {
        if (!_ctx || !_backroomMgr)
            return;
...
function _hideRegularHUD() {
    ...
    if (_ctx.regularHUD.hide) {
        ...
        document.getElementById('hud-root')?.style.setProperty('display', 'none');
    }
}
function _showRegularHUD() {
    if (!_ctx)
        return;
    if (_ctx.regularHUD.show) {
        ...
        document.getElementById('hud-root')?.style.removeProperty('display');
    }
}
function _buildTeleportFloors() {
    if (!_ctx || !_hud)
        return;
    const floors = TOWER_FLOOR_DEFS
        ...
        onTeleport: () => {
            try {
                _ctx.sceneManager.loadRoomImmediate(`tower_${def.id}_chamber`);
            }
            catch { /* */ }
...
function _toggleQuestBuilder() {
  ...
  `;
    const questState = { name: '', desc: '', objectives: [], xp: 100 };
    const renderPanel = () => {
        panel.innerHTML = `
    ...
    `;
        ...
        panel.querySelector('#qb-name')?.addEventListener('input', e => { questState.name = e.target.value; });
        panel.querySelector('#qb-desc')?.addEventListener('input', e => { questState.desc = e.target.value; });
        panel.querySelector('#qb-xp')?.addEventListener('input', e => { questState.xp = parseInt(e.target.value) || 0; });
        panel.querySelector('#qb-add-obj')?.addEventListener('click', () => { questState.objectives.push({ type: 'kill', target: '', count: 1 }); renderPanel(); });
        panel.querySelectorAll('[data-oi]').forEach(el => {
            const i = parseInt(el.dataset['oi'] ?? '0');
            el.addEventListener('input', ev => {
                const val = ev.target.value;
                const field = el.dataset['field'];
                if (field === 'count')
                    questState.objectives[i].count = parseInt(val) || 1;
                else
                    questState.objectives[i][field] = val;
            });
        });
        ...
        panel.querySelector('#qb-save')?.addEventListener('click', async () => {
            if (!questState.name.trim()) {
                _hudToast('Quest needs a name');
                return;
            }
            ...
            try {
                ...
                _hudToast(`✓ Quest built: ${id} (no server — use Ctrl+S to save state)`);
                panel.remove();
            }
        });
    ...
    panel.querySelector('#qb-name')?.focus();
}
// ── Portal proximity ──────────────────────────────────────────────────────────
let _portalTransitioning = false;
function _checkPortalProximity() {
    if (!_ctx || !_backroomMgr || _portalTransitioning)
        return;
        ...
        _ctx.scene.traverse(obj => {
            if (obj.userData['returnPortal'] && !_portalTransitioning) {
                ...
                if (Math.sqrt(dx * dx + dz * dz) < 1.2) {
                    ...
                    const onEnter = obj.userData['onEnter'];
                    if (onEnter) {
                        // Fade out and exit
...
async function _enterBackroomWithFade(roomId, fromPos) {
    // Fade to black
    const fade = document.createElement('div');
    fade.style.cssText = 'position:fixed;inset:0;background:black;opacity:0;z-index:9999;pointer-events:none;transition:opacity 0.3s';
    document.body.appendChild(fade);
...
function _openSkinPicker() {
    ...
    `;
        ...
        const grid = overlay.querySelector('#skin-grid');
        const search = overlay.querySelector('#skin-search');
        const state = getCreativeState();
        let roleFilter = 'all';
        ...
        const renderSkins = (filter = '') => {
            ...
            if (roleFilter === 'princess') {
              ...
              `;
                            ...
                            card.addEventListener('click', async () => {
                                if (!dna || !_ctx || !_ctx.player.applyPrincess) {
                                    _hudToast('Princess data unavailable');
                                    return;
                                }
                                try {
                                    ...
                                    _hudToast('Failed to load princess');
                                }
                            });
            ...
            const filtered = CHAR_MODELS.filter(m => {
                const roleOk = roleFilter === 'all' ? true : m.roles.includes(roleFilter);
        ...
        `;
                card.title = `${m.name}\n${m.id}\nRoles: ${m.roles.join(', ')}`;
                card.addEventListener('click', async () => {
                    if (!_ctx?.player.applyAssetModel) {
                        _hudToast('Skin swap not available');
                        return;
                    }
                    try {
...
async function _pickAssetForSlot(path, _slot) {
    if (_placement)
        await _placement.holdAsset(path);
}
// ── Per-session keyboard handler (C = browser, P = char sheet, Shift tracking) ──
function _attachCreativeKeys() {
    const onDown = (e) => {
        if (!isCreativeActive())
            return;
        ...
        if ((e.ctrlKey || e.metaKey) && (e.key === 'i' || e.key === 'I') && !e.shiftKey) {
            ...
            input.type = 'file';
            input.accept = '.ttt-scenario.json,.json';
