...
// ── Asset Showcase Hall ───────────────────────────────────────────────────────
export async function buildAssetShowcase(scene) {
    const group = new THREE.Group();
    group.name = 'backroom_asset_showcase';
    // Floor
    const floor = new THREE.Mesh(new THREE.PlaneGeometry(120, 120), new THREE.MeshBasicMaterial({ color: 0x0d0820, side: THREE.DoubleSide }));
    ...
    for (const row of kitGroups) {
        ...
        kits.forEach(async (kit, i) => {
            ...
            const wz = row.z;
            // Pedestal
            const ped = new THREE.Mesh(new THREE.CylinderGeometry(0.6, 0.8, 0.4, 8), new THREE.MeshBasicMaterial({ color: 0x1a0e2a }));
...
export function buildDungeonPrototype(scene) {
    const group = new THREE.Group();
    group.name = 'backroom_dungeon_prototype';
    // Floor
    const floor = new THREE.Mesh(new THREE.PlaneGeometry(20, 20), new THREE.MeshBasicMaterial({ color: 0x1a0e28, side: THREE.DoubleSide }));
    floor.rotation.x = -Math.PI / 2;
    group.add(floor);
    // Walls (4 sides)
...
export function buildSpellLab(scene) {
    const group = new THREE.Group();
    group.name = 'backroom_spell_lab';
    const floor = new THREE.Mesh(new THREE.PlaneGeometry(30, 30), new THREE.MeshBasicMaterial({ color: 0x080418, side: THREE.DoubleSide }));
    floor.rotation.x = -Math.PI / 2;
    group.add(floor);
    ...
    const wbMarker = new THREE.Mesh(new THREE.CylinderGeometry(0.5, 0.5, 0.1, 8), new THREE.MeshBasicMaterial({ color: 0x4488ff, transparent: true, opacity: 0.7 }));
    ...
    const keyHandler = (e) => {
        ...
        if ((e.key === 'r' || e.key === 'R') && !e.ctrlKey) {
            const wb = document.getElementById('spell-workbench');
            if (wb?._doCast) {
                ...
                const last = window.__game?.lastSpellCast;
                if (last) {
                    _openSpellWorkbench(last);
                    // Fire after DOM settles
                    requestAnimationFrame(() => {
                        const wb2 = document.getElementById('spell-workbench');
                        wb2?._doCast?.();
                    });
                }
                else {
...
function _openSpellWorkbench(prefill) {
  ...
  `;
    ...
    // ── Stat tables ──────────────────────────────────────────────────────────
    const ELEM_DMG = { fire: 15, ice: 12, lightning: 20, arcane: 18, shadow: 14, nature: 10 };
    const ELEM_MANA = { fire: 20, ice: 15, lightning: 25, arcane: 30, shadow: 22, nature: 12 };
    const SHAPE_MULT = { bolt: 1.0, burst: 0.7, nova: 0.6, beam: 1.5, chain: 0.8, mine: 1.2 };
    const SHAPE_CAST = { bolt: 0.3, burst: 0.5, nova: 0.6, beam: 1.0, chain: 0.4, mine: 0.8 };
    const SHAPE_AOE = { bolt: 0, burst: 4, nova: 6, beam: 0, chain: 3, mine: 5 };
    const MOD_MULT = { none: 1.0, double: 2.0, pierce: 1.1, bounce: 0.9, slow: 0.8, burn: 1.3 };
    const MOD_DUR = { none: 0, double: 0, pierce: 0, bounce: 0, slow: 4, burn: 3 };
    const computeStats = (e, s, m) => {
    ...
    `;
        const onChange = () => {
            elem = (panel.querySelector('#wb-elem'))?.value ?? elem;
            shape = (panel.querySelector('#wb-shape'))?.value ?? shape;
            mod = (panel.querySelector('#wb-mod'))?.value ?? mod;
            renderWB();
        };
        ...
        panel.querySelector('#wb-slo')?.addEventListener('click', () => {
            ...
            window.__game?.setGameSpeed?.(slowMoActive ? 0.1 : 1);
            renderWB();
        });
        const doCast = () => {
            ...
            if (castLog.length > 20)
                castLog.shift();
            window.__game = { ...window.__game, lastSpellCast: { element: elem, shape, modifier: mod, name } };
        ...
        panel._doCast = doCast; // expose for R-key handler
        panel.querySelector('#wb-export')?.addEventListener('click', () => {
            ...
            a.download = `spell_${elem}_${shape}.json`;
            a.click();
...
// ── Combat Arena ──────────────────────────────────────────────────────────────
export function buildCombatArena(scene) {
    const group = new THREE.Group();
    group.name = 'backroom_combat_arena';
    const floor = new THREE.Mesh(new THREE.CircleGeometry(18, 32), new THREE.MeshBasicMaterial({ color: 0x1a0808, side: THREE.DoubleSide }));
    floor.rotation.x = -Math.PI / 2;
    group.add(floor);
    // Arena ring
    const ring = new THREE.Mesh(new THREE.TorusGeometry(18, 0.3, 8, 64), new THREE.MeshBasicMaterial({ color: 0x882222 }));
 ...
 */
export async function buildBuildingLab(scene, physics) {
    const sceneGroups = [];
    const sceneLights = [];
    const physBodies = [];
    ...
    // ── Import helpers ────────────────────────────────────────────────────────
    const [{ generateInterior }, { STYLE_COLORS }, { getDefaultNpcDna }, { buildNpc },] = await Promise.all([
    ...
    for (let i = 0; i < TOTAL; i++) {
        ...
        if (physics) {
            ...
            for (const [ox, oz, ew, ed] of [
                ...
                physBodies.push(physics.createStaticBox(new THREE.Vector3(wx + ox, wallH, wz + oz), new THREE.Vector3(ew, wallH, ed)));
            }
        }
        ...
        // ── NPC inside floor 0 ────────────────────────────────────────────────
        try {
            const npcDna = getDefaultNpcDna(species, role, (i + 7) * 0x9E3779B9 >>> 0);
    ...
    const pathMesh = new THREE.Mesh(new THREE.PlaneGeometry(4, F1_OFFSET_Z), new THREE.MeshLambertMaterial({ color: 0x222240 }));
...
export function buildEmptyRoom(scene, name, color) {
    const group = new THREE.Group();
    group.name = `backroom_${name}`;
    const floor = new THREE.Mesh(new THREE.PlaneGeometry(20, 20), new THREE.MeshBasicMaterial({ color: 0x0a0a14, side: THREE.DoubleSide }));
    floor.rotation.x = -Math.PI / 2;
    group.add(floor);
...
// ── Helper ────────────────────────────────────────────────────────────────────
function _makeLabel(text, color, pos, scale) {
