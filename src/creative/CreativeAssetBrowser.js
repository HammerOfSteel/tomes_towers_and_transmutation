...
import { SPAWN_CATEGORIES, ALL_SPAWN_ITEMS } from './SpawnPalette';
// ── CSS ───────────────────────────────────────────────────────────────────────
const CSS = `
#cab-root {
  position: fixed;
...
let _cacheLoading = new Set();
async function loadKitAssets(kitId) {
    if (_assetCache.has(kitId))
        return _assetCache.get(kitId);
    try {
        ...
        const assets = raw.map(a => ({ ...a, category: a.category, kitId }));
...
export class CreativeAssetBrowser {
    ...
    _renderTabs() {
        ...
        if (!bar)
            return;
        bar.innerHTML = '';
        const tabStyle = (active) => `font-size:9px;padding:2px 8px;background:${active ? 'rgba(100,40,180,0.4)' : 'transparent'};` +
        ...
        mkTab('📦 Models', 'models');
        if (getCreativeState().codeFirstAssets)
            mkTab('⚙ Code', 'code');
    }
    _renderKits() {
    ...
    _select(kitId) {
        ...
        this._el?.querySelectorAll('.cab-kit-btn').forEach(b => {
    ...
    async _renderGrid() {
        ...
        // Apply search filter
        if (this._search) {
            assets = assets.filter(a => a.name.toLowerCase().includes(this._search) ||
