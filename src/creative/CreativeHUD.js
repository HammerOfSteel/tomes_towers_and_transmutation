 ...
 *  └─────────────────────────────────────────────────────────────────┘
 */
import { getCreativeState, isCreativeActive, getSpeedMultiplier, setActiveTool, setActiveHotbarSlot, setNoClip, setGridSnap, setCodeFirstAssets, cycleSpeedUp, cycleSpeedDown, } from './CreativeModeState';
// ── CSS injected once ─────────────────────────────────────────────────────────
const CSS = `
#creative-hud-root {
...
`;
function injectCSS() {
    if (document.getElementById('creative-hud-css'))
        return;
...
export class CreativeHUD {
    ...
    `;
        ...
        // Keyboard shortcuts
        this._kbHandler = (e) => this._onKey(e);
    ...
    refresh() {
        ...
        const s = getCreativeState();
        if (this._zoneEl)
            this._zoneEl.textContent = s.currentZone;
        if (this._speedEl)
            this._speedEl.textContent = `🚀 ${getSpeedMultiplier()}×`;
        if (this._noclipEl) {
    ...
    _onKey(e) {
        ...
        switch (e.key) {
            ...
            case 'E':
                ...
                this._quickTools?.classList.toggle('hidden', !this._toolsVisible);
                break;
            case 'p':
            case 'P':
                setActiveTool('place');
            ...
            case 't':
            case 'T':
                this._toggleTeleport();
                break;
            case 'b':
            case 'B':
                this.cb.onOpenBackrooms();
                break;
            case 'k':
            case 'K':
                this.cb.onOpenSkinPicker();
                break;
            case '[':
                cycleSpeedDown();
                this.refresh();
    ...
    _toggleSettings() {
        ...
        const rebuild = () => {
            ...
            const addToggle = (label, desc, value, onChange) => {
        ...
        `;
                row.querySelector('.cs-toggle').addEventListener('click', () => {
                    onChange(!value);
                    rebuild();
                    this.refresh();
                });
    ...
    _closeTeleport() {
        if (this._teleportPanel)
            this._teleportPanel.style.display = 'none';
    }
    _updateHotbarActive(slot) {
        if (!this._hotbar)
            return;
        this._hotbar.querySelectorAll('.chb-slot').forEach((el, i) => {
    ...
    _updateHotbarSlots() {
        ...
        this._hotbar.querySelectorAll('.chb-slot').forEach((el, i) => {
            ...
            if (name)
                name.textContent = path ? (path.split('/').pop()?.replace(/\.(gltf|glb)$/, '') ?? '') : 'Empty';
