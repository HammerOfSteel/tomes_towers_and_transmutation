...
function factionWardLabel(faction, type) {
...
function rateMarket(w, centre) { return dist(w.seed, centre); }
function rateChurch(_w, _c, i) { return i; } // low index = central
function rateInn(w, centre) { return dist(w.seed, centre) * 0.6; }
function ratePatriciate(w, centre) { return dist(w.seed, centre) * 0.35; }
function rateSlum(w, centre) { return -dist(w.seed, centre); }
// ── Core generator ────────────────────────────────────────────────────────────
export function buildSettlement(p) {
...
export function generateBaseSeeds(p) {
...
export function assignWardLayouts(wards, centre, radius, type, global) {
    for (const ward of wards) {
        if (!ward.withinCity) {
            ward.wardLayout = 'linear';
            continue;
        }
        if (global !== 'auto') {
        ...
        const zone = r < 0.35 ? 'inner' : r < 0.65 ? 'mid' : 'outer';
        const palette = ZONE_PALETTES[type]?.[ward.type]?.[zone] ?? [['organic', 1]];
 ...
 */
export function buildFromSeeds(seeds, p) {
    ...
    for (let i = 0; i < seeds.length; i++) {
        ...
        if (poly)
            polygons.push(poly.map(([x, y]) => ({ x, y })));
        else
            polygons.push([]);
    }
    ...
    const wards = sortedIdx.map((si, _rank) => ({
        type: 'craftsmen',
        seed: seeds[si],
        polygon: polygons[si] ?? [],
        withinCity: true,
    ...
    if (p.hasPlaza)
        assign('market', w => rateMarket(w, centre));
    assign('church', w => rateChurch(w, centre, inner.indexOf(w)));
    assign('inn', w => rateInn(w, centre));
    assign('smithy', () => rand());
    if (p.nPatches >= 8)
        assign('patriciate', w => ratePatriciate(w, centre));
    if (p.nPatches >= 10)
        assign('merchant', () => rand());
    if (p.nPatches >= 12)
        assign('slum', w => rateSlum(w, centre));
    if (p.nPatches >= 16)
        assign('park', () => rand());
    ...
    if (factionExtras) {
        for (const [type, minP, placement] of factionExtras) {
            if (p.nPatches >= minP && unassigned.length > 0) {
                const rateFn = placement === 'central' ? (w) => dist(w.seed, centre)
                    : placement === 'outer' ? (w) => -dist(w.seed, centre)
                        : () => rand();
                assign(type, rateFn);
            }
        }
...
function convexHullExpanded(pts, centre, expand) {
    if (pts.length < 3)
        return pts;
    // Find leftmost point
...
function dominantEdgeAngle(poly) {
    let max = 0, ang = 0;
...
function minDistToEdge(pt, poly) {
    let minD = Infinity;
    for (let i = 0; i < poly.length; i++) {
        const a = poly[i], b = poly[(i + 1) % poly.length];
...
function drawBldg(ctx, cx, cy, bw, bh, rot, fill) {
    ...
    blocked(cx, cy, bw, bh, angle) {
        ...
        for (let gx = x0; gx <= x1; gx++)
            for (let gy = y0; gy <= y1; gy++)
                if (this.cells.has(this._key(gx, gy)))
                    return true;
        return false;
    }
    /** Mark the rotated rect as occupied. */
    mark(cx, cy, bw, bh, angle) {
        const C = OccupancyGrid.CELL;
...
function samplePerimeter(poly, t) {
 ...
 */
function fillWardOrganically(ctx, poly, wardType, wardSeed, occ) {
    if (poly.length < 3)
        return;
    ...
    for (let row = 0; row < nRows; row++) {
        ...
        for (let si = 0; si < nSamples; si++) {
            ...
            if (!pointInPolygon({ x: bx, y: by }, poly))
                continue;
            if (minDistToEdge({ x: bx, y: by }, poly) < STREET - 2)
                continue;
            if (_activeRoads.length > 0 && minDistToRoads({ x: bx, y: by }, _activeRoads) < ROAD_CLEARANCE)
                continue;
            ...
            if (occ.blocked(bx, by, aw, dh, finalAngle))
                continue;
...
function minDistToRoads(pt, roads) {
    let minD = Infinity;
...
function fillWardGrid(ctx, poly, wardType, seed, occ) {
    if (poly.length < 3)
        return;
    ...
    for (let ry = minY + STREET; ry + BH < maxY - STREET + 1; ry += BH + GAP) {
        for (let rx = minX + STREET; rx + BW < maxX - STREET + 1; rx += BW + GAP) {
            ...
            if (!pointInPolygon({ x: wcx, y: wcy }, poly))
                continue;
            if (minDistToEdge({ x: wcx, y: wcy }, poly) < STREET - 1)
                continue;
            if (_activeRoads.length > 0 && minDistToRoads({ x: wcx, y: wcy }, _activeRoads) < ROAD_CLEARANCE)
                continue;
            const aw = BW * (0.82 + rand() * 0.2), dh = BH * (0.82 + rand() * 0.2);
            if (occ.blocked(wcx, wcy, aw, dh, angle))
                continue;
 ...
 */
function fillWardLinear(ctx, poly, wardType, seed, occ) {
    if (poly.length < 3)
        return;
    const rand = mulberry32(seed);
    const col = wardType === 'church' ? CARTO.bldg_dk : CARTO.bldg;
 ...
 */
function fillWardTerraced(ctx, poly, wardType, seed, occ) {
    if (poly.length < 3)
        return;
    const rand = mulberry32(seed);
    const col = wardType === 'church' ? CARTO.bldg_dk : CARTO.bldg;
    ...
    for (let ry = minY + STREET; ry + ROW_H < maxY - STREET + 1; ry += ROW_H + ROW_GAP) {
        ...
        for (let h = 0; h < nHouses; h++) {
            ...
            if (_activeRoads.length > 0 && minDistToRoads({ x: wcx, y: wcy }, _activeRoads) < ROAD_CLEARANCE)
                continue;
            if (occ.blocked(wcx, wcy, bw, bh, angle))
                continue;
 ...
 */
function fillWardPerimeter(ctx, poly, wardType, seed, occ) {
    if (poly.length < 3)
        return;
    const rand = mulberry32(seed);
    const col = wardType === 'church' ? CARTO.bldg_dk : CARTO.bldg;
    ...
    for (let si = 0; si < nSamples; si++) {
        ...
        if (!pointInPolygon({ x: bx, y: by }, poly))
            continue;
        if (minDistToEdge({ x: bx, y: by }, poly) < STREET - 1)
            continue;
        if (_activeRoads.length > 0 && minDistToRoads({ x: bx, y: by }, _activeRoads) < ROAD_CLEARANCE)
            continue;
 ...
 */
function fillWardRadial(ctx, poly, wardType, seed, occ) {
    if (poly.length < 3)
        return;
    const rand = mulberry32(seed);
    const col = wardType === 'church' ? CARTO.bldg_dk : CARTO.bldg;
 ...
 */
function fillWardClustered(ctx, poly, wardType, seed, occ) {
    if (poly.length < 3)
        return;
    const rand = mulberry32(seed);
    const col = wardType === 'church' ? CARTO.bldg_dk : CARTO.bldg;
    ...
    for (let attempt = 0; attempt < warmupIter; attempt++) {
        ...
        const bbox = poly.reduce((b, p) => ({ minX: Math.min(b.minX, p.x), maxX: Math.max(b.maxX, p.x),
            minY: Math.min(b.minY, p.y), maxY: Math.max(b.maxY, p.y) }), { minX: Infinity, maxX: -Infinity, minY: Infinity, maxY: -Infinity });
        ...
        if (!pointInPolygon(candidate, poly))
            continue;
        if (minDistToEdge(candidate, poly) < STREET + DEPTH)
            continue;
        if (_activeRoads.length > 0 && minDistToRoads(candidate, _activeRoads) < ROAD_CLEARANCE + DEPTH)
            continue;
        if (clusterCentres.some(c => Math.hypot(c.x - cx, c.y - cy) < CLUSTER_MIN_DIST))
            continue;
...
function fillWard(ctx, ward, occ) {
...
export function drawSettlement2D5(model, canvas, showLabels = true, layout = 'organic', faction = 'human') {
    ...
    for (const ward of model.wards) {
        if (ward.withinCity || !ward.polygon.length)
            continue;
        ctx.beginPath();
        ...
        if (ward.type === 'market') {
            ...
            for (let i = 0; i < ward.polygon.length; i++) {
                ...
                for (let t = 0.1; t < 0.9; t += 12 / len) {
                    ...
                    if (minDistToEdge({ x: sx, y: sy }, ward.polygon) < 3)
                        continue;
                    if (occ.blocked(sx, sy, 6, 8, ang))
                        continue;
            ...
            ctx.fillStyle = CARTO.water;
            ctx.fill();
            ctx.strokeStyle = CARTO.bldg_dk;
            ctx.lineWidth = 1;
            ctx.stroke();
            continue;
        }
...
// ── Canvas 2D Renderer ────────────────────────────────────────────────────────
// ── Canvas 2D Renderer ────────────────────────────────────────────────────────
export function drawSettlement(model, canvas, showLabels = true, showBuildings = true) {
    const ctx = canvas.getContext('2d');
    const W = canvas.width, H = canvas.height;
    ctx.clearRect(0, 0, W, H);
    // Background
    ...
    for (const ward of model.wards) {
        ...
        for (let i = 1; i < ward.polygon.length; i++)
            ctx.lineTo(ward.polygon[i].x, ward.polygon[i].y);
        ctx.closePath();
    ...
    if (model.wall && model.wall.length) {
        ...
        for (let i = 1; i < model.wall.length; i++)
            ctx.lineTo(model.wall[i].x, model.wall[i].y);
        ctx.closePath();
...
let persistentSeeds = null; // mutable seeds for interactive warp
let lastParams = null;
let activeTool = 'select';
let viewMode = sessionStorage.getItem('ow-view') ?? 'iso';
function setTool(name) {
    ...
    if (name === 'warp')
        canvas.classList.add('tool-warp');
}
...
function redraw() {
    ...
    const showBuildings = document.getElementById('show-buildings').checked;
    const layoutParam = document.querySelector('#layout-pills .pill.active')?.dataset.layout ?? 'organic';
    const faction = document.querySelector('#faction-pills .pill.active')?.dataset.faction ?? 'human';
    if (viewMode === 'iso')
        drawSettlement2D5(currentModel, canvas, showLabels, layoutParam, faction);
    else
        drawSettlement(currentModel, canvas, showLabels, showBuildings);
}
function getParams() {
    const type = document.querySelector('#type-pills .pill.active')?.dataset.type ?? 'village';
    const layout = document.querySelector('#layout-pills .pill.active')?.dataset.layout ?? 'organic';
    const faction = document.querySelector('#faction-pills .pill.active')?.dataset.faction ?? 'human';
    return {
        ...
        layout,
        faction,
        nPatches: parseInt(document.getElementById('patches').value),
        warp: parseFloat(document.getElementById('warp').value),
        nGates: parseInt(document.getElementById('roads').value),
        walled: document.getElementById('walls').checked,
        hasCitadel: document.getElementById('citadel').checked,
        hasPlaza: document.getElementById('plaza').checked,
        width: canvas.width,
...
for (const [id, valId] of [['patches', 'patches-val'], ['warp', 'warp-val'], ['roads', 'roads-val']]) {
...
document.getElementById('btn-roll').addEventListener('click', () => {
    ...
    if (studioMode === 'cave')
        generateCaveView();
    else if (studioMode === 'dungeon')
        generateDungeonView();
    else if (studioMode === 'realm')
        generateRealmView();
    else if (studioMode === 'solar')
        generateSolarView();
    else
        generate();
});
...
window.addEventListener('keydown', e => {
    ...
    switch (e.key) {
        ...
        case '+':
        case '=': {
            const el = document.getElementById('patches');
        ...
        case 'W': {
            ...
            generate(true);
            break; // keep seeds when toggling walls
        }
        case 's':
        case 'S':
            setTool('select');
...
function _updateBreadcrumb() {
    ...
    items.querySelectorAll('[data-nav-idx]').forEach(a => {
...
function _realmHitTest(px, py) {
    ...
    for (const s of settlements) {
        ...
        if (Math.hypot(px - sx, py - sy) <= settleDotR)
    ...
    for (const d of dungeons) {
        ...
        if (Math.hypot(px - dx, py - dy) <= dungeonDotR)
            return { kind: 'dungeon', data: d };
    }
    return null;
}
canvas.addEventListener('click', e => {
    ...
    if (hit.kind === 'settlement') {
        ...
        const factionBtn = document.querySelector(`#faction-pills [data-faction="${s.faction}"]`);
        factionBtn?.click();
...
canvas.addEventListener('pointermove', e => {
    if (!warpDragging || activeTool !== 'warp' || !persistentSeeds || !lastParams)
        return;
    ...
    octx.lineWidth = 1;
    for (const [ax, ay, bx, by] of [[mx - 6, my, mx + 6, my], [mx, my - 6, mx, my + 6]]) {
        octx.beginPath();
        octx.moveTo(ax, ay);
        octx.lineTo(bx, by);
        octx.stroke();
...
canvas.addEventListener('pointermove', e => {
    if (activeTool !== 'warp' || warpDragging)
        return;
    const [mx, my] = canvasXY(e);
    const WARP_R = Math.min(canvas.width, canvas.height) * WARP_RADIUS_FRAC;
    const octx = overlay.getContext('2d');
...
canvas.addEventListener('mousemove', e => {
    ...
    for (const ward of currentModel.wards) {
        if (!ward.withinCity || !ward.polygon.length)
            continue;
        if (pointInPolygon({ x: mx, y: my }, ward.polygon)) {
            found = ward;
            break;
        }
    }
    if (found) {
        ...
        if (found.polygon.length) {
            ...
            for (let i = 1; i < found.polygon.length; i++)
                octx.lineTo(found.polygon[i].x, found.polygon[i].y);
            octx.closePath();
...
canvas.addEventListener('dblclick', e => {
    if (studioMode !== 'settlement' || !currentModel)
        return;
    const rect = canvas.getBoundingClientRect();
    const mx = (e.clientX - rect.left) * (canvas.width / rect.width);
    const my = (e.clientY - rect.top) * (canvas.height / rect.height);
    ...
    if (!kind)
        return;
    const factionStr = document.querySelector('#faction-pills .pill.active')?.dataset.faction ?? 'human';
    const FACTION_MAP = {
    ...
    const faction = FACTION_MAP[factionStr] ?? 'human_town';
...
function _bModalSetup() {
    ...
    window.addEventListener('mouseup', () => {
        ...
        if (_bModalCanvas)
            _bModalCanvas.style.cursor = 'grab';
    });
    _bModalCanvas.addEventListener('dblclick', _bModalResetView);
}
function _bModalResetView() {
    _bModalZoom = 1.0;
    _bModalPanX = 0;
    _bModalPanY = 0;
    _bModalRedraw();
}
function _bModalRedraw() {
    if (!_bModalPlan || !_bModalCanvas)
        return;
...
function hideBuildingModal() {
    if (_bModal)
        _bModal.style.display = 'none';
    hoverEl.textContent = 'hover a ward · double-click to enter building';
}
// Escape key closes modal
window.addEventListener('keydown', e => {
    if (e.key === 'Escape' && _bModal?.style.display !== 'none')
        hideBuildingModal();
});
...
function buildRoomTiles(w, d) {
    ...
    if (w >= 9 && d >= 9) {
        const ip = 2;
        for (const [px, pz] of [[ip, ip], [w - 1 - ip, ip], [ip, d - 1 - ip], [w - 1 - ip, d - 1 - ip]]) {
...
function placeInteractables(types, w, d, rand) {
    const result = [];
    const used = new Set();
...
function placeSpawns(count, w, d, rand) {
    const result = [];
    const used = new Set();
...
const CATEGORY_SEQ = ['library', 'barracks', 'armory', 'ritual', 'treasure', 'library', 'barracks'];
/**
 * Generate a branching dungeon floor plan with typed rooms.
 * Returns a DungeonPlan compatible with drawDungeonFloorPlan().
 */
export function generateRichFloorPlan(seed, complexity) {
    const rand = mulberry32(seed);
    const occupied = new Set();
    const defs = [];
    let idx = 0;
    function tryPlace(cat, col, row) {
        const key = `${col},${row}`;
        if (occupied.has(key))
            return null;
        const sizes = ROOM_SIZES[cat];
        const [w, d] = sizes[Math.floor(rand() * sizes.length)];
        const def = { id: `room_${idx++}`, cat, col, row, w, d, connections: [] };
        occupied.add(key);
        defs.push(def);
        return def;
    }
    ...
    while (queue.length && defs.length < targetRooms) {
        ...
        for (const [dir, dc, dr] of shuffledDirs) {
            if (added >= branches || defs.length >= targetRooms)
                break;
            // Skip dir if parent already has a connection that way
            if (parent.connections.some(c => c.dir === dir))
                continue;
            const nc = parent.col + dc, nr = parent.row + dr;
            const cat = defs.length >= targetRooms - 1 ? 'boss'
    ...
    if (defs.length >= 3) {
        const furthest = defs.reduce((a, b) => Math.abs(b.col) + Math.abs(b.row) > Math.abs(a.col) + Math.abs(a.row) ? b : a);
...
function guessRoomLabel(id, floorType) {
    if (id.includes('cell_start'))
        return 'Cell';
    if (id.includes('library_large'))
    ...
    if (id.includes('boss'))
        return 'Boss';
    if (floorType === 'stone_alchemy')
        return 'Ritual';
    if (floorType === 'stone_scorched')
        return 'Forge';
    if (floorType === 'wood')
        return 'Library';
    return '';
 ...
 */
export function drawDungeonFloorPlan(plan, canvas, floorFilter) {
    const ctx = canvas.getContext('2d');
    const W = canvas.width, H = canvas.height;
    // Clear
    ...
    while (queue.length) {
        ...
        for (const stair of room.staircases) {
            if (!stair.targetId || visited.has(stair.targetId))
                continue;
            if (!rooms.has(stair.targetId))
                continue;
    ...
    for (const [rid, pos] of positions) {
        const room = rooms.get(rid);
        for (const door of room.doors) {
            if (!door.targetId || !positions.has(door.targetId))
                continue;
            if (!rooms.has(door.targetId))
                continue;
    ...
    for (const [rid, pos] of positions) {
        ...
        for (const door of room.doors) {
            ...
            ctx.beginPath();
            ctx.arc(dcx, dcy, CELL * 0.3, 0, Math.PI * 2);
            ctx.fill();
        ...
        for (const spawn of room.spawns) {
            ...
            ctx.beginPath();
            ctx.arc(spx, spy, CELL * 0.28, 0, Math.PI * 2);
            ctx.fill();
        }
...
let studioMode = 'settlement';
let currentDungeonPlan = null;
function generateDungeonView() {
    ...
    const complexity = parseInt(document.getElementById('dfloors').value);
    try {
        if (dtype === 'tower') {
...
function redrawDungeon() {
    ...
    let floorFilter;
    if (dtype === 'tower') {
        floorFilter = parseInt(document.getElementById('dfloor').value);
    }
...
document.getElementById('dungeon-type-pills').addEventListener('click', e => {
    ...
    document.getElementById('tower-floor-row').style.display = isTower ? '' : 'none';
    currentDungeonPlan = null;
    generateDungeonView();
});
document.getElementById('dfloors').addEventListener('input', () => {
    document.getElementById('dfloors-val').textContent =
        document.getElementById('dfloors').value;
    currentDungeonPlan = null;
    generateDungeonView();
});
document.getElementById('dfloor')?.addEventListener('input', () => {
    const v = document.getElementById('dfloor').value;
    document.getElementById('dfloor-val').textContent = v;
    redrawDungeon();
});
...
function getBiomePal(data) {
    if (data.type === 'cave')
        return CAVE_BIOMES[data.biome] ?? CAVE_BIOMES.stone;
    return GLADE_BIOMES[data.biome] ?? GLADE_BIOMES.forest;
}
/**
...
function generateCaveData(seed, type, biome = type === 'cave' ? 'stone' : 'forest', size = 2, density = 0.47) {
    const SIZES = { 1: [52, 40], 2: [72, 54], 3: [92, 68] };
    const [W, H] = SIZES[size] ?? SIZES[2];
    const rand = mulberry32(seed);
    let grid;
    if (type === 'cave') {
        const ca = BIOME_CA[biome] ?? BIOME_CA.stone;
            ...
            for (let sx = 1; sx < W - 1; sx++) {
                ...
                while (q.length) {
                    ...
                    if (cx < 0 || cy < 0 || cx >= W || cy >= H)
                        continue;
                    if (visited[cy][cx] || !grid[cy][cx])
                        continue;
                    visited[cy][cx] = true;
                    region.push([cx, cy]);
                    q.push([cx + 1, cy], [cx - 1, cy], [cx, cy + 1], [cx, cy - 1]);
                }
                if (region.length > bestCells.length)
                    bestCells = region;
            }
        }
    ...
    if (type === 'cave') {
        ...
        for (let y = 1; y < H - 1; y++)
            for (let x = 1; x < W - 1; x++) {
                ...
                if (adjFloor && rand() > 0.45)
                    features.push({ x, y, kind: 'tree' });
            }
        for (const [x, y] of shuffled(floorCells).slice(0, 2))
            features.push({ x, y, kind: 'spawn' });
    }
 ...
 */
export function drawCaveGlade(data, canvas) {
    const ctx = canvas.getContext('2d');
    const { grid, W, H, type, features, seed } = data;
    const biome = data.biome;
    const pal = getBiomePal(data);
    const CELL = Math.min(Math.floor((canvas.width - 8) / W), Math.floor((canvas.height - 8) / H));
    const offX = Math.floor((canvas.width - W * CELL) / 2);
    const offY = Math.floor((canvas.height - H * CELL) / 2);
    ...
    for (const feat of features) {
        ...
        switch (feat.kind) {
            ...
            case 'mushroom':
                ...
                ctx.beginPath();
                ctx.arc(cx, cy - CELL * 0.08, CELL * 0.28, 0, Math.PI * 2);
                ctx.fill();
                break;
        }
    }
    ...
    const featLabels = type === 'cave'
        ? (CAVE_FEAT_LABELS[biome] ?? {})
        : (GLADE_FEAT_LABELS[biome] ?? {});
    ...
    for (const [kind, label] of Object.entries(featLabels)) {
        if (!label || !seenKinds.has(kind))
            continue;
...
// ── Cave mode state ───────────────────────────────────────────────────────────
let currentCaveData = null;
function getCaveParams() {
    const type = document.querySelector('#cave-type-pills .pill.active')?.dataset.ctype ?? 'cave';
    const biome = type === 'cave'
        ? (document.querySelector('#cave-biome-pills .pill.active')?.dataset.biome ?? 'stone')
        : (document.querySelector('#glade-biome-pills .pill.active')?.dataset.gbiome ?? 'forest');
    const size = parseInt(document.getElementById('cave-size')?.value ?? '2');
    const density = parseInt(document.getElementById('cave-density')?.value ?? '47') / 100;
...
function generateCaveView() {
    ...
    genTimeEl.textContent = `${(CAVE_BIOMES[biome] ?? GLADE_BIOMES[biome])?.name ?? biome}  ·  ${floors} open cells  ·  seed ${seed}`;
}
function redrawCave() { if (currentCaveData)
    drawCaveGlade(currentCaveData, canvas); }
const REALM_BIOME_COLOR = {
...
function fbmR(noise, x, y, oct, scale = 1) {
...
export function generateRealmData(seed, W = 96, H = 72, nSettlements = 6, shape = 'island', climate = 'temperate', roughness = 0.5) {
    const rand = mulberry32(seed);
    ...
    // ── Build cell grid ──────────────────────────────────────────────────────────
    const cells = Array.from({ length: H }, (_, cy) => Array.from({ length: W }, (_, cx) => {
    ...
    // ── Settlements ──────────────────────────────────────────────────────────────
    const VALID = new Set(['grassland', 'forest', 'savanna', 'taiga', 'desert']);
    const validCells = [];
    for (let y = 4; y < H - 4; y++)
        for (let x = 4; x < W - 4; x++)
            if (VALID.has(cells[y][x].biome))
                validCells.push({ x, y });
    const sv = [...validCells].sort(() => rand() - 0.5);
    const settlements = [];
    const MIN_DIST = Math.floor(Math.min(W, H) / (nSettlements + 2));
    const FACTIONS = ['human', 'elven', 'dwarven', 'orcish', 'vulperia', 'slime', 'vampire', 'undead', 'fae'];
    for (const cell of sv) {
        if (settlements.length >= nSettlements)
            break;
        ...
        if (settlements.every(s => Math.hypot(s.x - cell.x, s.y - cell.y) >= MIN_DIST)) {
            ...
            const sz = td > MIN_DIST * 2.5 && (b === 'forest' || b === 'grassland') ? 'city'
    ...
    // ── Dungeons ─────────────────────────────────────────────────────────────────
    const DUNGEON_BIOMES = new Set(['grassland', 'forest', 'taiga', 'desert', 'savanna', 'tundra', 'snow']);
    const nDungeons = 3 + Math.floor(rand() * 4);
    const dungeons = [];
    const dungeonCands = [...validCells].filter(c => DUNGEON_BIOMES.has(cells[c.y][c.x].biome) &&
        Math.hypot(c.x - towerX, c.y - towerY) > MIN_DIST).sort(() => rand3() - 0.5);
    for (const cell of dungeonCands) {
        if (dungeons.length >= nDungeons)
            break;
        ...
        if (farFromSettlements && farFromOtherDungeons)
            dungeons.push({ x: cell.x, y: cell.y });
    }
...
function realmLayout(data, canvas) {
    const { W, H } = data;
    const CELL = Math.max(2, Math.min(Math.floor((canvas.width - 4) / W), Math.floor((canvas.height - 4) / H)));
    const offX = Math.floor((canvas.width - W * CELL) / 2);
    const offY = Math.floor((canvas.height - H * CELL) / 2);
...
export function drawRealm(data, canvas) {
    const ctx = canvas.getContext('2d');
    ...
    for (let y = 0; y < H; y++) {
        for (let x = 0; x < W; x++) {
            const c = cells[y][x];
            if (c.biome === 'ocean' || c.biome === 'deep_ocean')
                continue;
            if (c.elevation > 0.60) {
    ...
    // ── Contour lines ─────────────────────────────────────────────────────────
    if (CELL >= 3) {
        for (const [level, alpha] of [[0.50, 0.12], [0.62, 0.16], [0.74, 0.22], [0.85, 0.28]]) {
            ...
            for (let y = 0; y < H; y++)
                for (let x = 0; x < W; x++) {
                    ...
                    if ((e >= level) !== (eR >= level)) {
                        ctx.beginPath();
                        ctx.moveTo(offX + (x + 1) * CELL, offY + y * CELL);
                        ctx.lineTo(offX + (x + 1) * CELL, offY + (y + 1) * CELL);
                        ctx.stroke();
                    }
                    if ((e >= level) !== (eD >= level)) {
                        ctx.beginPath();
                        ctx.moveTo(offX + x * CELL, offY + (y + 1) * CELL);
                        ctx.lineTo(offX + (x + 1) * CELL, offY + (y + 1) * CELL);
                        ctx.stroke();
                    }
                }
        ...
        for (let y = 0; y < H; y++)
            for (let x = 0; x < W; x++)
                if (cells[y][x].biome === 'forest' && (x * 3 + y * 7) % 5 === 0) {
                    ctx.beginPath();
                    ctx.arc(offX + x * CELL + CELL / 2, offY + y * CELL + CELL / 2, CELL * 0.22, 0, Math.PI * 2);
                    ctx.fill();
                }
    }
    ...
    // ── Coastline border ───────────────────────────────────────────────────────
    ctx.strokeStyle = 'rgba(20,30,80,0.45)';
    ctx.lineWidth = 0.7;
    for (let y = 0; y < H; y++)
        for (let x = 0; x < W; x++) {
    ...
    if (settlements.length > 1) {
        ctx.strokeStyle = 'rgba(170,140,70,0.55)';
        ctx.lineWidth = 0.8;
        ctx.setLineDash([2, 3]);
    ...
    // Glow
    ctx.strokeStyle = 'rgba(240,200,30,0.3)';
    ctx.lineWidth = 4;
    ctx.beginPath();
    for (let i = 0; i < 6; i++) {
        const a = (i / 6) * Math.PI * 2 - Math.PI / 6;
        i === 0 ? ctx.moveTo(tx + tr * 1.5 * Math.cos(a), ty2 + tr * 1.5 * Math.sin(a)) : ctx.lineTo(tx + tr * 1.5 * Math.cos(a), ty2 + tr * 1.5 * Math.sin(a));
    }
    ctx.closePath();
    ctx.stroke();
    // Hex ring
    ...
    for (let i = 0; i < 6; i++) {
        const a = (i / 6) * Math.PI * 2 - Math.PI / 6;
        i === 0 ? ctx.moveTo(tx + tr * Math.cos(a), ty2 + tr * Math.sin(a)) : ctx.lineTo(tx + tr * Math.cos(a), ty2 + tr * Math.sin(a));
    }
    ctx.closePath();
    ctx.stroke();
    ...
    ctx.font = `${Math.max(6, Math.min(9, CELL + 1))}px Georgia, serif`;
    ctx.textBaseline = 'top';
    for (const d of data.dungeons) {
        const dx = offX + (d.x + 0.5) * CELL, dy = offY + (d.y + 0.5) * CELL;
        ctx.fillStyle = 'rgba(0,0,0,0.6)';
        ctx.beginPath();
        ctx.arc(dx, dy, dR + 1.5, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.textAlign = 'center';
        ctx.fillText('⚔', dx, dy + dR + 1);
    }
    // ── Settlements ───────────────────────────────────────────────────────────
    const dotR = Math.max(2, CELL * 0.75);
    ctx.font = `${Math.max(7, Math.min(10, CELL + 2))}px Georgia, serif`;
    ctx.textBaseline = 'top';
    for (const s of settlements) {
        ...
        ctx.fill();
        ctx.fillStyle = SETTLEMENT_SIZE_COLOR[s.size];
        ctx.beginPath();
        ...
        ctx.textAlign = 'center';
        ctx.fillText(s.name, sx, sy + dotR + 1);
    }
    // ── Compass rose ───────────────────────────────────────────────────────────
    ...
    ctx.beginPath();
    ctx.arc(0, 0, cr * 1.1, 0, Math.PI * 2);
    ctx.fill();
    // N petal
    ctx.fillStyle = '#d8c888';
    ctx.beginPath();
    ctx.moveTo(0, -cr);
    ctx.lineTo(cr * 0.28, 0);
    ctx.lineTo(0, cr * 0.38);
    ctx.closePath();
    ctx.fill();
    // S petal
    ...
    // N label
    ctx.fillStyle = '#d8c888';
    ctx.font = `bold ${Math.max(6, cr * 0.5)}px Georgia,serif`;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'bottom';
    ...
    const lbs = ['ocean', 'beach', 'desert', 'savanna', 'grassland', 'forest', 'taiga', 'tundra', 'snow'].filter(b => present.has(b));
    const LH = 11, LX = offX + 5, LY = offY + 5;
    ctx.fillStyle = 'rgba(0,0,0,0.6)';
    ctx.fillRect(LX - 2, LY - 2, 86, LH * lbs.length + 4);
    lbs.forEach((b, i) => {
        ctx.fillStyle = REALM_BIOME_COLOR[b];
        ctx.fillRect(LX, LY + i * LH + 2, 8, 7);
        ctx.fillStyle = 'rgba(240,235,220,0.92)';
        ctx.font = '7px Georgia, serif';
        ctx.textAlign = 'left';
        ctx.textBaseline = 'top';
        ctx.fillText(REALM_BIOME_LABEL[b], LX + 11, LY + i * LH + 2);
    });
    // ── Settlement size key ───────────────────────────────────────────────────
    const KX = offX + W * CELL - 68, KY = offY + 5;
    ctx.fillStyle = 'rgba(0,0,0,0.6)';
    ctx.fillRect(KX - 2, KY - 2, 68, 40);
    let ki = 0;
    for (const [sz, lab] of [['village', 'Village'], ['town', 'Town'], ['city', 'City']]) {
        ctx.fillStyle = SETTLEMENT_SIZE_COLOR[sz];
        ctx.beginPath();
        ctx.arc(KX + 5, KY + 5 + ki * 12, 3, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = 'rgba(240,235,220,0.92)';
        ctx.font = '7px Georgia, serif';
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ctx.fillText(lab, KX + 11, KY + 5 + ki * 12);
        ki++;
    }
    // ── Title ─────────────────────────────────────────────────────────────────
...
function buildCloudGrid(W, H, seed) {
...
export function drawRealmPlanet(data, canvas) {
    const ctx = canvas.getContext('2d');
    ...
    ctx.beginPath();
    ctx.arc(pcx, pcy, planetR * 1.14, 0, Math.PI * 2);
    ctx.fill();
    ...
    if (tDz > 0.15) { // on visible hemisphere
        ...
        for (let i = 0; i < 6; i++) {
            const a = (i / 6) * Math.PI * 2 - Math.PI / 6;
            if (i === 0)
                ctx.moveTo(tSx + tr2 * Math.cos(a), tSy + tr2 * Math.sin(a));
            else
                ctx.lineTo(tSx + tr2 * Math.cos(a), tSy + tr2 * Math.sin(a));
        }
        ctx.closePath();
        ctx.stroke();
    }
...
let currentPlanetType = 'terran';
// ── Solar System state ────────────────────────────────────────────────────────
let solarRenderer = null;
let currentSolarData = null;
function getSolarRenderer() {
    if (!solarRenderer) {
        ...
        solarRenderer.onPlanetHover(p => {
            ...
            if (el)
                el.textContent = p ? `${PLANET_META_SS[p.type]} ${p.name}` : '—';
        });
        solarRenderer.onPlanetClick(p => {
            ...
            const pTypePill = document.querySelector(`#planet-type-pills [data-ptype="${currentPlanetType}"]`);
            ...
            const cfg = PLANET_REALM[currentPlanetType] ?? { shape: 'island', climate: 'temperate', roughness: 50, settlements: 6 };
            // Apply shape pill
            document.querySelectorAll('#realm-shape-pills .pill').forEach(x => x.classList.remove('active'));
            document.querySelector(`#realm-shape-pills [data-shape="${cfg.shape}"]`)?.classList.add('active');
            // Apply climate pill
            document.querySelectorAll('#realm-climate-pills .pill').forEach(x => x.classList.remove('active'));
            document.querySelector(`#realm-climate-pills [data-climate="${cfg.climate}"]`)?.classList.add('active');
            // Apply roughness slider
            const roughnessEl = document.getElementById('realm-roughness');
            if (roughnessEl) {
                roughnessEl.value = String(cfg.roughness);
                const lbl = document.getElementById('realm-roughness-val');
                if (lbl)
                    lbl.textContent = cfg.roughness < 25 ? 'Flat' : cfg.roughness < 50 ? 'Rolling' : cfg.roughness < 75 ? 'Moderate' : 'Rugged';
            }
            // Apply settlements slider
            const settleEl = document.getElementById('realm-settlements');
            if (settleEl) {
                settleEl.value = String(cfg.settlements);
                const lbl = document.getElementById('realm-settlements-val');
                if (lbl)
                    lbl.textContent = String(cfg.settlements);
            }
            ...
            document.querySelector('#realm-view-pills [data-view="hex"]')?.classList.add('active');
...
function generateSolarView() {
    ...
    if (starEl)
        starEl.textContent = `${currentSolarData.star.spectral}-type · ${currentSolarData.planets.length} planets`;
    // Populate planet list
    const listEl = document.getElementById('solar-planet-list');
    if (listEl)
        listEl.innerHTML = currentSolarData.planets.map(p => `<div>${PLANET_META_SS[p.type] ?? ''} ${p.name}${p.isTowerPlanet ? ' ⬡' : ''}</div>`).join('');
}
function showPlanetCanvas(show) {
    ...
    if (layerCtrl)
        layerCtrl.style.display = show ? '' : 'none';
    if (typeSection)
...
function redrawRealm() {
    ...
    if (realmViewMode === 'planet') {
        ...
        const sunDir = new THREE.Vector3(Math.cos(sunElev) * Math.sin(sunLon) * 0.6, // reduced X/Z so Z stays positive
        -Math.sin(sunElev) * 0.4, Math.cos(sunElev) * Math.cos(sunLon) * 0.6 + 0.6).normalize();
        ...
        const showClouds = document.getElementById('planet-show-clouds')?.checked ?? true;
        const showAtmos = document.getElementById('planet-show-atmos')?.checked ?? true;
        const dayOnly = document.getElementById('planet-day-only')?.checked ?? false;
        ...
        const sunLon = ((d.seed * 0.00137) % 1) * Math.PI * 2;
        const sunElev = 0.35 + ((d.seed ^ 0x7F2A) & 0xFF) / 0xFF * 0.25;
        const sunDir = new THREE.Vector3(Math.cos(sunElev) * Math.sin(sunLon) * 0.6, -Math.sin(sunElev) * 0.4, Math.cos(sunElev) * Math.cos(sunLon) * 0.6 + 0.6).normalize();
        // Derive subdivision from realm Size slider: S=6, M=8, L=12, XL=16, Planet=20
        const SIZE_TO_SUB = { 1: 6, 2: 8, 3: 12, 4: 16, 5: 24 };
        void SIZE_TO_SUB; // kept for reference
        const realmSize = parseInt(document.getElementById('realm-size')?.value ?? '2');
        const roughness = parseFloat(document.getElementById('realm-roughness')?.value ?? '50') / 100;
        const shape = document.querySelector('#realm-shape-pills .pill.active')?.dataset.shape ?? 'island';
        const climate = document.querySelector('#realm-climate-pills .pill.active')?.dataset.climate ?? 'temperate';
        void shape;
        void climate; // shape+climate baked into cells
        ...
        if (hexSubVal)
            hexSubVal.textContent = `${tileCount} tiles (sub 24)`;
        ...
        hr.setAutoRotate(document.getElementById('planet-auto-rotate')?.checked ?? true);
        hr.setDayOnly(document.getElementById('planet-day-only')?.checked ?? false);
        hr.start();
    }
    else {
...
function generateRealmView() {
    const seed = parseInt(seedInput.value) || Date.now();
    const size = parseInt(document.getElementById('realm-size')?.value ?? '2');
    const nS = parseInt(document.getElementById('realm-settlements')?.value ?? '6');
    const shape = document.querySelector('#realm-shape-pills .pill.active')?.dataset.shape ?? 'island';
    const climate = document.querySelector('#realm-climate-pills .pill.active')?.dataset.climate ?? 'temperate';
    const roughness = parseFloat(document.getElementById('realm-roughness')?.value ?? '50') / 100;
    const REALM_SIZES = { 1: [64, 48], 2: [96, 72], 3: [160, 120], 4: [220, 164], 5: [300, 225] };
...
document.getElementById('studio-tabs').addEventListener('click', e => {
    ...
    document.querySelector('.map-toolbar').style.visibility = mode === 'settlement' ? '' : 'hidden';
...
document.getElementById('planet-type-pills').addEventListener('click', e => {
    const pill = e.target.closest('.pill');
    if (!pill?.dataset.ptype)
        return;
    currentPlanetType = pill.dataset.ptype;
...
// ── Realm sliders ─────────────────────────────────────────────────────────────
const SIZE_LABELS = { '1': 'XS', '2': 'S', '3': 'M', '4': 'L', '5': 'XL' };
const ROUGHNESS_LABELS = (v) => v < 25 ? 'Flat' : v < 50 ? 'Rolling' : v < 75 ? 'Moderate' : v < 90 ? 'Rugged' : 'Extreme';
document.getElementById('realm-size')?.addEventListener('input', function () {
    const lbl = document.getElementById('realm-size-val');
    if (lbl)
        lbl.textContent = SIZE_LABELS[this.value] ?? this.value;
    if (studioMode === 'realm') {
        currentRealmData = null;
        generateRealmView();
    }
});
document.getElementById('realm-roughness')?.addEventListener('input', function () {
    const lbl = document.getElementById('realm-roughness-val');
    if (lbl)
        lbl.textContent = ROUGHNESS_LABELS(parseInt(this.value));
    if (studioMode === 'realm') {
        currentRealmData = null;
        generateRealmView();
    }
});
document.getElementById('realm-settlements')?.addEventListener('input', function () {
    const lbl = document.getElementById('realm-settlements-val');
    if (lbl)
...
let _libraryTypeFilter = 'all';
let _librarySelectedId = null;
// ── Thumbnail helpers ─────────────────────────────────────────────────────────
function _renderThumbnail(_type) {
    const thumbCanvas = document.getElementById('library-thumb-canvas');
    if (!thumbCanvas)
        return null;
...
function _renderLibraryGrid() {
    ...
    const filtered = search.trim()
        ...
        : entries;
    if (count)
        count.textContent = `(${assetLibrary.size})`;
...
function _getLibraryTag(entry, key) {
    const tag = entry.tags.find(t => t.startsWith(`${key}:`));
    return tag ? tag.slice(key.length + 1) : null;
}
function _setActivePillByDataset(containerId, datasetKey, value) {
    if (!value)
        return;
    ...
    const match = pills.find(p => p.dataset[datasetKey] === value);
    if (!match)
        return;
    pills.forEach(p => p.classList.remove('active'));
    match.classList.add('active');
}
function _syncCaveTypeSections(type) {
    const isCave = type === 'cave';
    const denseLabel = document.getElementById('cave-density-row');
    if (denseLabel)
        denseLabel.style.display = isCave ? '' : 'none';
    const caveSection = document.getElementById('cave-biome-section');
    const gladeSection = document.getElementById('glade-biome-section');
    if (caveSection)
        caveSection.style.display = isCave ? '' : 'none';
    if (gladeSection)
        gladeSection.style.display = isCave ? 'none' : '';
}
function _setStudioModeForLibraryPreview(mode) {
    studioMode = mode;
    _navStack = [];
    _updateBreadcrumb();
    document.querySelectorAll('.studio-tab').forEach(t => t.classList.remove('active'));
    ...
    if (mapToolbar)
        mapToolbar.style.visibility = mode === 'settlement' ? '' : 'hidden';
    if (mode !== 'settlement')
        hideBuildingModal();
    if (mode !== 'realm' && planetRenderer) {
        planetRenderer.stop();
...
function _previewLibraryEntry(entry) {
    window.__owStudioLastLibraryPreview = entry
    ...
    if (entry.type === 'settlement') {
        ...
        const faction = _getLibraryTag(entry, 'faction');
        _setActivePillByDataset('faction-pills', 'faction', faction);
        if (faction)
            updateLegend(faction);
        currentModel = entry.data;
        persistentSeeds = null;
        lastParams = null;
        redraw();
        if (currentModel) {
    ...
    if (entry.type === 'dungeon' || entry.type === 'building') {
        ...
        if (currentDungeonPlan && floorInput && isTower) {
            ...
            if (floorVal)
                floorVal.textContent = `F${floorInput.value}`;
        }
        redrawDungeon();
        if (currentDungeonPlan) {
    ...
    if (entry.type === 'cave') {
        ...
        const ctype = _getLibraryTag(entry, 'ctype') ?? 'cave';
        _setActivePillByDataset('cave-type-pills', 'ctype', ctype);
        _syncCaveTypeSections(ctype);
        if (ctype === 'cave')
            _setActivePillByDataset('cave-biome-pills', 'biome', _getLibraryTag(entry, 'biome'));
        else
            _setActivePillByDataset('glade-biome-pills', 'gbiome', _getLibraryTag(entry, 'biome'));
        currentCaveData = entry.data;
        redrawCave();
        if (currentCaveData) {
...
// ── Toggle ────────────────────────────────────────────────────────────────────
function _setLibraryOpen(open) {
    _libraryOpen = open;
    window.__owStudioLibraryOpen = open;
    ...
    for (const pid of controlPanels) {
        ...
        if (p)
            p.style.display = open ? 'none' : (pid === `${studioMode}-controls` ? '' : 'none');
    }
}
...
document.getElementById('library-type-pills')?.addEventListener('click', (e) => {
    const btn = e.target.closest('[data-ltype]');
    if (!btn)
        return;
    _libraryTypeFilter = btn.dataset.ltype;
