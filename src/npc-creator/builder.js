 ...
 */
export async function buildNpc(dna) {
    ...
    const pDna = {
        ...
        dress: { ...{ style: 'aline', flare: 1, length: 1, trim: false, sash: false, puffSleeves: false }, ...(base.dress ?? {}) },
        face: {
            eyeStyle: 'round',
            eyeSize: 1,
            eyeSpacing: 1,
    ...
    if (!safeDna)
        throw new Error(`[buildNpc] sanitizeDna failed for NPC "${dna.name}"`);
...
export function buildNpcSync(dna) {
    // Build a placeholder immediately; replace async once factory loads
    const placeholder = {
        root: (() => { const { Group } = require('three'); return new Group(); })(),
        dna,
        update: () => { },
        dispose: () => { },
        speak: () => { },
