 ...
 */
import { PLANET_META } from './planet-dna';
// ── Seeded RNG ────────────────────────────────────────────────────────────────
function mulberry(n) {
    let s = (n >>> 0) + 1;
    return () => {
...
function planetName(starName, idx, rng) {
...
// ── Generator ────────────────────────────────────────────────────────────────
export function generateSolarSystem(seed) {
    const rng = mulberry(seed);
    // Star
    const spectralTypes = ['F', 'G', 'G', 'G', 'K', 'K', 'M'];
    ...
    for (let i = 0; i < nPlanets; i++) {
        ...
        let type;
        if (i <= 1) {
            type = ['rocky', 'volcanic', 'desert'][Math.floor(rng() * 3)];
        }
        else if (i <= 4) {
            type = isTower ? 'terran' : ['terran', 'ocean', 'verdant', 'dead'][Math.floor(rng() * 4)];
        }
        else {
            type = ['ice', 'gas_giant', 'ringed', 'dead'][Math.floor(rng() * 4)];
        }
    ...
    const comets = Array.from({ length: nComets }, () => ({
...
export class SolarSystemRenderer {
    ...
    onPlanetClick(cb) { this.onClickPlanet = cb; }
    onPlanetHover(cb) { this.onHoverPlanet = cb; }
    start() {
        if (this.active)
            return;
        this.active = true;
    ...
    resize(w, h) {
        ...
        if (this.data)
            this._buildNebula(this.data.seed); // rebuild nebula for new size
    }
    // ── Private helpers ──────────────────────────────────────────────────────
    _cx() { return this.canvas.width / 2; }
    _cy() { return this.canvas.height / 2; }
    _buildAsteroids(belt) {
