...
const roomEl = document.getElementById('bv-room-name');
...
function loadPlan() {
    ...
    try {
        ...
        if (!data.rooms || !data.startRoomId)
            throw new Error('plan missing rooms or startRoomId');
        return {
        ...
        window.__buildingViewerError = String(e);
        return null;
    }
}
// ── Main ──────────────────────────────────────────────────────────────────────
async function main() {
    ...
    sceneManager.onRoomLoaded = (bp) => {
        ...
        _floorBody = physics.createStaticBox(new THREE.Vector3(0, -0.1, 0), new THREE.Vector3(hw, 0.1, hd));
    ...
    // ── 14. Ready ─────────────────────────────────────────────────────────────────
    window.__buildingViewerReady = true;
    window.__bvRoomId = plan.startRoomId;
