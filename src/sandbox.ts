...
const canvas3d    = document.getElementById('c3d') as HTMLCanvasElement;
const annotCanvas = document.getElementById('annotation-canvas') as HTMLCanvasElement;
...
const groundMesh = new THREE.Mesh(
  new THREE.PlaneGeometry((GHALF * 2 + 2) * T, (GHALF * 2 + 2) * T),
...
const ghostIndicator = new THREE.Mesh(
  new THREE.PlaneGeometry(T * 0.88, T * 0.88),
...
function setPreviewCells(cells: Array<{ col: number; row: number }>) {
  ...
  for (let i = n; i < MAX_PREVIEW; i++) previewPool[i]!.visible = false;
}

function clearPreviewCells() {
  for (let i = 0; i < MAX_PREVIEW; i++) previewPool[i]!.visible = false;
}
...
interface PlacedTile {
  ...
  floor:  number;
  rotY:   number;
  offX:   number; // sub-grid X offset in WU
  offZ:   number; // sub-grid Z offset in WU
  group:  THREE.Group;
}
...
let selectedCodeItem: string | null = null;
let selectedCodeBtn:  HTMLButtonElement | null = null;
let currentRotDeg: 0 | 90 | 180 | 270 = 0;
let currentFloor                   = 0;
let mode: 'pencil' | 'erase' | 'line' | 'rect' = 'pencil';
...
let lineStartPos:  { col: number; row: number } | null = null;
let rectStartPos:  { col: number; row: number } | null = null;

let cursorCol = 0;
let cursorRow = 0;
let cursorInViewport = false;
let ghostLoadedPath: string | null  = null;

// ── Auto-categorized tile catalog from manifest ───────────────────────────────

/** Derive a human-readable sub-category from the GLB filename. */
function autoCategory(pack: string, filename: string): string {
  const n = filename.replace('.glb', '');

  if (pack === 'nature') {
    if (n.startsWith('ground_grass'))  return 'Grass';
    if (n.startsWith('ground_path'))   return 'Paths';
    if (n.startsWith('ground_river'))  return 'Rivers';
    if (n.startsWith('ground_water') || n === 'water') return 'Water';
    if (n.startsWith('tree') || n.startsWith('bush'))  return 'Vegetation';
    if (n.startsWith('rock'))          return 'Rocks';
    return 'Ground';
  }
  if (pack === 'buildings') {
    if (n.startsWith('wall'))         return 'Walls';
    if (n.startsWith('roof'))         return 'Roofs';
    if (n.startsWith('floor') || n.startsWith('wood-floor')) return 'Floors';
    if (n.startsWith('tower'))        return 'Towers';
    if (n.startsWith('stairs'))       return 'Stairs';
    if (n.startsWith('battlement'))   return 'Battlements';
    if (n.startsWith('fence'))        return 'Fences';
    if (n.startsWith('overhang'))     return 'Overhangs';
    if (n.startsWith('dock'))         return 'Dock';
    if (n.startsWith('structure'))    return 'Structure';
    if (n.startsWith('column'))       return 'Columns';
    if (n.startsWith('detail') || n.startsWith('barrel') || n.startsWith('pulley') || n.startsWith('ladder')) return 'Props';
    return 'Misc';
  }
  if (pack === 'town') {
    ...
    if (n.startsWith('roof'))         return 'Roofs';
    if (n.startsWith('stairs'))       return 'Stairs';
    if (n.startsWith('fountain'))     return 'Fountains';
    if (n.startsWith('hedge'))        return 'Vegetation';
    if (n.startsWith('fence'))        return 'Fences';
    if (n.startsWith('banner') || n.startsWith('blade') || n.startsWith('cart') ||
        n.startsWith('lantern') || n.startsWith('rock'))  return 'Props';
    if (n.startsWith('chimney') || n.startsWith('balcony') || n.startsWith('planks') ||
        n.startsWith('poles') || n.startsWith('pillar') || n.startsWith('overhang')) return 'Architecture';
    return 'Misc';
  }
  if (pack === 'castle') {
    if (n.includes('tower-square'))   return 'Square Tower';
    if (n.includes('tower-round'))    return 'Round Tower';
    if (n.includes('tower-pentagon')) return 'Pentagon Tower';
    if (n.startsWith('wall'))         return 'Walls';
  ...
  if (pack === 'dungeon') {
    if (n.startsWith('gate'))         return 'Gates';
    if (n.startsWith('corridor'))     return 'Corridors';
    if (n.startsWith('room'))         return 'Rooms';
    if (n.startsWith('stairs'))       return 'Stairs';
    if (n.startsWith('template'))     return 'Templates';
    return 'Misc';
  }
  return 'Misc';
}
...
interface CatalogGroup { pack: string; subcat: string; color: string; paths: string[] }
...
const collapsedPacks = new Set<string>(); // empty = all packs open
...
function buildPaletteUI() {
  ...
  const byPack = new Map<string, CatalogGroup[]>();
  for (const group of CATALOG) {
    if (!byPack.has(group.pack)) byPack.set(group.pack, []);
  ...
  for (const [pack, groups] of byPack) {
    ...
    ph.addEventListener('click', () => {
      const content = packSection.querySelector<HTMLElement>('.pack-content')!;
      const arrow   = ph.querySelector<HTMLElement>('.ph-arrow')!;
...
const CODE_BUILDINGS: Array<{ key: string; icon: string; name: string; desc: string; footprint: string }> = [
...
const CODE_NATURE: Array<{ key: string; icon: string; name: string; desc: string }> = [
...
function buildCodePaletteUI() {
  ...
  for (const item of CODE_BUILDINGS) {
    ...
    if (item.key === selectedCodeItem) btn.classList.add('selected');
    btn.innerHTML =
...
function filterPalette(query: string) {
  ...
  if (!q) {
    ...
    document.querySelectorAll<HTMLElement>('#palette-scroll .pack-section').forEach(sec => {
      sec.style.display = '';
      const pack    = sec.dataset['pack'] ?? '';
      const content = sec.querySelector<HTMLElement>('.pack-content')!;
      if (content) content.style.display = collapsedPacks.has(pack) ? 'none' : '';
      sec.querySelectorAll<HTMLElement>('.tile-btn').forEach(b => b.style.display = '');
      sec.querySelectorAll<HTMLElement>('.subcat-hdr').forEach(h => h.style.display = '');
    });
    return;
  }
  ...
  document.querySelectorAll<HTMLElement>('#palette-scroll .pack-section').forEach(sec => {
    ...
    sec.querySelectorAll<HTMLElement>('.tile-btn').forEach(btn => {
      const matches = (btn.textContent ?? '').toLowerCase().includes(q);
      btn.style.display = matches ? '' : 'none';
      if (matches) packHasMatch = true;
    });
    sec.querySelectorAll<HTMLElement>('.subcat-hdr').forEach(h => h.style.display = '');
    sec.style.display = packHasMatch ? '' : 'none';
    if (content) content.style.display = packHasMatch ? '' : 'none';
  });
}
...
async function placeTileAt(
  ...
  if (!model) { setStatus(`⚠ Could not clone: ${path}`); return; }
...
function removeTileAt(col: number, row: number, floor: number): boolean {
...
function freeGroup(g: THREE.Group): void {
  g.traverse((child) => {
...
function refreshCodeGhost() {
  ...
  while (ghostTileGroup.children.length > 0) ghostTileGroup.remove(ghostTileGroup.children[0]!);
  ghostLoadedPath = null;

  if (!selectedCodeItem) return;

  let src: THREE.Group | null = null;

  if (selectedCodeItem.startsWith('building:')) {
    const type = selectedCodeItem.slice('building:'.length) as BuildingType;
  ...
  src.traverse(child => {
    ...
    mesh.material = mats.map(mat => {
      ...
      c.transparent = true; c.opacity = 0.35; c.depthWrite = false;
      return c;
    });
  });
...
async function refreshGhostModel(): Promise<void> {
  if (!selectedPath || ghostLoadedPath === selectedPath) return;
  ...
  try {
    ...
    if (ghostLoadedPath !== pathSnapshot) return; // selection changed
...
function _makeProceduralTree(seed: number): THREE.Group {
  ...
  const canopy = new THREE.Mesh(
    new THREE.ConeGeometry(0.55 + rng() * 0.2, 1.2 + rng() * 0.4, 7),
...
function _makeProceduralRock(seed: number): THREE.Group {
  const rng = mulberry32(seed);
  const g   = new THREE.Group();
...
function _makeProceduralBush(seed: number): THREE.Group {
  const rng = mulberry32(seed);
  const g   = new THREE.Group();
...
function _makeProceduralStump(): THREE.Group {
  const g = new THREE.Group();
  const stump = new THREE.Mesh(
    new THREE.CylinderGeometry(0.22, 0.26, 0.28, 7),
    new THREE.MeshStandardMaterial({ color: 0x6a4820, roughness: 0.95 }),
  );
...
function placeCodeItem(col: number, row: number) {
  if (!selectedCodeItem) return;
...
function screenToTile(clientX: number, clientY: number): { col: number; row: number } | null {
...
function updateGhostPosition(col: number, row: number) {
  ...
  (ghostIndicator.material as THREE.MeshBasicMaterial).color.setHex(indicatorColor);
}
...
async function placeMany(cells: Array<{ col: number; row: number }>) {
  // Code mode
  if (selectedCodeItem) {
...
canvas3d.addEventListener('pointermove', (e) => {
  ...
  if (isPointerDown) {
    if (mode === 'pencil' && (snap.col !== lastPaintCol || snap.row !== lastPaintRow)) {
      lastPaintCol = snap.col; lastPaintRow = snap.row;
...
canvas3d.addEventListener('pointerleave', () => {
  ...
  ghostIndicator.visible = false;
  ghostTileGroup.visible = false;
  if (!isPointerDown) clearPreviewCells();
});

canvas3d.addEventListener('pointerdown', (e) => {
  if (e.button !== 0 || annotActive) return;
  const snap = screenToTile(e.clientX, e.clientY);
  if (!snap) return;
...
canvas3d.addEventListener('pointerup', (e) => {
  if (annotActive) { isPointerDown = false; return; }
...
canvas3d.addEventListener('contextmenu', (e) => {
  e.preventDefault();
  const snap = screenToTile(e.clientX, e.clientY);
  if (snap) removeTileAt(snap.col, snap.row, currentFloor);
});

// ── Tool wiring ───────────────────────────────────────────────────────────────

function setMode(m: 'pencil' | 'erase' | 'line' | 'rect') {
  mode = m;
  lineStartPos = null;
  rectStartPos = null;
  clearPreviewCells();
...
function setRotation(deg: 0 | 90 | 180 | 270) {
  currentRotDeg = deg;
  document.querySelectorAll<HTMLElement>('[data-rot]').forEach(btn =>
    btn.classList.toggle('active', btn.dataset['rot'] === String(deg)),
  );
...
function cycleRotation() {
  setRotation(((currentRotDeg + 90) % 360) as 0 | 90 | 180 | 270);
}
...
function setOffset(x: number, z: number) {
  const clamp = (v: number) => Math.round(Math.max(-T, Math.min(T, v)) / OFFSET_STEP) * OFFSET_STEP;
  tileOffX = clamp(x);
  tileOffZ = clamp(z);
  const el = document.getElementById('offset-display');
  if (el) el.textContent = `X: ${tileOffX.toFixed(2)}  Z: ${tileOffZ.toFixed(2)}`;
...
document.getElementById('btn-clear')!.addEventListener('click',  () => {
  if (confirm('Clear all tiles?')) clearAll();
});
...
document.querySelectorAll<HTMLElement>('[data-rot]').forEach(btn =>
  btn.addEventListener('click', () =>
    setRotation(parseInt(btn.dataset['rot'] ?? '0', 10) as 0 | 90 | 180 | 270),
  ),
);

document.querySelectorAll<HTMLElement>('[data-cam]').forEach(btn =>
  btn.addEventListener('click', () =>
    setCameraPreset(btn.dataset['cam'] as 'iso' | 'top' | 'front' | 'side'),
  ),
);

// ── Camera presets ────────────────────────────────────────────────────────────

function setCameraPreset(preset: 'iso' | 'top' | 'front' | 'side') {
  const d = 26;
  const pos: Record<typeof preset, [number, number, number]> = {
...
window.addEventListener('keydown', (e) => {
  ...
  switch (e.code) {
    case 'KeyP':  setMode('pencil');  break;
    case 'KeyX':  setMode('erase');   break;
    case 'KeyL':  setMode('line');    break;
    case 'KeyB':  setMode('rect');    break;
    case 'KeyR':  cycleRotation();    break;
    case 'KeyQ':  setFloor(currentFloor - 1); break;
    case 'KeyE':  setFloor(currentFloor + 1); break;
    case 'Escape':
      // Cancel line / rect start
      if (lineStartPos || rectStartPos) {
        lineStartPos = null; rectStartPos = null;
        clearPreviewCells();
        setStatus('Cancelled');
      }
      break;
    case 'ArrowLeft':  e.preventDefault(); setOffset(tileOffX - OFFSET_STEP, tileOffZ); break;
    case 'ArrowRight': e.preventDefault(); setOffset(tileOffX + OFFSET_STEP, tileOffZ); break;
    case 'ArrowUp':    e.preventDefault(); setOffset(tileOffX, tileOffZ - OFFSET_STEP); break;
    case 'ArrowDown':  e.preventDefault(); setOffset(tileOffX, tileOffZ + OFFSET_STEP); break;
    case 'Delete':
    case 'Backspace':
      if (cursorInViewport) removeTileAt(cursorCol, cursorRow, currentFloor);
      break;
    case 'KeyZ':
...
function toggleAnnotation() {
  ...
  (document.getElementById('annot-toggle') as HTMLInputElement).checked = annotActive;
...
document.getElementById('annot-toggle')!.addEventListener('change', (e) => {
  annotActive = (e.target as HTMLInputElement).checked;
  annotCanvas.classList.toggle('active', annotActive);
});
...
annotCanvas.addEventListener('pointermove', (e) => {
  if (!isDrawing || !annotActive) return;
  annotCtx.beginPath();
...
function updateHUD() {
  ...
  (document.getElementById('hud-mode')  as HTMLElement).textContent = modeLabel[mode];
  let name = '— none —';
  if (selectedPath)     name = selectedPath.split('/').pop()!;
  if (selectedCodeItem) name = selectedCodeItem.replace('building:', '').replace('nature:', '').replace('_', ' ');
  (document.getElementById('hud-tile')  as HTMLElement).textContent = name;
  (document.getElementById('hud-floor') as HTMLElement).textContent = `Floor ${currentFloor}`;
  (document.getElementById('hud-rot')   as HTMLElement).textContent = `${currentRotDeg}°`;
}

function setStatus(msg: string) {
  (document.getElementById('status-bar') as HTMLElement).textContent = msg;
}

function captureThumb(): string {
  renderer.render(scene, camera);
  try { return canvas3d.toDataURL('image/jpeg', 0.55); } catch { return ''; }
}
...
interface SavedTile {
  path: string; col: number; row: number; floor: number; rotY: number;
  offX?: number; offZ?: number;
}

interface SavedCreation {
  id: string; name: string; folder: string;
  tiles: SavedTile[]; thumbnail: string;
...
class CreationStore {
  private items: SavedCreation[] = [];
  constructor() { this._load(); }
  private _load() {
    try { this.items = JSON.parse(localStorage.getItem(STORE_KEY) ?? '[]') as SavedCreation[]; }
    catch { this.items = []; }
  }
  private _persist() { localStorage.setItem(STORE_KEY, JSON.stringify(this.items)); }
  getAll():             readonly SavedCreation[]     { return this.items; }
  get(id: string):      SavedCreation | undefined    { return this.items.find(c => c.id === id); }
  save(name: string, folder: string, tiles: SavedTile[], thumbnail: string): SavedCreation {
    const now = Date.now();
    const c: SavedCreation = {
  ...
  update(id: string, patch: Partial<Pick<SavedCreation, 'name' | 'folder' | 'tiles' | 'thumbnail'>>): boolean {
  ...
  delete(id: string): boolean {
    ...
    if (i === -1) return false;
    this.items.splice(i, 1); this._persist(); return true;
  }
  buildTree(): FolderNode {
    const root: FolderNode = { name: '', path: '', children: new Map(), items: [] };
    for (const c of this.items) {
      if (!c.folder) { root.items.push(c); continue; }
      const parts = c.folder.split('/').filter(Boolean);
      let node = root; let sofar = '';
...
class CreationsPanel {
  ...
  private openFolders: Set<string> = new Set();
  private onLoad: (c: SavedCreation) => void;
  private onEdit: (c: SavedCreation) => void;

  constructor(store: CreationStore, el: HTMLElement,
              onLoad: (c: SavedCreation) => void, onEdit: (c: SavedCreation) => void) {
    this.store = store; this.container = el;
    this.onLoad = onLoad; this.onEdit = onEdit;
    this.refresh();
  }
  ...
  private _count(n: FolderNode): number {
    return n.items.length + [...n.children.values()].reduce((s, c) => s + this._count(c), 0);
  }

  private _renderNode(node: FolderNode, parent: HTMLElement, depth: number) {
    for (const [, child] of node.children) {
      ...
      if (open) this._renderNode(child, parent, depth + 1);
    }
    for (const c of node.items) {
      ...
      el.addEventListener('click', (e) => {
        if ((e.target as HTMLElement).closest('.ci-actions')) return;
        this.onLoad(c);
      });
      (el.querySelector('.ci-edit') as HTMLElement).addEventListener('click', (e) => {
        e.stopPropagation(); this.onEdit(c);
      });
      (el.querySelector('.ci-del') as HTMLElement).addEventListener('click', (e) => {
        e.stopPropagation();
        if (confirm(`Delete "${c.name}"?`)) {
          this.store.delete(c.id); this.refresh();
...
(window as any).__sandbox = {
  ...
  async assembleBuilding(type: BuildingType, seed = 1, col = 0, row = 0) {
    await _placeBuilding(type, seed, col, row);
  },

  /** Move camera to an arbitrary world position. */
  setCamera(x: number, y: number, z: number, lx = 0, ly = 0, lz = 0) {
  ...
  setCameraPreset(preset: 'iso' | 'top' | 'front' | 'side') {
...
async function init() {
  ...
  const codeModeBtn  = document.getElementById('sb-code-btn')  as HTMLButtonElement;
  const assetModeBtn = document.getElementById('sb-asset-btn') as HTMLButtonElement;
  const packsWrap    = document.getElementById('sb-packs-wrap') as HTMLElement;
  ...
  function applyAssetModeToUI() {
    ...
    const searchInput = document.getElementById('tile-search') as HTMLInputElement;
    ...
    packsWrap.querySelectorAll<HTMLInputElement>('input[type=checkbox]').forEach(cb => {
    ...
    if (isKenney) {
      for (const pack of Object.keys(ASSET_PACKS)) {
        if (!cfg.assetPacks.includes(pack)) collapsedPacks.add(pack);
      }
      buildPaletteUI();
    } else {
      buildCodePaletteUI();
  ...
  packsWrap.querySelectorAll<HTMLInputElement>('input[type=checkbox]').forEach(cb => {
    cb.addEventListener('change', () => {
      const cfg = loadWorldGenConfig();
      cfg.assetPacks = [...packsWrap.querySelectorAll<HTMLInputElement>('input:checked')]
        .map(c => c.value);
      saveWorldGenConfig(cfg);
      applyAssetModeToUI();
    });
  ...
  function setActiveTab(tab: 'tiles' | 'creations') {
    const tilesEl   = document.getElementById('palette-scroll')!;
    const creatEl   = document.getElementById('creation-scroll')!;
    const searchEl  = document.getElementById('tile-search') as HTMLInputElement;
  ...
  // Search filter
  (document.getElementById('tile-search') as HTMLInputElement).addEventListener('input', (e) => {
  ...
  let saveEditId: string | null = null;

  function openSaveModal(existing?: SavedCreation) {
    saveEditId = existing?.id ?? null;
    (document.getElementById('sm-name')   as HTMLInputElement).value  = existing?.name   ?? '';
    (document.getElementById('sm-folder') as HTMLInputElement).value  = existing?.folder ?? '';
    (document.getElementById('sm-title')  as HTMLElement).textContent = existing ? 'Edit Creation' : 'Save Creation';
    (document.getElementById('sm-save-btn') as HTMLElement).textContent = existing ? 'Update' : 'Save';
    const thumb = existing?.thumbnail ?? captureThumb();
    const count = existing?.tiles.length ?? placedTiles.length;
    (document.getElementById('sm-thumb') as HTMLElement).innerHTML =
      thumb ? `<img src="${thumb}" style="max-width:110px;max-height:72px;border-radius:4px;"/>` : '';
    (document.getElementById('sm-count') as HTMLElement).textContent =
      `${count} tile${count !== 1 ? 's' : ''}`;
    document.getElementById('save-modal')!.classList.remove('hidden');
    setTimeout(() => (document.getElementById('sm-name') as HTMLInputElement).focus(), 40);
  }
  ...
  const creationPanel = new CreationsPanel(
    creationStore,
    document.getElementById('creation-scroll')!,
    // onLoad — clear scene and restore all tiles
    async (creation) => {
      if (!confirm(`Load "${creation.name}"?\nThis will clear the current scene.`)) return;
      clearAll();
      setStatus(`Loading "${creation.name}"…`);
      for (const t of creation.tiles) await placeTileAt(t.path, t.col, t.row, t.floor, t.rotY, t.offX ?? 0, t.offZ ?? 0);
  ...
  document.getElementById('sm-save-btn')!.addEventListener('click', () => {
    const name   = (document.getElementById('sm-name')   as HTMLInputElement).value.trim();
    const folder = (document.getElementById('sm-folder') as HTMLInputElement).value.trim();
    if (!name) { (document.getElementById('sm-name') as HTMLInputElement).focus(); return; }
    if (saveEditId) {
  ...
  [document.getElementById('sm-name')!, document.getElementById('sm-folder')!].forEach(inp =>
    inp.addEventListener('keydown', (e) => {
      if (e.key === 'Enter')  (document.getElementById('sm-save-btn') as HTMLButtonElement).click();
      if (e.key === 'Escape') closeSaveModal();
    }),
  );

  document.getElementById('btn-save-creation')!.addEventListener('click', () => {
    if (placedTiles.length === 0) { setStatus('Nothing to save — place some tiles first!'); return; }
    openSaveModal();
  });
  ...
  Object.assign((window as any).__sandbox, {
    saveCreation(name: string, folder = '') {
    ...
    async loadCreation(id: string) {
      const c = creationStore.get(id);
      if (!c) return false;
      clearAll();
      for (const t of c.tiles) await placeTileAt(t.path, t.col, t.row, t.floor, t.rotY, t.offX ?? 0, t.offZ ?? 0);
      return true;
    },
    listCreations() { return [...creationStore.getAll()]; },
    deleteCreation(id: string) {
      const ok = creationStore.delete(id);
      if (ok) creationPanel.refresh();
      return ok;
    },
  });
  ...
  let _slimeIM: THREE.InstancedMesh | null = null;
  const _slimeDummy  = new THREE.Object3D();
  const _slimeColor  = new THREE.Color();

  function _refreshSlimeStats(count: number) {
  ...
  function _spawnSlimesInSandbox(count: number) {
    if (_slimeIM) { scene.remove(_slimeIM); _slimeIM = null; }
    if (count <= 0) { _refreshSlimeStats(0); return; }

    _slimeIM = createSlimeBodyIM(count);

    // Spiral layout so they're all visible from the iso camera
    const COLORS = [0x44bb55, 0xffdd44, 0x9955ff] as const; // hostile / flee / recruit
    ...
    if (_slimeIM.instanceColor) _slimeIM.instanceColor.needsUpdate = true;
  ...
  document.getElementById('btn-spawn-slimes')!.addEventListener('click', () => {
    const n = parseInt((document.getElementById('slime-count') as HTMLInputElement).value, 10);
  ...
  (window as any).__sandbox.slime = {
     ...
     */
    getStats(): { count: number; frustumCulled: boolean | null; drawCalls: number } {
      if (!_slimeIM) return { count: 0, frustumCulled: null, drawCalls: 0 };
      return {
