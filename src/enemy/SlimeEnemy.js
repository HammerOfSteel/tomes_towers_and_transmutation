...
import { PatrolBehavior } from '@/enemy/PatrolBehavior';
 ...
 */
export function createSlimeBodyIM(maxInstances) {
...
export class SlimeEnemy {
    ...
    _knockbackDir = new THREE.Vector3();
    _knockbackSpeed = 0;
    ...
    _modelFadeTimer = -1.0;
    MODEL_FADE_DURATION = 1.5; // seconds
    constructor(spawnPosition, physics, onAttackPlayer) {
        this.onAttackPlayer = onAttackPlayer;
        // Physics — kinematic capsule so it can be moved manually like the player
        const { body, collider } = physics.createKinematicCapsule(spawnPosition, SLIME_RADIUS * 0.5, SLIME_RADIUS);
        this.body = body;
        ...
        this.health = new HealthComponent(SLIME_HP, SLIME_IFRAME_DURATION, () => this.onHit(), () => this.onDead());
     ...
     */
    forceFlee() {
        if (this.state === 'dead' || this.state === 'recruited')
            return;
        this.state = 'flee';
        this.bodyMesh.material.color.setHex(SLIME_FLEE_COLOR);
    }
    // ── B4: patrol + aggro ────────────────────────────────────────────────────
    /**
    ...
    recruit() {
        ...
        this._guardPos = null;
        this.bodyMesh.material.color.setHex(SLIME_RECRUIT_COLOR);
    }
    /** True while this slime is stationed at a Watch Perch. */
    get isGuarding() { return this.state === 'guard'; }
    /**
     ...
     */
    assignGuard(perchPos) {
        this.state = 'guard';
        this._guardPos = perchPos.clone();
        this._followerTarget = null;
        this.bodyMesh.material.color.setHex(SLIME_GUARD_COLOR);
    }
    ...
    update(playerPos, dt) {
        ...
        if (this._patrol && !this.isDead && this.state !== 'flee' && this.state !== 'recruited') {
            ...
            if (pout.state === 'dead') {
                ...
                if (pout.justDetected)
                    AggroSystem.instance.shout(this);
            }
        ...
        if (this._isTaming) {
            ...
            if (this.flashTimer <= 0 && this.tameReactTimer <= 0) {
                ...
                this.bodyMesh.material.color.setRGB(0.52 + pulse * 0.08, 0.28 + pulse * 0.05, 0.72 + pulse * 0.1);
            }
        }
     ...
     */
    updateAsFollower(playerPos, hostileHash, dt) {
        if (this.isDead) {
            this._tickDeathAnim(dt);
            return;
        }
        if (this.state === 'guard') {
            this._tickGuard(hostileHash, dt);
            return;
        }
        ...
        if (!this._followerTarget) {
            const candidates = hostileHash.queryRadius(this._pos.x, this._pos.z, FOLLOWER_AGGRO_RANGE);
    ...
    _tickGuard(hostileHash, dt) {
        ...
        if (!this._followerTarget) {
            const candidates = hostileHash.queryRadius(this._guardPos.x, this._guardPos.z, GUARD_AGGRO_RANGE);
    ...
    applyDeathKnockback(fromPos, speed = 4.0) {
        this._knockbackDir.subVectors(this.worldPosition, fromPos).setY(0);
        if (this._knockbackDir.lengthSq() < 0.001)
            this._knockbackDir.set(1, 0, 0);
    ...
    onDead() {
        ...
        this.bodyMesh.material.color.setHex(SLIME_COLOR);
        ...
        for (let i = 0; i < DEATH_CHUNK_COUNT; i++) {
            ...
            chunk.position.set((_slimeRand() - 0.5) * 0.4, 0.1 + _slimeRand() * 0.25, (_slimeRand() - 0.5) * 0.4);
            this.group.add(chunk);
            const angle = (i / DEATH_CHUNK_COUNT) * Math.PI * 2 + (_slimeRand() - 0.5) * 0.7;
            const speed = 3.0 + _slimeRand() * 3.5;
            this._deathChunks.push({
                mesh: chunk,
                vel: new THREE.Vector3(Math.cos(angle) * speed, 2.0 + _slimeRand() * 3.0, Math.sin(angle) * speed),
            });
        }
    }
    ...
    _tickDeathAnim(dt) {
        ...
        for (const { mesh, vel } of this._deathChunks) {
            ...
            if (t > 0.35) {
                ...
                mesh.material.opacity = Math.max(0, 1 - cf);
            }
        }
    ...
    writeToIM(im, idx) {
        ...
        im.setColorAt(idx, _imTmpC.set(this.bodyMesh.material.color));
    }
    static buildMesh() {
...
export const SLIME_SPAWN_POSITIONS = [
