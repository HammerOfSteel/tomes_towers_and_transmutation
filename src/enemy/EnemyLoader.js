 ...
 */
function resolveClips(clips, mixer) {
    const result = {
    ...
    for (const key of Object.keys(CLIP_PATTERNS)) {
        ...
        if (clip)
            result[key] = mixer.clipAction(clip);
    }
    return result;
}
// ── EnemyLoader ───────────────────────────────────────────────────────────────
 ...
 */
export async function loadEnemyModel(def, spawnPos) {
    ...
    group.position.set(spawnPos.x, spawnPos.y - centredBox.min.y, spawnPos.z);
 ...
 */
export function disposeEnemyRig(rig) {
 ...
 */
export function getEnemyManifestEntry(enemyId) {
 ...
 */
export function getEnemyModelDef(enemyId) {
