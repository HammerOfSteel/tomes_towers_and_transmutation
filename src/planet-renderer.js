...
export class PlanetRenderer {
    ...
    loadPlanet(tex) {
        ...
        for (const s of tex.settlements) {
            ...
            const pos = new THREE.Vector3(r * Math.cos(lat) * Math.sin(lon), r * Math.sin(lat), r * Math.cos(lat) * Math.cos(lon));
    ...
    start() {
        ...
        const loop = () => {
            ...
            if (this.planetMesh)
                this.planetMesh.rotation.y = t * 0.03;
    ...
    setVisible(layer, show) {
        if (layer === 'clouds' && this.cloudMesh)
...
export function buildDayTexture(cells, W, H) {
    const tw = 512, th = 256;
...
export function buildNightTexture(settlements, W, H) {
    const tw = 256, th = 128;
    const c = document.createElement('canvas');
    c.width = tw;
    c.height = th;
...
export function buildSpecularTexture(cells, W, H) {
    const tw = 256, th = 128;
    const c = document.createElement('canvas');
    c.width = tw;
    c.height = th;
 ...
 */
export function buildCloudTexture(seed) {
    const TW = 512, TH = 256;
    ...
    const vn = (x, y, g, gw, gh) => valueNoise2D(x, y, g, gw, gh);
    /** Ridged noise: sharp peaks like cumulus towers. */
    const ridged = (x, y, g, gw, gh) => 1 - Math.abs(vn(x, y, g, gw, gh) * 2 - 1);
    const c = document.createElement('canvas');
    c.width = TW;
    c.height = TH;
    const ctx = c.getContext('2d');
...
export function buildSpecularCloudTexture(cells, W, H, seed) {
    const tw = 256, th = 128;
    const c = document.createElement('canvas');
    c.width = tw;
    c.height = th;
