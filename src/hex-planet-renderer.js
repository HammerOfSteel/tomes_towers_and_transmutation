...
import { PLANET_BIOME_COLORS, getGasGiantBandColor } from './planet-dna';
...
function makeGrid(GW, GH, rand) {
    const g = new Float32Array(GW * GH);
    for (let i = 0; i < g.length; i++)
        g[i] = rand();
    return g;
}
function valueNoise2D(x, y, g, GW, GH) {
...
function buildContinentMask(shape, rand) {
    if (shape === 'continents') {
...
export class HexPlanetRenderer {
    ...
    loadPlanet(s) {
        ...
        for (const { group } of this.moonGroups)
        ...
        const BASE_R = 2.0;
        const hs = new Hexasphere(BASE_R, s.subdivisions, 0.96);
        ...
        function sampleCell(cx, cy, cz, len) {
        ...
        const HexasphereFine = Hexasphere;
        ...
        for (const tile of hs_fine.tiles) {
            ...
            if (isGasGiant) {
                ...
                for (let ti2 = 0; ti2 < N; ti2++) {
                    ...
                    pos2.push(cx, cy2, cz, b0.x * s0, b0.y * s0, b0.z * s0, b1.x * s1, b1.y * s1, b1.z * s1);
        ...
        for (const settle of s.settlements) {
            ...
            const pos = new THREE.Vector3(r2 * Math.cos(lat) * Math.sin(lon), r2 * Math.sin(lat), r2 * Math.cos(lat) * Math.cos(lon));
    ...
    start() {
        ...
        const loop = () => {
            ...
            // Animate moons
            for (const { group, speed } of this.moonGroups)
            ...
            if (this.labelObjs.length > 0) {
                ...
                for (const obj of this.labelObjs) {
                    ...
                    obj.element.style.opacity = camDir.dot(wp.normalize()) > 0.05 ? '1' : '0';
                }
            }
    ...
    stop() { this.active = false; cancelAnimationFrame(this.raf); this.labelRenderer.domElement.style.visibility = 'hidden'; }
    setVisible(layer, show) {
        if (layer === 'atmosphere' && this.atmosMesh)
            this.atmosMesh.visible = show;
    }
    setDayOnly(enabled) {
