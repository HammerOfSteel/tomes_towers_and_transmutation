...
export class WallOcclusionManager {
    ...
    _hiddenCount = 0;
    _wPos = new THREE.Vector3();
    _wNDC = new THREE.Vector3();
    _camPos = new THREE.Vector3();
    // Three NDC sample points on the player (feet, waist, head)
    _pFeet = new THREE.Vector3();
    _pWaist = new THREE.Vector3();
    _pHead = new THREE.Vector3();
    constructor() {
        console.log('[WallOcclusion] initialized');
        window.__wallOccluded = 0;
    }
    /**
     * Call once per frame.
     *
     ...
     *                      meshes are tested.
     */
    update(camera, playerGroup, roomGroup) {
        // ── Restore previous frame's hidden objects ───────────────────────────
        const prevCount = this._hidden.size;
        for (const mesh of this._hidden)
            mesh.visible = true;
        this._hidden.clear();
        if (DEBUG && prevCount > 0)
            console.debug(`[WallOcclusion] restored ${prevCount}`);
        if (!roomGroup)
            return;
        // ── Collect tagged meshes (walls + pillars share userData.isWall = true) ─
        const wallMeshes = [];
        roomGroup.traverse((obj) => {
            if (obj.isMesh && obj.userData.isWall === true) {
                wallMeshes.push(obj);
            }
        });
        ...
        for (const mesh of wallMeshes) {
            ...
            if (wallCamDist >= playerCamDist - DEPTH_MARGIN)
                continue;
        ...
        window.__wallOccluded = this._hiddenCount;
    ...
    reset() {
        ...
        window.__wallOccluded = 0;
        if (count > 0)
            console.log(`[WallOcclusion] reset — restored ${count}`);
    }
}
