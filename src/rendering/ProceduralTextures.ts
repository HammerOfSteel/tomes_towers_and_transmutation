 ...
 */
export function makeHeraldStoneTexture(seed = 31): THREE.CanvasTexture {
  const SIZE = 256;
  const canvas = makeCanvas(SIZE);
  const ctx = getCtx(canvas);
  const rand = mulberry32(seed);
  ...
  for (const [tx, ty] of [[TILE/2, TILE/2], [TILE*1.5, TILE/2], [TILE/2, TILE*1.5], [TILE*1.5, TILE*1.5]]) {
    ...
    ctx.lineTo(tx, ty + s); ctx.lineTo(tx - s, ty);
 ...
 */
export function makeDampStoneTexture(seed = 55): THREE.CanvasTexture {
  const SIZE = 256;
  const canvas = makeCanvas(SIZE);
  const ctx = getCtx(canvas);
  const rand = mulberry32(seed);
 ...
 */
export function makeGrassTexture(seed = 61): THREE.CanvasTexture {
  const SIZE = 256;
  const canvas = makeCanvas(SIZE);
  const ctx = getCtx(canvas);
 ...
 */
export function makeFloorPlanksTexture(seed = 77): THREE.CanvasTexture {
  const SIZE     = 256;
  const canvas   = makeCanvas(SIZE);
  const ctx      = getCtx(canvas);
  const rand     = mulberry32(seed);
 ...
 */
export function makeAlchemyStoneTexture(seed = 13): THREE.CanvasTexture {
  const SIZE = 256;
  const canvas = makeCanvas(SIZE);
  const ctx = getCtx(canvas);
  const rand = mulberry32(seed);
 ...
 */
export function makeScorchedStoneTexture(seed = 71): THREE.CanvasTexture {
  const SIZE = 256;
  const canvas = makeCanvas(SIZE);
  const ctx = getCtx(canvas);
  const rand = mulberry32(seed);
 ...
 */
export function makeSealedStoneTexture(seed = 89): THREE.CanvasTexture {
  const SIZE = 256;
  const canvas = makeCanvas(SIZE);
  const ctx = getCtx(canvas);
  const rand = mulberry32(seed);
 ...
 */
export function makeCelestialStoneTexture(seed = 97): THREE.CanvasTexture {
  const SIZE = 256;
  const canvas = makeCanvas(SIZE);
  const ctx = getCtx(canvas);
  const rand = mulberry32(seed);
...
// ═══════════════════════════════════════════════════════════════════════════════
// World Editor — procedural material preset system
// ═══════════════════════════════════════════════════════════════════════════════
...
export interface EditorTexturePreset {
  id: string;
  label: string;
  emoji: string;
  defaultColor: string;
...
export const TEXTURE_PRESETS: EditorTexturePreset[] = [
  {
...
function _editorSeed(colorHex: string, p: Record<string, number>): number {
  let h = parseInt(colorHex.replace('#', ''), 16) ^ 0xBEEF_CAFE;
  for (const [k, v] of Object.entries(p)) {
    for (let i = 0; i < k.length; i++) h ^= k.charCodeAt(i) << (i % 24);
    h ^= Math.round(v * 1000);
...
function _editorRng(seed: number): () => number {
  let s = seed >>> 0;
  return (): number => {
    s ^= s << 13; s ^= s >>> 17; s ^= s << 5;
...
function _fill(ctx: CanvasRenderingContext2D, r: number, g: number, b: number, size: number): void {
...
function _genWood(ctx: CanvasRenderingContext2D, size: number, color: string, p: Record<string, number>, rng: () => number): void {
...
function _genStone(ctx: CanvasRenderingContext2D, size: number, color: string, p: Record<string, number>, rng: () => number): void {
  const [r, g, b] = _parseHex(color);
  _fill(ctx, r, g, b, size);
...
function _genFabric(ctx: CanvasRenderingContext2D, size: number, color: string, p: Record<string, number>, rng: () => number): void {
  const [r, g, b] = _parseHex(color);
  _fill(ctx, r, g, b, size);
  ...
  if (worn > 0.05) {
    for (let w = 0; w < Math.round(worn * 12); w++) {
      ...
      ctx.beginPath(); ctx.ellipse(wx, wy, wr, wr*0.5, rng()*Math.PI, 0, Math.PI*2); ctx.fill();
    }
  }
}

function _genMetal(ctx: CanvasRenderingContext2D, size: number, color: string, p: Record<string, number>, rng: () => number): void {
  const [r, g, b] = _parseHex(color);
  const hi = (v: number): number => Math.min(255, v + 55);
  const lo = (v: number): number => Math.max(0,   v - 40);
  ...
  for (let i = 0; i < scratchCount; i++) {
    ...
    ctx.beginPath();
    if (horiz) { ctx.moveTo(0, pos); ctx.lineTo(size, pos + wobble); }
    else        { ctx.moveTo(pos, 0); ctx.lineTo(pos + wobble, size); }
    ctx.stroke();
  }
...
function _genMarble(ctx: CanvasRenderingContext2D, size: number, color: string, p: Record<string, number>, rng: () => number): void {
  const [r, g, b] = _parseHex(color);
  ...
  for (let v = 0; v < veinCount; v++) {
    ...
    ctx.bezierCurveTo(x1, size*0.33, x2, size*0.66, x3, size); ctx.stroke();
    if (branch && rng() > 0.5) {
...
function _genLeather(ctx: CanvasRenderingContext2D, size: number, color: string, p: Record<string, number>, rng: () => number): void {
  const [r, g, b] = _parseHex(color);
  _fill(ctx, r, g, b, size);
  ...
  for (let x = 0; x < size; x += sp) {
    for (let y = 0; y < size; y += sp) {
      ...
      ctx.beginPath(); ctx.ellipse(x+ox, y+oy, poreR*0.7, poreR*0.45, rng()*Math.PI, 0, Math.PI*2); ctx.fill();
    }
  }
  ...
  if (tooled > 0.1) {
    ...
    for (let i = 0; i < tLines; i++) {
      ctx.beginPath(); ctx.moveTo(i*size/tLines, 0); ctx.lineTo(0, i*size/tLines); ctx.stroke();
    }
  }
  ...
  if (worn > 0.05) {
    for (let w = 0; w < Math.round(worn * 15); w++) {
      ...
      ctx.beginPath(); ctx.ellipse(wx, wy, wr, wr*0.4, rng()*Math.PI, 0, Math.PI*2); ctx.fill();
    }
  }
}

function _genPainted(ctx: CanvasRenderingContext2D, size: number, color: string, p: Record<string, number>, rng: () => number): void {
  const [r, g, b] = _parseHex(color);
  _fill(ctx, r, g, b, size);
  ...
  for (let i = 0; i < strokeCount; i++) {
    ...
    ctx.save(); ctx.translate(sx, sy); ctx.rotate(rng()*Math.PI);
  ...
  if (worn > 0.05) {
    for (let c = 0; c < Math.round(worn * 20); c++) {
      ...
      ctx.beginPath(); ctx.ellipse(cx, cy, cw, cw*(0.3+rng()*0.4), rng()*Math.PI, 0, Math.PI*2); ctx.fill();
    }
  }
}
...
export function buildMaterial(
  presetId: string,
  texParams: Record<string, number>,
  colorHex: string,
  ...
  switch (presetId) {
    case 'wood':    _genWood(ctx, SIZE, colorHex, texParams, rng);    break;
    case 'stone':   _genStone(ctx, SIZE, colorHex, texParams, rng);   break;
    case 'fabric':  _genFabric(ctx, SIZE, colorHex, texParams, rng);  break;
    case 'metal':   _genMetal(ctx, SIZE, colorHex, texParams, rng);   break;
    case 'marble':  _genMarble(ctx, SIZE, colorHex, texParams, rng);  break;
    case 'leather': _genLeather(ctx, SIZE, colorHex, texParams, rng); break;
    case 'painted': _genPainted(ctx, SIZE, colorHex, texParams, rng); break;
    default: {
  ...
  switch (presetId) {
    case 'wood':    mat.roughness = 0.88 - (texParams['polish'] ?? 0) * 0.55; mat.metalness = 0; break;
    case 'stone':   mat.roughness = 0.92;                                       mat.metalness = 0; break;
    case 'fabric':  mat.roughness = 0.95;                                       mat.metalness = 0; break;
    case 'metal':   mat.roughness = Math.max(0.05, 0.5 - (texParams['polish'] ?? 0.5) * 0.45);
                    mat.metalness = 0.75 + (texParams['polish'] ?? 0.5) * 0.2; break;
    case 'marble':  mat.roughness = texParams['roughness'] ?? 0.15;             mat.metalness = 0; break;
    case 'leather': mat.roughness = 0.78;                                       mat.metalness = 0; break;
    case 'painted': mat.roughness = texParams['roughness'] ?? 0.65;             mat.metalness = 0; break;
    default:        mat.roughness = texParams['roughness'] ?? 0.75;
...
export function buildEmissiveMaterial(colorHex: string, intensity = 1.8): THREE.MeshStandardMaterial {
