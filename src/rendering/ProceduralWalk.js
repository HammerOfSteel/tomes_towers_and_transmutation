...
const BOB_AMP = 0.030;
const LEG_KEYS = ['frontLegL', 'frontLegR', 'backLegL', 'backLegR'];
...
export class ProceduralWalkController {
    ...
    get naturalFootY() { return this._naturalFootY; }
    /** Whether this rig can use procedural walking (needs all 4 leg bones). */
    get isApplicable() {
        return (this._bones.frontLegL != null &&
     ...
     */
    update(dt, bodyPos, bodyYaw) {
        if (!this.isApplicable || dt <= 0)
            return;
        ...
        for (const key of LEG_KEYS) {
            ...
            const drift = Math.sqrt((s.foot.x - _v1.x) ** 2 + (s.foot.z - _v1.z) ** 2);
    ...
    getFootPositions() {
        ...
        for (const key of LEG_KEYS) {
            const s = this._states.get(key);
            if (s)
                out[key] = s.foot.clone();
        }
        return out;
    }
    // ── Private helpers ────────────────────────────────────────────────────────
    _diagonalStepping(key) {
    ...
    _computeIdeal(key, pos, yaw, scale, out) {
        ...
        out.set((sx * cosY - sz * sinY) * scale + pos.x, pos.y + this._naturalFootY * scale, (sx * sinY + sz * cosY) * scale + pos.z);
    }
}
