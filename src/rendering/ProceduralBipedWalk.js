...
export class ProceduralBipedWalkController {
    ...
    _gaitPhase = 0; // 0..1, increments with speed
    _torsoBaseY = 0;
    _inited = false;
    ...
    _idealFoot(side, pos, yaw, scale, out) {
        const xSign = side === 'L' ? -1 : 1;
        const spread = this._restX * scale * 1.15; // slightly wider than hip for stable stance
        const cosY = Math.cos(yaw), sinY = Math.sin(yaw);
        out.set(pos.x + xSign * spread * cosY, pos.y + this._naturalFootY * scale, pos.z + xSign * spread * sinY);
    }
    // ── Main update ───────────────────────────────────────────────────────────
    update(dt, bodyPos, bodyYaw) {
        ...
        if (torso) {
            const bob = speed > 0.1
                ...
                : 0;
            torso.position.y = THREE.MathUtils.lerp(torso.position.y, this._torsoBaseY + bob, Math.min(1, dt * 8));
        }
        if (speed > 0.1) {
        ...
        if (this._bones.armL)
            this._bones.armL.rotation.x = armSwing;
        if (this._bones.armR)
            this._bones.armR.rotation.x = -armSwing;
    }
    ...
    reset() {
        ...
        this._gaitPhase = 0;
        if (this._bones.armL)
            this._bones.armL.rotation.x = 0.06;
        if (this._bones.armR)
            this._bones.armR.rotation.x = 0.06;
    }
}
