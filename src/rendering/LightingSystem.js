...
export class LightingSystem {
    ...
    _ambientFadeFrom = 0;
    _ambientFadeTo = 0;
    _ambientFadeDuration = 0;
    _ambientFadeElapsed = 0;
    constructor(scene) {
        this._scene = scene;
    ...
    clearTorches() {
        for (const t of this._torches)
            this._scene.remove(t.light);
        this._torches.length = 0;
        for (const g of this._flameGroups)
            this._scene.remove(g);
        for (const g of this._flameGeos)
            g.dispose();
        for (const m of this._flameMats)
            m.dispose();
     ...
     */
    fadeAmbientIn(fromIntensity, duration) {
    ...
    applyPreset(preset) {
        ...
        for (const t of this._torches)
            t.light.color.setHex(p.torchColor);
    }
    // ── Per-frame update ─────────────────────────────────────────────────────
    update(dt) {
        this._t += dt;
