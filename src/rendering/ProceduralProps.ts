...
export function buildBed(
  ...
  for (const [lx, lz] of [[-0.52, -0.95], [0.52, -0.95], [-0.52, 0.95], [0.52, 0.95]] as [number, number][]) {
...
export function buildTable(
  ...
  const lx = w / 2 - 0.08;
  const lz = d / 2 - 0.08;
  for (const [px, pz] of [[-lx, -lz], [lx, -lz], [-lx, lz], [lx, lz]] as [number, number][]) {
 ...
 * Total height ≈ 1.0 WU.
 */
export function buildChair(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();
  const SEAT_H = 0.46;
 ...
 */
export function buildWardrobe(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();
  ...
  for (const [fx, fz] of [[-0.48, -0.22], [0.48, -0.22], [-0.48, 0.22], [0.48, 0.22]] as [number, number][]) {
...
export function buildCampfire(
  ...
  const emberGeo = GeometryCache.get('prop_campfire_ember', () =>
  ...
  // Flame cone on top
  const flameGeo = GeometryCache.get('prop_campfire_flame', () =>
    new THREE.ConeGeometry(0.12, 0.42, 7),
  );
 ...
 */
export function buildTelescope(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();
...
export interface ShelfOpts {
  /** Number of shelf boards stacked vertically (default 2). */
  shelfCount?: number;
  /** Bracket style: 0=flat slab, 1=angled brace, 2=ornate curved (default 1). */
  bracketStyle?: number;
}
...
export interface PillarOpts {
  /** Column style: 0=round, 1=octagonal, 2=square (default 0). */
  style?: number;
  /** Height of decorative capital on top (0=none, default 0.25). */
  capitalH?: number;
  /** Height of pedestal base (0=none, default 0.15). */
  baseH?: number;
  /** Entasis bulge factor 0–1 (default 0.06). */
  entasis?: number;
}
...
export interface AltarOpts {
  /** Number of stepped tiers (1–3, default 2). */
  tiers?: number;
  /** Top element: 0=plain flat, 1=crystal shard, 2=carved hollow (default 1). */
  topElement?: number;
}
...
export function buildRug(
  ...
  if (pattern === 1) {
    ...
    ctx.fillRect(0, 0, bw, SIZE); ctx.fillRect(SIZE-bw, 0, bw, SIZE);
    ...
    for (let i = 0; i < 8; i++) {
      const a = (i/8)*Math.PI*2;
      ctx.fillStyle = color2;
      ctx.beginPath(); ctx.arc(cx + Math.cos(a)*cr*0.65, cy + Math.sin(a)*cr*0.65, cr*0.12, 0, Math.PI*2); ctx.fill();
    }
  }
...
export function buildBanner(
  ...
  bannerCanvas.width = SIZE; bannerCanvas.height = Math.round(SIZE * (h / w));
  const bCtx = bannerCanvas.getContext('2d')!;
  const BH   = bannerCanvas.height;

  bCtx.fillStyle = color1; bCtx.fillRect(0, 0, SIZE, BH);

  if (pattern === 1) {
    bCtx.fillStyle = color2; bCtx.fillRect(0, BH*0.35, SIZE, BH*0.3);
  } else if (pattern === 2) {
    bCtx.fillStyle = color2;
    bCtx.beginPath(); bCtx.moveTo(0, 0); bCtx.lineTo(SIZE, 0); bCtx.lineTo(SIZE/2, BH*0.4); bCtx.closePath(); bCtx.fill();
  } else if (pattern === 3) {
    bCtx.fillStyle = color2;
  ...
  // Bottom trim
  bCtx.fillStyle = color2; bCtx.fillRect(0, BH-8, SIZE, 8);
...
export interface FireplaceOpts {
  /** Opening style: 0=flat lintel, 1=arched (default 1). */
  archOpening?: number;
  /** Mantel shelf height above lintel (0=none, default 0.18). */
  mantelH?: number;
}
...
export function buildFireplace(
  ...
  for (const [lx, lry] of [[-0.12, 0.4], [0.12, -0.4]] as [number, number][]) {
 ...
 */
export function buildPotionRack(
  frameMat: THREE.Material,
  glowMat: THREE.Material,
): THREE.Group {
  const grp = new THREE.Group();
 ...
 */
export function buildDistillationCoil(
  copperMat: THREE.Material,
 ...
 */
export function buildReadingTable(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();
  ...
  for (const [lx, lz] of [[-0.82, -0.38], [0.82, -0.38], [-0.82, 0.38], [0.82, 0.38]] as [number, number][]) {
    const leg = mesh(legGeo, mat, grp);
  ...
  for (const [px, rx] of [[-0.18, -0.12], [0.18, 0.12]] as [number, number][]) {
 ...
 */
export function buildGlobe(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();

  // Tripod base legs
  const baseLegGeo = GeometryCache.get('prop_globe_base_leg', () =>
    new THREE.CylinderGeometry(0.018, 0.022, 0.44, 6),
  );
  for (let i = 0; i < 3; i++) {
    const a = (i / 3) * Math.PI * 2;
  ...
  const shaftGeo = GeometryCache.get('prop_globe_shaft', () =>
    new THREE.CylinderGeometry(0.016, 0.016, 0.78, 7),
  );
  const shaft = mesh(shaftGeo, mat, grp);
  shaft.position.y = 0.39;

  // Globe sphere (slightly flattened at poles = oblate spheroid via scale)
  const sphereGeo = GeometryCache.get('prop_globe_sphere', () =>
  ...
  const meridianGeo = GeometryCache.get('prop_globe_meridian', () =>
    new THREE.TorusGeometry(0.26, 0.012, 6, 22),
  );
 ...
 */
export function buildAnvil(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();

  // Base foot
  const baseGeo = GeometryCache.get('prop_anvil_base', () =>
    new THREE.BoxGeometry(0.70, 0.18, 0.40),
  );
  const base = mesh(baseGeo, mat, grp);
  base.position.y = 0.09;

  // Waist
  const waistGeo = GeometryCache.get('prop_anvil_waist', () =>
    new THREE.BoxGeometry(0.38, 0.28, 0.30),
  );
  const waist = mesh(waistGeo, mat, grp);
  waist.position.y = 0.32;

  // Face (flat top)
  const faceGeo = GeometryCache.get('prop_anvil_face', () =>
    new THREE.BoxGeometry(0.72, 0.14, 0.34),
  );
  const face = mesh(faceGeo, mat, grp);
  face.position.y = 0.53;

  // Horn
  const hornPts: THREE.Vector2[] = [];
  for (let i = 0; i <= 8; i++) {
    const t = i / 8;
 ...
 */
export function buildCoolingTrough(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();

  // Outer iron hull
  const outerGeo = GeometryCache.get('prop_trough_outer', () =>
    new THREE.BoxGeometry(1.50, 0.44, 0.54),
  );
  ...
  // Three side supports
  const legGeo = GeometryCache.get('prop_trough_leg', () =>
 ...
 */
export function buildBunk(mat: THREE.Material, fabricMat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();

  // Four corner posts
  const postGeo = GeometryCache.get('prop_bunk_post', () =>
    new THREE.CylinderGeometry(0.04, 0.04, 1.80, 7),
  );
  ...
  // Lower mattress
  const mattGeo = GeometryCache.get('prop_bunk_mattress', () =>
    new THREE.BoxGeometry(1.00, 0.12, 0.38),
  );
  ...
  const railGeo = GeometryCache.get('prop_bunk_rail', () =>
 ...
 * @param mat  Wood material.
 */
export function buildMessTable(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();

  // Tabletop
  const topGeo = GeometryCache.get('prop_mess_top', () =>
    new THREE.BoxGeometry(2.20, 0.08, 0.70),
  );
  const top = mesh(topGeo, mat, grp);
  top.position.y = 0.78;

  // Four legs
  const legGeo = GeometryCache.get('prop_mess_leg', () =>
  ...
  const benchTopGeo = GeometryCache.get('prop_mess_bench_top', () =>
    new THREE.BoxGeometry(2.00, 0.06, 0.28),
  );
  const benchLegGeo = GeometryCache.get('prop_mess_bench_leg', () =>
 ...
 */
export function buildMapTable(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();

  // Table frame legs
  const legGeo = GeometryCache.get('prop_maptable_leg', () =>
    new THREE.CylinderGeometry(0.05, 0.07, 0.82, 8),
  );
  ...
  // Tabletop
  const topGeo = GeometryCache.get('prop_maptable_top', () =>
    new THREE.BoxGeometry(1.76, 0.06, 0.84),
  );
  const top = mesh(topGeo, mat, grp);
  top.position.y = 0.85;
 ...
 */
export function buildWeaponStand(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();

  // Base plinth
  const baseGeo = GeometryCache.get('prop_weapon_base', () =>
    new THREE.BoxGeometry(0.52, 0.10, 0.28),
  );
  const base = mesh(baseGeo, mat, grp);
  base.position.y = 0.05;

  // Vertical centre pole
  const poleGeo = GeometryCache.get('prop_weapon_pole', () =>
    new THREE.CylinderGeometry(0.025, 0.030, 1.10, 7),
  );
  const pole = mesh(poleGeo, mat, grp);
  pole.position.y = 0.65;

  // Horizontal cross-bar
  const barGeo = GeometryCache.get('prop_weapon_bar', () =>
    new THREE.CylinderGeometry(0.018, 0.018, 0.60, 6),
  );
 ...
 */
export function buildPlantPot(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();
 ...
 */
export function buildRaisedPlanter(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();

  // Stone walls (four slabs)
  const wallN = GeometryCache.get('prop_planter_wallN', () =>
    new THREE.BoxGeometry(1.60, 0.50, 0.10),
  );
  ...
  const wallE = GeometryCache.get('prop_planter_wallE', () =>
    new THREE.BoxGeometry(0.10, 0.50, 0.54),
  );
  ...
  // Soil base
  const soilGeo = GeometryCache.get('prop_planter_soil', () =>
    new THREE.BoxGeometry(1.40, 0.10, 0.44),
  );
  const soilMat = new THREE.MeshLambertMaterial({ color: 0x3a2a18 });
  const soil = new THREE.Mesh(soilGeo, soilMat);
 ...
 */
export function buildContainmentRing(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();

  // Outer ring
  const outerGeo = GeometryCache.get('prop_ward_outer', () =>
    new THREE.TorusGeometry(0.90, 0.035, 6, 48),
  );
  ...
  // Inner ring
  const innerGeo = GeometryCache.get('prop_ward_inner', () =>
    new THREE.TorusGeometry(0.62, 0.022, 6, 40),
  );
  ...
  const nodeGeo = GeometryCache.get('prop_ward_node', () =>
 ...
 */
export function buildAstrolabe(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();

  // Tripod legs (three)
  const legGeo = GeometryCache.get('prop_astro_leg', () =>
    new THREE.CylinderGeometry(0.018, 0.025, 0.68, 6),
  );
  for (let i = 0; i < 3; i++) {
  ...
  // Central shaft
  const shaftGeo = GeometryCache.get('prop_astro_shaft', () =>
    new THREE.CylinderGeometry(0.020, 0.020, 0.38, 7),
  );
  const shaft = mesh(shaftGeo, mat, grp);
  shaft.position.y = 0.86;

  // Main armillary sphere rings (3 nested rings)
  const ring1Geo = GeometryCache.get('prop_astro_ring1', () =>
    new THREE.TorusGeometry(0.32, 0.018, 7, 32),
  );
  const ring2Geo = GeometryCache.get('prop_astro_ring2', () =>
    new THREE.TorusGeometry(0.26, 0.014, 6, 28),
  );
  const ring3Geo = GeometryCache.get('prop_astro_ring3', () =>
  ...
  // Central orb
  const orbGeo = GeometryCache.get('prop_astro_orb', () =>
    new THREE.SphereGeometry(0.065, 9, 7),
  );
 ...
 */
export function buildFermentingVat(
  mat: THREE.Material,
  glassMat: THREE.Material,
): THREE.Group {
  const grp = new THREE.Group();
  ...
  const bodyGeo = GeometryCache.get('prop_ferment_body', () =>
    new THREE.CylinderGeometry(0.60, 0.55, 1.10, 16),
  );
  const body = mesh(bodyGeo, mat, grp);
  body.position.y = 0.55;

  // Top rim ring
  const topRimGeo = GeometryCache.get('prop_ferment_rim', () =>
    new THREE.TorusGeometry(0.62, 0.04, 7, 20),
  );
  ...
  const bandGeo = GeometryCache.get('prop_ferment_band', () =>
    new THREE.TorusGeometry(0.63, 0.025, 6, 20),
  );
 ...
 */
export function buildHerbBundle(mat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();
