...
export class RoundedBoxGeometry extends THREE.BufferGeometry {
  constructor(
    ...
    function addVertex(px: number, py: number, pz: number,
     ...
     */
    function buildFace(
      uAxis: THREE.Vector3, vAxis: THREE.Vector3, nAxis: THREE.Vector3,
      uHalf: number, vHalf: number, nOff: number,
      uvFlip: boolean,
    ) {
      ...
      for (let j = 0; j < rows; j++) {
        for (let i = 0; i < rows; i++) {
          ...
          let pu: number, pv: number, pn: number;
          let nu: number, nv: number, nn: number;
