...
function _phaseAndBlend(hour: number): { a: Phase; b: Phase; t: number } {
  // time ranges (hour)
  if (hour >= 22 || hour < 5) {
    // night
...
function _lerpHex(a: number, b: number, t: number): THREE.Color {
  _ca.setHex(a);
  _cb.setHex(b);
...
export class DayNightSystem {
  constructor(
    private readonly _hemi:     THREE.HemisphereLight,
