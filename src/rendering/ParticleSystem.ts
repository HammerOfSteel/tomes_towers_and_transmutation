...
// ── Continuous emitter config ─────────────────────────────────────────────────

export interface EmitterConfig {
  /** Hex colour, e.g. 0xff8833. */
  color:    number;
  /** Particles per second. */
  rate:     number;
  /** Base speed of each particle (WU/s). */
  speed:    number;
  /** Per-particle lifetime in seconds. */
  lifetime: number;
  /** Extra upward velocity added to each particle. */
  upBias:   number;
  /** Cone half-angle (radians) for direction randomisation. Default π = sphere. */
  spread:   number;
  /** If false, gravity is not applied to these particles. Default true. */
  gravity:  boolean;
}

const EMITTER_DEFAULTS: EmitterConfig = {
  color:    0xffffff,
  rate:     20,
  speed:    1.0,
  lifetime: 0.6,
...
export interface EmitterHandle {
  /** Move the emitter's origin in world space. */
...
// ── Internal emitter record ───────────────────────────────────────────────────

interface ContinuousEmitter extends EmitterConfig {
  x: number; y: number; z: number;
  timer: number;   // accumulated dt for sub-frame emission
  active: boolean;
}
...
export class ParticleSystem {
  ...
  private readonly _pos:      Float32Array = new Float32Array(MAX * 3);
  private readonly _vel:      Float32Array = new Float32Array(MAX * 3);
  private readonly _startCol: Float32Array = new Float32Array(MAX * 3);
  private readonly _colBuf:   Float32Array = new Float32Array(MAX * 3);
  private readonly _age:      Float32Array = new Float32Array(MAX);
  private readonly _life:     Float32Array = new Float32Array(MAX);
  private readonly _alive:    Uint8Array   = new Uint8Array(MAX);
  private readonly _hasGrav:  Uint8Array   = new Uint8Array(MAX);

  private readonly _free: number[] = Array.from({ length: MAX }, (_, i) => MAX - 1 - i);
  ...
  private readonly _emitters: ContinuousEmitter[] = [];
   ...
   */
  addEmitter(pos: THREE.Vector3, cfg: Partial<EmitterConfig> = {}): EmitterHandle {
    const e: ContinuousEmitter = {
      ...EMITTER_DEFAULTS,
      ...cfg,
      x: pos.x,
      y: pos.y,
    ...
    return {
      setPos(x: number, y: number, z: number): void { e.x = x; e.y = y; e.z = z; },
      stop():                                    void { e.active = false; },
      get active():                          boolean { return e.active; },
    };
  }
   ...
   */
  addTorchFire(pos: THREE.Vector3): EmitterHandle {
  ...
  addAmbientDust(
    ...
    const e = this._emitters[this._emitters.length - 1] as ContinuousEmitter & { dustRadius?: number };
    e.dustRadius = radius;
    return handle;
  }
  ...
  update(dt: number): void {
    ...
    if (this._emitters.length > 50) {
      for (let i = this._emitters.length - 1; i >= 0; i--) {
        if (!this._emitters[i].active) this._emitters.splice(i, 1);
      }
    }
    ...
    if (dirty || this._liveCount > 0) {
      ...
      (this._geo.attributes['color']    as THREE.BufferAttribute).needsUpdate = true;
    }
  }
  ...
  private _free_slot(slot: number): void {
  ...
  private _setColor(slot: number, col: THREE.Color): void {
    const i3 = slot * 3;
  ...
  private _emitFromContinuous(e: ContinuousEmitter): void {
    const slot = this._alloc();
    if (slot < 0) return;

    // Scatter position (for dust, _dustRadius is set)
    const dustR = (e as ContinuousEmitter & { dustRadius?: number }).dustRadius ?? 0;
