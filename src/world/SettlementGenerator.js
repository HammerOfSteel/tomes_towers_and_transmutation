...
// ── Entry point ───────────────────────────────────────────────────────────────
export function planSettlement(type, centerCol, centerRow, seed, grid) {
 ...
 */
export function applySettlementToGrid(plan, grid, id) {
...
function _valid(grid, col, row) {
    if (col < 1 || col >= grid.width - 1 || row < 1 || row >= grid.height - 1)
        return false;
    ...
    if (cell.feature === 'river')
        return false;
    if (cell.content === 'dungeon_entrance')
        return false;
    if (cell.elevation < 1)
        return false; // no buildings on bog/level-0
    return true;
}
/**
 ...
 */
function _noOverlap(placed, col, row, type, padding = 1) {
    ...
    for (const b of placed) {
        ...
        if (Math.abs(col - b.col) < hW + bhW + padding &&
...
// ── Village ───────────────────────────────────────────────────────────────────
function _planVillage(cc, cr, seed, grid, name) {
    ...
    const focalType = rand() < 0.6 ? 'well' : 'market_cross';
    ...
    for (let pi = 0; pi < PLOTS.length && mi < MIX.length; pi++) {
        ...
        const row = cr + dr;
        const btype = MIX[mi];
        if (roadSet.has(`${col},${row}`))
            continue; // never on road
        if (!_valid(grid, col, row) || !_noOverlap(buildings, col, row, btype, 1))
            continue;
        buildings.push({
            type: btype, col, row,
            rotation: rot,
...
// ── Town ──────────────────────────────────────────────────────────────────────
function _planTown(cc, cr, seed, grid, name) {
    ...
    for (let step = -8; step <= 8; step += 4) {
        for (const side of [-1, 1]) {
            ...
            if (!_valid(grid, col, row) || !_noOverlap(buildings, col, row, btype, 2))
                continue;
            buildings.push({
                type: MIX[mi++],
                col, row,
...
// ── City ──────────────────────────────────────────────────────────────────────
function _planCity(cc, cr, seed, grid, name) {
    ...
    for (const [qsc, qsr] of [[-1, -1], [1, -1], [-1, 1], [1, 1]]) {
        for (let bi = 0; bi < 5; bi++) {
            if (mi >= QUADRANT_MIX.length)
                break;
            ...
            if (!_valid(grid, col, row) || !_noOverlap(buildings, col, row, btype)) {
                mi++;
                continue;
            }
            buildings.push({
