...
// ── UI ────────────────────────────────────────────────────────────────────────
let _solmorPanel = null;
function _closeSolmorPanel() {
    if (_solmorPanel) {
        _solmorPanel.remove();
        _solmorPanel = null;
    }
 ...
 */
export function showSolmorEncounter(stage, species, onComplete) {
    injectHudTheme();
    _closeSolmorPanel();
    const dialogue = stage === 1 ? getStage1(species) :
...
export function isSolmorOpen() {
    return _solmorPanel !== null;
}
/** Stored ending choice from Stage 3 (if made). */
export function getSolmorEndingChoice() {
