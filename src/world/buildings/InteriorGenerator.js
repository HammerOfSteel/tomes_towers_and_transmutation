...
import { FLOOR_HEIGHT } from './BuildingDNA';
import { buildProp } from '@/prop-creator/builder';
import { MATERIAL_COLORS } from '@/prop-creator/types';
import { stoneTexture, renderTexture } from './TextureFactory';
 ...
 */
export function generatePlan(dna) {
    const fp = getBuildingFootprint(dna);
    const w = fp.w;
    const d = fp.d;
    const grid = new Uint8Array(w * d).fill(TILE_WALL);
    const rooms = [];
    function setFloor(x, z) {
        if (x >= 0 && z >= 0 && x < w && z < d)
            grid[x + w * z] = TILE_FLOOR;
    }
    function fillRoom(rx, rz, rw, rd, purpose) {
    ...
    function setDoor(x, z) {
        if (x >= 0 && z >= 0 && x < w && z < d)
            grid[x + w * z] = TILE_DOOR;
    }
...
function getBuildingFootprint(dna) {
...
export function deriveBlueprints(plan) {
    const isWall = (x, z) => {
        if (x < 0 || z < 0 || x >= plan.w || z >= plan.d)
            return true;
        return plan.grid[x + plan.w * z] !== TILE_FLOOR;
    };
    const isFloor = (x, z) => x >= 0 && z >= 0 && x < plan.w && z < plan.d && plan.grid[x + plan.w * z] === TILE_FLOOR;
        ...
        for (let tz = r.z; tz < r.z + r.d; tz++) {
            for (let tx = r.x; tx < r.x + r.w; tx++) {
                if (!isFloor(tx, tz))
                    continue;
                if (isWall(tx, tz - 1))
                    wallTiles.push({ tx, tz, face: 'S' });
                else if (isWall(tx, tz + 1))
                    wallTiles.push({ tx, tz, face: 'N' });
                else if (isWall(tx - 1, tz))
                    wallTiles.push({ tx, tz, face: 'E' });
                else if (isWall(tx + 1, tz))
                    wallTiles.push({ tx, tz, face: 'W' });
            }
        }
        return { ...r, wallTiles };
    });
...
export class Occupancy {
    ...
    placeable(tx, tz) {
        if (tx < 0 || tz < 0 || tx >= this.plan.w || tz >= this.plan.d)
            return false;
...
function placeAt(g, occ, tx, tz, kind, mat, theme, seed, rotation = 0) {
    if (!occ.placeable(tx, tz))
        return false;
    const dna = {
        v: 1, kind: 'prop', name: `${mat} ${kind}`, seed,
        propKind: kind, material: mat, theme, condition: 'weathered',
...
function anchorToWall(g, occ, room, kind, mat, theme, seed) {
...
function scatter(g, occ, room, kinds, count, mat, theme, seed) {
    ...
    for (let attempt = 0; attempt < count * 12 && placed < count; attempt++) {
        ...
        if (placeAt(g, occ, tx, tz, k, mat, theme, seed ^ attempt, r() * Math.PI * 2))
            placed++;
    }
}
// ── Floor surface ─────────────────────────────────────────────────────────────
function layFloorSurface(g, plan, style) {
...
// ── Wall surfaces ─────────────────────────────────────────────────────────────
function buildWallSurfaces(g, plan, style, h) {
    ...
    const isWall = (x, z) => x < 0 || z < 0 || x >= plan.w || z >= plan.d || plan.grid[x + plan.w * z] === TILE_WALL;
    const isPassable = (x, z) => x >= 0 && z >= 0 && x < plan.w && z < plan.d && plan.grid[x + plan.w * z] !== TILE_WALL;
    const isDoor = (x, z) => x >= 0 && z >= 0 && x < plan.w && z < plan.d && plan.grid[x + plan.w * z] === TILE_DOOR;
    // Is a tile on the building exterior (neighbours the boundary wall)?
    const isExterior = (x, z) => x <= 1 || z <= 1 || x >= plan.w - 2 || z >= plan.d - 2;
    ...
    for (let z = 0; z < plan.d; z++) {
        for (let x = 0; x < plan.w; x++) {
            ...
            for (const face of faces) {
                ...
                m.position.copy(face.pos);
                m.castShadow = m.receiveShadow = true;
                m.userData.isOccluder = true;
                m.userData._origOpacity = 1;
                g.add(m);
            }
            // ── Door arches at TILE_DOOR adjacencies ──────────────────────────────
            // Detect door to the north (z-1) or south (z+1) of this floor tile
            for (const [dx, dz, isNS] of [[0, -1, true], [0, 1, true], [1, 0, false], [-1, 0, false]]) {
                const nx = x + dx, nz = z + dz;
                if (!isDoor(nx, nz))
                    continue;
                const archKey = `${nx},${nz}`;
                if (windowSet.has('arch:' + archKey))
                    continue;
            ...
            for (const [dx, dz, isNS] of [[0, -1, true], [0, 1, true], [1, 0, false], [-1, 0, false]]) {
                ...
                if (isNS) {
                    ...
                    glass.position.set(x + 0.5, wBot + wH / 2, wz);
                    g.add(glass);
                    // Frame
                    ...
                    glass.position.set(wx, wBot + wH / 2, z + 0.5);
                    g.add(glass);
                    for (const [fw, fh, fx2, fz2] of [
                        [0.06, 0.05, wx, z + 0.5],
...
// ── Ceiling ───────────────────────────────────────────────────────────────────
function buildCeiling(g, plan, style, h) {
    ...
    for (let bx = beamSpacing; bx < plan.w; bx += beamSpacing) {
        ...
        beam.userData.isOccluder = true;
        beam.userData._origOpacity = 1;
        g.add(beam);
    }
}
// ── Furnishing rule engine ────────────────────────────────────────────────────
function furnishRoom(g, occ, room, style, theme, seed) {
    const lights = [];
 ...
 */
function buildStaircase(g, tx, tz, style, goUp) {
 ...
 */
export function generateInterior(dna, floorIndex = 0) {
    ...
    return {
        ...
        occluderMeshes: (() => {
            const out = [];
            root.traverse(obj => { if (obj.isMesh && obj.userData.isOccluder)
                out.push(obj); });
            return out;
        })(),
    };
...
function themeForKind(kind) {
    switch (kind) {
        case 'blacksmith':
        case 'apothecary': return 'alchemy';
        case 'chapel': return 'dungeon';
        case 'inn':
        case 'tavern':
        case 'shop': return 'residential';
