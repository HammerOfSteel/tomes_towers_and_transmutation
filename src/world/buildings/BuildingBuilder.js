...
import { FLOOR_HEIGHT, STYLE_COLORS, getFootprint } from './BuildingDNA';
...
function addWindow(g, x, y, z, rotY, lit, tMat, winW = 1.05, winH = 1.15) {
    const depth = 0.12;
...
function addDoor(g, x, y, z, rotY, dna, tMat) {
...
function addChimney(g, cx, cy, cz) {
...
function buildHouseOrShop(dna) {
    ...
    const plinth = new THREE.Mesh(new THREE.BoxGeometry(w + 0.5, plinthH, d + 0.5), new THREE.MeshStandardMaterial({ color: new THREE.Color('#6a6458'), roughness: 0.95 }));
    ...
    // Core shadow volume
    const core = new THREE.Mesh(new THREE.BoxGeometry(w - 0.5, wallH, d - 0.5), new THREE.MeshStandardMaterial({ color: new THREE.Color(dna.colors.walls), roughness: 0.95 }));
    ...
    let roof;
    if (dna.style === 'thatched')
        roof = thatchedRoof(w, d, rMat);
    else if (dna.style === 'stone')
        roof = hippedRoof(w, d, 0.45, 0.30, rMat);
    else if (dna.style === 'arcane')
        roof = hippedRoof(w, d, 0.60, 0.22, rMat);
    else
        roof = pitchedRoof(w, d, 0.52, rMat);
...
function buildTerraced(dna) {
    ...
    const tMat = trimStd(dna);
    // Plinth
    const plinth = new THREE.Mesh(new THREE.BoxGeometry(w + 0.4, plinthH, d + 0.4), new THREE.MeshStandardMaterial({ color: new THREE.Color('#6a6458'), roughness: 0.95 }));
    plinth.position.y = plinthH / 2;
...
function buildCottage(dna) {
    ...
    const base = new THREE.Mesh(new THREE.BoxGeometry(w + 0.8, plinthH, d + 0.8), new THREE.MeshStandardMaterial({ color: new THREE.Color('#585450'), roughness: 0.97 }));
...
function buildVilla(dna) {
    ...
    const plinth = new THREE.Mesh(new THREE.BoxGeometry(w + 0.6, plinthH, d + 0.6), new THREE.MeshStandardMaterial({ color: new THREE.Color('#808070'), roughness: 0.95 }));
    plinth.position.y = plinthH / 2;
    plinth.castShadow = plinth.receiveShadow = true;
    g.add(plinth);
    const yMid = plinthH + wallH / 2;
...
function buildBlacksmith(dna) {
    ...
    const rMat = roofStd(dna, w, d);
    const tMat = trimStd(dna);
    const plinth = new THREE.Mesh(new THREE.BoxGeometry(w + 0.4, plinthH, d + 0.4), new THREE.MeshStandardMaterial({ color: new THREE.Color('#505050'), roughness: 0.97 }));
    plinth.position.y = plinthH / 2;
    g.add(plinth);
...
function buildMarketStall(dna) {
    ...
    for (const [px, pz] of [[w / 2, 0], [-w / 2, 0], [w / 2, -d + 0.3], [-w / 2, -d + 0.3]]) {
...
function buildApothecary(dna) {
    ...
    const rMat = roofStd(dna, w, d);
    const tMat = trimStd(dna);
    const plinth = new THREE.Mesh(new THREE.BoxGeometry(w + 0.3, plinthH, d + 0.3), new THREE.MeshStandardMaterial({ color: new THREE.Color('#686460'), roughness: 0.95 }));
    plinth.position.y = plinthH / 2;
    g.add(plinth);
    const core = new THREE.Mesh(new THREE.BoxGeometry(w, wallH, d), wMat);
    core.position.y = plinthH + wallH / 2;
    ...
    for (const [jx, jy] of [[-(w / 2 - 0.7), plinthH + 1.2], [(w / 2 - 0.7), plinthH + 1.2]]) {
...
function buildGate(dna) {
    ...
    for (const tx of [-gateW / 2 + 1.1, gateW / 2 - 1.1]) {
        ...
        const bH = 0.6;
        for (const [mx, mz] of [[-0.55, 0], [0, 0.55], [0.55, 0], [0, -0.55]]) {
    ...
    switch (dna.style) {
        case 'vampiric': {
            ...
            for (let fl = 0; fl < 3; fl++) {
                ...
                band.position.set(0, by, d / 2 + 0.05);
                g.add(band);
                const band2 = band.clone();
                band2.position.set(0, by, -(d / 2 + 0.05));
                g.add(band2);
            }
            break;
        }
        ...
        case 'nomadic': {
            ...
            for (let fl = 0; fl < dna.floors; fl++) {
                const by = fl * FLOOR_HEIGHT;
                for (const [cx, cz] of [[w / 2, d / 2], [-w / 2, d / 2]]) {
...
// ── Main entry ────────────────────────────────────────────────────────────────
export function buildBuilding(dna) {
    const builder = KIND_BUILDERS[dna.buildingKind] ?? buildHouseOrShop;
    const fp = getFootprint(dna.buildingKind, dna.size);
    const fullDna = { ...dna, colors: { ...STYLE_COLORS[dna.style], ...dna.colors } };
