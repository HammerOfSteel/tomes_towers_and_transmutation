...
import { BUILDING_SPECS, type BuildingType } from './BuildingTypes';
...
// ── Wall variant sets per building type ───────────────────────────────────────

interface WallSet { plain: string; window: string; door: string }

const WALL_SETS: Record<BuildingType, WallSet> = {
...
function _paths(ws: WallSet): WallSet {
  return {
...
const WALL_PATHS = Object.fromEntries(
  Object.entries(WALL_SETS).map(([k, v]) => [k, _paths(v)]),
) as Record<BuildingType, WallSet>;
...
export const BUILDING_PRELOAD_PATHS: readonly string[] = [
  ...new Set([
...
export function assembleBuilding(
  ...
  // ── Special buildings ─────────────────────────────────────────────────────
  if (type === 'guard_tower') { _assembleGuardTower(loader, group, floors); return group; }
  if (type === 'well')        { _assembleWell(loader, group);               return group; }
  if (type === 'market_cross'){ _assembleMarketCross(loader, group);        return group; }
  if (type === 'market_stall'){ _assembleMarketStall(loader, group, cols, rows); return group; }
  ...
  for (let f = 0; f < floors; f++) {
    ...
    for (let iz = 0; iz < rows; iz++) {
      for (let ix = 0; ix < cols; ix++) {
        ...
        if (!isN && !isS && !isE && !isW) continue; // interior — skip
        ...
        const wallFaces: Array<{ rotY: number }> = [];
        if (isN) wallFaces.push({ rotY: 0 });
        if (isS) wallFaces.push({ rotY: Math.PI });
        if (isE) wallFaces.push({ rotY: -Math.PI / 2 });
        if (isW) wallFaces.push({ rotY:  Math.PI / 2 });
  ...
  for (let iz = 0; iz < rows; iz++) {
    for (let ix = 0; ix < cols; ix++) {
      ...
      if      (isN && isW) { path = ROOF_CORNER_GLB; rotY =  Math.PI;      }
      else if (isN && isE) { path = ROOF_CORNER_GLB; rotY =  Math.PI / 2;  }
      else if (isS && isW) { path = ROOF_CORNER_GLB; rotY = -Math.PI / 2;  }
      else if (isS && isE) { path = ROOF_CORNER_GLB; rotY =  0;            }
      else if (isN)        { path = ROOF_EDGE_GLB;   rotY =  Math.PI;      }
      else if (isS)        { path = ROOF_EDGE_GLB;   rotY =  0;            }
      else if (isE)        { path = ROOF_EDGE_GLB;   rotY = -Math.PI / 2;  }
      else if (isW)        { path = ROOF_EDGE_GLB;   rotY =  Math.PI / 2;  }
      else                 { path = ROOF_GLB;        rotY =  0;            }
 ...
 */
function _assembleGuardTower(loader: AssetLoader, group: THREE.Group, floors: number): void {
  // buildings/tower.glb is ~1 WU tall native → S = 2 WU per section.
  const tileH = 1.0 * S;
  const sections: Array<[string, number]> = [
    [TOWER_BASE_GLB, 0],
    ...Array.from<unknown, [string, number]>(
      { length: Math.max(0, floors - 2) },
      (_, i) => [TOWER_BODY_GLB, (i + 1) * tileH],
    ),
 ...
 */
function _assembleWell(loader: AssetLoader, group: THREE.Group): void {
  const ws = WALL_PATHS.well;
  const sc = S * 0.6;
  const r  = 0.5 * sc;
  const wallDirs: Array<[number, number, number]> = [
 ...
 */
function _assembleMarketCross(loader: AssetLoader, group: THREE.Group): void {
 ...
 */
function _assembleMarketStall(
  loader: AssetLoader, group: THREE.Group,
  ...
  for (let ix = 0; ix < cols; ix++) {
    _tile(loader, OVERHANG_GLB, group,
      (ix - cols / 2 + 0.5) * S,
      S * 0.5,
      (rows / 2 - 0.5) * S,
      Math.PI,
