...
let _thatchCanvas:      HTMLCanvasElement | null = null;
let _cobblestoneCanvas: HTMLCanvasElement | null = null;
...
function _buildStoneCanvas(): HTMLCanvasElement {
...
function _buildBrickCanvas(): HTMLCanvasElement {
  const c = document.createElement('canvas');
  c.width = c.height = 512;
  const g = c.getContext('2d');
  if (!g) return c;;
...
function _buildRenderCanvas(): HTMLCanvasElement {
  const c = document.createElement('canvas');
...
function _buildSlateCanvas(): HTMLCanvasElement {
  const c = document.createElement('canvas');
  c.width = c.height = 512;
  const g = c.getContext('2d');
  if (!g) return c;;
...
function _buildThatchCanvas(): HTMLCanvasElement {
  const c = document.createElement('canvas');
  c.width = c.height = 256;
  const g = c.getContext('2d');
  if (!g) return c;;
...
function _buildCobblestoneCanvas(): HTMLCanvasElement {
  const c = document.createElement('canvas');
  c.width = c.height = 256;
  const g = c.getContext('2d');
  if (!g) return c;;
...
export function stoneTexture(repX: number, repY: number): THREE.CanvasTexture {
  if (!_stoneCanvas) _stoneCanvas = _buildStoneCanvas();
...
export function brickTexture(repX: number, repY: number): THREE.CanvasTexture {
  if (!_brickCanvas) _brickCanvas = _buildBrickCanvas();
...
export function renderTexture(repX: number, repY: number): THREE.CanvasTexture {
  if (!_renderCanvas) _renderCanvas = _buildRenderCanvas();
...
export function slateTexture(repX: number, repY: number): THREE.CanvasTexture {
  if (!_slateCanvas) _slateCanvas = _buildSlateCanvas();
  return _wrap(new THREE.CanvasTexture(_slateCanvas), repX, repY);
}

/** Warm straw thatch with horizontal course lines. */
export function thatchTexture(repX: number, repY: number): THREE.CanvasTexture {
  if (!_thatchCanvas) _thatchCanvas = _buildThatchCanvas();
  return _wrap(new THREE.CanvasTexture(_thatchCanvas), repX, repY);
}

/** Irregular rounded cobblestones — roads, squares, courtyards. */
export function cobblestoneTexture(repX: number, repY: number): THREE.CanvasTexture {
  if (!_cobblestoneCanvas) _cobblestoneCanvas = _buildCobblestoneCanvas();
