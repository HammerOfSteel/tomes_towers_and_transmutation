 ...
 */
export function generateCharacterName(characterId) {
    switch (characterId) {
        // ── Human ──────────────────────────────────────────────────────────────
        case 'rogue':
        case 'rogue_hooded': {
 ...
 */
export function generateQualityName(characterId, tries = 6) {
    for (let i = 0; i < tries; i++) {
        const name = generateCharacterName(characterId);
        if (_isAcceptable(name))
            return name;
    }
    // Fall through to plain generation (better than infinite looping)
    return generateCharacterName(characterId);
}
function _isAcceptable(name) {
    if (name.length < 3)
        return false;
    // No double-vowel start after combination
    if (/^[aeiou]{2}/i.test(name))
        return false;
    // No trailing standalone single consonant (e.g. "Valc")
    if (/[^aeiou\s]$/i.test(name) && name.split(' ')[0].length < 4)
        return false;
    return true;
}
const HUMAN_IDS = ['rogue', 'mage', 'human_warrior', 'human_paladin', 'human_bard'];
const FOX_IDS = ['fox_rogue', 'fox_ranger', 'fox_mage', 'fox_mysterious'];
const SLIME_IDS = ['slime', 'slime_arcane', 'slime_philosopher', 'slime_young'];
const UNDEAD_IDS = ['skeleton_rogue', 'skeleton_mage', 'zombie', 'ghost', 'mystery_undead'];
/**
 ...
 */
export function generateNameForSpecies(species, tries = 6) {
    let pool;
    switch (species) {
        case 'human':
            pool = HUMAN_IDS;
        ...
        case 'undead':
            ...
            break;
        case 'any': {
            const ALL = [...HUMAN_IDS, ...FOX_IDS, ...SLIME_IDS, ...UNDEAD_IDS];
