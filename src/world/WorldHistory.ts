...
export type FactionType =
  | 'mages_guild' | 'merchants' | 'bandits' | 'forest_spirits' | 'undead_cult';
...
export interface WorldFaction {
  ...
  /** 0.0–1.0 — starts ~0.4, modified by simulation. */
  strength:         number;
}
...
const FACTION_TYPE_NAMES: Record<FactionType, string> = {
...
function factionName(rand: () => number, type: FactionType): string {
...
function pickDesc(arr: string[], rand: () => number): string {
  return arr[Math.floor(rand() * arr.length)] ?? arr[0]!;
}

// ── Main simulation ────────────────────────────────────────────────────────────

export function simulateWorldHistory(data: WorldData, seed: number): WorldHistoryResult {
  ...
  for (let turn = 6; turn <= 25; turn++) {
    for (const fa of factions) {
      if (fa.strength < 0.5) continue;
      if (rand() > 0.20) continue; // 20% chance of action

      // Find a target faction (different home settlement)
      const targets = factions.filter(fb =>
        fb.id !== fa.id &&
  ...
  for (let turn = 26; turn <= 40; turn++) {
    if (rand() < 0.35 && settlements.length >= 2) {
      ...
      if (ib === ia) ib = (ib + 1) % settlements.length;
