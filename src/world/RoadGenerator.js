    ...
    pop() {
        ...
        if (this._d.length > 0) {
            ...
            for (;;) {
                ...
                if (l < this._d.length && this._d[l][0] < this._d[m][0])
                    m = l;
                if (r < this._d.length && this._d[r][0] < this._d[m][0])
                    m = r;
                if (m === i)
                    break;
...
function _moveCost(grid, fc, fr, tc, tr, base) {
    if (tc < 0 || tc >= grid.width || tr < 0 || tr >= grid.height)
        return Infinity;
 ...
 */
function _aStar(grid, startC, startR, endC, endR) {
    const W = grid.width;
    const key = (c, r) => r * W + c;
 ...
 */
function _lShape(c1, r1, c2, r2) {
    const pts = [];
    const cStep = c1 <= c2 ? 1 : -1;
    for (let c = c1; c !== c2 + cStep; c += cStep)
        pts.push({ col: c, row: r1 });
    const rStep = r1 <= r2 ? 1 : -1;
    for (let r = r1 + rStep; r !== r2 + rStep; r += rStep)
        pts.push({ col: c2, row: r });
    return pts;
}
/**
 ...
 */
function _addLoopEdges(centres, mstEdges, count) {
    const N = centres.length;
    if (N < 3)
        return [];
    ...
    return candidates.slice(0, count).map(([, a, b]) => [a, b]);
}
/**
 ...
 */
export function buildInterSettlementRoads(settlements, grid) {
    if (settlements.length < 2)
        return { tiles: [] };
    const centres = settlements.map(s => ({
