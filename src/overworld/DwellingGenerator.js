...
// ── Generator ─────────────────────────────────────────────────────────────────
function scatterFurniture(room, rng) {
...
export function generateDwelling(seed, archetype = 'house_small', faction = 'human') {
...
export function drawDwellingFloorPlan(plan, canvas, floorFilter = 0) {
    ...
    const PADDING = 24;
    const cellSize = Math.floor(Math.min((W - PADDING * 2) / gridW, (H - PADDING * 2) / gridH, 32));
    ...
    for (const room of rooms) {
        ...
        for (const door of room.doors) {
            ...
            switch (door.wall) {
                case 'N':
                    dx = px + door.offset * cellSize + cellSize * 0.25;
                    dy = py - dh / 2;
                ...
                case 'E':
                    dx = px + pw - dh / 2;
                    dy = py + door.offset * cellSize + cellSize * 0.25;
                    ctx.fillRect(dx, dy, dh, dw);
                    break;
                case 'W':
                    dx = px - dh / 2;
                    dy = py + door.offset * cellSize + cellSize * 0.25;
