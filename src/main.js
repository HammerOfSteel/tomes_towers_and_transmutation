...
import { BaseScene, STRUCTURE_COSTS, STRUCTURE_META } from '@/scene/BaseScene';
...
import { speciesForCharacter } from '@/world/StoryQuestLine';
...
async function main() {
    ...
    composer.addPass(new EffectPass(cameraRig.camera, new BloomEffect({ intensity: 2.2, luminanceThreshold: 0.12, kernelSize: KernelSize.MEDIUM })));
    // ── Physics ────────────────────────────────────────────────────────────────
    ...
    let _activeBuildingDna = null;
    let _currentBuildingFloor = 0;
    let _activeInterior = null;
    ...
    let _selectedCreatureIdx = null;
    /** Yellow ring rendered under the selected spawned creature. */
    const _selectionRing = (() => {
        const ring = new THREE.Mesh(new THREE.RingGeometry(0.8, 1.05, 40), new THREE.MeshBasicMaterial({ color: 0xfbbf24, side: THREE.DoubleSide, transparent: true, opacity: 0.85 }));
    ...
    async function _mountInterior(dna, floorIndex) {
        ...
        const floorBody = physics.createStaticBox(new THREE.Vector3(0, INTERIOR_Y - 0.5, 0), new THREE.Vector3(40, 0.5, 40));
    ...
    function _unmountInterior() {
        if (!_activeInterior)
            return;
        scene.remove(_activeInterior.scene.group);
        for (const l of _activeInterior.scene.lights)
            scene.remove(l);
    ...
    async function enterBuildingInterior(dna) {
    ...
    async function _switchBuildingFloor(newFloor) {
        if (!_activeBuildingDna || !_activeInterior)
            return;
    ...
    function leaveBuildingInterior() {
        if (!_inBuildingInterior || !_activeInterior)
            return;
        _doFade(() => {
            _unmountInterior();
    ...
    function _makeOverworld(seed) {
        ...
        ow.onQuestGiven = (quest) => {
            questModal.show(quest, () => {
                ...
                minimap?.setQuestPins(questLog.getActive().map(q => ({ col: q.target.col, row: q.target.row })));
            });
        };
        return ow;
    }
    ...
    function switchToInterior(roomId) {
        overworld?.exit();
    ...
    sceneManager.onExitTrigger = () => {
        ...
        if (!_hasMasterKey && gameMode === 'interior') {
            ...
            _storyToast('The front door is sealed with heavy magic. You need the master key — it must be in the basement.', 'beat');
            return;
        }
    ...
    sceneManager.onTransitionAttempt = (_targetId, direction) => {
        if (direction === 'up' && !_hasMasterKey && gameMode === 'interior') {
            _storyToast("The staircase is sealed with an arcane ward.\nThe basement might have what you need first.", 'beat');
            return false;
        }
        return true;
    };
    ...
    let _activeDungeonId = null;
    let _questCheckTimer = 0;
    let _storyRunner = null;
    ...
    const _pendingBlueprints = new Set();
    ...
    window.addEventListener('keydown', (e) => {
        if (gameMenu.isOpen || spellBook.isOpen || statPanel.visible)
            return;
        if (e.code === 'KeyZ')
            consumables.usePotion('potion_heal_minor');
        if (e.code === 'KeyX')
            consumables.usePotion('potion_heal_major');
    });
    ...
    let _telescopePrevRoomId = null;
    function enterTelescopeMode() {
        if (telescopeView.active)
            return;
    ...
    interactables.onKeyPickup = () => {
        ...
        _hasMasterKey = true;
        _storyToast('You take the master key.\nThe binding ward dissolves in your palm.', 'act');
    ...
    interactables.onCraftingStation = (type) => {
        ...
        if (!_towerPrologueDone && gameMode === 'interior') {
            ...
            _storyToast(`The ${label} hums with unfamiliar wards.\n"Do not touch my equipment." — Solmor`, 'beat');
            return;
        }
        if (type === 'alchemy') {
    ...
    function startGame(seed, cfg) {
        ...
        for (const bonus of cfg?.statBonuses ?? []) {
            ...
            if (entry)
                progression.boostStat(entry[0], entry[1]);
        }
        ...
        //   Never fall back to old KayKit asset models for the playable character.
        {
            ...
            if (dnaToApply) {
                ...
                player.applyPrincess(dnaToApply).catch((e) => console.error('[main] ❌ applyPrincess failed:', e));
            }
            else {
                ...
                import('@/princess-creator/defaults/PrincessDefaults').then(({ getDefaultPrincessForCharId }) => {
                    ...
                    console.log('[main] applyPrincess (default blueprint) → char:', cfg?.characterId, '→ dna:', defaultDna.name, defaultDna.species);
                    player.applyPrincess(defaultDna).catch((e) => console.error('[main] ❌ applyPrincess (default) failed:', e));
                });
            }
        }
        ...
        if (cfg?.characterId || cfgSpecies) {
            ...
            talentSystem.activeSpecies = _characterSpecies;
            ...
            _storyRunner = cfg?.characterId
                ...
                : new StoryRunner(_characterSpecies, questLog);
            // C1: Update quest journal species tab
            const _sl = import('@/world/StoryQuestLine').then(({ getStoryLineBySpecies }) => {
                const line = getStoryLineBySpecies(_characterSpecies);
                if (line)
                    questLog.setSpeciesTitle(line.displayTitle);
            });
            ...
            _storyRunner.onActBegin = (title, intro) => {
                ...
                if (title !== 'Prologue — The Tower')
                    _towerPrologueDone = true;
    ...
    function _startDevPanelInGame() {
        ...
        _sandboxUi = new DevSandbox({
            ...
            onInstantCooldowns: (v) => { spells.instantCooldowns = v; },
            getHpInfo: () => ({ hp: player.health.hp, maxHp: player.health.maxHp }),
            onFillHp: () => { player.health.reset(); if (deathTriggered) {
                deathTriggered = false;
                deathScreen.hide();
            } },
            onSetHp: (v) => { player.health.forceSetHp(v); if (deathTriggered && v > 0) {
                deathTriggered = false;
                deathScreen.hide();
            } },
            onKillAllInRoom: () => sceneManager.getActiveEnemies().forEach(e => { if (!e.isDead)
                e.health.takeDamage(9999); }),
            onForceFlee: () => {
            ...
            onFastTravel: (pos) => { if (gameMode !== 'exterior')
                switchToExterior(); player.teleport(new THREE.Vector3(pos.x, pos.y + 2, pos.z)); },
            onRunWave: (_count, _interval, hp, damage) => {
                // Called once per enemy by the wave timer in DevSandbox
                const playerPos = player.group.position;
                const angle = Math.random() * Math.PI * 2;
                const r = 4 + Math.random() * 2;
                const pos = new THREE.Vector3(playerPos.x + Math.cos(angle) * r, 1.5, playerPos.z + Math.sin(angle) * r);
                const en = new SlimeEnemy(pos, physics, (dmg) => { player.health.takeDamage(dmg); if (dmg > 0) {
                    cameraRig.shake(0.12, 0.22);
                    hud.flashHit();
                    gameLoop.freeze(2);
                    dmgNumbers.spawn(player.group.position.clone().setY(player.group.position.y + 1.5), dmg, "damage");
                } });
                en.health.forceSetHp(hp);
                en._baseDamage = damage;
                scene.add(en.group);
            ...
            onSwapPlayerModel: (path) => {
                ...
                import('@/characters/charManifest').then(({ CHAR_MODELS }) => {
                    ...
                    if (def)
                        player.applyAssetModel(def).catch((e) => console.warn('[sandbox] model swap failed:', e));
                });
            },
            ...
            onPlayPreviewAnim: (clipName) => {
                const loaded = window['__previewLoaded'];
                if (!loaded?.mixer)
                    return;
                ...
                if (clip)
                    loaded.mixer.clipAction(clip).play();
            },
            ...
            onSpawnCreature: (dna) => {
                ...
                if (dna.archetype === 'biped') {
                    ...
                    if (qwc.isApplicable) {
                        ...
                        rig.root.traverse(obj => { if (obj instanceof THREE.Mesh)
                            box.union(new THREE.Box3().setFromObject(obj)); });
                        if (!box.isEmpty() && isFinite(box.min.y) && box.min.y > 0.01)
    ...
    function _shadowArrivalVfx(pos, targetScene) {
        for (let i = 0; i < 5; i++) {
    ...
    // ── Story toast ──────────────────────────────────────────────────────────
    function _storyToast(text, kind) {
        const el = document.createElement('div');
     ...
     */
    function _spawnBindingCircle(roomScene) {
        ...
        // Glowing rune disc
        const disc = new THREE.Mesh(new THREE.CircleGeometry(1.2, 32), new THREE.MeshBasicMaterial({ color: 0x4400aa, transparent: true, opacity: 0.35, side: THREE.DoubleSide }));
        disc.rotation.x = -Math.PI / 2;
        group.add(disc);
        // Thin outer ring
        const ring = new THREE.Mesh(new THREE.TorusGeometry(1.2, 0.04, 6, 48), new THREE.MeshBasicMaterial({ color: 0x6622ff, transparent: true, opacity: 0.6 }));
        ring.rotation.x = Math.PI / 2;
        group.add(ring);
        // Subtle pulse light
    ...
    if (import.meta.env?.DEV) {
        ...
        window.__game = {
            ...
            switchToInterior: (roomId) => switchToInterior(roomId),
            ...
            isGameRunning: () => gameLoop.running === true,
            ...
            getPrincessInfo: () => {
                const inst = player._princessInstance;
                if (!inst)
                    return null;
            ...
            spawnTestNpc: (species = 'human', role = 'merchant') => {
                import('@/npc-creator/defaults/NpcDefaults').then(({ getDefaultNpcDna }) => {
                    import('@/npc-creator/builder').then(({ buildNpc }) => {
                        const dna = getDefaultNpcDna(species, role, Date.now());
            ...
            spawnTestEnemy: (species = 'undead', role = 'melee', tier = 1) => {
                import('@/enemy-creator/defaults/EnemyDefaults').then(({ getDefaultEnemyDna }) => {
                    import('@/enemy-creator/builder').then(({ buildEnemy }) => {
                        const dna = getDefaultEnemyDna(species, role, tier, Date.now());
                ...
                import('@/world/buildings/BuildingBuilder').then(({ buildBuilding }) => {
                    import('@/world/buildings/BuildingDNA').then(({ STYLE_COLORS }) => {
                        const dna = {
                            ...
                            colors: STYLE_COLORS[style] ?? STYLE_COLORS['thatched'],
                            rotation: 0,
                        };
                    ...
                    import('@/world/buildings/BuildingBuilder').then(({ buildBuilding }) => {
                        import('@/world/buildings/BuildingDNA').then(({ STYLE_COLORS }) => {
                            ...
                            const dna = {
                                ...
                                condition: 'weathered', hasInterior: true, interiorLayout: 'single_room',
                                colors: STYLE_COLORS[style] ?? STYLE_COLORS['timber'],
                            ...
                            overworld?._buildingData?.push({ dna, pos: new THREE.Vector3(bx, 0, bz) });
            ...
            enterBackroom: async (id = 'building_lab') => {
                ...
                const prev = window.__activeBackroomCleanup;
                prev?.();
                ...
                window.__activeBackroomCleanup = () => {
                    sceneCleaner?.();
                    try {
                        physics.rapierWorld.removeRigidBody(groundBody);
                    }
            ...
            exitBackroom: () => {
                const cleanup = window.__activeBackroomCleanup;
                cleanup?.();
                window.__activeBackroomCleanup = null;
                window.__backroomReady = false;
            },
            hasAssetTrees: () => !!scene.__assetTreesLoaded,
            hasAssetRocks: () => !!scene.__assetRocksLoaded,
            hasAssetClutter: () => !!scene.__assetClutterLoaded,
            hasAssetRiver: () => !!scene.__assetRiverLoaded,
            hasAssetTower: () => !!scene.__assetTowerLoaded,
            hasAssetSettlement: () => !!scene.__assetSettlementLoaded,
            hasAssetDungeon: () => !!scene.__assetDungeonLoaded,
            ...
            quickPlayPrincess: (codeOrDna) => {
                ...
                import('@/princess-creator/dna').then(({ shareCodeToDna }) => {
                    ...
                    import('@/creatures/CreatureDNA').then(({ DEFAULT_PLAYER_DNA }) => {
                        mainMenu.hide();
                        startGame(undefined, {
                            name: resolved?.name ?? 'Princess',
                            boon: 'tome',
                            slotId: 0,
                            dna: { ...DEFAULT_PLAYER_DNA },
                            statBonuses: ['intelligence', 'magic_power'],
                            characterId: 'rogue',
                            princessDna: resolved ?? undefined,
                            princessSpecies: 'human',
                        });
                    });
                });
            ...
            enterCreativeMode: () => { if (import.meta.env.DEV)
                CreativeMode.enter(); },
            /** Exit creative mode (dev only). */
            exitCreativeMode: () => { if (import.meta.env.DEV)
                CreativeMode.exit(); },
            ...
            setGameSpeed: (scale) => { physics.cullingRadius; gameLoop.timeScale = Math.max(0.01, Math.min(4, scale)); },
            ...
            buildPrincess: (dna) => {
                import('@/princess-creator/dna').then(({ sanitizeDna, shareCodeToDna }) => {
                    const resolved = typeof dna === 'string' ? shareCodeToDna(dna) : sanitizeDna(dna);
                    if (!resolved) {
                        console.error('[__game.buildPrincess] invalid DNA');
                        return;
                    }
    ...
    window.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Escape') {
            ...
            if (!gameMenu.isOpen && !editMode.isActive)
                controlsOverlay.toggle();
        }
        else if (e.key === 'k' || e.key === 'K') {
            if (import.meta.env.DEV && CreativeMode.active)
                return;
            if (!gameMenu.isOpen && !editMode.isActive)
                spellBook.toggle();
        }
            ...
            if (import.meta.env.DEV && CreativeMode.active)
                return;
            if (!gameMenu.isOpen && !editMode.isActive)
                statPanel.toggle(progression);
        }
        else if (e.key === 't' || e.key === 'T') {
            if (import.meta.env.DEV && CreativeMode.active)
                return;
            if (!gameMenu.isOpen && !editMode.isActive)
                talentTree.toggle(progression, talentSystem);
        }
        else if (e.key === '`' || e.key === '~') {
            if (!gameMenu.isOpen)
                editMode.toggle(); // shortcut: direct editor toggle
        }
            ...
            if (!gameMenu.isOpen && devModeEnabled())
                owEditor?.toggle();
        }
    });
    ...
    function _buildConstructionMenu() {
        const TYPES = ['barrier_wall', 'watch_perch', 'healing_fountain', 'ward_stone'];
        ...
        for (const type of TYPES) {
            ...
            btn.addEventListener('click', () => {
                ...
                _rebuildGhost();
                // highlight active
                wrap.querySelectorAll('button').forEach(b => { b.style.background = 'rgba(30,20,50,0.9)'; });
    ...
    let _constructionMenuEl = null;
    function _toggleConstructionMode() {
    ...
    function _exitConstructionMode() {
        _constructionMode = false;
        _selectedStructure = null;
        _rebuildGhost(); // clears ghost
        if (_constructionMenuEl) {
            _constructionMenuEl.remove();
            _constructionMenuEl = null;
        }
    }
    ...
    renderer.domElement.addEventListener('click', (e) => {
        ...
        if (!_constructionMode && _spawnedRigs.length > 0) {
            ...
            raycaster.setFromCamera(new THREE.Vector2(((e.clientX - rect2.left) / rect2.width) * 2 - 1, -((e.clientY - rect2.top) / rect2.height) * 2 + 1), cameraRig.camera);
            const pickedMeshes = [];
            for (const { rig } of _spawnedRigs)
                rig.root.traverse(o => { if (o instanceof THREE.Mesh)
                    pickedMeshes.push(o); });
            ...
            if (hits.length > 0) {
                ...
                for (let ci = 0; ci < _spawnedRigs.length; ci++) {
                    let found = false;
                    _spawnedRigs[ci].rig.root.traverse(o => { if (o === hitObj)
                        found = true; });
                    if (found) {
                        newIdx = ci;
                        break;
        ...
        if (!_constructionMode || !_selectedStructure || gameMode !== 'exterior')
            return;
        e.stopPropagation();
        // Raycast to floor plane at y=0
        const rect = renderer.domElement.getBoundingClientRect();
        raycaster.setFromCamera(new THREE.Vector2(((e.clientX - rect.left) / rect.width) * 2 - 1, -((e.clientY - rect.top) / rect.height) * 2 + 1), cameraRig.camera);
        raycaster.ray.intersectPlane(floorPlane, mouseWorld);
        // Check resource cost
        const costs = STRUCTURE_COSTS[_selectedStructure];
        const canAfford = Object.entries(costs).every(([res, amt]) => inventory.get(res) >= (amt ?? 0));
        if (!canAfford) {
            // Brief flash on the menu — not enough resources
            if (_constructionMenuEl) {
                _constructionMenuEl.style.borderColor = '#cc2244';
                setTimeout(() => { if (_constructionMenuEl)
                    _constructionMenuEl.style.borderColor = '#553388'; }, 600);
            }
            return;
        }
        // Spend resources
        const costRecord = Object.fromEntries(Object.entries(costs).filter(([, v]) => v !== undefined));
        inventory.spendMulti(costRecord);
        hud.setResources(inventory.snapshot());
        craftingUI.refresh();
        // Place structure
    ...
    function _setExteriorPrompt(text) {
        if (!text) {
            exteriorPrompt.style.opacity = '0';
            return;
        }
    ...
    gameLoop.onTick((dt) => {
        ...
        if (!editMode.isActive && !gameMenu.isOpen && !deathScreen.isVisible && !spellBook.isOpen && !devPanel.isOpen) {
            ...
            if (player.hasPrincess)
                player.updatePrincess(performance.now() / 1000, dt);
                ...
                for (let _ci = 0; _ci < _spawnedRigs.length; _ci++) {
                    ...
                    if (walkCtrl) {
                        ...
                        if (isControlled) {
                            ...
                            if (cs.moveLeft)
                                rig.root.rotation.y += TURN_SPEED * dt;
                            if (cs.moveRight)
                                rig.root.rotation.y -= TURN_SPEED * dt;
                            let fwd = 0;
                            if (cs.moveForward)
                                fwd = CTRL_SPEED * (cs.run ? 1.6 : 1);
                            if (cs.moveBackward)
                                fwd = -CTRL_SPEED * 0.55;
                            if (fwd !== 0) {
            ...
            const _previewLoaded = window['__previewLoaded'];
            ...
            if (gameMode === 'interior') {
                ...
                overworld._timeHour = TimeSystem.instance.hour % 24;
                ...
                if (_questCheckTimer <= 0) {
                    ...
                    for (const q of questLog.getActive()) {
                        if (!q.fulfilled && checkQuestFulfillment(q, _gc.col, _gc.row, discoveryTracker.clearedDungeons)) {
                            ...
                            minimap?.setQuestPins(questLog.getActive().map(aq => ({ col: aq.target.col, row: aq.target.row })));
                        }
                    }
                    // Story runner tick (same 1 Hz throttle is fine)
                    if (_storyRunner) {
                        const _nearby = []; // settlement proximity — extended when OverworldScene exposes getSettlementsNear()
                        _storyRunner.tick({
                            killCount: sceneManager.killCount,
                            dungeonsClearedCount: discoveryTracker.clearedDungeons.size + _towerRoomsClearedCount,
                            itemsCraftedCount: _craftedItemCount,
                ...
                const _flee = overworld.getActiveEnemies().find(e => !e.isDead && e.isRecruitable && e.worldPosition.distanceTo(_pos) < 6.0);
                if (_flee) {
                        ...
                        if (_bld) {
                            ...
                            if (_nearNPC) {
                                ...
                                if (_wp && party.members.some(m => !m.isGuarding)) {
                                    ...
                                    const _LABELS = { ore: '⛏ Mine ore', timber: '🪵 Chop timber', essence: '✨ Harvest essence' };
                ...
                if (_harvestNodeIdx !== null) {
                    ...
                    if (pct >= 1) {
                        ...
                        inventory.add(nodeType, finalAmt);
                        hud.setResources(inventory.snapshot());
                        _cancelHarvest();
                    }
                }
            ...
            if (gameMode === 'interior') {
                ...
                const _nearFleeInt = sceneManager.getActiveEnemies().find(en => !en.isDead && en.isRecruitable
                ...
                interactables.overridePrompt(_nearFleeInt
            ...
            if (attackJustPressed && meleeCooldown <= 0) {
                meleeCooldown = 0.4;
                const meleeAngle = Math.atan2(mouseWorld.x - player.group.position.x, mouseWorld.z - player.group.position.z);
                ...
                (target, damage) => {
                    ...
                    if (pos)
                        dmgNumbers.spawn(pos.clone().setY(pos.y + 1.8), damage, 'damage');
            ...
            if (interactJustPressed && !bookReader.isOpen && !telescopeView.active && !tamingGame.active) {
                if (gameMode === 'interior') {
                    ...
                    const nearFleeInt = sceneManager.getActiveEnemies().find(en => !en.isDead && en.isRecruitable
                        && en.worldPosition.distanceTo(player.group.position) < 6.0);
                    ...
                    const nearFlee = overworld.getActiveEnemies().find((en) => !en.isDead && en.isRecruitable
                        && en.worldPosition.distanceTo(player.group.position) < 6.0);
                    if (nearFlee) {
                        if (!party.isFull) {
            ...
            if (castJustPressed) {
                ...
                if (activeSpell && progression.isSpellUnlocked(activeSpell)) {
                    spells.cast(activeSpell, player.group.position, mouseWorld, enemies, scene, 
                    ...
                    (target, damage) => {
                        ...
                        if (pos) {
                            ...
                            if (isCrit) {
                                cameraRig.shake(0.08, 0.25);
                                gameLoop.freeze(3);
                            } // G3: 3-frame hit-stop on crit
                        }
                    }, {
                        spellDamageMult: progression.derivedSpellDamageMult,
                        aoeRadiusMult: progression.mods.aoeRadiusMult,
                        party,
                        onForceFlee: (targets) => targets.forEach(e => e.forceFlee?.()),
                        ...
                        onBlink: (_origin) => {
                            ...
                            // Wall check
                            const rayHit = physics.castRayVsWalls(playerPos.clone().setY(playerPos.y + 0.8), dir, dist + 0.5);
            ...
            const _abCtx = {
                ...
                progression,
                enemies: sceneManager.getActiveEnemies(),
                playerHp: player.health,
            };
            if (s.ability1)
                abilities.trycast(0, _abCtx);
            if (s.ability2)
                abilities.trycast(1, _abCtx);
            if (s.ability3)
                abilities.trycast(2, _abCtx);
            if (s.ability4)
                abilities.trycast(3, _abCtx);
            ...
            if (gameMode === 'interior')
                _updateHealOrbs(dt, scene, player, particles);
        }
        // 8. Camera
        ...
        const activeEnemiesForBars = gameMode === 'interior'
            ? sceneManager.getActiveEnemies()
            : (overworld?.getActiveEnemies() ?? []);
        enemyBars.update(activeEnemiesForBars, player.group.position);
        ...
        if (currKillCount > prevKillCount) {
            ...
            for (const e of sceneManager.getActiveEnemies()) {
                if (e.isDead) {
                    const eid = e.group.userData['enemyId'];
                    if (eid)
                        _eliteEnemiesKilled.add(eid);
                }
            }
        }
        ...
        hud.update(player.health.hp, player.health.maxHp, sceneManager.killCount, sceneManager.totalEnemies, sceneManager.currentFloor, progression.getEquippedSlots(), input.activeSlot, player.dodgeReadyFraction, input.state.run, getFloorDef(sceneManager.currentFloor)?.name, progression.level, progression.xpProgress);
        ...
        if (gameMode === 'exterior') {
            ...
            if (_constructionMode && _ghostMesh && overworld) {
                raycaster.setFromCamera(mouseNDC, cameraRig.camera);
                raycaster.ray.intersectPlane(floorPlane, mouseWorld);
                _ghostMesh.position.set(Math.round(mouseWorld.x / 2) * 2, 1, Math.round(mouseWorld.z / 2) * 2);
            }
        ...
        hud.setCooldowns(equippedIds.map(id => id ? spells.cooldownFraction(id) : null), equippedIds.map(id => id ? spells.cooldownRemaining(id) : null));
        // Buff bar — tick and update
        consumables.tickBuffs();
        buffBar.update(consumables.activeBuffs);
        // Party strip — follower HP chips
        partyStrip.setMembers(party.members.map((m, idx) => ({
            name: `Follower ${idx + 1}`,
            icon: '🟣',
            hp: m.hp,
            maxHp: m.maxHp,
...
const _healOrbs = [];
function _spawnHealOrb(scene, player, particles) {
...
function _updateHealOrbs(dt, scene, player, particles) {
