...
import {
  buildCauldron, buildGoblet, buildArch, buildBookStack, buildLantern,
  buildBed, buildTable, buildChair, buildWardrobe, buildCampfire, buildTelescope,
  buildShelf, buildPillar, buildAltar, buildRug, buildBanner, buildFireplace,
} from '@/rendering/ProceduralProps';
import {
  TEXTURE_PRESETS, buildMaterial, buildEmissiveMaterial,
...
import {
  ...
  type TowerVersion,
  type TowerItem,
} from '@/world-editor/towerVersioning';
...
function _solidMat(hex: string, roughness = 0.8, metalness = 0): THREE.MeshStandardMaterial {
...
interface ParamDef { id: string; label: string; min: number; max: number; step: number; default: number; }

interface AssetTypeDef {
  id: string;
  label: string;
  emoji: string;
  category: 'furniture' | 'props' | 'structures' | 'light';
  params: ParamDef[];
  /** When true the second material is built as emissive (fire/glow). */
  emissive2?: boolean;
  build: (p: Record<string, number>, mat1: THREE.Material, mat2: THREE.Material) => THREE.Group;
}
...
const ASSET_TYPES: AssetTypeDef[] = [
  ...
  },
  {
    ...
    build: (p, _mat1, _mat2) => buildRug(
      (document.querySelector('#asset-color') as HTMLInputElement)?.value ?? '#8b5e3c',
      (document.querySelector('#asset-color2') as HTMLInputElement)?.value ?? '#d4aa00',
      p['w'] ?? 2.0, p['d'] ?? 1.4, Math.round(p['pattern'] ?? 2),
    ),
  },
  // ─────────────── STRUCTURES ─────────────────────────────────────────────────
  {
  ...
  },
  {
    ...
    build: (p, mat1) => buildBanner(
      mat1,
      (document.querySelector('#asset-color') as HTMLInputElement)?.value  ?? '#8b2222',
      (document.querySelector('#asset-color2') as HTMLInputElement)?.value ?? '#d4aa00',
      p['w'] ?? 0.65, p['h'] ?? 1.4, Math.round(p['pattern'] ?? 1),
    ),
  },
  {
...
function _buildBarrel(ht: number, rd: number, hoopCount: number, woodMat: THREE.Material, hoopMat: THREE.Material): THREE.Group {
...
function _buildCrate(sz: number, woodMat: THREE.Material, braceMat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();
  ...
  grp.add(body);
  // Cross brace on top
  for (const [w, d] of [[sz * 0.9, 0.06], [0.06, sz * 0.9]] as [number, number][]) {
...
function _buildChest(sz: number, woodMat: THREE.Material, metalMat: THREE.Material): THREE.Group {
  const grp = new THREE.Group();
  const scale = sz;
...
interface LibraryEntry {
  ...
  color2: string;
  createdAt: number;
  kind: 'asset' | 'building' | 'room';
  buildingType?: string;
  modelPath?: string;
  thumbnail?: string;   // base64 JPEG data-URL captured at save time
}
...
function addToLibrary(entry: LibraryEntry): void {
  const lib = loadLibrary();
  lib.push(entry);
  saveLibrary(lib);
}

function deleteFromLibrary(id: string): void {
...
const canvas = qs<HTMLCanvasElement>('#canvas3d');
const wrap   = qs<HTMLDivElement>('#viewport-wrap');
...
const ground = new THREE.Mesh(
  ...
  new THREE.MeshLambertMaterial({ color: 0x1e2430 }),
);
...
let _thumbRenderer: THREE.WebGLRenderer | null = null;

function _captureThumb(): string {
  if (!_thumbRenderer) {
...
function showPreview(grp: THREE.Group): void {
  clearPreview();
  previewGroup = grp;
  // Centre on ground
...
let activeTab: 'assets' | 'models' | 'tiles' | 'tower' | 'buildings' | 'library' = 'assets';

// ── Models tab state ──────────────────────────────────────────────────────────
type ModelEntry = { id: string; label: string; path: string; subcat: string; pack: string };
type ModelManifest = Record<string, ModelEntry[]>;
let _modelManifest: ModelManifest | null = null;
let _modelLoader: GLTFLoader | null = null;
let _currentModelGroup: THREE.Group | null = null;
let _modelMixer: THREE.AnimationMixer | null = null;
let _modelAnims: THREE.AnimationAction[] = [];

// ── Tower palette state ───────────────────────────────────────────────────────
let _towerPaletteMode: 'items' | 'library' = 'items';
let _towerLibSearch = '';
let _towerLibModelPath: string | null = null; // selected library model path

// ── Tower editor tool state ───────────────────────────────────────────────────
type TowerTool = 'place' | 'select';
let _towerTool: TowerTool = 'place';
let _towerSnapToGrid = true;
let _towerSelectedIdx: number | null = null;
let _towerSelectionBox: THREE.BoxHelper | null = null;
...
function animate(): void {
  ...
  orbit.update();
  if (_modelMixer) _modelMixer.update(delta);
  if (_towerSelectionBox) _towerSelectionBox.update();
  renderer.render(scene, camera);
}
animate();
let selectedAssetId: string | null = null;
let paramValues: Record<string, number> = {};
let libCat: 'assets' | 'buildings' | 'rooms' = 'assets';

// Material preset state (Asset Studio)
let mat1Preset = 'wood';
let mat1Params: Record<string, number> = {};
let mat2Preset = 'metal';
let mat2Params: Record<string, number> = {};

// Tower editor state
/** Items placed per floor — key is floorIndex. */
const towerFloorItems = new Map<number, TowerItem[]>();
/** Get (or lazily create) the items array for a specific floor. */
function _getTowerFloorItems(floor: number): TowerItem[] {
  if (!towerFloorItems.has(floor)) towerFloorItems.set(floor, []);
  return towerFloorItems.get(floor)!;
}
let towerPaletteType: InteractableType = 'barrel';
...
function _saveTowerVersionToStorage(label?: string): void {
  const v = createVersion(localStorage, towerFloorItems as Map<number, TowerItem[]>, label);
  renderTowerVersionHistory();
  toast(`Saved version "${v.label}"`);
}

function _restoreTowerVersion(v: TowerVersion): void {
  restoreVersion(v, towerFloorItems as Map<number, TowerItem[]>);
...
function renderTowerVersionHistory(): void {
  const container = qs<HTMLDivElement>('#tower-version-list');
  const countEl   = qs<HTMLSpanElement>('#tower-version-count');
...
// Building state
let currentBuildingGroup: THREE.Group | null = null;
...
function switchTab(tab: typeof activeTab): void {
  ...
  if (towerGridGroup) { scene.remove(towerGridGroup); towerGridGroup = null; }
  ...
  if (currentBuildingGroup) { scene.remove(currentBuildingGroup); currentBuildingGroup = null; }
  ...
  const libView    = qs<HTMLDivElement>('#library-view');
  const tilesView  = qs<HTMLDivElement>('#tiles-view');
  const leftAssets  = qs<HTMLDivElement>('#left-assets');
  const leftModels  = qs<HTMLDivElement>('#left-models');
  const leftTower   = qs<HTMLDivElement>('#left-tower');
  const leftBld     = qs<HTMLDivElement>('#left-buildings');
  const rightAssets  = qs<HTMLDivElement>('#right-assets');
  const rightModels  = qs<HTMLDivElement>('#right-models');
  const rightTower   = qs<HTMLDivElement>('#right-tower');
  const rightBld     = qs<HTMLDivElement>('#right-buildings');
  const vpLabel      = qs<HTMLSpanElement>('#vp-mode-label');
  ...
  if (tab === 'library') renderLibraryGrid();
  if (tab === 'tower')   initTowerEditor();
  if (tab === 'models')  { _clearTowerItems(); initModelsTab(); }
  if (tab !== 'tower')   { _clearGhost(); _deselectTowerItem(); }
}

document.querySelectorAll('.tab-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    const t = (btn as HTMLElement).dataset['tab'] as typeof activeTab;
    if (t) switchTab(t);
  });
});
...
function _getModelLoader(): GLTFLoader {
  if (!_modelLoader) _modelLoader = new GLTFLoader();
  return _modelLoader;
}

function _clearModelPreview(): void {
  if (_modelMixer) { _modelMixer.stopAllAction(); _modelMixer = null; }
  _modelAnims = [];
  if (_currentModelGroup) {
    ...
    _currentModelGroup.traverse(obj => {
      ...
      if (mesh.isMesh) {
        ...
        (Array.isArray(mesh.material) ? mesh.material : [mesh.material]).forEach(m => (m as THREE.Material).dispose());
      }
    });
...
function _fitCameraToModel(group: THREE.Group): void {
...
function _loadModelPreview(entry: ModelEntry): void {
  _clearModelPreview();
  qs<HTMLDivElement>('#model-preview-name').textContent = entry.label;
  qs<HTMLDivElement>('#model-preview-path').textContent = entry.path;
  qs<HTMLDivElement>('#model-copy-path').textContent = entry.path;
  qs<HTMLDivElement>('#model-anim-list').textContent = 'Loading…';

  _getModelLoader().load(
    entry.path,
    (gltf) => {
      ...
      group.traverse(obj => {
        ...
        if (mesh.isMesh) {
          ...
          for (const mat of mats) {
            ...
            if (std.isMeshStandardMaterial) { std.metalness = 0; std.roughness = Math.max(std.roughness, 0.6); }
          }
        }
      });
      scene.add(group);
      ...
      const animDiv = qs<HTMLDivElement>('#model-anim-list');
      if (clips.length) {
        _modelMixer = new THREE.AnimationMixer(group);
        _modelAnims = clips.map(c => _modelMixer!.clipAction(c));
        _modelAnims[0]!.play();
        animDiv.innerHTML = clips.map((c, i) =>
          `<button class="btn" style="margin:2px 2px 0;font-size:10px" data-anim="${i}">${c.name}</button>`
        ).join('');
        animDiv.querySelectorAll('button').forEach(btn => {
          btn.addEventListener('click', () => {
            const idx = parseInt((btn as HTMLElement).dataset['anim']!);
    ...
    undefined,
    (err) => {
      qs<HTMLDivElement>('#model-anim-list').textContent = 'Load error';
      console.warn('[Models] failed to load', entry.path, err);
    },
  );
}

function _buildModelList(cat: string, search: string): void {
  const container = qs<HTMLDivElement>('#model-list');
  container.innerHTML = '';
  if (!_modelManifest) { container.innerHTML = '<div style="padding:12px;color:#6a7a8a;font-size:12px">Loading manifest…</div>'; return; }
...
function initModelsTab(): void {
  const catSel   = qs<HTMLSelectElement>('#model-category');
  const searchIn = qs<HTMLInputElement>('#model-search');
  ...
  if (!_modelManifest) {
    fetch('/assets/manifest.json')
      ...
      .catch(() => { qs<HTMLDivElement>('#model-list').innerHTML = '<div style="padding:12px;color:#df6a5a;font-size:12px">Failed to load manifest.</div>'; });
  } else {
    refresh();
  }
  ...
  qs('#btn-model-reset-cam').onclick = () => {
    if (_currentModelGroup) _fitCameraToModel(_currentModelGroup);
  };

  qs('#model-copy-path').onclick = () => {
    const path = qs<HTMLDivElement>('#model-copy-path').textContent ?? '';
    if (path) { navigator.clipboard.writeText(path).then(() => toast('Path copied!')); }
  };

  qs('#btn-model-save').onclick = () => {
    const name = qs<HTMLDivElement>('#model-preview-name').textContent ?? '';
    const path = qs<HTMLDivElement>('#model-copy-path').textContent ?? '';
    if (!path) { toast('Select a model first'); return; }
...
function buildAssetTypeList(category: string): void {
  const container = qs<HTMLDivElement>('#asset-type-list');
  container.innerHTML = '';
...
function selectAsset(id: string): void {
  ...
  qs<HTMLDivElement>('#right-asset-name-lbl').textContent = `${def.emoji} ${def.label}`;
...
function populateTexParamSliders(
  slot: 'mat1' | 'mat2',
  presetId: string,
  paramsState: Record<string, number>,
): void {
  ...
  if (!preset || !preset.params.length) return;

  for (const pd of preset.params) {
    if (!(pd.id in paramsState)) paramsState[pd.id] = pd.default;
    ...
    slider.min = String(pd.min); slider.max = String(pd.max); slider.step = String(pd.step);
...
function initMatPresetSelects(): void {
  for (const slot of ['mat1', 'mat2'] as const) {
    const sel = document.getElementById(`${slot}-preset`) as HTMLSelectElement | null;
    if (!sel) continue;

    sel.innerHTML = TEXTURE_PRESETS.map(p =>
    ...
    if (preset) {
      ...
      for (const pd of preset.params) {
        if (!(pd.id in paramsRef)) paramsRef[pd.id] = pd.default;
      }
    }
...
function buildParamSliders(params: ParamDef[]): void {
  const container = qs<HTMLDivElement>('#asset-params-list');
  container.innerHTML = '';
...
function previewCurrentAsset(): void {
  if (!selectedAssetId) return;
  const def = ASSET_TYPES.find(t => t.id === selectedAssetId);
  if (!def) return;

  const color1 = qs<HTMLInputElement>('#asset-color').value;
  const color2 = qs<HTMLInputElement>('#asset-color2').value;
...
function randomizeCurrentAsset(): void {
  if (!selectedAssetId) return;
  const def = ASSET_TYPES.find(t => t.id === selectedAssetId);
  if (!def) return;
  ...
  qs<HTMLInputElement>('#asset-color').value  = preset1.defaultColor;
  qs<HTMLInputElement>('#asset-color2').value = preset1.defaultColor2;

  // Update UI
  const s1 = document.getElementById('mat1-preset') as HTMLSelectElement | null;
  const s2 = document.getElementById('mat2-preset') as HTMLSelectElement | null;
  if (s1) s1.value = mat1Preset;
  if (s2) s2.value = mat2Preset;
...
qs('#btn-save-asset').addEventListener('click', () => {
  if (!selectedAssetId) { toast('Select an asset type first.'); return; }
  const def = ASSET_TYPES.find(t => t.id === selectedAssetId)!;
  const name = qs<HTMLInputElement>('#asset-save-name').value.trim() || `${def.label} #${Date.now().toString(36)}`;
  const entry: LibraryEntry = {
    ...
    params: { ...paramValues },
    color: qs<HTMLInputElement>('#asset-color').value,
    color2: qs<HTMLInputElement>('#asset-color2').value,
    createdAt: Date.now(),
    kind: 'asset',
    thumbnail: _captureThumb(),
  };
  ...
  qs<HTMLInputElement>('#asset-save-name').value = '';
});

// Populate category select
qs('#asset-category').addEventListener('change', (e) => {
  buildAssetTypeList((e.target as HTMLSelectElement).value);
});
...
const INTERACTABLE_LABELS: Partial<Record<InteractableType, string>> = {
  barrel: '🛢 Barrel',
  crate: '📦 Crate',
  chest: '🎁 Chest',
  bookshelf: '📚 Bookshelf',
...
const GRID = TV_GRID;
const CELL = TV_CELL;
void GRID_CX; void worldToGrid; void deleteVersion; void TOWER_VERSION_KEY; // silence unused-import warnings

let towerGridGroup: THREE.Group | null = null;
let towerItemGroups: THREE.Group[] = [];

// ── Tower editor: ghost preview state ─────────────────────────────────────────
let _ghostGroup: THREE.Group | null = null;
let _ghostRotation: Rotation = 0;
let _ghostMaterial: THREE.MeshLambertMaterial | null = null; // lazy-init
function _getGhostMat(): THREE.MeshLambertMaterial {
  if (!_ghostMaterial) {
...
const _TOWER_ITEM_GLB: Partial<Record<InteractableType, string>> = {
...
const _towerGlbCache = new Map<string, THREE.Group>(); // path → template
let _towerGlbLoaderRef: GLTFLoader | null = null;
function _towerGlbLoader(): GLTFLoader {
  if (!_towerGlbLoaderRef) _towerGlbLoaderRef = new GLTFLoader();
  return _towerGlbLoaderRef;
}

// ── Tower palette builders ─────────────────────────────────────────────────────

function _makePaletteItem(label: string, selected: boolean, onClick: () => void): HTMLDivElement {
...
function _buildTowerItemsPalette(container: HTMLDivElement): void {
  ...
  for (const t of types) {
    ...
    container.appendChild(_makePaletteItem(
      (INTERACTABLE_LABELS[t] ?? t) + (hasGlb ? ' ✦' : ''),
      isSelected,
      () => {
        towerPaletteType = t;
...
function _buildTowerLibraryPalette(container: HTMLDivElement): void {
  if (!_modelManifest) {
    const msg = document.createElement('div');
    msg.style.cssText = 'font-size:11px;color:var(--text2);padding:8px 2px';
    msg.textContent = 'Loading library…';
    container.appendChild(msg);
    fetch('/assets/manifest.json').then(r => r.json()).then((m: ModelManifest) => {
      _modelManifest = m;
      if (_towerPaletteMode === 'library') { container.innerHTML = ''; _buildTowerLibraryPalette(container); }
  ...
  const byPackSubcat = new Map<string, Map<string, ModelEntry[]>>();
  for (const entries of Object.values(_modelManifest)) {
    for (const e of entries) {
      if (search && !e.label.toLowerCase().includes(search) && !e.pack.toLowerCase().includes(search) && !e.subcat.toLowerCase().includes(search)) continue;
      const pack = e.pack || 'Other';
      const sub  = e.subcat || 'Misc';
      if (!byPackSubcat.has(pack)) byPackSubcat.set(pack, new Map());
  ...
  const selectEntry = (e: ModelEntry) => {
    _towerLibModelPath = e.path;
    _ghostRotation = 0;
    _clearGhost();
    container.querySelectorAll('.asset-item').forEach(el => el.classList.remove('selected'));
    (container.querySelector(`[data-libpath="${CSS.escape(e.path)}"]`) as HTMLElement | null)?.classList.add('selected');
  };
...
function initTowerEditor(): void {
  // ── Populate floor selector once (not on every palette re-render) ──────────
  const floorSel = qs<HTMLSelectElement>('#tower-floor-sel');
  if (!_towerFloorSelectorReady) {
    ...
    if (!floorSel.value) floorSel.selectedIndex = 0;
    renderTowerVersionHistory();
  }
  ...
  // ── Wire tool buttons ────────────────────────────────────────────────────────
  const toolPlace  = qs<HTMLButtonElement>('#tower-tool-place');
  const toolSelect = qs<HTMLButtonElement>('#tower-tool-select');
  const snapGrid   = qs<HTMLInputElement>('#tower-snap-grid');
  ...
  const palette    = qs<HTMLDivElement>('#tower-palette');
  const tabItems   = qs<HTMLButtonElement>('#tower-tab-items');
  const tabLib     = qs<HTMLButtonElement>('#tower-tab-lib');
  const libSearch  = qs<HTMLInputElement>('#tower-lib-search');
  palette.innerHTML = '';
  ...
  libSearch.oninput = (e) => {
    _towerLibSearch = (e.target as HTMLInputElement).value.toLowerCase();
  ...
  for (const item of _getTowerFloorItems(floorIdx)) {
    _renderTowerItem(
      item.type, item.x, item.z, item.rotation as Rotation,
      item.modelPath, item.scale ?? 1,
      item.rotX ?? 0, item.rotY, item.rotZ ?? 0, item.y ?? 0,
    );
  }
...
function _towerItemColor(type: InteractableType): number {
  const colorMap: Partial<Record<InteractableType, number>> = {
...
function _makeBoxPlaceholder(type: InteractableType, mat?: THREE.Material): THREE.Group {
  const grp = new THREE.Group();
...
function _clearTowerItems(): void {
  _deselectTowerItem();
  for (const g of towerItemGroups) {
    scene.remove(g);
    g.traverse(obj => {
...
function _selectTowerItem(idx: number): void {
  _deselectTowerItem();
  ...
  const label = item?.type === 'prop'
    ...
    : (INTERACTABLE_LABELS[item?.type as InteractableType] ?? item?.type ?? '?');
  qs<HTMLDivElement>('#tower-sel-info').textContent = label;
  qs<HTMLDivElement>('#tower-sel-controls').style.display = '';
  // Scale
  qs<HTMLInputElement>('#tower-sel-scale').value = String(grp.scale.x);
  ...
  qs<HTMLInputElement>('#tower-sel-rotx').value = String(rx);
  qs<HTMLInputElement>('#tower-sel-roty').value = String(ry);
  qs<HTMLInputElement>('#tower-sel-rotz').value = String(rz);
  qs('#tower-sel-rotx-val').textContent = rx + '°';
  qs('#tower-sel-roty-val').textContent = ry + '°';
  qs('#tower-sel-rotz-val').textContent = rz + '°';
}

function _deselectTowerItem(): void {
  _towerSelectedIdx = null;
  _towerDragging = false;
  _towerDragMoved = false;
  if (_towerSelectionBox) { scene.remove(_towerSelectionBox); _towerSelectionBox = null; }
...
function _towerRaycastItems(event: MouseEvent): number | null {
  ...
  for (let i = 0; i < towerItemGroups.length; i++) {
    ...
    towerItemGroups[i]!.traverse(o => { if (o instanceof THREE.Mesh) meshes.push(o); });
    if (meshes.length && ray.intersectObjects(meshes, false).length > 0) return i;
  }
  return null;
}

function _clearGhost(): void {
  if (_ghostGroup) { scene.remove(_ghostGroup); _ghostGroup = null; }
}

function _renderTowerItem(
  type: InteractableType | 'prop',
  x: number, z: number,
  rotation: Rotation = 0,
  modelPath?: string, scale = 1,
  rotX = 0, rotY?: number, rotZ = 0, y = 0,
): THREE.Group {
  const grp = new THREE.Group();
  ...
  const glbPath = type === 'prop' ? (modelPath ?? null) : (_TOWER_ITEM_GLB[type as InteractableType] ?? null);

  // Start with colored box placeholder
  const placeholder = type === 'prop'
    ? _makeBoxPlaceholder('barrel' as InteractableType) // generic placeholder for props
    : _makeBoxPlaceholder(type as InteractableType);
  grp.add(placeholder);
  scene.add(grp);
  towerItemGroups.push(grp);

  if (glbPath) {
    const swapIn = (template: THREE.Group) => {
      if (!grp.parent) return;
      grp.remove(placeholder);
      placeholder.traverse(o => { if (o instanceof THREE.Mesh) { o.geometry.dispose(); (o.material as THREE.Material).dispose(); } });
    ...
    if (cached) { swapIn(cached); }
    else { _towerGlbLoader().load(glbPath, (gltf) => { _towerGlbCache.set(glbPath, gltf.scene); swapIn(gltf.scene); }); }
  }
  return grp;
}
...
function _buildGhostGroup(type: InteractableType | 'prop', modelPath?: string): THREE.Group {
  ...
  if (cached) {
    ...
    clone.traverse(o => { if (o instanceof THREE.Mesh) o.material = mat; });
    grp.add(clone);
  } else {
    const placeholder = type === 'prop' ? _makeBoxPlaceholder('barrel' as InteractableType, mat) : _makeBoxPlaceholder(type as InteractableType, mat);
    grp.add(placeholder);
...
function _refreshGhost(x: number, z: number, show: boolean): void {
  if (!show) { _clearGhost(); return; }
  _clearGhost();
...
function _towerRaycast(e: MouseEvent): { x: number; z: number } | null {
  const rect = canvas.getBoundingClientRect();
...
function _towerCurrentFloorIdx(): number {
  return parseInt(qs<HTMLSelectElement>('#tower-floor-sel').value, 10);
}

function _towerFloorR(): number {
...
canvas.addEventListener('mousemove', (e) => {
  ...
  if (_towerTool === 'select' && _towerDragging && _towerSelectedIdx !== null) {
    ...
    if (e.shiftKey) {
      ...
      if (ray.ray.intersectPlane(vertPlane, pt)) {
        ...
        if (_towerSnapToGrid) ny = Math.round(ny / CELL) * CELL;
      ...
      if (ray.ray.intersectPlane(_towerDragPlane, pt)) {
        ...
        if (_towerSnapToGrid) { nx = Math.round(nx / CELL) * CELL; nz = Math.round(nz / CELL) * CELL; }
  ...
  if (_towerTool === 'place' && e.buttons === 0) {
    ...
    if (!hit) { _clearGhost(); return; }
    const dist = Math.sqrt((hit.x / CELL) ** 2 + (hit.z / CELL) ** 2);
    _refreshGhost(hit.x, hit.z, dist <= _towerFloorR() - 1);
  }
});

canvas.addEventListener('mouseleave', () => { if (activeTab === 'tower') _clearGhost(); });

canvas.addEventListener('mousedown', (e) => {
  if (activeTab !== 'tower' || _towerTool !== 'select' || e.button !== 0) return;
  if (_towerSelectedIdx === null) return;
  // Require the click to actually land on the selected item
  if (_towerRaycastItems(e) !== _towerSelectedIdx) return;
...
canvas.addEventListener('mouseup', () => {
  if (_towerDragging) {
    ...
    if (_towerSelectedIdx !== null) {
      ...
      if (item) { item.x = grp.position.x; item.y = grp.position.y; item.z = grp.position.z; }
    }
  }
  _towerDragging = false;
});
...
canvas.addEventListener('click', (e) => {
  if (activeTab !== 'tower') return;
  if (_towerDragMoved) { _towerDragMoved = false; return; }

  if (_towerTool === 'select') {
    const hitIdx = _towerRaycastItems(e);
    if (hitIdx === null) { _deselectTowerItem(); }
    else if (hitIdx === _towerSelectedIdx) { _deselectTowerItem(); }
    else { _selectTowerItem(hitIdx); }
    return;
  }
  ...
  if (!hit) return;
  const dist = Math.sqrt((hit.x / CELL) ** 2 + (hit.z / CELL) ** 2);
  if (dist > _towerFloorR() - 1) { toast('Outside chamber bounds'); return; }
  ...
  if (existIdx !== -1) { toast('Cell occupied — right-click to remove'); return; }

  const isLib = _towerPaletteMode === 'library';
  if (isLib && !_towerLibModelPath) { toast('Select a model from the library first'); return; }
...
window.addEventListener('keydown', (e) => {
  ...
  if (_towerTool === 'select' && _towerSelectedIdx !== null) {
    ...
    const grp = towerItemGroups[_towerSelectedIdx];
    if (item && grp) {
      const curDeg = Math.round(((item.rotY ?? item.rotation) % 360 + 360) % 360) as Rotation;
      const newDeg = next[curDeg in next ? curDeg : 0] ?? 90;
      item.rotY = newDeg;
      grp.rotation.y = newDeg * (Math.PI / 180);
      qs<HTMLInputElement>('#tower-sel-roty').value = String(newDeg);
      qs('#tower-sel-roty-val').textContent = newDeg + '°';
      toast(`Yaw: ${newDeg}°`);
    }
  } else {
    _ghostRotation = next[_ghostRotation];
    if (_ghostGroup) _ghostGroup.rotation.y = _ghostRotation * (Math.PI / 180);
...
qs('#btn-tower-clear').addEventListener('click', () => {
  const floorIdxClear = parseInt(qs<HTMLSelectElement>('#tower-floor-sel').value, 10);
...
qs<HTMLInputElement>('#tower-sel-scale').addEventListener('input', (e) => {
  if (_towerSelectedIdx === null) return;
...
(['rotx', 'roty', 'rotz'] as const).forEach(axis => {
  qs<HTMLInputElement>(`#tower-sel-${axis}`).addEventListener('input', (ev) => {
    if (_towerSelectedIdx === null) return;
...
qs('#btn-tower-export').addEventListener('click', () => {
  const floorIdx = parseInt(qs<HTMLSelectElement>('#tower-floor-sel').value, 10);
  const floorDef = TOWER_FLOOR_DEFS.find(d => d.floorIndex === floorIdx);
  const output = {
  ...
  a.href = url; a.download = `${floorDef?.id ?? 'room'}_layout.json`;
  a.click();
...
qs('#btn-tower-save-version').addEventListener('click', () => {
  const label = qs<HTMLInputElement>('#tower-version-label').value.trim();
  _saveTowerVersionToStorage(label || undefined);
  qs<HTMLInputElement>('#tower-version-label').value = '';
});
...
qs('#btn-tower-load-game').addEventListener('click', () => {
  ...
  if (hasEdits) {
    if (!confirm('Load the game tower layout?\n\nYour current unsaved changes will be replaced. Save a version first if you want to keep them.')) return;
  }
  _loadTowerFromGame();
});
...
function _syncBuildingSlider(id: string): void {
  const slider = qs<HTMLInputElement>(`#${id}`);
  const valEl  = qs<HTMLSpanElement>(`#${id}-v`);
...
function generateBuildingPreview(): void {
  const typeKey  = qs<HTMLSelectElement>('#bld-type').value;
  const seed     = parseInt(qs<HTMLInputElement>('#bld-seed').value, 10) || 42;
...
qs('#btn-bld-random').addEventListener('click', () => {
  qs<HTMLInputElement>('#bld-seed').value = String(Math.floor(Math.random() * 99999));
  generateBuildingPreview();
});

qs('#btn-bld-save').addEventListener('click', () => {
  const typeKey = qs<HTMLSelectElement>('#bld-type').value;
  const seed    = qs<HTMLInputElement>('#bld-seed').value;
  const entry: LibraryEntry = {
    id: crypto.randomUUID(),
    ...
    params: {
      ...
      width: parseInt(qs<HTMLInputElement>('#bld-width').value, 10),
      depth: parseInt(qs<HTMLInputElement>('#bld-depth').value, 10),
      floors: parseInt(qs<HTMLInputElement>('#bld-floors').value, 10),
    },
    color: '#8a7a6a',
    color2: '#5a5a6a',
    createdAt: Date.now(),
...
document.querySelectorAll('.lib-cat-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    libCat = (btn as HTMLElement).dataset['libcat'] as typeof libCat;
    renderLibraryGrid();
  });
});
...
function renderLibraryGrid(): void {
  const grid    = qs<HTMLDivElement>('#lib-grid');
  const entries = loadLibrary().filter(e => {
    if (libCat === 'assets')    return e.kind === 'asset';
    if (libCat === 'buildings') return e.kind === 'building';
    if (libCat === 'rooms')     return e.kind === 'room';
    return false;
  });
  ...
  for (const entry of entries) {
    ...
    delBtn.addEventListener('click', (ev) => {
      ...
      card.remove();
      if (!grid.childElementCount) renderLibraryGrid(); // show empty state
    ...
    expBtn.addEventListener('click', (ev) => {
      ...
      const url  = URL.createObjectURL(blob);
      const a    = document.createElement('a');
      a.href = url; a.download = `${entry.name.replace(/[^a-z0-9]/gi, '_')}.json`;
...
function loadLibraryEntry(entry: LibraryEntry): void {
  if (entry.kind === 'asset') {
    switchTab('assets');
    const def = ASSET_TYPES.find(t => t.id === entry.typeId);
    if (!def) { toast('Unknown asset type'); return; }
    selectedAssetId = entry.typeId;
    paramValues = { ...entry.params };
    qs<HTMLInputElement>('#asset-color').value = entry.color;
    qs<HTMLInputElement>('#asset-color2').value = entry.color2;
    qs<HTMLSelectElement>('#asset-category').value = def.category;
    buildAssetTypeList(def.category);
    buildParamSliders(def.params.map(p => ({ ...p, default: paramValues[p.id] ?? p.default })));
    previewCurrentAsset();
    ...
    switchTab('buildings');
    qs<HTMLInputElement>('#bld-seed').value = String(entry.params['seed'] ?? 42);
    qs<HTMLSelectElement>('#bld-type').value = entry.buildingType ?? 'cottage';
    generateBuildingPreview();
    toast(`Loaded "${entry.name}"`);
  } else {
...
qs('#btn-lib-export-all').addEventListener('click', () => {
  ...
  if (!all.length) { toast('Library is empty'); return; }
  const blob = new Blob([JSON.stringify(all, null, 2)], { type: 'application/json' });
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement('a');
  a.href = url; a.download = 'ttt_library_export.json'; a.click();
  URL.revokeObjectURL(url);
...
qs('#btn-lib-import').addEventListener('click', () => {
  qs<HTMLInputElement>('#lib-import-input').click();
});
...
qs('#lib-import-input').addEventListener('change', (e) => {
  ...
  reader.onload = (ev) => {
    try {
      const imported = JSON.parse(ev.target?.result as string) as LibraryEntry[];
