...
export class MeleeArc {
    ...
    mat;
    geo;
    timer = SWEEP_LIFETIME;
    constructor(originPos, facingAngle, targets, onHit) {
    ...
    // ── Private ───────────────────────────────────────────────────────────────
    applyDamage(origin, facingAngle, targets, onHit) {
        ...
        for (const target of targets) {
            ...
            if (Math.abs(delta.y) > SWEEP_HEIGHT)
                continue;
            ...
            if (dot < Math.cos(SWEEP_ANGLE / 2))
                continue;
            const applied = target.takeDamage(MELEE_DAMAGE);
            if (applied > 0)
                onHit?.(target, applied);
        }
    }
    static buildMesh(facingAngle) {
...
export class CombatSystem {
    ...
    triggerMelee(origin, facingAngle, targets, scene, onHit) {
