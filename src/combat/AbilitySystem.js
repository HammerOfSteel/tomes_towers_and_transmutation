...
export function applyCharacterAbilities(abilities, characterId) {
    ...
    if (id.startsWith('elf_')) {
        ...
        const recall = {
            ...
            cast(ctx) {
                ...
                const lastSpell = ctx.lastCastSpellId;
                if (lastSpell) {
    ...
    if (id.startsWith('celestial_')) {
            ...
            cast(ctx) {
                ...
                for (const deg of ANGLES) {
                    ...
                    for (const e of ctx.enemies) {
                        ...
                        if (angle < 0.25 && diff.length() < 12) {
                            e.takeDamage(BASE_DMG);
                        }
                    }
                }
        ...
        const moonveil = {
            ...
            cast(ctx) {
                ctx.isMoonveiled = true;
                _levitateVfx(ctx.scene, ctx.playerPos.clone());
                setTimeout(() => { ctx.isMoonveiled = false; }, 3000);
            },
        };
            ...
            cast(ctx) {
                ...
                for (const e of ctx.enemies) {
                    if (e.isDead)
                        continue;
                    const diff = e.worldPosition.clone().sub(ctx.playerPos);
                    if (diff.length() > CONE_RANGE)
                        continue;
                    // Face direction — use player facing angle from group.rotation.y
                    const facing = new THREE.Vector3(Math.sin(ctx.playerFacingY ?? 0), 0, Math.cos(ctx.playerFacingY ?? 0));
                    if (diff.angleTo(facing) <= HALF_ANGLE)
                        e.takeDamage(BASE_DMG);
                }
...
function _starburstVfx(scene, pos) {
