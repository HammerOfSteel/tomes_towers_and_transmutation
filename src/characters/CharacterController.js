...
export class CharacterController {
    ...
    get isPlayingOneShot() { return this._oneShotTimer > 0; }
    /**
     * Transition to a sustained looping state.
     * Blocked while a one-shot is playing (unless `force` is true or state is 'die').
     */
    setState(next, crossFadeSec, force = false) {
        if (this._oneShotTimer > 0 && !force && next !== 'idle')
            return;
        if (next === this._loopState && this._oneShotTimer <= 0)
            return;
     ...
     */
    playOnce(state, returnTo = 'idle', onEnd) {
    ...
    // ── Private helpers ───────────────────────────────────────────────────────
    _playLoop(state, crossFadeSec = 0) {
        if (!this._mixer)
            return;
        const clip = findClip(this._clips, state);
        if (!clip)
