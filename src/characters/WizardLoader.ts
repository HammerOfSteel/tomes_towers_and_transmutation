...
interface _Entry { scene: THREE.Group; clips: THREE.AnimationClip[] }
const _cache   = new Map<string, _Entry>();
const _pending = new Map<string, Promise<_Entry>>();
const _loader  = new GLTFLoader();

function _loadGLB(path: string): Promise<_Entry> {
  const hit = _cache.get(path);
  if (hit) return Promise.resolve(hit);
  const inf = _pending.get(path);
  if (inf) return inf;

  const p = new Promise<_Entry>((resolve, reject) => {
    _loader.load(
      path,
      (gltf) => {
        const entry: _Entry = {
...
function _find(clips: THREE.AnimationClip[], name: string): THREE.AnimationClip | null {
 ...
 */
export async function loadWizard(def: WizardDef): Promise<LoadedWizard> {
  const entry = await _loadGLB(def.animPath);
  const group = skeletonClone(entry.scene) as THREE.Group;
  ...
  if (!walkClip) throw new Error(`WizardLoader: no walk clip "${def.clipWalk}" in ${def.animPath}`);
  if (!idleClip) throw new Error(`WizardLoader: no idle clip "${def.clipIdle}" in ${def.animPath}`);
