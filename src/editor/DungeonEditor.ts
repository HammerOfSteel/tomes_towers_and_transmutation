...
import type { DungeonDoc, DungeonRoom } from './EditorSchema';
import { EDITOR_SCHEMA_VERSION } from './EditorSchema';

// ── Room templates ─────────────────────────────────────────────────────────────

const ROOM_TEMPLATES: Record<string, { size: { w: number; d: number }; label: string }> = {
...
export class DungeonEditor {
  private _rooms:       DungeonRoom[] = [];
  private _activeRoom   = 0;
  private _dungeonId    = 'dungeon_new';
  private _dungeonName  = 'New Dungeon';
  ...
  addRoom(templateKey: string): DungeonRoom {
  ...
  connectRooms(fromId: string, toId: string): void {
  ...
  private async _switchToRoom(idx: number): Promise<void> {
    if (idx < 0 || idx >= this._rooms.length) return;
    if (this._rooms[this._activeRoom]) this._saveCurrentRoom();
  ...
  private _renderNodeMap(): void {
    ...
    const positions: { x: number; y: number }[] = this._rooms.map((_, i) => ({
      x: ((i % cols) + 1) * cw,
    ...
    for (const room of this._rooms) {
      ...
      for (const connId of room.connections) {
        ...
        if (toIdx < fromIdx) continue; // draw each connection once
        ctx.beginPath();
    ...
    panel.insertAdjacentHTML('beforeend', `
      ...
      <div style="display:flex;gap:3px;flex-wrap:wrap;margin-bottom:4px" id="dng-template-btns">
        ${Object.entries(ROOM_TEMPLATES).map(([k, v]) =>
          `<button class="dng-add-room" data-tpl="${k}" style="font-size:9px;padding:2px 4px;background:#1a1626;border:1px solid #2a1e40;color:#665588;border-radius:2px;cursor:pointer">${v.label}</button>`
        ).join('')}
      </div>
  ...
  private _bindEvents(panel: HTMLElement): void {
    panel.querySelectorAll<HTMLButtonElement>('.dng-add-room').forEach(btn => {
    ...
    panel.querySelector('#dng-spawn-enemy')?.addEventListener('click', () =>
      this.core.placeSpawn('enemy', new THREE.Vector3(0, 0, 0)));
    panel.querySelector('#dng-spawn-npc')?.addEventListener('click', () =>
      this.core.placeSpawn('npc', new THREE.Vector3(0, 0, 0)));
    panel.querySelector('#dng-exit-enter')?.addEventListener('click', () =>
      this.core.placeExit('dungeon_entrance', new THREE.Vector3(-4, 0, 0)));
    panel.querySelector('#dng-exit-out')?.addEventListener('click', () =>
      this.core.placeExit('dungeon_exit', new THREE.Vector3(4, 0, 0)));
