...
export const TOWER_FLOOR_DEFAULTS = [
...
export class TowerFloorEditor {
    ...
    _floorListEl;
    _propsPanel;
    constructor(core, _serial, container) {
        this.core = core;
    ...
    async _switchToFloor(idx) {
        if (idx < 0 || idx >= this._floors.length)
            return;
        // Save current before switching
        if (this._floors[this._activeIdx])
            this._saveCurrentFloor();
        this._activeIdx = idx;
    ...
    _syncPropsFromFloor(f) {
        f.name = document.getElementById('tfe-floor-name')?.value ?? f.name;
        f.lightPreset = document.getElementById('tfe-light-preset')?.value ?? f.lightPreset;
        f.quote = document.getElementById('tfe-quote')?.value ?? f.quote;
        f.bossRoom = document.getElementById('tfe-boss-room')?.checked ?? f.bossRoom;
    }
    _syncPropsToUI(f) {
        const byId = (id) => document.getElementById(id);
        const nameEl = byId('tfe-floor-name');
        const lightEl = byId('tfe-light-preset');
        const quoteEl = byId('tfe-quote');
        const bossEl = byId('tfe-boss-room');
        const idxEl = byId('tfe-floor-index');
        if (nameEl)
            nameEl.value = f.name;
        if (lightEl)
            lightEl.value = f.lightPreset;
        if (quoteEl)
            quoteEl.value = f.quote;
        if (bossEl)
            bossEl.checked = f.bossRoom;
        if (idxEl)
            idxEl.textContent = `Index: ${f.floorIndex}`;
    }
    // ── Floor boundary grid ─────────────────────────────────────────────────────
    _boundaryMesh = null;
    _updateFloorBoundary(f) {
        if (this._boundaryMesh) {
            this._boundaryMesh.parent?.remove(this._boundaryMesh);
        }
        const hw = f.size.w / 2;
        const hd = f.size.d / 2;
        const pts = [
        ...
        this.core.scene?.add(this._boundaryMesh);
    }
    // ── DOM: floor list panel ───────────────────────────────────────────────────
    _buildFloorListPanel(container) {
        ...
        `;
            ...
            if (this._floors.length > 1) {
                ...
                del.onclick = (e) => {
                    ...
                    if (this._activeIdx >= this._floors.length)
                        this._activeIdx = this._floors.length - 1;
                    this._renderFloorList();
    ...
    _buildFloorPropsPanel(container) {
        const panel = document.createElement('div');
    ...
    _bindPropsPanelEvents(panel) {
        ...
        panel.querySelector('#tfe-export-ts')?.addEventListener('click', () => {
            ...
            const btn = panel.querySelector('#tfe-export-ts');
    ...
    _pendingSpawnType = null;
    _pendingExitType = null;
