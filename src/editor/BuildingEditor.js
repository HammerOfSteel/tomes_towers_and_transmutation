...
export class BuildingEditor {
    ...
    _floors = 2;
    _activeFloor = 0;
    _footprint = { w: 4, d: 4 }; // in 2WU tiles
    _style = 'mixed';
    _roofStyle = 'gabled';
    _buildingId = 'building_new';
    _name = 'New Building';
    _interiorId = '';
    _panel;
    ...
    async autoGenerateRoof() {
        ...
        for (let xi = 0; xi < this._footprint.w; xi++) {
            for (let zi = 0; zi < this._footprint.d; zi++) {
                ...
                const path = 'corner' in roofPaths && isCorner ? roofPaths.corner : roofPaths.flat ?? Object.values(roofPaths)[0];
    ...
    // ── Room preset fill ─────────────────────────────────────────────────────────
    async fillRoomPreset(presetName) {
    ...
    _bindPanelEvents(panel) {
        panel.querySelectorAll('.bld-mode').forEach(btn => {
            btn.addEventListener('click', () => {
                panel.querySelectorAll('.bld-mode').forEach(b => {
                    b.style.background = '#1a1626';
                    b.style.color = '#665588';
                });
                btn.style.background = '#2a1645';
                btn.style.color = '#c090ff';
                this._mode = btn.dataset['mode'];
                const presetsEl = panel.querySelector('#bld-room-presets');
                if (presetsEl)
                    presetsEl.style.display = this._mode === 'interior' ? '' : 'none';
            });
        });
        panel.querySelector('#bld-name')?.addEventListener('change', e => { this._name = e.target.value; });
        panel.querySelector('#bld-floors')?.addEventListener('change', e => { this._floors = Math.max(1, Math.min(3, parseInt(e.target.value))); });
        panel.querySelector('#bld-active-floor')?.addEventListener('change', e => { this._activeFloor = parseInt(e.target.value); });
        panel.querySelector('#bld-style')?.addEventListener('change', e => { this._style = e.target.value; });
        panel.querySelector('#bld-roof-style')?.addEventListener('change', e => { this._roofStyle = e.target.value; });
        panel.querySelector('#bld-auto-roof')?.addEventListener('click', () => this.autoGenerateRoof());
        panel.querySelectorAll('.bld-preset').forEach(btn => {
            btn.addEventListener('click', () => this.fillRoomPreset(btn.dataset['preset']));
        });
