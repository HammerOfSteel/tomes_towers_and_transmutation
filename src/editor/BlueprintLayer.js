...
// ── Materials ─────────────────────────────────────────────────────────────────
function mat(color, opacity, wire = false) {
...
function addCell(group, cx, cz, w, height, depth, mat, yOffset = 0) {
    const wx = (cx - CX) * CELL;
    const wz = (cz - CZ) * CELL;
...
function buildTowerChamber(floorIndex) {
    ...
    for (let i = 0; i < Math.min(maxRooms, DOOR_SLOTS.length); i++) {
        ...
        for (const [ox, oz, w, d] of [
            ...
            const wm = new THREE.Mesh(tile(w * CELL, wallH, d * CELL), M.sideWall());
            wm.position.set((srCX - CX + ox) * CELL, wallH / 2, (srCZ - CZ + oz) * CELL);
...
function buildOverworldBlueprint(onDungeonClick) {
    const group = new THREE.Group();
    ...
    for (let row = 0; row < GH; row++) {
        for (let col = 0; col < GW; col++) {
            ...
            if (!biomeGroups[biome])
                biomeGroups[biome] = [];
            dummy.position.set((col - GW / 2 + 0.5) * TILE_SCALE, 0, (row - GH / 2 + 0.5) * TILE_SCALE);
    ...
    for (const biome of biomeIds) {
        ...
        if (!matrices.length)
            continue;
        const mesh = new THREE.InstancedMesh(tileGeo, new THREE.MeshBasicMaterial({ color: BIOME_COLORS[biome], transparent: true, opacity: 0.75 }), matrices.length);
...
function buildDungeonBlueprint(dungeonEntry) {
    const group = new THREE.Group();
    ...
    for (const bp of ALL_BLUEPRINTS) {
        ...
        const floor = new THREE.Mesh(new THREE.PlaneGeometry(floorW, floorD), new THREE.MeshBasicMaterial({ color: 0x2a1e3a, transparent: true, opacity: 0.5, side: THREE.DoubleSide }));
        ...
        for (const t of bp.tiles) {
            ...
            const wz = (t.z + 0.5) * cs;
            const wall = new THREE.Mesh(new THREE.BoxGeometry(cs * 0.96, wh, cs * 0.96), new THREE.MeshBasicMaterial({ color: 0x140d20, transparent: true, opacity: 0.75 }));
            wall.position.set(wx, wh / 2, wz);
            wall.userData['blueprint'] = true;
            group.add(wall);
            // Wireframe edge
            const edge = new THREE.Mesh(new THREE.BoxGeometry(cs, wh + 0.1, cs), new THREE.MeshBasicMaterial({ color: 0x8855cc, transparent: true, opacity: 0.25, wireframe: true }));
        ...
        for (const d of bp.doors ?? []) {
            ...
            const dz = (d.z + 0.5) * cs;
            const door = new THREE.Mesh(new THREE.BoxGeometry(cs * 0.3, wh * 0.7, cs * 0.9), new THREE.MeshBasicMaterial({ color: 0xffcc44, transparent: true, opacity: 0.7 }));
        ...
        for (const item of bp.interactables ?? []) {
            ...
            const marker = new THREE.Mesh(new THREE.SphereGeometry(0.3, 6, 6), new THREE.MeshBasicMaterial({ color: 0x88ffcc, transparent: true, opacity: 0.8 }));
...
export class BlueprintLayer {
    ...
    hide() { this._clear(); }
    get visible() { return this._group !== null; }
    _clear() {
        if (!this._group)
            return;
        this._group.traverse(obj => {
