...
export type IngredientType = ResourceType | 'recipe_card';

export interface CraftingIngredient {
  type:   IngredientType;
  amount: number;
  /** Display label shown in the UI slot. */
  label:  string;
}

// ── Result ─────────────────────────────────────────────────────────────────

export type ResultKind = 'potion' | 'equipment_token' | 'enchant_chip' | 'blueprint';
...
export type StationType = 'alchemy' | 'forge' | 'enchanting' | 'blueprint';

export interface CraftingRecipe {
  id:           string;
  stationType:  StationType;
  name:         string;
  /** Up to 3 ingredients (the UI has 3 slots). */
  ingredients:  [CraftingIngredient, ...CraftingIngredient[]];
  result:       CraftingResult;
  /** Seconds the brew/forge animation plays before awarding the result. */
  animDuration: number;
  /** Icon for the recipe list. */
  icon:         string;
}
...
export const ENCHANTING_RECIPES: CraftingRecipe[] = [
  {
    id: 'chip_blazing',
...
export const BLUEPRINT_RECIPES: CraftingRecipe[] = [
  {
...
export const RECIPES_BY_STATION: Record<StationType, CraftingRecipe[]> = {
