...
import { RECIPES_BY_STATION, } from './CraftingRecipes';
// ── Styles ─────────────────────────────────────────────────────────────────
const CSS = `
#crafting-panel {
  position: fixed;
...
export class CraftingUI {
    ...
    _showDetail(recipe) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (!ing) {
                ...
                const have = ing.type === 'recipe_card' ? 0 : this.inventory.get(ing.type);
        ...
        const canCraft = recipe.ingredients.every(ing => {
            if (ing.type === 'recipe_card')
                return true; // gated by loot; assume present for now
            return this.inventory.get(ing.type) >= ing.amount;
        });
    ...
    _startCraft() {
        ...
        if (!ok)
            return; // shouldn't happen (button was enabled), but guard anyway
        // Start animation
        this._animElapsed = 0;
