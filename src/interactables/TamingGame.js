...
class SongCircle {
    ...
    update(dt) {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            this._runes[i].position.set(Math.cos(a) * 1.15, 0.12 + Math.sin(t * 2.2 + i * 1.1) * 0.07, Math.sin(a) * 1.15);
        }
        // Decay pulse
        if (this._pulseT > 0) {
            ...
            this._outer.material.color.setHex(this._pulseT > 0.05 ? this._pulseColor : 0x8844cc);
        }
    }
    pulse(quality) {
        this._pulseT = 1.0;
    ...
    setResonance(fraction) {
        const f = Math.max(0, Math.min(1, fraction));
        this._inner.material.color.setHSL(f * 0.38, 0.9, 0.5);
    }
    dispose(scene) {
...
export class TamingGame {
    ...
    // ── Private — game logic ──────────────────────────────────────────────────
    _onWordChosen(wordIndex) {
        if (this._phase !== 'choosing' || !this._slime)
            return;
    ...
    _updateDots() {
        if (!this._dotsEl)
            return;
        this._dotsEl.querySelectorAll('[data-dot]').forEach(d => {
