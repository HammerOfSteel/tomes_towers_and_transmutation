...
export class Stage {
    ...
    constructor(canvas) {
        ...
        // ── Set dressing ──
        const floor = new THREE.Mesh(new THREE.CircleGeometry(60, 48).rotateX(-Math.PI / 2), new THREE.MeshStandardMaterial({ color: '#1d1730', roughness: 0.95 }));
        floor.position.y = -1.2;
        floor.receiveShadow = true;
        const pedestal = new THREE.Mesh(new THREE.CylinderGeometry(7.4, 8.2, 1.2, 36), new THREE.MeshStandardMaterial({ color: '#2a2140', roughness: 0.85, flatShading: true }));
        pedestal.position.y = -0.6;
        pedestal.receiveShadow = true;
        pedestal.castShadow = true;
        const ring = new THREE.Mesh(new THREE.TorusGeometry(7.35, 0.14, 10, 64).rotateX(Math.PI / 2), new THREE.MeshStandardMaterial({ color: '#e8b64c', roughness: 0.35, metalness: 0.85 }));
        ring.position.y = 0.02;
        const runes = new THREE.Mesh(new THREE.RingGeometry(5.6, 6.1, 48).rotateX(-Math.PI / 2), new THREE.MeshBasicMaterial({ color: '#5e4a9e', transparent: true, opacity: 0.35 }));
    ...
    update(t, dt) {
        ...
        for (let i = this.bursts.length - 1; i >= 0; i--) {
            ...
            for (let j = 0; j < bp.count; j++) {
                bp.setXYZ(j, bp.getX(j) + b.velocities[j * 3] * dt, bp.getY(j) + (b.velocities[j * 3 + 1] - b.age * 9) * dt, bp.getZ(j) + b.velocities[j * 3 + 2] * dt);
            }
            bp.needsUpdate = true;
            b.points.material.opacity = Math.max(0, 1 - b.age / 0.9);
            if (b.age > 0.9) {
    ...
    snapshot(size, portrait) {
        ...
        if (portrait) {
            ...
            this.scene.fog = null;
            for (const o of this.setDressing)
                o.visible = false;
