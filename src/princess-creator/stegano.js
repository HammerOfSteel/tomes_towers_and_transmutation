...
export function steganoCapacity(width) {
 ...
 */
export function encodeIntoPixels(pixels, width, height, payload) {
    const bytes = new TextEncoder().encode(payload);
    if (bytes.length > steganoCapacity(width))
        return false;
...
export function decodeFromPixels(pixels, width, height) {
    const startRow = height - STRIP_ROWS;
    if (startRow < 0)
        return null;
    ...
    if (length <= 0 || length > steganoCapacity(width) + 8)
        return null;
...
export async function embedInPngDataUrl(dataUrl, payload) {
    const img = new Image();
