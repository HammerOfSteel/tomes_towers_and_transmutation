...
export function bakeClip(def) {
    ...
    const channel = (get, neutral, lerp) => {
        const known = [];
        keys.forEach((key, i) => {
            const v = get(key);
            if (v !== undefined)
    ...
    const lerpV3 = (a, b, u) => [a[0] + (b[0] - a[0]) * u, a[1] + (b[1] - a[1]) * u, a[2] + (b[2] - a[2]) * u];
    const lerpN = (a, b, u) => a + (b - a) * u;
    const jointTracks = {};
    for (const j of JOINT_IDS) {
        jointTracks[j] = channel((key) => key.joints?.[j], NEUTRAL.joints[j], lerpV3);
    }
    ...
    return {
        ...
        keys: keys.map((key, i) => ({
            ...
            joints: Object.fromEntries(JOINT_IDS.map((j) => [j, jointTracks[j][i]])),
            rootY: rootYs[i],
            rootRot: rootRots[i],
            torsoScale: scales[i],
        })),
...
export function resolveClips(dna, tweaks = {}) {
...
export function speciesAnimInfo(species) {
