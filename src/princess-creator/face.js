 ...
 */
export function buildFace(socket, dna, kit, surfaceR) {
    ...
    const makeEye = (side) => {
        ...
        switch (f.eyeStyle) {
            ...
            case 'sleepy': {
                ...
                const lid = new THREE.Mesh(new THREE.SphereGeometry(r0 * 0.95, 16, 8, 0, Math.PI * 2, 0, Math.PI / 2.4), kit.skin);
    ...
    switch (f.mouth) {
        ...
        case 'teeth': {
            for (let i = -2; i <= 2; i++) {
                const tooth = new THREE.Mesh(new THREE.BoxGeometry(0.055 * R, 0.1 * R, 0.04 * R), kit.white);
                const angle = i * 0.16;
