...
import { ANIM_IDS } from './anim/clips';
import { tweaksFor, setTweak, clearTweaks, buildAnimationExport, importAnimationExport, } from './anim/tweaks';
...
let kit = createMaterialKit(store.dna);
let result = composePrincess(store.dna, kit);
...
function rebuild(dna, archetypeChanged) {
...
const ui = new Ui(store, {
    ...
    setSpecies: (s) => {
        ...
        next.seed = cur.seed;
        if (cur.pclass !== 'none')
            CLASS_DEFS[cur.pclass].apply(next);
        store.setDna(next);
    },
    setClass: (c) => {
    ...
    setSubtype: (id) => store.set('subtype', id),
    copyCode: () => {
    ...
    deleteGalleryEntry: (id) => ui.setGallery(removeFromGallery(id)),
    listClips: () => {
    ...
    exportAnims: () => {
        const blob = new Blob([JSON.stringify(buildAnimationExport())], { type: 'application/json' });
        downloadBlob(blob, 'princess-animations.anim.json');
    },
    undo: () => store.undo(),
    redo: () => store.redo(),
    applyPalette: (species, paletteId) => {
    ...
    startPaintDrag: (hex) => manipulator.startPaintDrag(hex),
});
...
store.subscribe((ev) => {
    ...
    if (ev.archetypeChanged)
        ui.onArchetypeChanged();
    ui.sync(ev.dna);
...
window.addEventListener('keydown', (e) => {
    ...
    if (mod && e.key.toLowerCase() === 'z') {
        ...
        if (s) {
            const card = document.querySelector(`.arch-card[data-species="${s}"]`);
            card?.click();
        }
    }
});
...
function importFeedback(ok) {
    const field = document.getElementById('import-code');
    if (ok)
        return;
    ...
    void (async () => {
        if (file.type === 'image/png' || file.name.endsWith('.png')) {
            ...
            try {
                ...
                if (parsed?.format === 'ttt-princess-anim') {
