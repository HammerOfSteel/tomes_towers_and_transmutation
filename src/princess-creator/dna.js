...
import { SPECIES_IDS, CLASS_IDS, AURA_STYLES, RANGES, DRESS_STYLES, EYE_STYLES, MOUTH_STYLES, HAIR_STYLES, CROWN_IDS, EAR_IDS, TAIL_IDS, BACK_IDS, HAND_ITEM_IDS, IDLE_STYLES, } from './types';
...
const NAME_DEFAULTS = {
    ...
    lamia: 'Zafirah', orc: 'Ghazka', troll: 'Brumhilde',
...
export function defaultDna(species) {
 ...
 */
export function sanitizeDna(raw) {
...
export function shareCodeToDna(code) {
    const trimmed = code.trim();
    const m = CODE_PREFIX.exec(trimmed);
    if (!m)
        return null;
    const version = parseInt(m[1], 10);
    if (!Number.isFinite(version) || version < 1 || version > DNA_VERSION)
        return null;
    try {
