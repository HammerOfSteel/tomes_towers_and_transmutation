...
import { wheelTargetFor, tiltTargetFor, resolveDrop, removalFor, tagPickables, paintSlotFor, DRAGGABLE, PULLABLE, } from '../interact';
const ALL_PICKS = [
...
describe('tagPickables', () => {
    it.each(SPECIES_IDS)('%s: default princess exposes hoverable parts and regions', (a) => {
        ...
        if (a === 'human')
            expect(picks.has('hair')).toBe(true);
    ...
    it('nearest tag wins: crown meshes are crown, not head', () => {
        ...
        for (const [mesh, pick] of map) {
            ...
            let found = false;
            while (cur) {
                if (cur.userData.pick === 'crown') {
                    found = true;
                    break;
                }
                if (cur.userData.pick === 'head')
                    break;
                cur = cur.parent;
            }
