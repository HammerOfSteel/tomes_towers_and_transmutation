...
import { ANIM_IDS, STATE_IDS, JOINT_IDS, CLIPS, NEUTRAL, bakeClip, resolveClips, speciesAnimInfo, } from '../anim/clips';
...
const finiteV3 = (v) => v.every((n) => Number.isFinite(n));
// ── Library validity ─────────────────────────────────────────────────────────
describe('clip library', () => {
        ...
        for (const id of ANIM_IDS) {
            ...
            for (const key of baked.keys) {
                ...
                for (const j of JOINT_IDS)
                    expect(finiteV3(key.joints[j])).toBe(true);
    ...
    it('states are loops; deaths hold their last frame', () => {
        for (const id of STATE_IDS)
            expect(CLIPS[id].loop).toBe(true);
        for (const id of ['die_1', 'die_2']) {
...
describe('resolveClips', () => {
        ...
        for (const species of SPECIES_IDS) {
            ...
            for (const id of ANIM_IDS) {
                ...
                for (const u of [0, 0.25, 0.5, 0.75, 1]) {
                    ...
                    for (const j of JOINT_IDS)
                        expect(finiteV3(pose.joints[j])).toBe(true);
    ...
    it('tweaks scale speed and amplitude', () => {
        ...
        const excursion = (set) => Math.max(...set.attack_1.keys.map((key) => Math.abs(key.joints.shoulderR[0] - neutralX)));
...
describe('sampleClip / ClipPlayer', () => {
    ...
    it('re-fires loop events every cycle', () => {
        ...
        for (let t = 0.001; t < d * 2.05; t += d / 20)
            player.sample(t);
...
describe('factory animation API', () => {
    ...
    it('surfaces gameplay events', () => {
        ...
        for (let t = 0.06; t < 0.06 + d; t += d / 30)
            p.update(t, 0.016);
...
describe('animation export', () => {
    ...
    it('import validates format and clamps tweak ranges', () => {
        ...
        expect(stored.bogus_species).toBeUndefined();
        localStorage.clear();
    });
});
