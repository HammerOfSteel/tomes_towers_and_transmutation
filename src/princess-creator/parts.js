...
// ── Crowns ───────────────────────────────────────────────────────────────────
function buildCrown(dna, kit, headR, totalLift) {
    ...
    if (id === 'halo') {
        ...
        for (let i = -1; i <= 1; i++) {
            ...
            const peak = shadowed(new THREE.Mesh(new THREE.ConeGeometry(headR * 0.055, headR * (i === 0 ? 0.22 : 0.14), 4), kit.metal));
        ...
        if (phase === 'full') {
            const ring = new THREE.Mesh(new THREE.TorusGeometry(headR * 0.34, headR * 0.05, 8, 24), kit.glow);
            ...
            discGeo.rotateX(Math.PI / 2);
            const rim = new THREE.Mesh(new THREE.TorusGeometry(headR * 0.31, headR * 0.035, 8, 20), kit.glow);
            moon.add(new THREE.Mesh(discGeo, kit.dark), rim);
        }
        else {
            // A clear waxing "C": half-ring, horns tipped up-right, slightly tapered
            const arc = new THREE.Mesh(new THREE.TorusGeometry(headR * 0.3, headR * 0.05, 8, 24, Math.PI), kit.glow);
        ...
        for (let i = 0; i < spikes; i++) {
            ...
            const spike = shadowed(new THREE.Mesh(new THREE.ConeGeometry(headR * 0.1, headR * 0.3, 4), kit.metal));
            ...
            if (id === 'crooked' && i === 1)
                spike.rotation.z = 0.7; // the bent one
            g.add(spike);
        }
        if (id === 'classic') {
            const gem = shadowed(new THREE.Mesh(new THREE.OctahedronGeometry(headR * 0.07), kit.accent));
...
function buildEarL(dna, kit, headR) {
...
function tailCountFor(dna) {
    if (dna.species !== 'foxling' || dna.parts.tail !== 'fluffy')
        return 1;
    return { '1': 1, '3': 3, '9': 9 }[dna.subtype] ?? 1;
}
function buildFluffyChain(dna, kit, size) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        const mesh = shadowed(new THREE.Mesh(new THREE.IcosahedronGeometry(scale, 1), i === n - 1 ? kit.hair : kit.skin));
...
function buildBack(dna, kit, hemDrop) {
    ...
    if (id === 'bow') {
        ...
        for (const side of [1, -1]) {
            ...
            for (const [sx, sy, r] of [[2.0, 1.85, 0.38], [1.2, 0.2, 0.25]]) {
        ...
        for (const side of [1, -1]) {
            ...
            for (const [len, wid, px, py, rz] of [
                ...
                // Center vein
                const vein = new THREE.Mesh(new THREE.BoxGeometry(wid * 0.12, len * 0.82, 0.05), kit.accent);
...
function buildHair(dna, kit, headR) {
    ...
    if (style === 'bob') {
        ...
        const curtainGeo = new THREE.CylinderGeometry(headR * 1.0, headR * 0.86, headR * 0.9 * len, seg, 1, true, Math.PI * 0.32, Math.PI * 1.36);
        ...
        for (let i = 0; i < n; i++) {
            ...
            const spike = shadowed(new THREE.Mesh(new THREE.ConeGeometry(headR * 0.16, spikeLen, 5), kit.hair));
            const dir = new THREE.Vector3(Math.cos(a) * Math.cos(el), Math.sin(el), Math.sin(a) * Math.cos(el) - 0.25).normalize();
        ...
        const main = shadowed(new THREE.Mesh(new THREE.SphereGeometry(headR * (0.85 + 0.35 * len), seg, seg), kit.hair));
        main.position.set(0, headR * 0.55, -headR * 0.12);
        g.add(main);
        for (const [ox, oy, r] of [
            [0.62, 0.25, 0.42], [-0.62, 0.25, 0.42], [0.35, 0.85, 0.38], [-0.35, 0.85, 0.38],
        ]) {
            const puff = shadowed(new THREE.Mesh(new THREE.SphereGeometry(headR * r * (0.8 + 0.3 * len), seg / 2, seg / 2), kit.hair));
...
function flameLayout(style, R, len) {
    const crown = (n, h, ring = 0.55) => Array.from({ length: n }, (_, i) => {
...
function buildFlameHair(dna, kit, headR) {
    const g = new THREE.Group();
    g.name = 'hair:flame';
    const tongues = [];
...
function buildMistHair(dna, kit, headR, disposers) {
    const g = new THREE.Group();
    g.name = 'hair:mist';
    const seg = 24;
...
function sprinkleGrowth(hairGroup, headR, kit, flowers = 0) {
    const SPOTS = [
    ...
    for (const [a, el, ring] of SPOTS) {
        ...
        const dir = new THREE.Vector3(Math.cos(a) * Math.cos(el), Math.sin(el), Math.sin(a) * Math.cos(el) - 0.2).normalize();
...
function addPearls(result, kit, p) {
    const n = 9;
    const r = p.topR * 0.92;
    for (let i = 0; i < n; i++) {
        ...
        pearl.position.set(Math.cos(a) * r, p.neckY - 0.25 - Math.sin(Math.abs(a - Math.PI / 2 - Math.PI * 0.35)) * 0.1, Math.sin(a) * r);
        result.rig.torso.add(pearl);
    }
}
/** Moonborn silver arcs / Verdant vines wrapped around the limbs + hem. */
function addLimbBands(result, material, p, arcs) {
...
function buildGlasses(dna, kit, headR) {
    const g = new THREE.Group();
    g.name = 'part:glasses';
    ...
    const bridge = new THREE.Mesh(new THREE.CylinderGeometry(rimR * 0.12, rimR * 0.12, ex * 2 - rimR * 1.8, 6), kit.metal);
...
// ── Attach pipeline ──────────────────────────────────────────────────────────
export function attachParts(result, dna, kit) {
    ...
    if (crown) {
        ...
        const halo = crown.userData.halo;
        if (halo) {
        ...
        if (earR) {
            ...
            result.hooks.tick.push(() => {
                if (Math.random() > 0.992)
                    flickL = 0.25 + Math.random() * 0.2;
                if (Math.random() > 0.992)
                    flickR = 0.25 + Math.random() * 0.2;
                flickL *= 0.9;
                flickR *= 0.9;
    ...
    if (dna.species === 'naiad')
        addPearls(result, kit, p);
    if (dna.species === 'moonborn')
        addLimbBands(result, kit.metal, p, true);
    if (dna.species === 'verdant')
        addLimbBands(result, kit.primary, p, false);
    ...
    if (!isSlime && dna.hair.style !== 'none') {
        if (dna.species === 'ignis') {
            ...
            if (hair) {
                ...
                if (dna.species === 'fae')
                    sprinkleGrowth(hair.group, p.headR, kit);
                if (dna.species === 'verdant')
                    sprinkleGrowth(hair.group, p.headR, kit, 4);
