...
function jelly(color, translucency) {
...
export function createMaterialKit(dna) {
    ...
    const skinGlowFor = (d) => {
        if (d.species === 'celestial')
            return 0.14 + d.aura.intensity * 0.1;
        if (d.species === 'high_elf')
            return 0.08 + d.aura.intensity * 0.06;
        if (d.species === 'undead')
            return 0.05 + d.aura.intensity * 0.05;
        if (d.species === 'ignis')
            return 0.12 + d.aura.intensity * 0.12;
        if (d.species === 'fae')
            return 0.06 + d.aura.intensity * 0.05;
        if (d.species === 'specter')
            return 0.06 + d.aura.intensity * 0.04;
        if (d.species === 'moonborn')
            return 0.07 + d.aura.intensity * 0.06;
        return 0;
    };
    const skinGlow = skinGlowFor(dna);
    if (skinGlow > 0 && 'emissive' in skin) {
        skin.emissive = new THREE.Color(c.glow);
        skin.emissiveIntensity = skinGlow;
    }
    // Specter: semi-translucent — she can appear almost solid or almost absent.
    if (dna.species === 'specter') {
        const ghost = (m, opacity) => {
            m.transparent = true;
            m.opacity = opacity;
        };
        ghost(skin, 0.72);
    ...
    if (dna.species === 'naiad') {
        ...
        const upgrade = (base, rough) => {
            const phys = new THREE.MeshPhysicalMaterial({ color: base.color.getHex(), roughness: rough });
            base.dispose();
            wet(phys);
            return phys;
        };
        skin = upgrade(skin, 0.35);
        primary = upgrade(primary, 0.4);
        hairMat = upgrade(hairMat, 0.25);
    }
    // Moonborn: hair lit from within.
    if (dna.species === 'moonborn' && 'emissive' in hairMat) {
        hairMat.emissive = new THREE.Color(c.glow);
        hairMat.emissiveIntensity = 0.22 + dna.aura.intensity * 0.15;
    }
    const kit = {
        archetype: a,
        flat,
        ...
        apply(next) {
            ...
            if (a === 'slime') {
                ...
                kit.primary.transmission = t;
                kit.skin.transmission = t;
                kit.hair.transmission = Math.min(0.85, t + 0.1);
            }
            const glow = skinGlowFor(next);
            if ('emissive' in kit.skin) {
                kit.skin.emissive ??= new THREE.Color(0x000000);
                kit.skin.emissive.set(glow > 0 ? n.glow : '#000000');
                kit.skin.emissiveIntensity = glow;
            }
