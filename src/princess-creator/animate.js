...
//  through `onEvent`.
import { resolveClips, STATE_IDS } from './anim/clips';
import { ClipPlayer } from './anim/player';
export const EMOTES = ['wave', 'twirl', 'dance', 'cast'];
const EMOTE_TO_CLIP = {
...
export class Animator {
    ...
    state = 'idle';
    oneShot = null;
    held = false; // frozen on a holdLast final frame
    nextBlink = 2.5;
    blinkStart = -1;
    lastNow = 0;
    ...
    get clips() { return this.clipSet; }
    get currentId() { return this.oneShot ?? this.state; }
    ...
    playEmote(id) {
        const clipId = EMOTE_TO_CLIP[id] ?? id;
        this.play(clipId);
    }
    update(t) {
        this.lastNow = t;
        if (!this.result || !this.dna || !this.clipSet)
            return;
        ...
        if (!this.held) {
            ...
            if (grounded) {
                if (style === 'float')
                    rootY += 0.16 + Math.sin(t * 1.6) * 0.12;
