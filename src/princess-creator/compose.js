...
function faceSurfaceR(dna, headR) {
...
export function composePrincess(dna, kit) {
    const result = SYNTHS[dna.archetype].build(dna, kit);
    const face = buildFace(result.sockets.face, dna, kit, faceSurfaceR(dna, result.proportions.headR));
