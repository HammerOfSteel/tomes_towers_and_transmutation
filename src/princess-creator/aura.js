...
export function attachAura(result, dna, kit) {
    const { style, intensity } = dna.aura;
    if (style === 'none' || intensity < 0.02)
        return;
    ...
    if (style === 'motes' || style === 'warm') {
        ...
        result.hooks.tick.push((t) => {
            for (const m of motes) {
                ...
                m.mesh.position.set(Math.cos(a) * m.r, m.h + Math.sin(t * 1.3 + m.phase * 2) * 0.35, Math.sin(a) * m.r);
    ...
    if (style === 'ember') {
        ...
        result.hooks.tick.push((t) => {
            for (const s of sparks) {
                ...
                s.mesh.position.set(Math.cos(a) * s.r * (1 - u * 0.4), u * rise, Math.sin(a) * s.r * (1 - u * 0.4));
    ...
    if (style === 'bubbles') {
        ...
        result.hooks.tick.push((t) => {
            for (const b of bubbles) {
                ...
                b.mesh.position.set(Math.cos(b.drift) * b.r + Math.sin(t * 1.1 + b.drift) * 0.35, u * rise, Math.sin(b.drift) * b.r + Math.cos(t * 0.9 + b.drift) * 0.35);
    ...
    if (style === 'cold' || style === 'warm') {
        ...
        const shell = new THREE.Mesh(new THREE.SphereGeometry(p.hemR + 2.2, 20, 14), shellMat);
