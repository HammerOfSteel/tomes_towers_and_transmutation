...
const HAND_POOL = [['none', 6], ['wand', 2], ['tome', 2], ['staff', 1], ['fan', 1]];
const CLASS_POOL = [['none', 4], ['scholar', 2], ['mage', 2], ['warrior', 2]];
...
function sampleBody(rng, species) {
    const lock = SPECIES_DEFS[species].lockBody ?? {};
    const out = {};
    for (const key of Object.keys(RANGES.body)) {
...
export function randomDna(species, seed) {
    ...
    if (def.subtypes) {
        dna.subtype = pickWeighted(rng, def.subtypes.map((s, i) => [s.id, SUBTYPE_WEIGHTS[i] ?? 1]));
    }
    ...
    if (chance(rng, 0.55))
        dna.colors.skin = pick(rng, def.skinTones);
    if (chance(rng, 0.55))
        dna.colors.hair = pick(rng, def.hairColors);
...
export function mutateDna(dna, seed) {
    const rng = mulberry32(seed);
    ...
    for (let i = 0; i < count; i++) {
        ...
        const range = RANGES[section][key];
        const target = out[section];
        const span = (range.max - range.min) * 0.15;
        target[key] = target[key] + (rng() * 2 - 1) * span;
    }
    if (chance(rng, 0.35))
        out.face.eyeStyle = pick(rng, EYE_STYLES);
    if (chance(rng, 0.25))
        out.face.mouth = pick(rng, MOUTH_STYLES);
