...
import { computeProportions, makeScaffold, buildDress, limbPart, shadowed, makeResult, } from './shared';
/** Bone: tapered shaft + icosahedron joint knob at the pivot. */
function bone(length, thickness, kit) {
    const g = new THREE.Group();
    const shaft = limbPart(new THREE.CylinderGeometry(thickness * 0.6, thickness, length, 5), kit.skin, -length / 2);
    g.add(shaft);
    const knob = shadowed(new THREE.Mesh(new THREE.IcosahedronGeometry(thickness * 1.35, 0), kit.skin));
    g.add(knob);
    return g;
}
export const skeletonSynth = {
    archetype: 'skeleton',
    build(dna, kit) {
        ...
        rig.head.add(nose);
        // Neck vertebra
        const vertebra = shadowed(new THREE.Mesh(new THREE.CylinderGeometry(0.14 * R, 0.16 * R, p.headCY - p.neckY, 6), kit.skin));
        vertebra.position.y = (p.headCY - p.neckY) / 2;
        rig.neck.add(vertebra);
        // Gown + collarbone
        rig.torso.add(buildDress(dna, kit, p));
        const clavicle = shadowed(new THREE.Mesh(new THREE.CylinderGeometry(bt * 0.7, bt * 0.7, p.shoulderX * 2, 6), kit.skin));
