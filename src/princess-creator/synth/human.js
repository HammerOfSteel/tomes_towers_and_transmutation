...
import { computeProportions, makeScaffold, buildDress, addPuffSleeves, limbPart, shadowed, makeResult, } from './shared';
export const humanSynth = {
    archetype: 'human',
    build(dna, kit) {
        ...
        rig.head.add(head);
        // Neck
        const neck = shadowed(new THREE.Mesh(new THREE.CylinderGeometry(p.headR * 0.16, p.headR * 0.2, p.headCY - p.neckY, 12), kit.skin));
        ...
        rig.torso.add(chest);
        if (dna.dress.puffSleeves)
            addPuffSleeves(kit, p, rig);
        // Arms (capsules) + hands
        for (let i = 0; i < 2; i++) {
            const upper = limbPart(new THREE.CapsuleGeometry(p.armThick, p.armUpper * 0.8, 4, 12), kit.skin, -p.armUpper / 2);
            rig.shoulders[i].add(upper);
            const lower = limbPart(new THREE.CapsuleGeometry(p.armThick * 0.88, p.armLower * 0.8, 4, 12), kit.skin, -p.armLower / 2);
        ...
        for (let i = 0; i < 2; i++) {
            const thigh = limbPart(new THREE.CapsuleGeometry(p.legThick, p.legUpper * 0.8, 4, 12), kit.skin, -p.legUpper / 2);
            rig.hips[i].add(thigh);
            const calf = limbPart(new THREE.CapsuleGeometry(p.legThick * 0.85, p.legLower * 0.8, 4, 12), kit.skin, -p.legLower / 2);
