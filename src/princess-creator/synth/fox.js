...
export const foxSynth = {
    ...
    build(dna, kit) {
        ...
        for (let i = 0; i < 2; i++) {
            const upper = limbPart(new THREE.CylinderGeometry(p.armThick * 1.15, p.armThick * 0.9, p.armUpper, 5), kit.skin, -p.armUpper / 2);
            rig.shoulders[i].add(upper);
            const lower = limbPart(new THREE.CylinderGeometry(p.armThick * 0.85, p.armThick * 0.72, p.armLower, 5), kit.hair, -p.armLower / 2);
        ...
        // Legs + little boots
        for (let i = 0; i < 2; i++) {
            const thigh = limbPart(new THREE.CylinderGeometry(p.legThick * 1.3, p.legThick, p.legUpper, 5), kit.skin, -p.legUpper / 2);
            rig.hips[i].add(thigh);
            const calf = limbPart(new THREE.CylinderGeometry(p.legThick, p.legThick * 0.8, p.legLower, 5), kit.hair, -p.legLower / 2);
