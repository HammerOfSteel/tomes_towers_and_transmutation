...
export function computeProportions(dna) {
    const b = dna.body;
 ...
 *           └ hipL/R ─ kneeL/R
 */
export function makeScaffold(dna, p) {
    ...
    const elbows = [new THREE.Group(), new THREE.Group()];
    elbows.forEach((e, i) => {
...
export function limbPart(geometry, material, yOffset) {
...
export function buildDress(dna, kit, p) {
...
export function addPuffSleeves(kit, p, rig) {
...
export function makeResult(scaffold, p, update, extraDispose) {
    const hooks = { tick: [], disposers: [] };
    return {
        ...
        dispose() {
            ...
            for (const fn of hooks.disposers)
                fn();
            extraDispose?.();
        },
    };
