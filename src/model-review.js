...
import { ENV_ASSETS, ENV_KITS } from '@/assets/envManifest';
...
function _setEnvKitFilter(id) {
    _envKitFilter = id;
    _buildEnvList();
    _buildEnvKitTree(); // rebuild to update active states
}
function _buildEnvKitTree() {
...
function _activateSubEditor(type) {
    ...
    if (type === 'tower_floor') {
        ...
        _blueprintLayer?.showOverworld((dungeonEntry) => {
            ...
            document.querySelector('[data-etype="dungeon"]')?.classList.add('active');
...
function _setEditorKitFilter(id) {
    _editorKitFilter = id;
    _buildEditorKitTree();
...
function _buildEditorKitTree() {
    ...
    for (const { group, label } of KIT_GROUPS) {
        ...
        for (const kit of kits) {
            if (!kit.extracted)
                continue; // editor only shows extracted kits
            const kitEl = document.createElement('div');
...
async function _loadFromGame() {
    ...
    if (!doc) {
        ...
        if (doc)
            _setEditorStatus('Loaded from autosave');
    }
    if (!doc) {
    ...
    if (hasObjects && !confirm(`Load "${doc.name}"?\n\nUnsaved changes will be replaced. Save a version first if you want to keep them.`))
        return;
    _editorDocId = doc.id;
    _editorDocName = doc.name;
    await _editorSerial.applyToCore(doc);
...
function _positionCameraForDungeon() {
...
canvas.addEventListener('click', (e) => {
    if (_editorType !== 'overworld' || !_blueprintLayer)
        return;
    ...
    if (hits.length > 0) {
        const onClick = hits[0].object.userData['onClick'];
        onClick?.();
    }
});
...
function _makeVersionRow(tag, label, v, doc) {
...
document.getElementById('ed-version-save')?.addEventListener('click', () => {
    if (!_editorSerial || !_editorCore)
        return;
    const label = document.getElementById('ed-version-label')?.value.trim();
    const doc = _editorSerial.build(_editorType, _editorDocId, _editorDocName);
    EditorVersioning.save(doc, label || undefined);
    document.getElementById('ed-version-label').value = '';
    _renderVersionPanel();
