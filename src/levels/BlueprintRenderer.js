...
import { buildCauldron, buildPotionRack, buildDistillationCoil, buildFermentingVat, buildHerbBundle, buildAnvil, buildCoolingTrough, buildBed, buildWardrobe, buildBunk, buildMessTable, buildReadingTable, buildGlobe, buildMapTable, buildWeaponStand, buildPlantPot, buildRaisedPlanter, buildContainmentRing, buildAstrolabe, buildBanner, buildRug, } from '@/rendering/ProceduralProps';
// ── Visual constants ──────────────────────────────────────────────────────
...
function buildStaircase(stair, bp, group, geometries, materials, doorTriggers, physics, bodies) {
    ...
    let ax = 0, az = 0;
    if (stair.facing === 'north')
        az = -1;
    else if (stair.facing === 'south')
        az = 1;
    else if (stair.facing === 'east')
        ax = 1;
    else
        ax = -1;
    ...
    for (let i = 0; i < STEPS; i++) {
        ...
        bodies.push(physics.createStaticBox(new THREE.Vector3(sx, sy, sz), new THREE.Vector3(ax !== 0 ? stepHalfD : stepHalfW, stepH / 2, az !== 0 ? stepHalfD : stepHalfW)));
    }
...
export function renderBlueprint(bp, physics, opts = {}) {
    ...
    const floorMat = (() => {
        if (floorType === 'wood')
            return MaterialLibrary.get('wood_plank');
        if (floorType === 'grass')
            return MaterialLibrary.get('grass_floor');
        if (floorType === 'dirt')
            return new THREE.MeshLambertMaterial({ color: FLOOR_COLORS.dirt });
        if (floorType === 'stone_alchemy')
            return MaterialLibrary.get('alchemy_stone_floor');
        if (floorType === 'stone_herald')
            return MaterialLibrary.get('herald_stone_floor');
        if (floorType === 'stone_damp')
            return MaterialLibrary.get('damp_stone_floor');
        if (floorType === 'stone_scorched')
            return MaterialLibrary.get('scorched_stone_floor');
        if (floorType === 'stone_sealed')
            return MaterialLibrary.get('sealed_stone_floor');
        if (floorType === 'stone_celestial')
            return MaterialLibrary.get('celestial_stone_floor');
    ...
    group.add(floorMesh);
    bodies.push(physics.createStaticBox(new THREE.Vector3(0, -0.025, 0), new THREE.Vector3(floorW / 2, 0.025, floorD / 2)));
    // ── Walls & pillars ───────────────────────────────────────────────────
    ...
    const stairBackingKeys = new Set();
    ...
    for (const tile of bp.tiles) {
        if (doorKeys.has(`${tile.x},${tile.z}`))
            continue;
        if (stairBackingKeys.has(`${tile.x},${tile.z}`))
            continue; // staircase opening
    ...
    {
        const wallTileSet = new Set(bp.tiles
            .filter(t => t.type === 'wall' && !doorKeys.has(`${t.x},${t.z}`) && !stairBackingKeys.has(`${t.x},${t.z}`))
            .map(t => `${t.x},${t.z}`));
        const placedCorners = new Set();
        for (const tile of bp.tiles) {
            if (tile.type !== 'wall' || doorKeys.has(`${tile.x},${tile.z}`) || stairBackingKeys.has(`${tile.x},${tile.z}`))
                continue;
            const { x: wx, z: wz } = cellToWorld(tile.x, tile.z, bp);
            const tH = tile.h ?? wallHeight;
            for (const [dx, dz] of [[1, 1], [1, -1], [-1, 1], [-1, -1]]) {
                if (!wallTileSet.has(`${tile.x + dx},${tile.z + dz}`))
                    continue;
    ...
    // ── Wall-top decorations — themed low-poly props on top of the wall ring ─
    {
        ...
        const wallPts = [];
        for (const tile of bp.tiles) {
            if (tile.type !== 'wall' || dkSet.has(`${tile.x},${tile.z}`))
                continue;
            const { x: wx, z: wz } = cellToWorld(tile.x, tile.z, bp);
            wallPts.push({ wx, wz });
        }
        ...
        if (ft === 'stone' && floorIdx === 5) {
            ...
            for (const { wx, wz } of pick(3)) {
                ...
                fl.position.set(wx + (rng() - 0.5) * 0.28, tH + 0.16 + rng() * 0.12, wz + (rng() - 0.5) * 0.28);
                group.add(fl);
            }
        }
    ...
    for (const door of bp.doors) {
        ...
        const lintelGeo = new THREE.BoxGeometry(isNS ? cellSize : 0.22, lintelH, isNS ? 0.22 : cellSize);
        ...
        if (item.type === 'bookshelf') {
            ...
            for (let comp = 0; comp < 3; comp++) {
                ...
                while (cursor < usableW / 2 - 0.05) {
                    ...
                    book.position.set(ix + (isRotated ? 0 : localX), compartmentYs[comp] + bH / 2 - bH * 0.04, iz + (isRotated ? localX : 0));
            ...
            bodies.push(physics.createStaticBox(new THREE.Vector3(ix, shelfH / 2, iz), new THREE.Vector3(isRotated ? cellSize * 0.14 : shelfW / 2, shelfH / 2, isRotated ? shelfW / 2 : cellSize * 0.14)));
        }
            ...
            top.castShadow = true;
            group.add(base, top);
            bodies.push(physics.createStaticBox(new THREE.Vector3(ix, 0.525, iz), new THREE.Vector3(0.275, 0.525, 0.19)));
        }
            ...
            bodies.push(physics.createStaticBox(new THREE.Vector3(ix, 0.75, iz), new THREE.Vector3(1.1, 0.75, 1.1)));
        }
            ...
            cap.rotation.x = -0.55;
            group.add(cap);
            bodies.push(physics.createStaticBox(new THREE.Vector3(ix, 1.0, iz), new THREE.Vector3(0.32, 1.0, 0.32)));
        }
            ...
            group.add(hood);
            bodies.push(physics.createStaticBox(new THREE.Vector3(ix, 0.45, iz), new THREE.Vector3(0.8, 0.45, 0.6)));
        }
            ...
            bodies.push(physics.createStaticBox(new THREE.Vector3(ix, 1.1, iz), new THREE.Vector3(0.75, 0.9, 0.1)));
        }
            ...
            bodies.push(physics.createStaticBox(new THREE.Vector3(ix, 0.36, iz), new THREE.Vector3(0.28, 0.36, 0.28)));
        }
            ...
            for (const isX of [true, false]) {
                const braceGeo = new THREE.BoxGeometry(isX ? 0.72 : 0.055, 0.055, isX ? 0.055 : 0.72);
                geometries.push(braceGeo);
                const brace = new THREE.Mesh(braceGeo, crateTrimMat);
                brace.position.set(ix, 0.725, iz);
                group.add(brace);
            }
            bodies.push(physics.createStaticBox(new THREE.Vector3(ix, 0.36, iz), new THREE.Vector3(0.36, 0.36, 0.36)));
        }
            ...
            group.add(clasp);
            bodies.push(physics.createStaticBox(new THREE.Vector3(ix, 0.32, iz), new THREE.Vector3(0.41, 0.32, 0.26)));
        }
            ...
            top.castShadow = true;
            group.add(top);
            for (const [lx, lz] of [[-0.5, -0.27], [0.5, -0.27], [-0.5, 0.27], [0.5, 0.27]]) {
            ...
            group.add(bow);
            // Two key teeth
            for (const [tx, ty] of [[-0.08, 0.06], [-0.04, 0.06]]) {
            ...
            group.add(halo);
            bodies.push(physics.createStaticBox(new THREE.Vector3(ix, 0.44, iz), new THREE.Vector3(0.95, 0.44, 0.425)));
        }
        else {
