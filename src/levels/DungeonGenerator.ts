...
/** The output of the generator: a set of room instances + the starting room. */
export interface DungeonPlan {
  /** Keyed by unique instance ID (e.g. "room_0", "room_1"). */
  rooms: Map<string, Blueprint>;
  /** Instance ID of the room the player spawns in. */
  startRoomId: string;
  seed: number;
}
...
const OPPOSITE: Record<DoorFacing, DoorFacing> = {
  north: 'south',
  south: 'north',
  east:  'west',
  west:  'east',
...
function cloneRoom(base: Blueprint, instanceId: string): Blueprint {
  return {
    ...base,
    id: instanceId,
    // Reset door targets so generator wires them from scratch
    doors:         base.doors.map((d: DoorEntry) => ({ ...d, targetId: null })),
...
function findFreeDoor(room: Blueprint, facing: DoorFacing): number {
  return room.doors.findIndex((d) => d.facing === facing && d.targetId === null);
}

// ── Base blueprint pool ───────────────────────────────────────────────────────

function buildBasePool(): Map<string, Blueprint> {
...
let _basePool: Map<string, Blueprint> | null = null;
function getBasePool(): Map<string, Blueprint> {
  if (!_basePool) _basePool = buildBasePool();
  return _basePool;
}
...
export function generateDungeon(
  seed: number,
  floorCount: number,
  basePrints: ReadonlyMap<string, Blueprint> = getBasePool(),
): DungeonPlan {
  ...
  function allocRoom(bpId: string): Blueprint {
    const base = basePrints.get(bpId);
    if (!base) throw new Error(`DungeonGenerator: unknown blueprint "${bpId}"`);
  ...
  function connect(fromRoom: Blueprint, exitFacing: DoorFacing, toRoom: Blueprint): void {
    const fromIdx = findFreeDoor(fromRoom, exitFacing);
    const toIdx   = findFreeDoor(toRoom,   OPPOSITE[exitFacing]);
    if (fromIdx === -1) throw new Error(
      `DungeonGenerator: room "${fromRoom.id}" (${fromRoom.id}) has no free ${exitFacing} door`);
    if (toIdx === -1) throw new Error(
      `DungeonGenerator: room "${toRoom.id}" has no free ${OPPOSITE[exitFacing]} door`);
  ...
  let prevRoom: Blueprint = startRoom;
  let exitFacing: DoorFacing = 'north'; // cell_start's free door faces north
