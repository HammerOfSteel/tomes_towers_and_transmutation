...
function isFloor(x: number, z: number): boolean {
  const dx = x - CX;
  const dz = z - CZ;
 ...
 */
export function generateGreenhouse(_seed: number): DungeonPlan {
  ...
  const room: Blueprint = {
    ...
    interactables: [
      // The Greenhouse Orb — centrepiece of the chamber
      {
        ...
        type: 'greenhouse_orb',
        content:
          'A crystalline orb pulses with slow green light.  Ancient runes etched '
      ...
      // Plant lore lecterns in the NW and NE quadrants
      {
        ...
        type: 'lectern',
        content:
          '"On the Cultivation of Sapient Fungi: given adequate moonlight and '
      ...
      },
      {
        ...
        type: 'lectern',
        content:
          '"The Slimewort vine requires no soil — only warmth, moisture, and '
