...
import type { Blueprint, TileEntry } from './blueprint';

function buildSandboxBlueprint(): Blueprint {
  const W = 14;
  const D = 14;
...
export function generateSandboxArena(): DungeonPlan {
  const bp = buildSandboxBlueprint();
  const rooms = new Map<string, Blueprint>([[bp.id, bp]]);
