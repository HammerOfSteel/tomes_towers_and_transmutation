...
function solidMat(color = '#8b7355'): THREE.MeshStandardMaterial {
  return new THREE.MeshStandardMaterial({ color: new THREE.Color(color) });
}

function emissiveMat(): THREE.MeshStandardMaterial {
...
function countMeshes(grp: THREE.Object3D): number {
  let count = 0;
  grp.traverse(obj => { if (obj instanceof THREE.Mesh) count++; });
  return count;
}

/** Dispose a group's geometry and materials to free memory. */
function disposeGroup(grp: THREE.Object3D): void {
  grp.traverse(obj => {
...
function assertValidGroup(grp: THREE.Group, minMeshes = 1): void {
