...
function makeStorage(initial: Record<string, string> = {}): VersionStorage {
  const store = { ...initial };
  return {
...
function makeItem(type = 'barrel', x = 0, z = 0): TowerItem {
  return { type: type as TowerItem['type'], x, z, rotation: 0 };
}

function makeFloorMap(floors: Record<number, TowerItem[]>): Map<number, TowerItem[]> {
...
describe('restoreVersion', () => {
  it('populates the floor map from the version snapshot', () => {
    ...
    const floorItems = new Map<number, TowerItem[]>();
  ...
  it('handles a version with no floors without error', () => {
    const version: TowerVersion = { version: 1, label: 's', savedAt: 0, floors: {} };
    const floorItems = new Map<number, TowerItem[]>();
