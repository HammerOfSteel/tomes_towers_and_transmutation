...
scene.add(fill);
// ── Checkerboard floor ────────────────────────────────────────────────────────
function makeCheckerFloor(size = 40, tileCount = 40) {
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(mid, mid - 20);
    ctx.lineTo(mid, mid + 20);
    ctx.stroke();
...
function autoPlace(group, spacing = 4) {
...
function registerSpawned(group, disposeFn) {
...
function _floatLabel(text, x, y, z) {
...
const showroom = {
    ...
    async spawnNpc(species = 'human', role = 'merchant', x, z) {
        const { getDefaultNpcDna } = await import('@/npc-creator/defaults/NpcDefaults');
        const { buildNpc } = await import('@/npc-creator/builder');
        const dna = getDefaultNpcDna(species, role, Date.now() ^ (Math.random() * 0xFFFF) >>> 0);
    ...
    async spawnEnemy(species = 'undead', role = 'melee', tier = 1, x, z) {
        ...
        const dna = getDefaultEnemyDna(species, role, tier, Date.now() ^ (Math.random() * 0xFFFF) >>> 0);
    ...
    spawnBuilding(kind = 'house', style = 'thatched', size = 'small', x, z) {
    ...
    /** Spawn a prop. */
    spawnProp(kind = 'chest', material = 'wood', x, z) {
        import('@/prop-creator/builder').then(({ buildProp }) => {
            import('@/prop-creator/types').then(({ MATERIAL_COLORS }) => {
                const dna = {
                    ...
                    size: 1, colors: MATERIAL_COLORS[material] ?? MATERIAL_COLORS['wood'],
    ...
    async spawnEnemyTiers() {
        ...
        for (const { species, role, z } of configs) {
            for (const [tier, x] of [[1, -8], [2, 0], [3, 8]]) {
    ...
    spawnInterior(kind = 'house', style = 'timber', x = 0, z = 0) {
     ...
     */
    async spawnBuildingWalkthrough() {
        const PAIRS = [
        ...
        const [{ STYLE_COLORS, factionBuildingDna: _fbd }, { buildBuilding }, { generateInterior }, { getDefaultNpcDna }, { buildNpc },] = await Promise.all([
        ...
        for (let i = 0; i < PAIRS.length; i++) {
            ...
            try {
                const npcDna = getDefaultNpcDna(species, role, i * 0x9E3779B9 >>> 0);
        ...
        import('@/world/buildings/BuildingDNA').then(({ factionBuildingDna }) => {
            import('@/world/buildings/BuildingBuilder').then(({ buildBuilding }) => {
                ...
                const kinds = ['cottage', 'watchtower', 'villa', 'chapel', 'blacksmith', 'watchtower',
                    'watchtower', 'watchtower', 'cottage', 'tent', 'barn', 'tent'];
                for (let fi = 0; fi < factions.length; fi++) {
                    const row = Math.floor(fi / 4);
                    const col = fi % 4;
                    const dna = factionBuildingDna(kinds[fi], factions[fi], fi * 0x9E3779B9 >>> 0, 'medium', 2);
                    const inst = buildBuilding(dna);
...
window.__showroomScene = scene; // exposed for Playwright tests
// ── HUD instructions ──────────────────────────────────────────────────────────
