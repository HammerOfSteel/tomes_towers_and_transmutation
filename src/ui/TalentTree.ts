...
function nodePos(pathIndex: number, r: number): [number, number] {
  const a = BASE_ANGLE + pathIndex * STEP;
  return [CX + Math.cos(a) * r, CY + Math.sin(a) * r];
}

/** For cross-path nodes between path i and path (i+1). */
function crossPos(pathI: number, pathJ: number): [number, number] {
...
TALENT_NODES.forEach(n => {
  if (n.path === 'cross') {
    ...
    const pathI = PATH_ORDER.indexOf(
      TALENT_NODES.find(x => x.id === n.prerequisites[0])?.path as never
    );
    const pathJ = PATH_ORDER.indexOf(
      TALENT_NODES.find(x => x.id === n.prerequisites[1])?.path as never
    );
    if (pathI !== -1 && pathJ !== -1) {
      NODE_POS.set(n.id, crossPos(pathI, pathJ));
    }
  } else {
    const pi = PATH_ORDER.indexOf(n.path as never);
...
export class TalentTree {
  ...
  private _infoEl: HTMLElement | null = null;
  private _talentPointsEl: HTMLElement | null = null;
  private _nodeEls = new Map<string, HTMLElement>();
  private _prog: ProgressionSystem | null = null;
  private _talents: TalentSystem | null = null;

  get visible(): boolean { return !!this._el; }

  open(prog: ProgressionSystem, talents: TalentSystem): void {
  ...
  toggle(prog: ProgressionSystem, talents: TalentSystem): void {
  ...
  private _makeNode(
    ...
    opts: {
      ...
      color: string;
      bought: boolean; canBuy: boolean; locked: boolean; isCenter: boolean;
    },
  ): HTMLElement {
    ...
    if (opts.isCenter) {
      ...
      if (opts.bought) dot.style.boxShadow = `0 0 6px ${opts.color}`;
      div.appendChild(dot);
    }
  ...
  private _showInfo(nodeId: string): void {
  ...
  private _refresh(): void {
    if (!this._el || !this._prog || !this._talents) return;
    ...
    this._nodeEls.forEach((div, id) => {
      ...
      // Update label
      const lbl = div.querySelector('div:not([style*="border-radius"])') as HTMLElement | null;
      if (lbl) {
  ...
  private _drawLines(): void {
    if (!this._canvas || !this._talents) return;
  ...
  private _updatePointsDisplay(): void {
    if (!this._talentPointsEl || !this._prog) return;
