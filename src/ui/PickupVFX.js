...
export function spawnPickupVFX(scene, pos, color = 0xffd966) {
    const group = new THREE.Group();
    group.position.copy(pos);
    group.position.y += 0.5;
    // Core burst sphere
    const burst = new THREE.Mesh(new THREE.SphereGeometry(0.08, 8, 6), new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 1 }));
    group.add(burst);
    // Expanding ring
    const ring = new THREE.Mesh(new THREE.TorusGeometry(0.1, 0.025, 6, 24), new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.7, side: THREE.DoubleSide }));
    ...
    const EXPAND = 120;
    const TOTAL = 300;
    const mat = burst.material;
    const rMat = ring.material;
    function tick() {
