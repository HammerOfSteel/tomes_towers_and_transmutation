...
export class AssetCharBrowser {
  ...
  private readonly _onSelect: (def: CharModelDef) => void;
  private readonly _models:   CharModelDef[];

  private _selected: CharModelDef | null = null;
  private _cardMap   = new Map<string, HTMLElement>();
  private _activeFilters = new Set<string>(); // active pack IDs ('all' = no filter)
  ...
  constructor(
    ...
    this._models = CHAR_MODELS.filter(
      (m) => m.roles.includes('player') && activePacks.includes(m.packId),
    );
  ...
  get selected(): CharModelDef | null { return this._selected; }

  dispose(): void { this._root.remove(); }

  // ── Private helpers ───────────────────────────────────────────────────────

  private _visibleModels(): CharModelDef[] {
    if (this._activeFilters.size === 0) return this._models;
  ...
  private _selectCard(def: CharModelDef, card: HTMLElement): void {
    // Deselect previous
    if (this._selected) {
  ...
  private _updateFilterChips(filterRow: HTMLElement, changedId: string): void {
    filterRow.querySelectorAll<HTMLElement>('.acb-filter-chip').forEach((chip) => {
