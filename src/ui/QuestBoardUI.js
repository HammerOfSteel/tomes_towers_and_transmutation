...
export const QuestBoardUI = {
    ...
    open(questLog, worldSeed, floor) {
        ...
        const boardQuests = [];
        const ctxTemplates = [
            { npcName: 'Anonymous Postings', npcRole: 'guard' },
            { npcName: 'Settlement Council', npcRole: 'scholar' },
            { npcName: 'Travelling Merchant', npcRole: 'merchant' },
        ];
        for (let i = 0; i < ctxTemplates.length; i++) {
            ...
            const q = generateQuest({
                npcName: ctx.npcName,
                npcRole: ctx.npcRole,
                settlement: { id: i, plan: { name: 'the Region', type: 'village', centerCol: 25, centerRow: 25, buildings: [], roads: [], population: 0 }, seed: worldSeed ^ (i * 0x1234) },
                dungeons: [],
                nearbyEvents: [],
...
function _mkQuestRow(q, alreadyActive, onPin) {
