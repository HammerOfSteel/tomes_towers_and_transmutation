...
export class EnemyHealthBars {
  ...
  private readonly _bars   = new Map<number, BarEntry>();
  ...
  update(
    ...
    for (let i = 0; i < enemies.length; i++) {
      ...
      if (dist > SHOW_RANGE) {
        ...
        if (bar) bar.wrap.style.display = 'none';
        continue;
      }
  ...
  clearAll(): void {
    for (const [, bar] of this._bars) bar.wrap.remove();
    this._bars.clear();
  }

  private _createBar(id: number): BarEntry {
