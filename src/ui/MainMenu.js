...
import { loadWorldGenConfig, saveWorldGenConfig, randomiseSeed, KENNEY_PACKS, } from '@/world/WorldGenConfig';
...
export function patchSaveSlot(slotId, patch) {
...
export class MainMenu {
    ...
    _buildDOM() {
        ...
        nav.append(mkBtn('Play', 'mm-btn', () => { this._renderSaveSlots(); this._openModal('mm-play'); }), mkBtn('Settings', 'mm-btn', () => this._openModal('mm-settings')), mkBtn('Controls', 'mm-btn', () => this._openModal('mm-controls')), mkBtn('Lore', 'mm-btn', () => { this._refreshLoreSpecies(); this._setLorePage(0); this._openModal('mm-lore'); }), devLabBtn, navSep, this._buildMusicPlayer());
        ...
        ov.append(header, section, this._buildPlayModal(), this._buildSettingsModal(), this._buildControlsModal(), this._buildLoreModal());
        return ov;
    }
    ...
    _renderSaveSlots() {
        ...
        for (let i = 0; i < NUM_SLOTS; i++) {
          ...
          <button class="mm-slot-btn mm-slot-btn--play" data-i="${i}">New Game</button>`;
            ...
            row.querySelectorAll('.mm-slot-btn--play').forEach(b => {
            ...
            row.querySelectorAll('.mm-slot-btn--del').forEach(b => {
                b.addEventListener('click', () => {
    ...
    `);
        ...
        // ── Volume ──────────────────────────────────────────────────────────────
        const slider = card.querySelector('#mm-vol');
        const valEl = card.querySelector('#mm-vol-val');
        slider.addEventListener('input', () => {
            valEl.textContent = `${slider.value}%`;
            localStorage.setItem(LS_VOL, slider.value);
            if (this._audio)
                this._audio.volume = parseInt(slider.value) / 100;
        });
        const fsCb = card.querySelector('#mm-fs');
        ...
        const musicToggle = card.querySelector('#mm-music-toggle');
        musicToggle.addEventListener('change', () => {
            const playerEl = document.getElementById('mm-player');
            if (playerEl)
                playerEl.style.display = musicToggle.checked ? 'flex' : 'none';
            localStorage.setItem(LS_MUSIC_VISIBLE, String(musicToggle.checked));
        });
        const devToggle = card.querySelector('#mm-dev-toggle');
        ...
        // ── Colour-blind mode ────────────────────────────────────────────────────
        const cbToggle = card.querySelector('#mm-cb-toggle');
        if (cbToggle) {
            // Apply on load
        ...
        // ── Text scale ───────────────────────────────────────────────────────────
        const textScaleSlider = card.querySelector('#mm-text-scale');
        const textScaleVal = card.querySelector('#mm-text-scale-val');
        ...
        // ── World Gen ───────────────────────────────────────────────────────────
        const saveWg = () => saveWorldGenConfig(wg);
        const sizeRadios = card.querySelectorAll('input[name="mm-wg-size"]');
        sizeRadios.forEach(r => r.addEventListener('change', () => {
            wg.worldSize = parseInt(r.value);
            saveWg();
        }));
        const seedInput = card.querySelector('#mm-wg-seed');
        seedInput.addEventListener('change', () => {
            wg.seed = Math.max(0, Math.min(0xFFFF_FFFF, parseInt(seedInput.value) || 0));
            seedInput.value = String(wg.seed);
            saveWg();
        });
        const randBtn = card.querySelector('#mm-wg-rand');
        ...
        const mkSlider = (id, valId, key) => {
            const el = card.querySelector(id);
            const val = card.querySelector(valId);
            el.addEventListener('input', () => {
                wg[key] = parseInt(el.value);
        ...
        const cityToggle = card.querySelector('#mm-wg-city');
        cityToggle.addEventListener('change', () => { wg.hasCity = cityToggle.checked; saveWg(); });
        // ── Asset mode + pack selection ───────────────────────────────────────────────────────
        const assetModeCb = card.querySelector('#mm-asset-mode');
        const assetPacksRow = card.querySelector('#mm-asset-packs-row');
        ...
        card.querySelectorAll('.mm-pack-cb').forEach(cb => {
            cb.addEventListener('change', () => {
                wg.assetPacks = [...card.querySelectorAll('.mm-pack-cb')]
                    .filter(c => c.checked).map(c => c.value);
                saveWg();
            });
        });
        // ── Character mode + pack selection ──────────────────────────────────────
        const charModeCb = card.querySelector('#mm-char-mode');
        const charPacksRow = card.querySelector('#mm-char-packs-row');
        ...
        card.querySelectorAll('.mm-char-pack-cb').forEach(cb => {
            cb.addEventListener('change', () => {
                wg.charPacks = [...card.querySelectorAll('.mm-char-pack-cb')]
                    .filter(c => c.checked).map(c => c.value);
                saveWg();
            });
        });
        const shareBtn = card.querySelector('#mm-wg-share');
        ...
        const loadInput = card.querySelector('#mm-wg-load-input');
        const loadBtn = card.querySelector('#mm-wg-load');
    ...
    _buildControlsModal() {
        ...
        if (rc) {
            ...
            rebindSection.insertAdjacentHTML('beforeend', '<div class="mm-setting-row mm-setting-row--section-hdr">' +
            ...
            const rebindListener = (e) => {
                if (!listeningAction || !listeningBtn)
                    return;
                e.preventDefault();
                e.stopPropagation();
                if (e.code === 'Escape') {
                ...
                rc.rebind(listeningAction, e.code);
                ...
                rebindSection.querySelectorAll('.mm-rebind-btn').forEach(btn => {
                    const a = btn.dataset['action'] ?? '';
                    if (a in newB) {
                        btn.dataset['code'] = newB[a];
                        btn.textContent = _codeLabel(newB[a]);
                    }
                });
        ...
        table.insertAdjacentHTML('beforeend', '<div class="mm-setting-row mm-setting-row--section-hdr">' +
        ...
        if (rc) {
            footer.appendChild(mkBtn('Reset to Defaults', 'mm-slot-btn', () => {
                ...
                card.querySelectorAll('.mm-rebind-btn').forEach(btn => {
                    const a = btn.dataset['action'] ?? '';
                    if (a in b) {
                        const code = b[a];
                        btn.dataset['code'] = code;
    ...
    _buildLoreModal() {
        ...
        nav.id = 'mm-book-nav';
        this._prevBtn = mkBtn('◄ Previous', 'mm-book-nav-btn', () => this._flipLore(-1));
        const pageNum = mkEl('span', 'mm-book-page-num');
        pageNum.id = 'mm-book-pnum';
        this._nextBtn = mkBtn('Next ►', 'mm-book-nav-btn', () => this._flipLore(1));
    ...
    _refreshLoreSpecies() {
        if (!this._loreSpecies) {
            const stored = localStorage.getItem(LS_CHARACTER_SPECIES);
    ...
    _setLorePage(idx) {
        ...
        `;
            ...
            if (pnum)
                pnum.textContent = `${this._lorePage + 1} \u2013 ${pages.length}`;
            if (this._prevBtn)
                this._prevBtn.disabled = this._lorePage === 0;
            if (this._nextBtn)
                this._nextBtn.disabled = this._lorePage === pages.length - 1;
        }
    }
    ...
    _buildMusicPlayer() {
        ...
        this._playPauseBtn = mkBtn('▶', 'mm-player-btn mm-player-btn--pp', () => this._togglePlayPause());
        this._playPauseBtn.title = 'Play / Pause';
        const nextBtn = mkBtn('⏭', 'mm-player-btn', () => this._nextTrack());
        nextBtn.title = 'Next';
        this._shuffleBtn = mkBtn('⇄', 'mm-player-btn mm-player-btn--shuf', () => this._toggleShuffle());
    ...
    _playTrack(idx, autoStart = false) {
        if (!this._audio)
            return;
    ...
    _updatePlayerUI() {
        if (this._songNameEl)
            this._songNameEl.textContent = TRACKS[this._trackIdx];
        if (this._playPauseBtn)
            this._playPauseBtn.textContent = this._audio?.paused !== false ? '▶' : '⏸';
    }
    // ── Modal helpers ──────────────────────────────────────────────────────
    _openModal(id) {
        ...
        preload.onload = () => {
            ...
            requestAnimationFrame(() => requestAnimationFrame(() => {
                ...
                setTimeout(() => {
                    ...
                    this.timers[i] = setTimeout(() => this._swapTile(i), SWAP_MIN_MS + randInt(0, SWAP_MAX_MS - SWAP_MIN_MS));
                }, FADE_MS + 80);
            }));
        };
        preload.onerror = () => {
    ...
    _ensureFonts() {
        if (document.getElementById('mm-gfonts'))
            return;
...
function mkModal(id, titleText, onBackdropClick) {
    ...
    const h2 = mkEl('h2', 'mm-modal-title');
