...
export class QuestAcceptModal {
    ...
    _rewardEl;
    _onAccept = null;
    get isOpen() { return this._el.classList.contains('qam--open'); }
    constructor() {
        injectHudTheme();
        ...
        this._descEl.className = 'qam-desc';
        this._targetEl = document.createElement('div');
        this._targetEl.className = 'qam-target';
        this._rewardEl = document.createElement('div');
        this._rewardEl.className = 'qam-reward';
        ...
        this._el.addEventListener('click', (e) => { if (e.target === this._el)
            this.close(); });
        // Close on Esc
        window.addEventListener('keydown', (e) => { if (e.code === 'Escape' && this.isOpen)
            this.close(); });
