...
export class HUD {
    ...
    _slotSpells = [null, null, null, null];
    /** HP animated trail */
    _hpTrailFill;
    _hpTrailTimer = null;
    ...
    /** Potion quick-slots */
    _potRoot;
    _potCounts = [document.createElement('span'), document.createElement('span')];
    _potSlots = [document.createElement('div'), document.createElement('div')];
    /** Slot glyph elements, updated with the spell icon */
    _slotGlyphs = [];
    /** G2: Quest tracker widget */
    _questTracker;
    _questTrackerCollapsed = false;
    _questTrackerData = null;
    constructor() {
        this._ensureStyles();
        ...
        xpTrack.style.width = '80px';
        xpTrack.style.height = '3px';
        ...
        this._addTooltip(hpRow, () => 'Hit Points', () => 'Drains when you take damage.\nZero ends the ritual.');
        this._addTooltip(dodgeEl, () => 'Dodge Roll  [F]', () => 'Dash in your movement direction with i-frames.\nPips show cooldown — 3 lit means ready.');
        this._addTooltip(this.runEl, () => 'Sprint  [⇧]', () => 'Hold Shift to move faster.\nNo stamina cost.');
        // ── Action bar ──────────────────────────────────────────────────────
        ...
        for (const { key, icon, label, slotIdx } of POT_DEFS) {
            ...
            const iconEl = document.createElement('span');
            iconEl.className = 'hud-pot-icon';
            iconEl.textContent = icon;
            const keyEl = document.createElement('span');
            keyEl.className = 'hud-pot-key';
            keyEl.textContent = key;
            const countEl = this._potCounts[slotIdx];
            countEl.className = 'hud-pot-count';
            countEl.textContent = '0';
    ...
    update(hp, maxHp, kills, total, floor = 0, equippedSlots = [], activeSlot = 0, dodgeReady = 1, isRunning = false, floorName, level = 1, xpProgress = 0) {
        // XP bar
        ...
        if (pct < trailPct - 0.5) {
            ...
            if (this._hpTrailTimer)
                clearTimeout(this._hpTrailTimer);
        ...
        if (slotKey !== this._lastSlotKey) {
            ...
            this.barSlots.forEach((slotEl, i) => {
                ...
                const nameEl = slotEl.querySelector('.hud-slot-name');
     ...
     */
    setAbilities(slotInfos, manaFrac) {
        // Mana bar
    ...
    _weatherIcon = '';
    // ── Tooltip helpers ───────────────────────────────────────────────────
    _addTooltip(el, getTitle, getBody) {
        el.style.pointerEvents = 'auto';
        el.addEventListener('mouseenter', (e) => {
            this._tooltipTimer = setTimeout(() => {
                this._showTooltip(getTitle(), getBody(), e.clientX, e.clientY);
            }, 360);
        });
    ...
    _showTooltip(title, body, cx, cy) {
        const titleEl = this._tooltip.querySelector('.hud-tt-title');
        const bodyEl = this._tooltip.querySelector('.hud-tt-body');
    ...
    setCooldowns(fractions, remaining) {
        for (let i = 0; i < 4; i++) {
            const slot = this.barSlots[i];
            const cdEl = slot.querySelector('.hud-slot-cd');
            if (!cdEl)
                continue;
