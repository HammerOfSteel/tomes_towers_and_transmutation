...
class Preview3D {
    ...
    _tR = 5.5;
    _th = 0.4;
    _tTh = 0.4; // azimuth (radians)
    _ph = 0.48;
    _tPh = 0.48; // elevation from horizontal
    // Character
    _charGroup = null;
    _charMixer = null;
    _charPos = new THREE.Vector3();
    _charTarget = new THREE.Vector3();
    _charFacing = 0;
    _animState = 'idle';
    _idleAction = null;
    _walkAction = null;
    _walkSpeed = 2.8; // units / second
    // Loop
    _rafId = null;
    _prevTime = 0;
    // Drag / click tracking
    _ptDown = { x: 0, y: 0, thStart: 0, phStart: 0 };
    _ptDragging = false;
    _ptMoved = false;
    // ResizeObserver
    _ro;
    ...
    constructor(canvas) {
        ...
        const bRing = new THREE.Mesh(new THREE.RingGeometry(this._arenaR - 0.08, this._arenaR, 64), new THREE.MeshBasicMaterial({ color: 0x7050cc, transparent: true, opacity: 0.8 }));
    ...
    // ── Public API ───────────────────────────────────────────────────────────────
    setCharacter(group, mixer, clips) {
        // Remove previous
        if (this._charGroup)
            this._scene.remove(this._charGroup);
        if (this._charMixer)
        ...
        if (mixer && clips.length > 0) {
            const find = (names) => names.map(n => THREE.AnimationClip.findByName(clips, n)).find(Boolean) ?? null;
    ...
    // ── Camera ───────────────────────────────────────────────────────────────────
    _updateCamera() {
        const lookAt = new THREE.Vector3(this._charPos.x, 0.85, this._charPos.z);
        const cp = Math.cos(this._ph), sp = Math.sin(this._ph);
        const ct = Math.cos(this._th), st = Math.sin(this._th);
        this._camera.position.set(lookAt.x + this._r * cp * st, lookAt.y + this._r * sp, lookAt.z + this._r * cp * ct);
    ...
    _bindInteraction(canvas) {
        ...
        canvas.addEventListener('pointermove', (e) => {
            ...
            if (this._ptMoved) {
                ...
                this._tPh = Math.max(0.08, Math.min(1.35, this._ptDown.phStart + (ddy / H) * Math.PI));
            }
        });
...
export class CharacterCreationV2 {
    ...
    _princessMode = false;
    _princessDna = null;
    _princessSpecies = null;
    _princessPanel = null;
    _princessToggleBtn = null;
    ...
    show(slotId) {
        ...
        if (!this._browser) {
            this._browser = new AssetCharBrowser(this._browserWrap, wg.charPacks, (def) => this._onModelSelected(def));
        }
    ...
    _setBoon(b) {
        this._boon = b;
        this._boonCards.forEach((card, id) => card.classList.toggle('ccv2-boon--on', id === b));
    }
    // ── HTML build ────────────────────────────────────────────────────────────────
    _buildHTML() {
        ...
        for (const b of BOONS) {
            ...
            icon.className = 'ccv2-boon-icon';
            icon.textContent = b.icon;
            const texts = document.createElement('div');
            const ttl = document.createElement('div');
            ttl.className = 'ccv2-boon-title';
            ttl.textContent = b.title;
            const eff = document.createElement('div');
            eff.className = 'ccv2-boon-effect';
            eff.textContent = b.effect;
        ...
        beginBtn.onclick = () => {
            ...
            if (this._princessMode) {
                ...
                const cfg = {
                    ...
                    boon: this._boon,
                    slotId: this._slotId,
                    dna: { /* not used in princess mode */},
                    princessDna: this._princessDna,
                    princessSpecies: this._princessSpecies ?? 'human',
                };
                this.onComplete?.(cfg);
                return;
            }
    ...
    _updatePrincessToggleLabel() {
        if (!this._princessToggleBtn)
            return;
        if (this._princessMode) {
    ...
    _openPrincessPanel() {
        if (!this._princessPanel) {
            this._princessPanel = new PrincessLibraryPanel(this._overlay.ownerDocument.body, {
                onSelect: (dna, gameSpecies) => {
                    ...
                    this._charBadge.className = 'ccv2-char-badge';
                    if (!this._nameInput.value.trim())
                        this._nameInput.value = dna.name;
                },
                onClose: () => {
