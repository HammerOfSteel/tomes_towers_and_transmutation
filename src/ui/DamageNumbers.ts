...
export type DamageKind = 'damage' | 'heal' | 'crit' | 'ability' | 'miss';
...
const KIND_SIZE: Record<DamageKind, string> = {
  damage:  '15px',
  heal:    '14px',
  crit:    '22px',
  ability: '16px',
...
interface DamageEntry {
  ...
  startTime:  number;
  duration:   number;
  startY:     number;   // screen Y at spawn
  riseAmount: number;   // total pixel rise
  jitterX:    number;   // horizontal wobble offset
}
...
export class DamageNumbers {
  private readonly _entries: DamageEntry[] = [];
   ...
   */
  spawn(worldPos: THREE.Vector3, value: number, kind: DamageKind): void {
  ...
  private _worldToScreen(worldPos: THREE.Vector3): THREE.Vector2 {
    _ndc.copy(worldPos);
    _ndc.project(this._camera);

    const rect = this._canvas.getBoundingClientRect();
    _screen.set(
      ( _ndc.x + 1) / 2 * rect.width  + rect.left,
      (-_ndc.y + 1) / 2 * rect.height + rect.top,
    );
