...
export class GameMenu {
    ...
    constructor(actions) {
        ...
        this._el.addEventListener('click', (e) => { if (e.target === this._el)
            this.close(); });
    ...
    toggle(tab) { this._open ? this.close() : this.open(tab); }
    ...
    _selectTab(id) {
        ...
        for (const btn of this._sidebar.querySelectorAll('.gm-tab')) {
    ...
    _renderLauncher(icon, name, key, desc, onOpen) {
    ...
    _renderSystem() {
        const btns = [
            ...
            { label: '📋  Controls & Help  [H]', action: () => { this.close(); /* H key handles it */ /* H key handles it */ document.dispatchEvent(new KeyboardEvent('keydown', { key: 'h', code: 'KeyH', bubbles: true })); } },
