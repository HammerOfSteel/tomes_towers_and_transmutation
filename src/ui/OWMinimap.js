...
export class OWMinimap {
    _wrap;
    _bg; // static terrain (redrawn on init)
    _overlay; // player + dynamic dots (every frame)
    _bgCtx;
    _ovCtx;
    _gw;
    _gh;
    ...
    _cx(col) { return (col / (this._gw - 1)) * (MAP_PX - 1); }
    /** Map a grid row to canvas Y (0..MAP_PX-1). */
    _cy(row) { return (row / (this._gh - 1)) * (MAP_PX - 1); }
    _renderTerrain(grid) {
    ...
    _renderSettlements(settlements) {
        const ctx = this._bgCtx;
