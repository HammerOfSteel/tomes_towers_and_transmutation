...
export class FloatingDialogue3D {
    ...
    _speechGroup = new THREE.Group();
    _choiceGroup = new THREE.Group();
    ...
    choose(choices) {
        return new Promise((resolve) => {
            ...
            setTimeout(() => {
                if (this._resolveChoice === null)
                    return; // forceChoice already resolved — don't show tiles
                this._purge();
    ...
    _updateHover() {
        ...
        if (next !== this._hoveredMesh) {
            if (this._hoveredMesh)
                this._hoveredMesh.material.color.setHex(0xffffff);
            if (next)
                next.material.color.setHex(0xffaa00);
    ...
    _onMouseMove = (e) => {
        ...
        const c = this._renderer.domElement;
        this._mouse.set((e.clientX / c.clientWidth) * 2 - 1, -(e.clientY / c.clientHeight) * 2 + 1);
    };
    _onClick = () => {
        if (this._isDragging) {
            this._isDragging = false;
        ...
        this._isDragging = false;
        if (!this._hoveredMesh || !this._resolveChoice)
            return;
    ...
    _spawnShatter(clicked) {
        ...
        for (let i = 0; i < COUNT; i++) {
            ...
            mesh.position.set(cx + (Math.random() - 0.5) * w * 0.7, cy + (Math.random() - 0.5) * h * 0.7, 0.005);
