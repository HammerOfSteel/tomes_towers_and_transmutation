...
export class QuestJournal extends QuestLog {
    ...
    _activeTab = 'species';
    _speciesTabLabel = '📖 Species Quests';
    _onKeyJournal;
    constructor() {
        super();
        injectHudTheme();
    ...
    setSpeciesTitle(title) {
        this._speciesTabLabel = `📖 ${title}`;
        if (this._journalVisible)
            this._renderJournal();
    }
    toggleJournal() {
    ...
    hideJournal() {
        ...
        if (this._journalPanel)
            this._journalPanel.style.display = 'none';
    }
    _ensureJournalPanel() {
        if (this._journalPanel)
            return;
