 ...
 *   panel.show();
 */
import { loadGallery, removeFromGallery } from '@/princess-creator/gallery';
...
function getBadgeColor(species) {
...
export class PrincessLibraryPanel {
    ...
    `;
        ...
        this._root.addEventListener('click', (e) => { if (e.target === this._root)
            this.hide(); });
    ...
    refresh() {
        if (this._root.classList.contains('plp-visible'))
            this._render();
    }
    _render() {
