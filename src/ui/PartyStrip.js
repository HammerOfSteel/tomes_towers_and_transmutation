...
export class PartyStrip {
    ...
    setMembers(members) {
        ...
        // Update each chip
        const iconEls = this._root.querySelectorAll('.hud-party-icon');
        const nameEls = this._root.querySelectorAll('.hud-party-name');
        for (let i = 0; i < this._chips.length; i++) {
            ...
            if (i < members.length) {
                ...
                chip.style.display = '';
                if (iconEls[i])
                    iconEls[i].textContent = m.icon;
                if (nameEls[i])
                    nameEls[i].textContent = m.name;
