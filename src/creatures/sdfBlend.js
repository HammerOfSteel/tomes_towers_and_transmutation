 ...
 */
function _marchGrid(evalSDF, cx, cy, cz, boxR, steps) {
    const positions = [];
    const indices = [];
    ...
    // March each cell
    const _vi = (ix, iy, iz) => field[iz][iy][ix];
    for (let iz = 0; iz < steps; iz++) {
        for (let iy = 0; iy < steps; iy++) {
            for (let ix = 0; ix < steps; ix++) {
                ...
                const active = edgePts.filter(p => p !== null);
                if (active.length < 3)
                    continue;
 ...
 * ```
 */
export function buildSeamMesh(jointA, jointB, radiusA, radiusB, opts = {}) {
