...
import type { FaceType, MouthType, Expression, EyeShape, SkinPattern, BrowStyle } from './CreatureDNA';
...
function _draw(cv: HTMLCanvasElement, spec: FaceSpec): void {
  ...
  switch (spec.faceType) {
    case 'cute':     _cute(ctx, cx, cy, eyeHex, spec.mouthType, spec.expression, spec.eyeShape); break;
    case 'angry':    _angry(ctx, cx, cy, eyeHex, spec.mouthType, spec.eyeShape); break;
    case 'cyclops':  _cyclops(ctx, cx, cy, eyeHex, spec.mouthType, spec.eyeShape); break;
    case 'skull':    _skull(ctx, cx, cy); break;
    case 'compound': _compound(ctx, cx, cy, eyeHex); break;
    case 'blank':    break;
    case 'cherubic': _cherubic(ctx, cx, cy, eyeHex, spec.mouthType, spec.expression, spec.eyeShape); break;
    case 'gaunt':    _gaunt(ctx, cx, cy, eyeHex, spec.mouthType, spec.eyeShape); break;
    case 'cat':      _cat(ctx, cx, cy, eyeHex, spec.mouthType, spec.expression, spec.eyeShape); break;
    case 'lizard':   _lizard(ctx, cx, cy, eyeHex, spec.mouthType, spec.eyeShape); break;
    case 'bird':     _bird(ctx, cx, cy, eyeHex); break;
    case 'insect':   _insect(ctx, cx, cy, eyeHex); break;
    case 'demon':    _demon(ctx, cx, cy, eyeHex, spec.mouthType, spec.eyeShape); break;
    case 'ancient':  _ancient(ctx, cx, cy, eyeHex); break;
  }
}

// ── Skin Pattern Overlay ─────────────────────────────────────────────────────

function _drawSkinPattern(ctx: CanvasRenderingContext2D, w: number, h: number, pattern: SkinPattern, markHex: string): void {
  ...
  switch (pattern) {
    case 'stripes':
      for (let i = -10; i < 20; i++) {
        ctx.beginPath();
        ctx.moveTo(i * 12, 0); ctx.moveTo(i * 12 + 6, 0);
        ctx.lineTo(i * 12 + h, h); ctx.lineTo(i * 12 + h - 6, h);
    ...
    case 'spots':
      for (let i = 0; i < 15; i++) {
        const sx = ((i * 37) % w);
        const sy = ((i * 53) % h);
        ctx.beginPath(); ctx.ellipse(sx, sy, 4 + (i % 4), 3 + (i % 3), 0, 0, Math.PI * 2); ctx.fill();
      }
      break;
    case 'scales':
      for (let row = 0; row < h; row += 10) {
        for (let col = 0; col < w; col += 10) {
          ...
          ctx.beginPath(); ctx.arc(col + offset, row, 5, 0, Math.PI, true); ctx.fill();
        }
      }
      break;
    case 'gradient':
      const grad = ctx.createLinearGradient(0, 0, 0, h);
      grad.addColorStop(0, markHex); grad.addColorStop(1, 'transparent');
      ctx.fillStyle = markHex;
      ctx.fillRect(0, 0, w, h);
      break;
    case 'cracks':
      ctx.strokeStyle = markHex; ctx.lineWidth = 1;
      for (let i = 0; i < 12; i++) {
        let x = (i * 47) % w, y = (i * 31) % h;
        ctx.beginPath(); ctx.moveTo(x, y);
        for (let j = 0; j < 4; j++) {
          x += (Math.sin(i + j) * 10); y += (Math.cos(i + j) * 8);
          ctx.lineTo(x, y);
        }
        ctx.stroke();
      }
    ...
    case 'fur':
      for (let i = 0; i < 60; i++) {
        ...
        ctx.beginPath(); ctx.ellipse(fx, fy, 2, 1, (i % 5) * 0.4, 0, Math.PI * 2); ctx.fill();
      }
      break;
  }
  ctx.globalAlpha = 1;
...
function _drawBrows(ctx: CanvasRenderingContext2D, cx: number, cy: number, style: BrowStyle, expr: Expression): void {
...
function _drawEye(ctx: CanvasRenderingContext2D, ex: number, ey: number, eyeHex: string, shape: EyeShape | undefined): void {
  ...
  if (isVoid) {
    ctx.beginPath(); ctx.ellipse(ex, ey + 1, 9, 9, 0, 0, Math.PI * 2); ctx.fill();
    // Void: dark inner ring
    ctx.fillStyle = '#000';
    ctx.beginPath(); ctx.ellipse(ex, ey + 1, 5, 5, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = eyeHex;
    ctx.beginPath(); ctx.ellipse(ex, ey + 1, 2, 2, 0, 0, Math.PI * 2); ctx.fill();
  } else if (isStar) {
    // Star pupil: 4-point star
    ctx.beginPath(); ctx.ellipse(ex, ey + 1, 8, 8, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#fff';
    _star(ctx, ex, ey + 1, 4, 7, 3, 4);
  } else if (isSlit) {
    ctx.beginPath(); ctx.ellipse(ex, ey + 1, 7, 8, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#000';
    ctx.beginPath(); ctx.ellipse(ex, ey + 1, 2, 7, 0, 0, Math.PI * 2); ctx.fill();
  } else {
    ctx.beginPath(); ctx.ellipse(ex, ey + 1, 7, 8, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#111';
    ctx.beginPath(); ctx.ellipse(ex, ey + 2, 4, 5, 0, 0, Math.PI * 2); ctx.fill();
  }
...
function _star(ctx: CanvasRenderingContext2D, cx: number, cy: number, innerR: number, outerR: number, points: number, rotation: number): void {
  ctx.beginPath();
...
function _cute(
  ...
  eyeHex: string, mouth: MouthType, expr: Expression, eyeShape?: EyeShape,
): void {
  const ey = cy - 8;
  ...
  if (expr === 'casting') {
    ...
    ctx.beginPath(); ctx.ellipse(cx - 22, cy - 8, 16, 14, 0, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(cx + 22, cy - 8, 16, 14, 0, 0, Math.PI * 2); ctx.fill();
  }
...
function _angry(
  ctx: CanvasRenderingContext2D, cx: number, cy: number, eyeHex: string, mouth: MouthType, eyeShape?: EyeShape,
): void {
  ...
  for (const [ex, side] of pairs) {
    ...
    if (eyeShape === 'slit') {
      ctx.fillStyle = eyeHex;
      ctx.beginPath(); ctx.ellipse(ex, cy - 1, 6, 7, 0, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = '#000';
      ctx.beginPath(); ctx.ellipse(ex, cy - 1, 2, 6, 0, 0, Math.PI * 2); ctx.fill();
    } else {
      ctx.fillStyle = eyeHex;
...
function _cyclops(
  ...
  if (eyeShape === 'void') {
    ctx.fillStyle = eyeHex;   ctx.beginPath(); ctx.ellipse(cx, cy - 8, 20, 17, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#000';   ctx.beginPath(); ctx.ellipse(cx, cy - 8, 12, 12, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = eyeHex;  ctx.beginPath(); ctx.ellipse(cx, cy - 8, 5, 5, 0, 0, Math.PI * 2); ctx.fill();
  } else {
...
function _cherubic(
  ...
  for (const ex of [cx - 24, cx + 24]) {
    // Larger eyes
    ctx.fillStyle = '#fffaf0';
    ctx.beginPath(); ctx.ellipse(ex, ey, 14, 14, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = eyeHex;
    ctx.beginPath(); ctx.ellipse(ex, ey + 2, 9, 10, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#111';
    ctx.beginPath(); ctx.ellipse(ex, ey + 3, 5, 6, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,0.8)';
    ctx.beginPath(); ctx.ellipse(ex - 3, ey - 2, 4, 5, -0.3, 0, Math.PI * 2); ctx.fill();
    if (eyeShape === 'star') {
      ctx.fillStyle = '#fff';
  ...
  ctx.beginPath(); ctx.ellipse(cx - 30, cy + 10, 14, 9, 0, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(cx + 30, cy + 10, 14, 9, 0, 0, Math.PI * 2); ctx.fill();
  // Button nose
...
function _gaunt(
  ctx: CanvasRenderingContext2D, cx: number, cy: number,
  eyeHex: string, mouth: MouthType, eyeShape?: EyeShape,
): void {
  ...
  for (const ex of [cx - 20, cx + 20]) {
    ...
    ctx.fillStyle = '#fffaf0';
    ctx.beginPath(); ctx.ellipse(ex, ey, 10, 8, 0, 0, Math.PI * 2); ctx.fill();
    if (eyeShape === 'void') {
      ctx.fillStyle = '#000';
      ...
      ctx.fillStyle = '#000';
      ctx.beginPath(); ctx.ellipse(ex, ey, 3, 5, 0, 0, Math.PI * 2); ctx.fill();
    }
    ctx.fillStyle = 'rgba(255,255,255,0.5)';
    ctx.beginPath(); ctx.ellipse(ex - 2, ey - 2, 2, 3, -0.3, 0, Math.PI * 2); ctx.fill();
  }
  // Hollow cheeks (dark shadows)
  ctx.fillStyle = 'rgba(30,15,20,0.2)';
  ctx.beginPath(); ctx.ellipse(cx - 26, cy + 8, 12, 8, 0.3, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(cx + 26, cy + 8, 12, 8, -0.3, 0, Math.PI * 2); ctx.fill();
  // Sharp cheekbone lines
  ctx.strokeStyle = 'rgba(60,30,40,0.3)'; ctx.lineWidth = 1.5;
  for (const s of [-1, 1]) {
    ctx.beginPath(); ctx.moveTo(cx + s * 14, cy - 2); ctx.lineTo(cx + s * 30, cy + 12); ctx.stroke();
  }
...
function _cat(
  ctx: CanvasRenderingContext2D, cx: number, cy: number,
  eyeHex: string, mouth: MouthType, expr: Expression, _eyeShape?: EyeShape,
): void {
  ...
  for (const ex of [cx - 22, cx + 22]) {
    // Almond-shaped eye
    ctx.fillStyle = '#fffaf0';
    ctx.beginPath(); ctx.ellipse(ex, ey, 12, 9, (ex < cx ? -0.2 : 0.2), 0, Math.PI * 2); ctx.fill();
    ...
    ctx.beginPath(); ctx.ellipse(ex - 2, ey - 2, 2, 3, 0, 0, Math.PI * 2); ctx.fill();
  }
  // Small triangular nose
  ctx.fillStyle = '#e09090';
  ctx.beginPath();
  ctx.moveTo(cx - 4, cy + 4); ctx.lineTo(cx + 4, cy + 4); ctx.lineTo(cx, cy + 8);
  ctx.closePath(); ctx.fill();
  // Whisker lines
...
function _lizard(
  ctx: CanvasRenderingContext2D, cx: number, cy: number,
  eyeHex: string, mouth: MouthType, _eyeShape?: EyeShape,
): void {
  // No visible nose, slit pupils, scale texture hint
  const ey = cy - 6;
  for (const ex of [cx - 22, cx + 22]) {
    ctx.fillStyle = '#e8e0c0';
    ctx.beginPath(); ctx.ellipse(ex, ey, 11, 9, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = eyeHex;
    ctx.beginPath(); ctx.ellipse(ex, ey, 8, 7, 0, 0, Math.PI * 2); ctx.fill();
    // Slit pupil
    ctx.fillStyle = '#000';
    ctx.beginPath(); ctx.ellipse(ex, ey, 1.5, 6, 0, 0, Math.PI * 2); ctx.fill();
    // No highlight — reptilian matte eye
  }
  // Scale hint lines around eyes
  ctx.strokeStyle = 'rgba(80,60,20,0.2)'; ctx.lineWidth = 1;
  for (const s of [-1, 1]) {
    for (let i = 0; i < 3; i++) {
      ctx.beginPath();
...
function _bird(ctx: CanvasRenderingContext2D, cx: number, cy: number, eyeHex: string): void {
  ...
  for (const ex of [cx - 18, cx + 18]) {
    ...
    ctx.fillStyle = '#000';
    ctx.beginPath(); ctx.ellipse(ex, ey, 2, 2, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,0.7)';
    ctx.beginPath(); ctx.ellipse(ex - 1, ey - 2, 1.5, 2, 0, 0, Math.PI * 2); ctx.fill();
  }
  // Beak
  ...
  ctx.lineTo(cx, cy + 14);
  ctx.closePath(); ctx.fill();
  ctx.strokeStyle = '#a07020'; ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(cx, cy + 2); ctx.lineTo(cx, cy + 14); ctx.stroke();
}

function _insect(ctx: CanvasRenderingContext2D, cx: number, cy: number, eyeHex: string): void {
  // Multi-facet compound eyes fill most of face
  for (const [ex, ey] of [[cx - 26, cy - 6], [cx + 26, cy - 6]] as [number, number][]) {
    const n = 7, r = 22;
    for (let i = 0; i < n; i++) {
      for (let j = 0; j < n; j++) {
        const px = ex - r + (i / (n - 1)) * r * 2;
        ...
        ctx.beginPath(); ctx.ellipse(px, py, 4, 4, 0, 0, Math.PI * 2); ctx.fill();
      }
    }
    ctx.globalAlpha = 1;
    // Outline
    ctx.strokeStyle = '#201008'; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.ellipse(ex, ey, r + 1, r * 0.72 + 1, 0, 0, Math.PI * 2); ctx.stroke();
  }
  // Mandibles
  ctx.strokeStyle = '#40200a'; ctx.lineWidth = 3;
  for (const s of [-1, 1]) {
...
function _demon(
  ...
  for (const ex of [cx - 20, cx + 20]) {
    ...
    ctx.beginPath(); ctx.ellipse(ex, ey, 2, 6, 0, 0, Math.PI * 2); ctx.fill();
    // Glow
...
function _ancient(ctx: CanvasRenderingContext2D, cx: number, cy: number, eyeHex: string): void {
  ...
  for (const [ex, ey] of eyePositions) {
    ctx.fillStyle = '#e8e0c0';
    ctx.beginPath(); ctx.ellipse(ex, ey, 8, 6, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = eyeHex;
    ctx.beginPath(); ctx.ellipse(ex, ey, 5, 4, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#000';
    ctx.beginPath(); ctx.ellipse(ex, ey, 2, 3, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,0.5)';
    ctx.beginPath(); ctx.ellipse(ex - 1, ey - 1, 1.5, 1.5, 0, 0, Math.PI * 2); ctx.fill();
  }
  // Mystical glow
  ...
  switch (mouth) {
    case 'smile': {
      ...
      if (expr === 'surprised' || expr === 'scared') {
        // O-shaped mouth
        ctx.beginPath(); ctx.ellipse(cx, cy + 2, 7, 8, 0, 0, Math.PI * 2); ctx.stroke();
      } else {
        ctx.beginPath();
