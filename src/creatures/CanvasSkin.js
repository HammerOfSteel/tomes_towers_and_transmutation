...
export function makeSkinTexture(baseColor, patternColor, pattern, scale, opacity) {
...
function _drawPattern(ctx, sz, pattern, scale) {
    switch (pattern) {
        case 'stripes':
            _stripes(ctx, sz, scale);
            break;
        case 'spots':
            _spots(ctx, sz, scale);
            break;
        case 'scales':
            _scales(ctx, sz, scale);
            break;
        case 'gradient':
            _gradient(ctx, sz);
            break;
        case 'cracks':
            _cracks(ctx, sz, scale);
            break;
        case 'fur':
            _fur(ctx, sz, scale);
            break;
...
function _cracks(ctx, sz, scale) {
    ...
    for (let i = 0; i < segs; i++) {
        ...
        ctx.moveTo(x, y);
        for (let j = 0; j < 3; j++) {
            ctx.lineTo(x + (_lcg(i * 5 + j * 11) - 0.5) * sz * 0.35, y + (_lcg(i * 3 + j * 7) - 0.5) * sz * 0.35);
        }
        ctx.stroke();
    }
}
function _fur(ctx, sz, scale) {
