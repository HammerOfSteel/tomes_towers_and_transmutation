...
export class SnakeLocomotion {
  ...
  private readonly _headWP = new THREE.Vector3();
  private readonly _invMat = new THREE.Matrix4();
  private readonly _tmpWP  = new THREE.Vector3();

  constructor(segmentCount: number, spacing: number) {
   ...
   */
  update(root: THREE.Object3D, segments: THREE.Group[]): void {
    root.getWorldPosition(this._headWP);

    if (!this._inited) {
      this._init(this._headWP, (root as THREE.Object3D & { rotation: THREE.Euler }).rotation.y);
    }
