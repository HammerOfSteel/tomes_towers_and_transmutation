...
function _hsl(h: number, s: number, l: number): number {
...
function _rgbToHsl(rgb: number): { h: number; s: number; l: number } {
...
function _pick<T>(rand: () => number, arr: readonly T[]): T {
...
const ARCH_WEIGHTS: [Archetype, number][] = [
  ['biped', 0.30], ['quadruped', 0.25], ['avian', 0.20], ['serpent', 0.15], ['amoeba', 0.10],
];

const PROP_POOLS: Partial<Record<Archetype, PropId[]>> = {
...
const TOP_POOL:  OutfitTopId[]  = ['none', 'tunic', 'robe_top', 'armor_chest', 'wrap'];
const LEGS_POOL: OutfitLegsId[] = ['none', 'trousers', 'skirt', 'shorts', 'loincloth', 'robe_skirt'];
 ...
 */
export function randomDNA(seed: number, forceArch?: Archetype): CreatureDNA {
  ...
  if (forceArch) {
    ...
    let cumul = 0;
    arch = 'biped';
    for (const [a, w] of ARCH_WEIGHTS) { cumul += w; if (r < cumul) { arch = a; break; } }
  }
...
export function mutateDNA(dna: CreatureDNA, strength: number, seed: number): CreatureDNA {
  ...
  const nudge = (v: number, lo: number, hi: number): number =>
    Math.min(hi, Math.max(lo, v + (rand() - 0.5) * 2 * s * (hi - lo) * 0.4));
  ...
  if (rand() < 0.20 * (1 + s)) {
    ...
    if (pool.length > 0) {
      if (d.props.length > 0 && rand() < 0.5) {
        ...
        if (!d.props.includes(next)) d.props.push(next);
      }
    }
  }
