...
function _bezier(p0: number, p1: number, p2: number, p3: number, t: number): number {
  const u = 1 - t;
...
function _sampleBezier(
  ...
  for (let i = 0; i <= n; i++) {
    ...
    pts.push(new THREE.Vector2(
      _bezier(x0, x1, x2, x3, t),
      _bezier(y0, y1, y2, y3, t),
    ));
  }
  return pts;
}
 ...
 */
export function dressFlaredProfile(
  height:   number,
  waistR:   number,
  hemR:     number,
  hemFlare: number,
  ...
  const upper = _sampleBezier(
    waistR,      waistR * 0.9, hemR * 0.85, hemR,
    height,      height * 0.6, height * 0.3, height * 0.15,
    8,
  );
  // Bottom hem: slight outward flare at the very bottom
  const lower = _sampleBezier(
    hemR,        hemR + hemFlare * 0.6, hemR + hemFlare, hemR + hemFlare,
    height * 0.15, height * 0.06, height * 0.01, 0,
    6,
  );
  return [...upper, ...lower];
}
...
export function robeLayeredProfile(height: number, chest: number): THREE.Vector2[] {
  ...
  const upper = _sampleBezier(
    chest,       chest * 1.06, stepR * 0.97, stepR,
    height,      height * 0.68, stepY + 0.04, stepY,
    7,
  );
  // Lower robe — step to hem
  const lower = _sampleBezier(
    stepR,       stepR * 1.05, hem * 0.98, hem,
    stepY,       stepY * 0.65, height * 0.05, 0,
    8,
  );
  return [...upper, ...lower];
}
...
export function skirtGatheredProfile(height: number, hip: number): THREE.Vector2[] {
  const hem = hip * 1.22;
  return _sampleBezier(
    hip, hip * 1.05, hem * 1.02, hem,
    height, height * 0.65, height * 0.18, 0,
    12,
  );
}
...
export function tunicProfile(height: number, chest: number): THREE.Vector2[] {
  ...
  return _sampleBezier(
    chest, chest * 1.02, hem * 0.99, hem,
    height, height * 0.6, height * 0.1, 0,
    10,
  );
}
