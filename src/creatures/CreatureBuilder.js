...
import { makeFaceTexture } from './CanvasFace';
...
let _matPool = new Map();
function _clearMatPool() { _matPool = new Map(); }
// ── Entry ─────────────────────────────────────────────────────────────────────
export function buildCreature(dna) {
    _clearMatPool();
 ...
 */
export function computeQuadNaturalFootY(rig) {
    ...
    if (kneeFL) {
        ...
        kneeFL.children.forEach(child => {
            if (child instanceof THREE.Mesh && child.position.y < pawY)
                pawY = child.position.y;
        });
        if (pawY === 0)
            pawY = -(Math.abs(kneeFL.position.y) * (25 / 27));
...
function _m(color, dna, overrides) {
...
function _free(meshes, textures) {
    return () => {
...
function _faceplane(dna, hs) {
    const spec = { faceType: dna.face.type, eyeColor: dna.face.eyeColor, mouthType: dna.face.mouthType, expression: dna.face.expression };
...
function _ears(earShape, hs, head, mat, ms) {
    if (earShape === 'none' || earShape === 'round')
        return; // round ears = no visible geometry
    if (earShape === 'pointed') {
...
// ── Props ─────────────────────────────────────────────────────────────────────
function _props(props, head, torso, dna, ms) {
    ...
    if (props.includes('horns_small') && head) {
        for (const s of [-1, 1]) {
            ...
            m.position.set(s * 0.13, 0.18, 0.02);
            m.rotation.z = s * 0.3;
            head.add(m);
            ms.push(m);
        }
    ...
    if (props.includes('horns_large') && head) {
        for (const s of [-1, 1]) {
            ...
            m.position.set(s * 0.17, 0.25, -0.04);
            m.rotation.z = s * 0.55;
            m.rotation.x = -0.18;
            head.add(m);
            ms.push(m);
    ...
    if (props.includes('hair_long') && head) {
        ...
        flow.position.set(0, -hs * 1.1, -hs * 0.72);
    ...
    if (props.includes('wings_bat') && torso) {
        for (const s of [-1, 1]) {
            ...
            shape.moveTo(0, 0);
            shape.lineTo(s * 0.82, 0.2);
            shape.lineTo(s * 0.92, -0.45);
            shape.lineTo(s * 0.4, -0.72);
            shape.lineTo(0, -0.3);
            shape.closePath();
    ...
    if (props.includes('aura') && torso) {
        const m = new THREE.Mesh(new THREE.SphereGeometry(0.62, 10, 7), new THREE.MeshBasicMaterial({ color: new THREE.Color(dna.colors.emissive), transparent: true, opacity: 0.15, side: THREE.BackSide }));
        torso.add(m);
        ms.push(m);
    }
    // ── CC-6 new props ───────────────────────────────────────────────────────
    if (props.includes('antlers') && head) {
        ...
        for (const s of [-1, 1]) {
            ...
            base.position.set(s * 0.10, 0.16, 0);
            base.rotation.z = s * 0.35;
            head.add(base);
            ms.push(base);
            for (const [bx, by, bz, rx, rz] of [[0.06, 0.24, 0, 0, s * 0.6], [-0.02, 0.22, 0.04, -0.3, s * 0.2]]) {
                const tine = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.025, 0.14, 4), am);
                tine.position.set(s * bx, by, bz);
                tine.rotation.set(rx, 0, rz);
                base.add(tine);
                ms.push(tine);
            }
        }
    ...
    if (props.includes('mane') && torso) {
        ...
        torso.add(maneIm);
        ms.push({ geometry: maneGeo, material: mm });
    }
    if (props.includes('feather_crest') && head) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            f.position.set(t * 0.15, 0.22, -0.04);
            f.rotation.z = t * 0.3;
            head.add(f);
            ms.push(f);
        }
    ...
    if (props.includes('tusk_lower') && head) {
        ...
        for (const s of [-1, 1]) {
            ...
            t.position.set(s * 0.08, -0.14, 0.06);
            t.rotation.set(0.3, 0, s * 0.15);
            head.add(t);
            ms.push(t);
        }
    }
    if (props.includes('scale_ridges') && torso) {
        ...
        ms.push({ geometry: ridgeGeo, material: sm2 });
    }
...
// ── Outfit ────────────────────────────────────────────────────────────────────────────────
function _outfit(dna, torso, ms) {
    const o = dna.outfit ?? { top: 'none', legs: 'none', over: 'none' };
    ...
    if (o.top === 'wrap') {
        for (let i = 0; i < 3; i++) {
            const ring = new THREE.Mesh(new THREE.CylinderGeometry(0.215, 0.215, 0.055, 10, 1, true), _m(0xe8dcc8, dna, { roughness: 0.9, clearcoat: 0 }));
            ring.position.y = 1.08 + i * 0.12;
            torso.add(ring);
            ms.push(ring);
        }
    }
    ...
    if (o.legs === 'trousers') {
        ...
        for (const s of [-1, 1]) {
            ...
            leg.position.set(s * 0.13, 0.37, 0);
            torso.add(leg);
            ms.push(leg);
        }
    }
    ...
    if (o.over === 'cloak') {
        ...
        m.position.set(0, 1.52, -0.24);
        m.rotation.x = 0.13;
        torso.add(m);
        ms.push(m);
        const hood = new THREE.Mesh(new THREE.SphereGeometry(0.19, 8, 6, 0, Math.PI * 2, 0, Math.PI * 0.6), new THREE.MeshPhysicalMaterial({ color: new THREE.Color(dna.colors.secondary), roughness: 0.88, metalness: 0, clearcoat: 0.1 }));
        hood.position.set(0, 1.73, -0.06);
        hood.rotation.x = 0.3;
        torso.add(hood);
        ms.push(hood);
    }
...
function _biped(dna) {
    ...
    for (const s of [-1, 1]) {
        ...
        sh.position.set(s * 0.31 * tx * sw, 1.44 * ty, 0);
    ...
    head.position.y = 0.065 + 0.22 * p.headSize;
    bones.head = head;
    neck.add(head);
    ...
    const hm = new THREE.Mesh(_wobble(_headgeo(dna.face.type, p.headSize), 0.018, dna, 1), pm);
    head.add(hm);
    ms.push(hm);
    ...
    // CC-12: SDF seam meshes — smooth-blend the neck→torso and head→neck joints
    {
        ...
        const seamNT = new THREE.Mesh(buildSeamMesh({ x: 0, y: neckBaseY, z: 0 }, { x: 0, y: torsoTopY, z: 0 }, 0.085 * nt, 0.19 * tx * sw, { blendK: 0.20 }), pm);
        torso.add(seamNT);
        ms.push(seamNT);
        const seamHN = new THREE.Mesh(buildSeamMesh({ x: 0, y: neckTopY, z: 0 }, { x: 0, y: headCentreY, z: 0 }, 0.07 * nt, p.headSize * 0.40, { blendK: 0.18 }), pm);
        torso.add(seamHN);
        ms.push(seamHN);
    }
    // Legs — hidden when a full-length over garment or skirt is worn
    const _ot = dna.outfit ?? { top: 'none', legs: 'none', over: 'none' };
    ...
    if (!_hideLegs) {
        ...
        for (const s of [-1, 1]) {
            ...
            foot.position.set(0.02, -L2_leg, 0.04);
            kneeGroup.add(foot);
            ms.push(foot);
            torso.add(hip);
            if (s === -1)
...
function _quad(dna) {
    ...
    ms.push(body);
    const legDefs = [
        { x: -0.28 * tx, z: 0.4 * tz, k: 'frontLegL', kk: 'frontLegLKnee' },
        { x: 0.28 * tx, z: 0.4 * tz, k: 'frontLegR', kk: 'frontLegRKnee' },
        { x: -0.28 * tx, z: -0.4 * tz, k: 'backLegL', kk: 'backLegLKnee' },
        { x: 0.28 * tx, z: -0.4 * tz, k: 'backLegR', kk: 'backLegRKnee' },
    ];
    ...
    for (const d of legDefs) {
        ...
        paw.position.y = -L2;
        paw.scale.set(1.3, 0.65, 1.5);
        kneeGroup.add(paw);
        ms.push(paw);
    }
    const neck = new THREE.Group();
    neck.position.set(0, 0.3, 0.5 * tz);
    neck.rotation.x = -0.7;
    bones.neck = neck;
    torso.add(neck);
    const neckM = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.12, 0.38 * p.neckLength, 8), pm);
    neckM.position.y = 0.19 * p.neckLength;
    neck.add(neckM);
    ms.push(neckM);
    const head = new THREE.Group();
    head.position.y = 0.38 * p.neckLength;
    bones.head = head;
    neck.add(head);
    const hm = new THREE.Mesh(_headgeo(dna.face.type, p.headSize * 0.88), pm);
    head.add(hm);
    ms.push(hm);
    const { tex, plane } = _faceplane(dna, p.headSize * 0.88);
    head.add(plane);
    ms.push(plane);
    if (p.tailLength > 0) {
        const tail = new THREE.Group();
        tail.position.set(0, 0.1, -0.45 * tz);
        tail.rotation.x = -1.0;
        bones.tail = tail;
        const tM = new THREE.Mesh(new THREE.ConeGeometry(0.1, 0.55 * p.tailLength, 8), sm);
        tM.position.y = 0.275 * p.tailLength;
        tail.add(tM);
        ms.push(tM);
        torso.add(tail);
    }
...
// ── Amoeba ────────────────────────────────────────────────────────────────────
function _amoeba(dna) {
    const ms = [], bones = { blobs: [] };
    const p = dna.proportions;
    ...
    blob.scale.set(p.torso[0], p.torso[1], p.torso[2]);
    torso.add(blob);
    ms.push(blob);
    // Face plane: place just outside blob surface (blob z-extent = 0.5 * headSize * torso[2])
    const head = new THREE.Group();
    head.position.z = 0.52 * p.headSize * p.torso[2] + 0.04;
    bones.head = head;
    torso.add(head);
    const { tex, plane } = _faceplane(dna, p.headSize * 0.8);
    head.add(plane);
    ms.push(plane);
...
function _avian(dna) {
    ...
    for (const s of [-1, 1]) {
        const wg = new THREE.Group();
        wg.position.set(s * 0.32 * tx, 0.18 * ty, 0);
        const sh = new THREE.Shape();
        sh.moveTo(0, 0);
        sh.bezierCurveTo(s * ws * 0.5, 0.15, s * ws, -0.05, s * ws, -0.3);
        sh.bezierCurveTo(s * ws * 0.8, -0.5, s * ws * 0.4, -0.4, 0, -0.2);
        sh.closePath();
    ...
    const neck = new THREE.Group();
    neck.position.set(0, 0.36 * ty, -0.1);
    neck.rotation.x = -0.35;
    bones.neck = neck;
    torso.add(neck);
    const neckM = new THREE.Mesh(new THREE.CylinderGeometry(0.055, 0.075, 0.28 * p.neckLength, 7), pm);
    neckM.position.y = 0.14 * p.neckLength;
    neck.add(neckM);
    ms.push(neckM);
    const head = new THREE.Group();
    head.position.y = 0.3 * p.neckLength;
    bones.head = head;
    neck.add(head);
    const hm = new THREE.Mesh(new THREE.SphereGeometry(0.2 * p.headSize, 12, 8), pm);
    head.add(hm);
    ms.push(hm);
    const { tex, plane } = _faceplane(dna, p.headSize * 0.75);
    head.add(plane);
    ms.push(plane);
    _props(dna.props, head, torso, dna, ms);
    return { root, bones, faceTex: tex, dispose: _free(ms, [tex]) };
...
function _serpent(dna) {
    const ms = [], bones = { segments: [] };
    const p = dna.proportions;
    const pm = _bodyMat(dna), sm = _m(dna.colors.secondary, dna);
    const root = new THREE.Group();
    root.scale.setScalar(p.global);
    ...
    for (let i = 0; i < n; i++) {
        ...
        g.add(seg);
        root.add(g); // FLAT — all direct children of root
        bones.segments.push(g);
        ms.push(seg);
        if (i === 0)
            bones.torso = g;
    }
