...
type B = CreatureRig['bones'];
const _isSerpent = (b: B) => (b.segments?.length ?? 0) > 0;
const _isAmoeba  = (b: B) => (b.blobs?.length  ?? 0) > 0;
const _isAvian   = (b: B) => b.wingL != null && b.armL == null;
const _isQuad    = (b: B) => b.frontLegL != null;
...
export function animateCreature(rig: CreatureRig, ctx: AnimContext): void {
  ...
  const t = ctx.time;
  switch (ctx.state) {
    case 'idle':       _idle(b, t);                             break;
    case 'walk':       _walk(b, t, ctx.velocity ?? 0.5, false); break;
    case 'run':        _walk(b, t, ctx.velocity ?? 1.0, true);  break;
    case 'hit':        _hit(b, ctx.timeSinceHit ?? 0);          break;
    case 'hypnotized': _hypno(b, t);                            break;
    case 'death':      _death(b, t);                            break;
  }
}

// ── Idle ──────────────────────────────────────────────────────────────────────

function _idle(b: B, t: number): void {
  if (_isSerpent(b)) { _idleSerpent(b, t); return; }
  if (_isAmoeba(b))  { _idleAmoeba(b, t);  return; }
  if (_isAvian(b))   { _idleAvian(b, t);   return; }
  _idleDefault(b, t);
}

function _idleDefault(b: B, t: number): void {
  ...
  if (b.tail)  b.tail.rotation.x = -1.0 + Math.sin(t * 1.3) * 0.12;
}
...
function _idleAmoeba(b: B, t: number): void {
  if (b.torso) { const p = 1 + Math.sin(t * 2.2) * 0.09; b.torso.scale.set(p, 1 / p, p); }
  if (b.head)  b.head.rotation.y = Math.sin(t * 0.6) * 0.12;
  if (b.blobs) b.blobs.forEach((g, i) => {
...
function _idleAvian(b: B, t: number): void {
  if (b.torso) { b.torso.position.y = Math.sin(t * 4.8) * 0.028; b.torso.rotation.z = Math.sin(t * 1.0) * 0.025; }
  if (b.head)  { b.head.rotation.y = Math.sin(t * 0.6) * 0.08; b.head.rotation.x = Math.sin(t * 1.5) * 0.025; }
  if (b.wingL) b.wingL.rotation.z =  Math.sin(t * 7.0) * 0.10 + 0.06;
  if (b.wingR) b.wingR.rotation.z = -Math.sin(t * 7.0) * 0.10 - 0.06;
  if (b.legL)  b.legL.rotation.x = 0.36;
  if (b.legR)  b.legR.rotation.x = 0.36;
}

// ── Walk / Run ────────────────────────────────────────────────────────────────

function _walk(b: B, t: number, vel: number, run: boolean): void {
  if (_isSerpent(b)) { _walkSerpent(b, t, run); return; }
  if (_isAmoeba(b))  { _walkAmoeba(b, t, run);  return; }
  if (_isAvian(b))   { _walkAvian(b, t, run);   return; }
  if (_isQuad(b))    { _walkQuad(b, t, run);    return; }
  _walkBiped(b, t, run);
  void vel;
}

function _walkBiped(b: B, t: number, run: boolean): void {
...
function _walkQuad(b: B, t: number, run: boolean): void {
  const sp = run ? 10 : 5.5, str = run ? 0.50 : 0.28;
  const ph = t * sp, bob = Math.sin(ph * 2) * (run ? 0.04 : 0.02);
  if (b.torso) { b.torso.position.y = bob; b.torso.rotation.x = bob * 0.35; }
  ...
  if (b.head) b.head.rotation.x = -bob * 1.5;
  if (b.tail) b.tail.rotation.x = -1.0 + Math.sin(ph * 1.3) * 0.28;
}
...
function _walkAmoeba(b: B, t: number, run: boolean): void {
  const ph = t * (run ? 4.5 : 2.8);
  const sx = 1 + Math.abs(Math.sin(ph)) * (run ? 0.22 : 0.14);
  if (b.torso) { b.torso.scale.set(1 / sx, 1 / (sx * 0.92), sx); b.torso.position.y = Math.abs(Math.sin(ph)) * (run ? 0.05 : 0.03); }
  if (b.head)  b.head.rotation.y = Math.sin(ph) * 0.14;
  if (b.blobs) b.blobs.forEach((g, i) => {
...
function _walkAvian(b: B, t: number, run: boolean): void {
  const ph = t * (run ? 22 : 14);
  if (b.torso) { b.torso.position.y = Math.sin(t * (run ? 6 : 4.8)) * (run ? 0.046 : 0.028); b.torso.rotation.x = run ? -0.14 : -0.05; }
  if (b.head)  b.head.rotation.x = run ? 0.10 : 0.0;
  if (b.wingL) b.wingL.rotation.z =  Math.sin(ph) * (run ? 0.56 : 0.38);
  if (b.wingR) b.wingR.rotation.z = -Math.sin(ph) * (run ? 0.56 : 0.38);
  if (b.legL)  b.legL.rotation.x = 0.55;
  if (b.legR)  b.legR.rotation.x = 0.55;
}

// ── Hit ───────────────────────────────────────────────────────────────────────

function _hit(b: B, tsh: number): void {
...
function _hypno(b: B, t: number): void {
...
function _death(b: B, t: number): void {
