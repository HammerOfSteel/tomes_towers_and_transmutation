...
export type StructureType = 'barrier_wall' | 'watch_perch' | 'healing_fountain' | 'ward_stone';

export interface PlacedStructure {
  id:   string;
  type: StructureType;
  /** World-space X (snapped to nearest 2 WU grid). */
  wx:   number;
  /** World-space Z (snapped to nearest 2 WU grid). */
  wz:   number;
}

/** Resource costs per structure (matches blueprint recipe ingredients). */
export const STRUCTURE_COSTS: Record<StructureType, Partial<Record<string, number>>> = {
...
export const STRUCTURE_META: Record<StructureType, { name: string; icon: string; description: string }> = {
...
export class BaseScene {
  private readonly _structures: PlacedStructure[] = [];
  private readonly _meshes = new Map<string, THREE.Group>();
  private _idCounter = 0;

  /** Fountain heal tick timer (seconds). */
  private _fountainTimer = 0;

  // ── Callbacks ─────────────────────────────────────────────────────────

  /** Called each fountain tick — heal player + minions within range. */
  onFountainHeal?: (pos: THREE.Vector3, radius: number, amount: number) => void;
  /** Called when a ward stone is placed/loaded — register repel zone. */
  onWardStonePlaced?: (pos: THREE.Vector3, radius: number, id: string) => void;
  /** Called when a ward stone is removed. */
  onWardStoneRemoved?: (id: string) => void;

  get structures(): readonly PlacedStructure[] { return this._structures; }
   ...
   * exists at that cell.
   */
  place(type: StructureType, wx: number, wz: number): PlacedStructure | null {
    const sx = Math.round(wx / SNAP) * SNAP;
    const sz = Math.round(wz / SNAP) * SNAP;

    // Block overlapping placements at the same grid cell
    if (this._structures.some(s => s.wx === sx && s.wz === sz)) return null;

    const id   = `struct_${Date.now()}_${this._idCounter++}`;
    const entry: PlacedStructure = { id, type, wx: sx, wz: sz };
  ...
  remove(id: string): void {
    ...
    if (idx === -1) return;
    const [entry] = this._structures.splice(idx, 1) as [PlacedStructure];
    this._save();

    const mesh = this._meshes.get(id);
    if (mesh) { this.scene.remove(mesh); this._freeGroup(mesh); this._meshes.delete(id); }
  ...
  nearWatchPerch(pos: THREE.Vector3, range = 3.5): PlacedStructure | null {
    let best: PlacedStructure | null = null;
    let bestD2 = range * range;
    for (const s of this._structures) {
      if (s.type !== 'watch_perch') continue;
      const dx = pos.x - s.wx;
      const dz = pos.z - s.wz;
  ...
  update(dt: number, playerPos: THREE.Vector3): void {
    ...
    if (hasFountain && this.onFountainHeal) {
      ...
      if (this._fountainTimer <= 0) {
        ...
        for (const s of this._structures) {
          if (s.type !== 'healing_fountain') continue;
  ...
  enter(): void {
    for (const mesh of this._meshes.values()) this.scene.add(mesh);
  }
  ...
  dispose(): void {
    this.exit();
    for (const mesh of this._meshes.values()) this._freeGroup(mesh);
    this._meshes.clear();
  }
  ...
  private _load(): void {
    try {
      ...
      if (!raw) return;
      const list = JSON.parse(raw) as PlacedStructure[];
      for (const s of list) {
  ...
  private _buildMesh(s: PlacedStructure): THREE.Group {
    const grp = new THREE.Group();
    grp.position.set(s.wx, 0, s.wz);

    switch (s.type) {
      case 'barrier_wall':  this._makeBarrierWall(grp);    break;
      case 'watch_perch':   this._makeWatchPerch(grp);     break;
      case 'healing_fountain': this._makeHealingFountain(grp); break;
      case 'ward_stone':    this._makeWardStone(grp);      break;
    }
    return grp;
  }

  /** 3-unit tall solid wall — 2WU wide, 0.3WU deep grey stone planks. */
  private _makeBarrierWall(grp: THREE.Group): void {
  ...
  private _makeWatchPerch(grp: THREE.Group): void {
    ...
    const railPositions: Array<[number, number, number, number]> = [
  ...
  private _makeHealingFountain(grp: THREE.Group): void {
    // Basin
  ...
  private _makeWardStone(grp: THREE.Group): void {
