...
export class NewGameScene {
    ...
    _buildCampfire() {
        ...
        const toyMat = (color, roughness = 0.5, clearcoat = 0.2, emissive = 0, flatShading = false) => new THREE.MeshPhysicalMaterial({
        ...
        for (let i = 0; i < 15; i++) {
            ...
            flame.position.set((Math.random() - 0.5) * 0.6, Math.random() * 1.5, (Math.random() - 0.5) * 0.6);
            flame.userData = {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            mesh.position.set((Math.random() - 0.5) * 1.5, Math.random() * 3 + 1, (Math.random() - 0.5) * 1.5);
            this._scene.add(mesh);
    ...
    _buildGround() {
        ...
        // Uneven mounds
        for (let i = 0; i < 15; i++) {
            const mound = new THREE.Mesh(new THREE.SphereGeometry(Math.random() * 2 + 1, 16, 16), groundMat);
            mound.position.set((Math.random() - 0.5) * 30, -0.5 - Math.random() * 1.5, (Math.random() - 0.5) * 30);
            mound.scale.y = 0.3;
    ...
    _buildTrees() {
        ...
        for (let i = 0; i < 120; i++) {
            ...
            if (radius < 10)
                continue;
            if (radius < 22 && angle > -1.5 && angle < 1.5)
                continue;
    ...
    _buildClouds() {
        ...
        for (let i = 0; i < 12; i++) {
            ...
            for (let j = 0; j < clumps; j++) {
                ...
                clump.position.set((Math.random() - 0.5) * 4, (Math.random() - 0.5) * 1.5, (Math.random() - 0.5) * 4);
            ...
            group.position.set((Math.random() - 0.5) * 120, 25 + Math.random() * 10, (Math.random() - 0.5) * 80 - 10);
    ...
    triggerGesture() {
        ...
        let gestureClip = null;
        for (const name of GESTURE_NAMES) {
            const found = w.allClips.find(c => c.name.toLowerCase().includes(name.toLowerCase()));
        ...
        if (!gestureClip)
            return; // no gesture clip available — silently skip
    ...
    _tick(dt) {
        ...
        for (const e of this._emberMeshes) {
            ...
            if (e.mesh.position.y > 4) {
                e.mesh.position.set((Math.random() - 0.5) * 0.5, 0.5, (Math.random() - 0.5) * 0.5);
            }
        }
        // ── star twinkling ────────────────────────────────────────────────────────
        ...
        this._camera.rotation.x += ((this._lookTgtX + breatheY * 0.5 + this._nodOffset) - this._camera.rotation.x) * 0.05;
        // ── user frame callback ───────────────────────────────────────────────────
        this.onFrame?.(dt);
        // ── floating dialogue tick ────────────────────────────────────────────────
        if (this._dialogueTick)
            this._dialogueTick(t);
        // ── render ────────────────────────────────────────────────────────────────
