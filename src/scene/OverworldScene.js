...
const BIOME_RIVER = [0.18, 0.38, 0.62]; // blue channel
const BIOME_WATER = [0.14, 0.26, 0.48]; // deep water
// ── OverworldScene ────────────────────────────────────────────────────────────
export class OverworldScene {
    ...
    _GHW;
    _GHH;
    _FR; // flat-zone radius in tiles (~28% of half-width)
    ...
    // ── Constructor ───────────────────────────────────────────────────────────
    constructor(scene, physics, player, worldData) {
        this.scene = scene;
    ...
    enter() {
        ...
        for (const rm of this._roadMeshes)
            this.scene.add(rm);
        for (const tr of this._trees)
            this.scene.add(tr.group);
        for (const rk of this._rocks)
            this.scene.add(rk.mesh);
        for (const ru of this._ruins)
            this.scene.add(ru);
        ...
        for (const dg of this._dungeonGroups)
            this.scene.add(dg);
        for (const bg of this._buildingGroups)
            this.scene.add(bg);
        for (const npc of this._npcs)
            npc.addToScene(this.scene);
        for (const cl of this._clutter)
            this.scene.add(cl);
        for (const rg of this._resourceGroups)
            this.scene.add(rg);
        ...
        if (this._riverGroups.length > 0) {
            ...
            for (const rg of this._riverGroups)
                this.scene.add(rg);
        }
        ...
        if (this._roadTileGroups.length > 0) {
            for (const rm of this._roadMeshes)
                rm.visible = false;
            for (const rg of this._roadTileGroups)
                this.scene.add(rg);
        }
    }
    ...
    exit() {
        ...
        for (const b of this._staticBodies)
            this.physics.rapierWorld.removeRigidBody(b);
        this._staticBodies = [];
        ...
        for (const tr of this._trees)
            this.scene.remove(tr.group);
        for (const rk of this._rocks)
            this.scene.remove(rk.mesh);
        for (const ru of this._ruins)
            this.scene.remove(ru);
        for (const en of this._enemies)
            this.scene.remove(en.group);
        this.scene.remove(this._slimeIM); // Phase 7h.2
        for (const dg of this._dungeonGroups)
            this.scene.remove(dg);
        for (const bg of this._buildingGroups)
            this.scene.remove(bg);
        for (const npc of this._npcs)
            npc.removeFromScene(this.scene);
        for (const cl of this._clutter)
            this.scene.remove(cl);
        for (const rg of this._riverGroups)
        ...
        for (const rg of this._resourceGroups)
            this.scene.remove(rg);
        if (this._waterMesh)
            this._waterMesh.visible = true; // restore for next enter
        for (const rm of this._roadMeshes)
            rm.visible = true; // restore
    }
    /** Per-frame enemy AI tick. */
    update(dt, inputE = false, camera) {
        this._skyT += dt;
        ...
        for (const en of this._enemies) {
            if (!en.isDead && !en.isRecruited)
                this._hostileHash.insert(en);
        }
        for (const en of this._enemies) {
    ...
    dispose() {
        ...
        for (const tr of this._trees)
            this._freeGroup(tr.group);
        for (const rk of this._rocks) {
            rk.mesh.geometry.dispose();
            rk.mesh.material.dispose();
        }
        for (const ru of this._ruins)
            this._freeGroup(ru);
        for (const en of this._enemies)
            en.dispose(this.physics);
        for (const npc of this._npcs)
            npc.dispose();
        this._slimeIM.geometry.dispose();
        this._slimeIM.material.dispose();
        for (const dg of this._dungeonGroups)
            this._freeGroup(dg);
        for (const bg of this._buildingGroups)
            this._freeGroup(bg);
     ...
     */
    async upgradeTreesWithAssets(loader) {
        ...
        this.scene.__assetTreesLoaded = true;
    }
     ...
     */
    async upgradeRocksWithAssets(loader) {
        const SMALL = [
        ...
        this.scene.__assetRocksLoaded = true;
    }
    ...
    async addGroundClutter(loader) {
        ...
        for (const [px, pz] of pts) {
            ...
            if (cell.feature !== 'none')
                continue; // don't clutter roads/rivers
            if (cell.content !== 'empty')
                continue;
            if (cell.settlementId > 0)
                continue;
            ...
            if (cell.biome === 'bog') {
                ...
                if (hash % 5 === 0)
                    path = MUSHROOM[hash % MUSHROOM.length];
                else if (hash % 4 === 0)
                    path = KK_ROCK_SMALL[hash % KK_ROCK_SMALL.length];
                else if (hash % 2 === 0)
                    path = KK_BUSH[hash % KK_BUSH.length];
                else
                    path = KK_GRASS[hash % KK_GRASS.length];
                propScale = 1.2;
            }
                ...
                if (hash % 3 === 0)
                    path = FLOWERS[hash % FLOWERS.length];
                else
                    path = GRASS[hash % GRASS.length];
            }
            else {
        ...
        this.scene.__assetClutterLoaded = true;
    }
     ...
     */
    async replaceWaterWithRiverTiles(loader) {
        const { _GW: GW, _GH: GH, _GHW: GHW, _GHH: GHH } = this;
        const PATHS = [
        ...
        const isRiver = (col, row) => this._wg.get(col, row).feature === 'river';
        ...
        this.scene.__assetRiverLoaded = true;
    }
     ...
     */
    async upgradeTowerWithAssets(loader) {
        ...
        const layers = [BASE, MID_D, MID, MID_W, MID, TOP, ROOF];
        ...
        this.scene.__assetTowerLoaded = true;
    }
     ...
     */
    async upgradeSettlementsWithAssets(loader, worldData) {
        ...
        const _decor = [];
        const addProp = (path, wx, wy, wz, rotY = 0, s = 1.8) => {
        ...
        for (const entry of settlements) {
            ...
            for (const r of roads) {
                ...
                if (hash % 10 !== 0)
                    continue; // ~10% of road-edge tiles
        ...
        this.scene.__assetSettlementLoaded = true;
    }
     ...
     */
    async upgradeBuildingsWithAssets(loader, worldData) {
        const { assembleBuilding, BUILDING_PRELOAD_PATHS, } = await import('@/world/buildings/AssetBuildingAssembler');
        ...
        this.scene.__assetBuildingsLoaded = true;
    }
     ...
     */
    async upgradeRoadsWithAssets(loader, worldData) {
        ...
        if (settlements) {
            for (const { plan } of settlements) {
                ...
                for (const r of plan.roads) {
                    ...
                    if (settlementRoadsSeen.has(key))
                        continue;
        ...
        for (const rg of roadGroups)
            this._roadTileGroups.push(rg);
        this.scene.__assetRoadsLoaded = true;
    }
     ...
     */
    async upgradeDungeonEntrancesWithAssets(loader) {
        ...
        this.scene.__assetDungeonLoaded = true;
    }
    ...
    getActiveEnemies() { return this._enemies; }
    ...
    _spawnEditorCamp(wx, wz, count) {
        const rand = mulberry32((Math.round(wx * 100) ^ Math.round(wz * 100)) >>> 0);
        for (let i = 0; i < count; i++) {
            ...
            const enemy = new SlimeEnemy(spawnPos, this.physics, (dmg) => this.player.health.takeDamage(dmg));
    ...
    _addEditorResourceNode(wx, wz, type) {
     ...
     */
    _buildResourceNodeMesh(wx, wz, type) {
        const grp = new THREE.Group();
    ...
    _createTerrainCollider() {
        ...
        const body = this.physics.rapierWorld.createRigidBody(RAPIER.RigidBodyDesc.fixed());
        this.physics.rapierWorld.createCollider(RAPIER.ColliderDesc.heightfield(GW - 1, GH - 1, heights, new RAPIER.Vector3((GW - 1) * T, 1.0, (GH - 1) * T)), body);
        return body;
    }
    /** Create a fixed static rigid body with the given collider at (x, y, z). */
    _addStaticBody(x, y, z, desc) {
        const body = this.physics.rapierWorld.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(x, y, z));
    ...
    _buildTerrain() {
        ...
        for (let row = 0; row < GH; row++) {
            for (let col = 0; col < GW; col++) {
                ...
                addFace([wx, wy, wz], [wx, wy, wz1], [wx1, wy, wz1], [wx1, wy, wz], 0, 1, 0, tr, tg, tb);
                // ── SOUTH wall (+Z face, at wz1) ─────────────────────────────────
                const Hs = lvl(col, row + 1);
                if (Hs < H) {
                    const wy2 = Hs * SH;
                    const d = 0.76;
                    addFace([wx1, wy, wz1], [wx, wy, wz1], [wx, wy2, wz1], [wx1, wy2, wz1], 0, 0, 1, tr * d, tg * d, tb * d);
                }
                ...
                if (Hn < H) {
                    const wy2 = Hn * SH;
                    const d = 0.50;
                    addFace([wx, wy, wz], [wx1, wy, wz], [wx1, wy2, wz], [wx, wy2, wz], 0, 0, -1, tr * d, tg * d, tb * d);
                }
                ...
                if (He < H) {
                    const wy2 = He * SH;
                    const d = 0.63;
                    addFace([wx1, wy, wz], [wx1, wy, wz1], [wx1, wy2, wz1], [wx1, wy2, wz], 1, 0, 0, tr * d, tg * d, tb * d);
                }
                ...
                if (Hw < H) {
                    const wy2 = Hw * SH;
                    const d = 0.55;
                    addFace([wx, wy, wz1], [wx, wy, wz], [wx, wy2, wz], [wx, wy2, wz1], -1, 0, 0, tr * d, tg * d, tb * d);
                }
            }
        }
    ...
    _buildWaterMesh() {
        ...
        for (let row = 0; row < GH; row++) {
            for (let col = 0; col < GW; col++) {
                ...
                const base = pos.length / 3;
                pos.push(wx, wy, wz, wx + T, wy, wz, wx + T, wy, wz + T, wx, wy, wz + T);
                idx.push(base, base + 1, base + 2, base, base + 2, base + 3);
            }
        }
        if (pos.length === 0)
    ...
    _buildTower() {
        ...
        // ── Three tower floors ────────────────────────────────────────────────
        const floorDefs = [
            { rt: 4.2, rb: 4.5, h: 5.0, cy: 5.0, hex: 0x706860 },
            { rt: 3.9, rb: 4.2, h: 5.0, cy: 10.0, hex: 0x686058 },
            { rt: 3.6, rb: 3.9, h: 5.0, cy: 15.0, hex: 0x706860 },
        ];
        for (const { rt, rb, h, cy, hex } of floorDefs) {
            ...
            for (const [wx, wz, ry] of winDefs) {
                const win = new THREE.Mesh(new THREE.BoxGeometry(0.62, 0.92, 0.16), m(0x14100c));
    ...
    _plantTrees(rand) {
        ...
        for (const [px, pz] of pts) {
            ...
            if (d < treeInner || d > treeOuter)
                continue;
            const c = Math.floor(wx / T + GHW);
            const r = Math.floor(wz / T + GHH);
            const cell = this._wg.get(c, r);
            if (cell.elevation < 1)
                continue; // no trees on bog/water
            if (cell.feature === 'road')
                continue; // no trees on roads
            if (cell.feature === 'road_dirt')
                continue;
            if (cell.content !== 'empty')
                continue; // no trees on buildings/entrances
            if (cell.settlementId > 0)
                continue; // no trees inside settlement zones
    ...
    _makeTree(rand) {
        ...
        // Trunk
        const trunk = new THREE.Mesh(new THREE.CylinderGeometry(trunkR * 0.72, trunkR, trunkH, 6), new THREE.MeshLambertMaterial({ color: 0x4a2810 }));
        trunk.position.y = trunkH / 2;
        g.add(trunk);
        // Lower canopy cone
        const greenBase = 0x1a4610 + Math.floor(rand() * 6) * 0x010100;
        const coneL = new THREE.Mesh(new THREE.ConeGeometry(coneR, coneH, 6), new THREE.MeshLambertMaterial({ color: greenBase }));
        coneL.position.y = trunkH + coneH * 0.48;
        g.add(coneL);
        // Upper canopy cone (smaller, slightly lighter)
        const coneU = new THREE.Mesh(new THREE.ConeGeometry(coneR * 0.65, coneH * 0.70, 6), new THREE.MeshLambertMaterial({ color: greenBase + 0x040800 }));
    ...
    _placeRocks(rand) {
        ...
        for (const [px, pz] of pts) {
            ...
            const wz = pz - H / 2;
            const d = Math.sqrt(wx * wx + wz * wz);
            if (d < rockInner || d > rockOuter)
                continue;
            const c = Math.floor(wx / T + GHW);
            const r = Math.floor(wz / T + GHH);
            const cell = this._wg.get(c, r);
            if (cell.feature === 'road')
                continue; // no rocks on roads
            if (cell.feature === 'road_dirt')
                continue;
            if (cell.content !== 'empty')
                continue; // no rocks on buildings
            if (cell.settlementId > 0)
                continue; // no rocks inside settlement zones
            const level = cell.elevation;
            const wy = level * SH;
            ...
            const mesh = new THREE.Mesh(new THREE.DodecahedronGeometry(radius, 0), new THREE.MeshLambertMaterial({ color }));
    ...
    _spawnCamps(rand, campCount) {
        ...
        for (const [px, pz] of pts) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                this._enemies.push(new SlimeEnemy(new THREE.Vector3(ex, level * SH + 0.9, ez), this.physics, (dmg) => this.player.health.takeDamage(dmg)));
            }
            camps++;
        }
    }
    ...
    _addRuins(rand) {
        ...
        for (const [px, pz] of pts) {
            if (this.buildingEntrances.length >= 2)
                break;
            const wx = px - W / 2;
            const wz = pz - H / 2;
            const d = Math.sqrt(wx * wx + wz * wz);
            if (d < ruinInner || d > ruinOuter)
                continue;
            const c = Math.floor(wx / T + GHW);
            const r = Math.floor(wz / T + GHH);
            const level = this._wg.get(c, r).elevation;
    ...
    _makeRuin(cx, cy, cz, rand) {
        ...
        const flr = new THREE.Mesh(new THREE.CylinderGeometry(RING_R + 0.28, RING_R + 0.28, 0.16, 16), floorMat);
        flr.position.y = 0.08;
        grp.add(flr);
        ...
        for (let i = 0; i < PILLARS; i++) {
            ...
            const pillar = new THREE.Mesh(new THREE.CylinderGeometry(0.20, 0.24, ph, 6), stoneMat);
        ...
        const glw = new THREE.Mesh(new THREE.SphereGeometry(0.32, 8, 6), new THREE.MeshLambertMaterial({
            color: 0x44cc88,
            emissive: 0x44cc88,
    ...
    // ── Dungeon entrances ─────────────────────────────────────────────────────
    _placeDungeonEntrances(dungeons, rand) {
        const { _GHW: GHW, _GHH: GHH } = this;
    ...
    _buildEntranceMesh(key, wx, wy, wz, rand) {
        switch (key) {
    ...
    _makeCaveArch(cx, cy, cz, rand) {
        const grp = new THREE.Group();
        grp.position.set(cx, cy, cz);
        const stone = new THREE.MeshLambertMaterial({ color: 0x4a4540 });
        for (const side of [-1, 1]) {
            const h = 1.2 + rand() * 0.6;
            const rock = new THREE.Mesh(new THREE.DodecahedronGeometry(0.7 + rand() * 0.3, 0), stone);
        ...
        // Arch cap
        const cap = new THREE.Mesh(new THREE.DodecahedronGeometry(0.55, 0), new THREE.MeshLambertMaterial({ color: 0x39332e }));
        cap.position.set(0, 1.9, 0);
        cap.scale.set(1.6, 0.45, 0.7);
        grp.add(cap);
        // Debris pebbles at the base
        for (let i = 0; i < 4; i++) {
            const p = new THREE.Mesh(new THREE.DodecahedronGeometry(0.14 + rand() * 0.1, 0), stone);
    ...
    _makeCryptDoor(cx, cy, cz, rand) {
        const grp = new THREE.Group();
        grp.position.set(cx, cy, cz);
        const slab = new THREE.MeshLambertMaterial({ color: 0x5a5248 });
        // Two frame posts
        for (const side of [-1, 1]) {
            const post = new THREE.Mesh(new THREE.BoxGeometry(0.38, 2.4, 0.38), slab);
            post.position.set(side * 0.9, 1.2, 0);
            post.rotation.z = side * 0.05;
            grp.add(post);
        }
        // Lintel
        const lintel = new THREE.Mesh(new THREE.BoxGeometry(2.1, 0.32, 0.40), slab);
        lintel.position.set(0, 2.4, 0);
        grp.add(lintel);
        // Slightly leaning door slab
        const door = new THREE.Mesh(new THREE.BoxGeometry(1.4, 2.2, 0.20), new THREE.MeshLambertMaterial({ color: 0x3a322c }));
    ...
    _makeRuinPillars(cx, cy, cz, rand) {
        const grp = new THREE.Group();
        grp.position.set(cx, cy, cz);
        ...
        for (let i = 0; i < COUNT; i++) {
            ...
            const col = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.22, h, 6), stoneMat);
            col.position.set(Math.cos(angle) * R, h / 2, Math.sin(angle) * R);
            col.rotation.z = (rand() - 0.5) * 0.22;
            grp.add(col);
            // Rubble at base
            for (let j = 0; j < 2; j++) {
                const peb = new THREE.Mesh(new THREE.DodecahedronGeometry(0.13 + rand() * 0.09, 0), stoneMat);
                peb.position.set(Math.cos(angle) * (R + (rand() - 0.5) * 0.9), 0.1, Math.sin(angle) * (R + (rand() - 0.5) * 0.9));
    ...
    _makeMineShaft(cx, cy, cz, rand) {
        const grp = new THREE.Group();
        grp.position.set(cx, cy, cz);
        const wood = new THREE.MeshLambertMaterial({ color: 0x5c3d1e });
        // Two angled support posts forming an A
        for (const side of [-1, 1]) {
            const post = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.11, 2.2, 5), wood);
            post.position.set(side * 0.7, 1.1, 0);
            post.rotation.z = side * 0.22;
            grp.add(post);
        }
        // Crossbeam
        const beam = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 1.8, 5), wood);
        beam.rotation.z = Math.PI / 2;
        beam.position.set(0, 1.65, 0);
        grp.add(beam);
        // Dark opening (square frame inset)
        const opening = new THREE.Mesh(new THREE.BoxGeometry(1.2, 1.6, 0.12), new THREE.MeshLambertMaterial({ color: 0x1a1008 }));
        opening.position.set(0, 0.8, 0.06);
        grp.add(opening);
        // Plank floor
        const plank = new THREE.Mesh(new THREE.BoxGeometry(1.0 + rand() * 0.4, 0.08, 0.18), wood);
    ...
    _makeBookPortal(cx, cy, cz, rand) {
        ...
        for (const side of [-1, 1]) {
            const page = new THREE.Mesh(new THREE.PlaneGeometry(0.8, 1.1), bookMat);
            page.rotation.y = side * 0.35;
            page.position.set(side * 0.38, 1.5, 0);
            grp.add(page);
        }
        // Central glow orb
        const glowMesh = new THREE.Mesh(new THREE.SphereGeometry(0.22, 8, 6), new THREE.MeshLambertMaterial({
            color: 0x88aaff,
            emissive: 0x4466ff,
        ...
        for (let i = 0; i < 5; i++) {
            ...
            sp.position.set((rand() - 0.5) * 1.2, 0.8 + rand() * 1.4, (rand() - 0.5) * 0.8);
            grp.add(sp);
        }
        return grp;
    }
    // ── Settlements ────────────────────────────────────────────────────────────
    _buildSettlements(worldData) {
        const { settlements } = worldData;
        if (!settlements || settlements.length === 0)
            return;
        const { _GHW: GHW, _GHH: GHH } = this;
    ...
    // ── NPC spawning ──────────────────────────────────────────────────────────
    _spawnSettlementNPCs(worldData) {
        ...
        const VILLAGE_ROLES = ['citizen', 'citizen', 'citizen', 'merchant', 'guard'];
        const TOWN_ROLES = ['citizen', 'citizen', 'merchant', 'merchant', 'guard', 'guard', 'innkeeper', 'blacksmith'];
        const CITY_ROLES = ['citizen', 'citizen', 'merchant', 'merchant', 'guard', 'guard', 'innkeeper', 'blacksmith', 'scholar'];
        for (const entry of settlements) {
            ...
            let nearRiverDir;
            const RIVER_SCAN = 20;
            outerRiver: for (let dist = 1; dist <= RIVER_SCAN; dist++) {
                for (let d = -dist; d <= dist; d++) {
                    for (const [dc, dr] of [[d, -dist], [d, dist], [-dist, d], [dist, d]]) {
            ...
            for (let i = 0; i < npcCount; i++) {
                ...
                this._npcs.push(new NPCEntity(npcCol, npcRow, npcWx, npcWz, role, entry, nearby, nearDungeonName, nearDungeonDir, nearRiverDir, dungeons));
                const npc = this._npcs[this._npcs.length - 1];
                if (this.onQuestGiven)
                    npc.onQuestGiven = this.onQuestGiven;
                if (this.onMerchant)
                    npc.onOpenMerchant = this.onMerchant;
