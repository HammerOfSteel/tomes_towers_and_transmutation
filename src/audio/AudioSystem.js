...
export class AudioSystem {
    ...
    setVolume(category, value) {
        this._gains[category].gain.setTargetAtTime(Math.max(0, Math.min(1, value)), this._ctx.currentTime, 0.05);
    }
    getVolume(category) {
        return this._gains[category].gain.value;
    }
    setMasterVolume(value) {
        for (const cat of Object.keys(this._gains)) {
            this.setVolume(cat, value * this.getVolume(cat));
        }
    }
    // ── Ambient scene ─────────────────────────────────────────────────────────
    setAmbientScene(scene) {
        if (scene === this._ambientScene)
            return;
        this._ambientScene = scene;
        this._stopAmbient(0.8);
        if (scene !== 'off') {
            setTimeout(() => { if (this._ambientScene === scene)
                this._startAmbient(scene); }, 900);
        }
    }
    _stopAmbient(fadeTime = 0.4) {
    ...
    _addDrone(freq, amp, lfoRate, lfoDepth) {
        const ctx = this._ctx;
    ...
    _addWind(amp, lowFreq, highFreq, lfoRate) {
        const ctx = this._ctx;
