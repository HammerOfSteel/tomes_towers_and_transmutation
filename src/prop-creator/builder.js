...
// ── Material factory ──────────────────────────────────────────────────────────
function makeMat(hex, emissive, emissiveIntensity = 0.4) {
 ...
 */
export function buildProp(dna) {
    const builder = BUILDERS[dna.propKind];
    if (!builder)
        throw new Error(`[buildProp] unknown propKind: "${dna.propKind}"`);
    const { root, collision, light } = builder(dna);
    // Apply size scaling
    if (dna.size !== 1)
        root.scale.setScalar(dna.size);
    ...
    if (dna.condition === 'damaged' || dna.condition === 'ruined') {
        ...
        root.traverse(obj => {
            if (obj instanceof THREE.Mesh && obj.material instanceof THREE.MeshLambertMaterial) {
                ...
                obj.material.color.multiplyScalar(0.75);
            }
        });
    }
