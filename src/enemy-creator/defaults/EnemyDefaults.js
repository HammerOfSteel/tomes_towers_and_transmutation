...
// ── Main factory ──────────────────────────────────────────────────────────────
export function getDefaultEnemyDna(species, role, tier, seed, isBoss = false) {
    ...
    const baseHpByTier = { 1: 20, 2: 45, 3: 90, 4: 300 };
    const baseDmgByTier = { 1: 4, 2: 8, 3: 16, 4: 40 };
    const aggroByRole = {
...
export function getEnemiesForFloor(floor, count, seed) {
    const tier = tierForFloor(floor);
    const r = mulberry32(seed ^ 0xF100_DEAD);
    const roles = ['melee', 'melee', 'ranged', 'caster', 'tank', 'swarm'];
    const species = ['undead', 'undead', 'human', 'elf', 'draconic', 'slime'];
