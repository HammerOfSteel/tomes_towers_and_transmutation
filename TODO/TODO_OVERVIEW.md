...
| G16 | **Organic World Tiles** — terrain/shorelines, building corners, and settlement plot layout are all still fundamentally square-grid-typed (per-cell/per-tile, neighbour-smoothing bolted on after the fact — the just-shipped shoreline noise-wobble is the latest example). Research into Townscaper's dual-grid marching-squares + jittered-triangle relaxed-mesh technique is complete; a phased roadmap (Phase 0 shared case-table infra, Phase 1 real dual-grid shorelines, Phase 2 building corners, Phase 3 organic settlement plots, Phase 4 dungeons **[scope check-in needed before starting]**, Phase 5 props) is written but **not started** | Medium — art direction, not blocking | `organic_world_tiles_todo.md` (TODO root, cross-cuts 02 + 03) |
