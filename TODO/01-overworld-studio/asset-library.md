...
- [x] Actions shipped: Import JSON, Rename, Duplicate, Delete, Export JSON, Pin to location (`assetLibrary.pinToLocation()`/`unpinFromLocation()` — adds/removes a `<type>:<locationId>` tag matching the existing runtime override tag convention; click-to-unpin chips render in the preview panel), Edit DNA — raw JSON editor in the Library preview panel (`AssetLibrary.updateData()`); marks the entry `isCustom`
