...
export class BotReporter {
    ...
    addSteps(steps) { steps.forEach(s => this._steps.push(s)); }
    get passed() { return this._steps.filter(s => s.status === 'pass').length; }
    get failed() { return this._steps.filter(s => s.status === 'fail').length; }
