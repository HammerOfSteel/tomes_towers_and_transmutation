...
export async function princessCreatorScenario(bot) {
    ...
    await bot.step('spawn foxling princess via __game.buildPrincess', async () => {
        ...
        if (code) {
            await bot.page.evaluate((shareCode) => {
                window.__game?.buildPrincess?.(shareCode);
            }, code);
        }
        else {
            // Fallback: use the first default
            await bot.page.evaluate((dna) => {
                window.__game?.buildPrincess?.(dna);
    ...
    await bot.step('open princess-creator in new tab via __game', async () => {
        ...
        const hasAtelier = await bot.page.evaluate(() => {
            return typeof window.__game?.buildPrincess === 'function';
        });
...
function expect_contains(arr, val, label) {
    if (!arr.includes(val)) {
