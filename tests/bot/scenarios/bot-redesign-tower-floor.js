...
export async function botRedesignTowerFloorScenario(bot) {
    ...
    await bot.step('pick barrel asset', async () => {
        const name = await CreativeActions.pickAsset(bot, 'barrel').catch(() => CreativeActions.pickAsset(bot));
    ...
    for (let i = 0; i < wallPositions.length; i++) {
        ...
        await bot.step(`place barrel ${i + 1}/${wallPositions.length}`, async () => {
            await bot.page.evaluate(([px, py, pz]) => window.__game.teleportPlayer?.(px, py, pz), [x + 0.5, y, z + 0.5]);
    ...
    await bot.step('open inventory — pick candelabra/torch asset', async () => {
        await CreativeActions.openInventory(bot);
        await CreativeActions.selectKit(bot, 'KayKit');
        const name = await CreativeActions.pickAsset(bot, 'candel').catch(() => CreativeActions.pickAsset(bot, 'torch').catch(() => CreativeActions.pickAsset(bot)));
