...
export async function botDesignDungeonRoomScenario(bot) {
    ...
    await bot.step('enter Dungeon Prototype backroom', async () => {
        ...
        if (visible) {
            ...
            await bot.page.evaluate(function () {
                window.__game?.enterBackroom?.('dungeon_prototype');
            });
        }
    ...
    await bot.step('place enemy spawn at room centre', async () => {
        await bot.page.evaluate(function () {
            window.__game.teleportPlayer?.(0.5, 0, 0.5);
        });
    ...
    await bot.step('pick chest / crate asset', async () => {
        const name = await CreativeActions.pickAsset(bot, 'chest').catch(() => CreativeActions.pickAsset(bot) // any asset if no chest
        );
