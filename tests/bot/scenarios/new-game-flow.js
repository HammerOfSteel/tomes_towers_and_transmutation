...
export async function newGameFlowScenario(bot) {
    ...
    await bot.step('Dev mode enabled and game reloads', async () => {
        ...
        await bot.page.waitForFunction(() => !!window.__game, { timeout: 30_000 });
    });
    // ── Step 2: Start game ───────────────────────────────────────────────────────
    ...
    await bot.step('Teleport to F0 — Grand Entrance Hall', async () => {
        ...
        if (floor !== 0 && floor !== -99)
            throw new Error(`Expected floor 0, got ${floor}`);
    });
    ...
    await bot.step('Switch to exterior overworld', async () => {
        ...
        const mode = await MovementActions.getGameMode(bot);
        if (mode !== 'exterior')
            throw new Error(`Expected exterior, got ${mode}`);
    });
