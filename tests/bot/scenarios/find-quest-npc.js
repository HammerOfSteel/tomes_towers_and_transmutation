...
export async function findQuestNpcScenario(bot) {
    ...
    await bot.step('find Archivist NPC on floor 0', async () => {
        ...
        console.log(`    near NPC at (${pos.x?.toFixed(1)}, ${pos.z?.toFixed(1)})`);
    });
    await bot.step('trigger NPC dialogue', async () => {
        ...
        if (text)
            console.log(`    dialogue: "${text.slice(0, 60)}..."`);
        else
            console.log('    (no DOM dialogue found — may be canvas-rendered)');
    });
