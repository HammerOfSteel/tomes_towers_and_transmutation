...
export async function botPlaceForestScenario(bot) {
    ...
    for (let i = 0; i < treePositions.length; i++) {
        ...
        await bot.step(`place tree ${i + 1}/12`, async () => {
            await bot.page.evaluate(([px, py, pz]) => window.__game.teleportPlayer?.(px, py, pz), [x + 0.5, y, z + 0.5] // stand just beside target spot
            );
    ...
    for (let i = 0; i < rockPositions.length; i++) {
        ...
        await bot.step(`place rock ${i + 1}/6`, async () => {
            await bot.page.evaluate(([px, py, pz]) => window.__game.teleportPlayer?.(px, py, pz), [x + 0.3, y, z + 0.3]);
            await bot.page.waitForTimeout(150);
            await CreativeActions.placeAtCenter(bot);
        });
    }
