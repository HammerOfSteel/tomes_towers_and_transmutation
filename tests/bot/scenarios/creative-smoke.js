 ...
 */
export async function creativeSmokeScenario(bot) {
    // ── Step 1: Load game ───────────────────────────────────────────────────
    ...
    await bot.step('Dev mode enabled', async () => {
        ...
        const isDevMode = await bot.page.evaluate(() => localStorage.getItem('ttt_dev_mode') === 'true');
        if (!isDevMode)
            throw new Error('Dev mode not set in localStorage');
    });
    // ── Step 3: Start new game ──────────────────────────────────────────────
    await bot.step('New game starts with fixed seed', async () => {
        await bot.startNewGame(0xBEEF_CAFE);
        // Verify game mode
        const mode = await bot.page.evaluate(() => window.__game.getGameMode());
        if (!mode)
            throw new Error('Game mode not set — game may not have started');
    });
    ...
    await bot.step('Creative HUD has status bar and hotbar', async () => {
        ...
        if (slotCount !== 8)
            throw new Error(`Expected 8 hotbar slots, got ${slotCount}`);
    });
    ...
    await bot.step('Clicking KayKit group shows kit list', async () => {
        ...
        if (kitCount === 0)
            throw new Error('No kit buttons visible after clicking KayKit group');
    });
    await bot.step('Dungeon Pack shows asset cards', async () => {
        ...
        if (cardCount === 0)
            throw new Error('No asset cards — Dungeon Pack shows empty grid');
    });
    ...
    await bot.step('Active hotbar slot shows asset name', async () => {
        ...
        if (text === '—')
            throw new Error('Hotbar slot 1 still shows — (empty) after picking');
    });
    ...
    await bot.step('Exit creative mode restores normal state', async () => {
        ...
        if (hudGone > 0)
            throw new Error('Creative HUD still present after exit');
    });
}
