...
export const PathfindingActions = {
     ...
     */
    async walkToPos(bot, target, stopRadius = 1.5, timeoutMs = 15_000) {
        ...
        while (Date.now() < deadline) {
            ...
            await bot.page.evaluate(([nx, ny, nz]) => window.__game.teleportPlayer?.(nx, ny, nz), [pos.x + (dx / dist) * step, pos.y, pos.z + (dz / dist) * step]);
     ...
     */
    async walkToRoom(bot, roomId) {
        await bot.page.evaluate((id) => window.__game.onTeleportRoom?.(id), roomId);
     ...
     */
    async findAndApproach(bot, type, radius = 2) {
        const found = await bot.page.evaluate((t) => {
            const g = window.__game;
            if (g?.findNearestObject)
                return g.findNearestObject(t);
            return null;
        }, type);
        if (!found) {
     ...
     */
    async findNPC(bot, npcName) {
        const pos = await bot.page.evaluate((name) => {
            const g = window.__game;
            if (g?.findNPC)
                return g.findNPC(name);
            return null;
        }, npcName);
        if (!pos) {
     ...
     */
    async findEnemy(bot, enemyType) {
        const pos = await bot.page.evaluate((t) => {
            const g = window.__game;
            if (g?.findNearestEnemy)
                return g.findNearestEnemy(t);
            return null;
        }, enemyType);
        if (!pos) {
     ...
     */
    async evadeEnemies(bot, duration = 2_000) {
        const enemyPos = await bot.page.evaluate(function () {
            const g = window.__game;
            return g?.getNearestEnemyPos?.() ?? null;
        });
        if (!enemyPos)
            return;
        const myPos = await bot.page.evaluate(function () {
            return window.__game.getPlayerPos?.() ?? { x: 0, y: 0, z: 0 };
        });
     ...
     */
    async getCurrentFloorRooms(bot) {
