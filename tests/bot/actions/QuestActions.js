...
export const QuestActions = {
     ...
     */
    async advanceDialogue(bot, times = 1) {
     ...
     */
    async waitForDialogue(bot) {
        await bot.page.waitForSelector('.dialogue-box, #dialogue-root, [id*="dialogue"], [class*="dialogue"]', { state: 'visible', timeout: 5_000 });
    },
    /**
     * Read the current dialogue text (first dialogue box found).
     */
    async getDialogueText(bot) {
     ...
     */
    async assertQuestActive(bot, idOrName) {
        // Try __game API first
        const apiActive = await bot.page.evaluate((id) => {
            const g = window.__game;
            if (g?.getQuestState)
                return g.getQuestState(id) === 'active';
            return false;
        }, idOrName);
        if (!apiActive) {
            ...
            if (dom === 0)
                throw new Error(`Quest "${idOrName}" not found in active quests`);
        }
    },
    /**
     * Assert a quest is marked complete.
     */
    async assertQuestComplete(bot, idOrName) {
        const apiDone = await bot.page.evaluate((id) => {
            const g = window.__game;
            if (g?.getQuestState)
                return g.getQuestState(id) === 'complete';
            return false;
        }, idOrName);
        if (!apiDone) {
            ...
            if (dom === 0)
                throw new Error(`Quest "${idOrName}" not marked complete`);
        }
    },
    /**
     ...
     */
    async walkToward(bot, target, stopRadius = 2, timeoutMs = 10_000) {
        ...
        while (Date.now() < deadline) {
            ...
            await bot.page.evaluate(([px, py, pz]) => window.__game.teleportPlayer?.(px, py, pz), [nx, pos.y, nz]);
     ...
     */
    async collectItem(bot, itemSelector) {
