...
export const CreativeActions = {
    ...
    async enter(bot) {
        await bot.page.evaluate(function () { window.__game.enterCreativeMode(); });
    ...
    async pickAsset(bot, nameFragment) {
    ...
    async assertAssetCards(bot, min) {
        const count = await bot.page.locator('.cab-card').count();
        if (count < min)
            throw new Error(`Expected ≥${min} asset cards, got ${count}`);
        return count;
    },
    /** Toggle the settings panel (O key). */
    async openSettings(bot) {
