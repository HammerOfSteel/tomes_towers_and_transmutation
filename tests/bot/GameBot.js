...
export class GameBot {
    ...
    async close() {
        if (this.cfg.record)
            await this._ctx?.close(); // flushes video
    ...
    async waitForGame() {
        ...
        await this.page.waitForFunction(() => !!window.__game, { timeout: 30_000 });
     ...
     */
    async startNewGame(seed = 0xDEAD_BEEF) {
        await this.page.evaluate((s) => window.__game.startGame(s), seed);
    ...
    async enterCreative() {
        await this.page.evaluate(() => window.__game.enterCreativeMode());
        await this.page.locator('#creative-hud-root').waitFor({ state: 'attached', timeout: 8_000 });
    }
    /** Exit creative mode. */
    async exitCreative() {
        await this.page.evaluate(() => window.__game.exitCreativeMode());
    ...
    async teleport(x, y, z) {
        await this.page.evaluate(([px, py, pz]) => window.__game.teleportPlayer(px, py, pz), [x, y, z]);
        await this.page.waitForTimeout(200);
    }
    /** Teleport to a room by ID. */
    async teleportToRoom(roomId) {
        await this.page.evaluate((id) => window.__game.onTeleportRoom?.(id), roomId);
    ...
    // ── Assertions ────────────────────────────────────────────────────────────
    async assertVisible(selector, timeout = 5000) {
    ...
    async assertText(selector, text) {
        ...
        if (!actual?.includes(text))
            throw new Error(`Expected "${text}" in "${actual}"`);
    }
    async assertCount(selector, min) {
        const n = await this.page.locator(selector).count();
        if (n < min)
            throw new Error(`Expected ≥${min} "${selector}", got ${n}`);
    }
    async evaluate(fn) {
    ...
    async instruct(text) {
        ...
        if (floorMatch) {
            ...
            await p.evaluate((id) => window.__game.onTeleportRoom?.(id), roomId);
            await p.waitForTimeout(600);
            return `teleported to floor ${n}`;
        }
        // go to overworld / exterior
        if (/go\s+to\s+(overworld|exterior)/.test(t)) {
            await p.evaluate(function () { window.__game.switchToExterior?.(); });
        ...
        if (roomMatch) {
            await p.evaluate((id) => window.__game.onTeleportRoom?.(id), roomMatch[1]);
            await p.waitForTimeout(600);
            return `teleported to room: ${roomMatch[1]}`;
        }
        // enter creative
        if (/enter\s+creative/.test(t)) {
            await p.evaluate(function () { window.__game.enterCreativeMode(); });
        ...
        if (/exit\s+creative/.test(t)) {
            await p.evaluate(function () { window.__game.exitCreativeMode(); });
        ...
        if (placeMatch) {
            ...
            if (x !== undefined && z !== undefined) {
                const pos = await p.evaluate(function () {
                    return window.__game.getPlayerPos?.() ?? { x: 0, y: 0, z: 0 };
                });
                await p.evaluate(([px, py, pz]) => window.__game.teleportPlayer?.(px, py, pz), [x, pos.y, z]);
        ...
        if (findMatch) {
            ...
            const pos = await p.evaluate((n) => {
                const g = window.__game;
                return g?.findNPC?.(n) ?? g?.findNearestObject?.(n) ?? null;
            }, name);
            if (pos) {
                await p.evaluate(([px, py, pz]) => window.__game.teleportPlayer?.(px - 1, py, pz - 1), [pos.x, pos.y, pos.z]);
                await p.waitForTimeout(400);
