...
function makeSlimeStub(opts: {
  ...
  let state: 'recruited' | 'guard' | 'dead' = opts.isDead ? 'dead' : 'recruited';
  const stub = {
    ...
    assignGuard: vi.fn((_pos: THREE.Vector3) => { state = 'guard'; }),
  };
  if (opts.isGuarding) state = 'guard';
  return stub as unknown as SlimeEnemy;
}
...
describe('Guard assignment via party members', () => {
  ...
  it('skips dead party members when selecting a guard', () => {
    ...
    const members: ReturnType<typeof makeSlimeStub>[] = [dead, alive];
