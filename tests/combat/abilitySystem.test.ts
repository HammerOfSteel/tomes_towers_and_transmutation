...
import { AbilitySystem, ManaPool, type Ability, type AbilityCastContext } from '@/combat/AbilitySystem';
import * as THREE from 'three';

// ── Mock cast context ─────────────────────────────────────────────────────────

function makeMockCtx(overrides?: Partial<AbilityCastContext>): AbilityCastContext {
  return {
    ...
    progression: { mods: { meleeDamageMult: 1, spellDamageMult: 1 } } as unknown as AbilityCastContext['progression'],
    enemies:     [],
...
function makeAbility(id: string, cooldown: number, manaCost: number, castFn = vi.fn()): Ability {
...
describe('AbilitySystem — trycast', () => {
  let sys: AbilitySystem;
  let castFn: ReturnType<typeof vi.fn>;
  let ctx: AbilityCastContext;
...
describe('applyCharacterAbilities — species ability assignment', () => {
  ...
  it('each ability has positive cooldown and manaCost', async () => {
    const { AbilitySystem: AS, applyCharacterAbilities: apply } = await import('@/combat/AbilitySystem');
    const chars = ['human_warrior', 'skeleton_mage', 'fox_rogue', 'slime'] as const;
