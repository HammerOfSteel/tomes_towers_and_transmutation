...
import { validateEncounterDef, totalEnemyCount, ENCOUNTER_POOLS, ENCOUNTER_POOL_F1, ENCOUNTER_POOL_F2, } from '@/levels/RoomEncounterDef';
// ── Helpers ───────────────────────────────────────────────────────────────────
...
describe('validateEncounterDef', () => {
    ...
    it('rejects waveCount < 1', () => {
        expect(validateEncounterDef(makeSwarm({ waveCount: 0 }))).toEqual(expect.arrayContaining([expect.stringContaining('waveCount')]));
    });
    it('rejects waveKillThreshold < 1', () => {
        expect(validateEncounterDef(makeSwarm({ waveKillThreshold: 0 }))).toEqual(expect.arrayContaining([expect.stringContaining('waveKillThreshold')]));
    });
    ...
    it('rejects missing id', () => {
        expect(validateEncounterDef(makeSwarm({ id: '' }))).toEqual(expect.arrayContaining([expect.stringContaining('id')]));
    });
    it('rejects empty enemies array', () => {
        expect(validateEncounterDef(makeSwarm({ enemies: [] }))).toEqual(expect.arrayContaining([expect.stringContaining('at least one enemy group')]));
    });
});
// ── totalEnemyCount ───────────────────────────────────────────────────────────
