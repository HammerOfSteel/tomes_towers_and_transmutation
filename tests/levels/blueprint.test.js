...
import { validateBlueprint, parseBlueprint, serializeBlueprint, BlueprintError, isInsideTrigger, doorSpawnPosition, cellToWorld, } from '@/levels/blueprint';
...
describe('validateBlueprint', () => {
    ...
    it('throws on unknown tile type', () => {
        expect(() => validateBlueprint({ ...VALID_BP, tiles: [{ x: 0, z: 0, type: 'door' }] })).toThrow(BlueprintError);
    });
    ...
    it('throws on unknown spawn type', () => {
        expect(() => validateBlueprint({ ...VALID_BP, spawns: [{ x: 1, z: 1, type: 'dragon' }] })).toThrow(BlueprintError);
    });
    ...
    it('includes descriptive info in error message', () => {
        try {
            ...
            expect(e.message).toContain('test_room');
...
function makeMockPhysics() {
    const bodies = [];
    return {
        createStaticBox: vi.fn((_pos, _half) => {
...
describe('renderBlueprint', () => {
    it('returns a group with at least one mesh', () => {
        ...
        const room = renderBlueprint(VALID_BP, physics);
    ...
    it('places a wall mesh at the correct world-space X/Z', () => {
        ...
        const physics = makeMockPhysics();
        const room = renderBlueprint(VALID_BP, physics);
        const wallMeshes = room.group.children.filter((c) => c instanceof THREE.Mesh && c.geometry instanceof THREE.BoxGeometry);
    ...
    it('creates a static physics body for each wall tile', () => {
        const physics = makeMockPhysics();
        renderBlueprint(VALID_BP, physics);
...
describe('doorSpawnPosition', () => {
    ...
    it('insets north-facing door southward (increasing z)', () => {
        const northDoor = { x: 1, z: 0, facing: 'north', targetId: null };
