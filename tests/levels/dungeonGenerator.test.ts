...
function planSignature(plan: DungeonPlan): string {
...
function assertSymmetric(plan: DungeonPlan): void {
  for (const [id, bp] of plan.rooms) {
    for (const door of bp.doors) {
      if (door.targetId === null) continue; // exterior exit — fine
      const target = plan.rooms.get(door.targetId);
      expect(target, `target room "${door.targetId}" (from "${id}") must exist`).toBeDefined();
      const backDoor = (target as Blueprint).doors.find((d) => d.targetId === id);
