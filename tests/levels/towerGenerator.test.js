...
import { TOWER_FLOOR_DEFS, FLOORS_ORDERED, PLAYER_START_FLOOR_INDEX, getFloorDef, } from '@/levels/TowerFloorDef';
...
describe('generateTower', () => {
        ...
        for (const [id, bp] of plan.rooms) {
            ...
            for (const door of bp.doors) {
                if (door.targetId === null)
                    continue; // exterior exit, no room to resolve
    ...
    it('living quarters has no enemy spawns (safe floor)', () => {
        ...
        for (const [id, bp] of plan.rooms) {
            if (!id.startsWith('tower_floor_quarters_room_'))
                continue;
    ...
    it('each chamber has its key fixture at the centre cell', () => {
        for (const def of TOWER_FLOOR_DEFS) {
            ...
            expect(bp).toBeDefined();
            const centre = bp.interactables.find((i) => i.type === def.keyFixture.type && i.x === 8 && i.z === 8);
