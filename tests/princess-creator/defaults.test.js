...
describe('PrincessDefaults — base templates', () => {
    ...
    it.each(DEFAULT_PRINCESSES.map(d => [d.name, d]))('%s: sanitizeDna returns a valid DNA object', (_, dna) => {
        const clean = sanitizeDna(dna);
        expect(clean).toBeTruthy();
        expect(clean.v).toBe(2);
        expect(typeof clean.species).toBe('string');
    });
    it.each(DEFAULT_PRINCESSES.map(d => [d.name, d]))('%s: buildPrincess returns an instance with root and update', (_, dna) => {
        expect(() => {
    ...
    it.each(DEFAULT_PRINCESSES.map(d => [d.name, d]))('%s: targetHeight 1.6 produces height in range [1.3, 2.0] WU', (_, dna) => {
        const inst = buildPrincess(dna, { targetHeight: 1.6 });
