...
import { getDefaultEnemyDna, getEnemiesForFloor, BOSS_DEFAULTS, tierForFloor, } from '@/enemy-creator/defaults/EnemyDefaults';
import { scaledHp, scaledDmg } from '@/enemy-creator/types';
import { encodeShareCode, decodeShareCode } from '@/procedural/ProceduralDNA';
const ALL_ROLES = ['melee', 'ranged', 'caster', 'support', 'tank', 'swarm'];
const ALL_SPECIES = ['human', 'undead', 'vulperia', 'slime', 'elf', 'celestial', 'draconic'];
...
describe('getEnemiesForFloor', () => {
    ...
    it('all have valid combat roles', () => {
        ...
        for (const e of enemies)
            expect(ALL_ROLES).toContain(e.combatRole);
    });
});
