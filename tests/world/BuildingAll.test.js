...
import { STYLE_COLORS, FACTION_PRESETS, factionBuildingDna, } from '@/world/buildings/BuildingDNA';
...
const KEY_KINDS_FOR_FACTIONS = [
    'house', 'cottage', 'watchtower', 'chapel', 'blacksmith',
];
function makeDna(kind, overrides = {}) {
    return {
...
const OVERLAY_STYLES = ['vampiric', 'elven', 'dwarven', 'gothic', 'nordic', 'fae', 'orcish', 'nomadic'];
const OVERLAY_KINDS = ['house', 'villa', 'watchtower', 'cottage', 'tavern'];
...
describe('Key faction showcase combos (from BUILDINGS.md)', () => {
    const combos = [
        ...
        ['tower', 'arcane', 'mage tower — uses factionBuildingDna fallback'],
    ];
    for (const [kind, faction, label] of combos) {
        it(`${label}: builds without throwing`, () => {
            const dna = typeof faction === 'string' && faction in FACTION_PRESETS
                ? factionBuildingDna(kind, faction, 99999)
                : makeDna(kind, { style: faction, colors: STYLE_COLORS[faction] ?? STYLE_COLORS['stone'] });
            expect(() => buildBuilding(dna)).not.toThrow();
...
describe('Condition variants on house', () => {
    for (const condition of ['pristine', 'weathered', 'damaged', 'ruined']) {
...
describe('Terrace modes', () => {
    for (const terrace of ['none', 'left', 'right', 'both']) {
