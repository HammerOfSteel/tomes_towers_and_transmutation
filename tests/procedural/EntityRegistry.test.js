...
import { resolveBuilder, registeredKinds, registerBuilder, } from '@/procedural/EntityRegistry';
...
describe('EntityRegistry', () => {
    ...
    it('resolveBuilder returns a function for every kind', async () => {
        for (const kind of registeredKinds()) {
            const fn = await resolveBuilder(kind);
    ...
    it('registerBuilder overwrites an existing entry', async () => {
        ...
        const dna = { v: 1, kind: 'npc', name: 'test', seed: 1 };
        fn(dna);
    ...
    it('resolveBuilder throws for unknown kind', async () => {
        await expect(resolveBuilder('unknown')).rejects.toThrow('[EntityRegistry]');
    });
});
