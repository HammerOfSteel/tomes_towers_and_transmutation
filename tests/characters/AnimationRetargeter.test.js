...
import { normaliseTrackNames, prepareKayKitClips, countMatchingTracks, } from '@/characters/AnimationRetargeter';
// ── helpers ───────────────────────────────────────────────────────────────────
function makeClip(name, tracks) {
    const threeTrack = tracks.map(({ trackName }) => new THREE.QuaternionKeyframeTrack(trackName, [0, 1], [0, 0, 0, 1, 0, 0, 0, 1]));
    return new THREE.AnimationClip(name, -1, threeTrack);
}
function makeBoneScene(boneNames) {
