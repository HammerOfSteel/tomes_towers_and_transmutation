...
function makeClip(name, duration = 1) {
    const track = new THREE.QuaternionKeyframeTrack('Root.quaternion', [0, duration], [0, 0, 0, 1, 0, 0, 0, 1]);
    return new THREE.AnimationClip(name, duration, [track]);
}
function makeLoadedChar(clipNames) {
...
describe('CharacterController — setState (looping)', () => {
    let ctrl;
    beforeEach(() => {
        ctrl = new CharacterController(makeLoadedChar(['Idle', 'Walk', 'Run', 'Jump_Idle', 'Levitate']));
    });
...
describe('CharacterController — getClip', () => {
    it('resolves KayKit clip names to states', () => {
        const ctrl = new CharacterController(makeLoadedChar(['Idle_A', 'Walking_A', 'Running_A', 'Attack', 'Hit_A', 'Death_A',
