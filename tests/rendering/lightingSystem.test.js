...
function makeBlueprint(id = 'test_room', width = 10, depth = 10, wallHeight = 4) {
    return {
        id, version: 1,
        width, depth,
        cellSize: 2,
...
describe('LightingSystem', () => {
    ...
    it('pulse light has the correct color', () => {
        ...
        const pulse = scene.children.find(c => c instanceof THREE.PointLight && c.color.getHex() === color);
    ...
    it('applyPreset greenhouse has different fog than dungeon', () => {
        ...
        const dungeonFar = scene.fog.far;
        ls.applyPreset('greenhouse');
        const greenFar = scene.fog.far;
    ...
    it('update modulates torch intensity within [0.1, 2.0]', () => {
        ...
        ls.update(0.1);
        ls.update(0.2);
        const torches = scene.children.filter(c => c instanceof THREE.PointLight);
