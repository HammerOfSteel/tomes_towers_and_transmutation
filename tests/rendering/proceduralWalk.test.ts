 ...
 */
function makeQuadRig(): CreatureRig {
...
function makeBipedRig(): CreatureRig {
  const root  = new THREE.Group();
  const torso = new THREE.Group();
  root.add(torso);
  return {
...
describe('ProceduralWalkController — stepping', () => {
  let rig:  CreatureRig;
  let ctrl: ProceduralWalkController;
  ...
  it('feet remain planted when body is stationary', () => {
    ...
    for (let i = 0; i < 10; i++) ctrl.update(0.016, rig.root.position, 0);
    const after = ctrl.getFootPositions();
    for (const key of ['frontLegL', 'frontLegR', 'backLegL', 'backLegR'] as const) {
  ...
  it('feet move to new positions after large body displacement', () => {
    ...
    for (let i = 0; i < 60; i++) ctrl.update(0.016, rig.root.position, 0);
    const after = ctrl.getFootPositions();
    // At least some feet should have moved
    let moved = 0;
    for (const key of ['frontLegL', 'frontLegR', 'backLegL', 'backLegR'] as const) {
      const dz = after[key].z - before[key].z;
      if (Math.abs(dz) > 0.3) moved++;
    }
  ...
  it('hip quaternions are updated (non-identity) after movement', () => {
    ...
    for (let i = 0; i < 30; i++) ctrl.update(0.016, rig.root.position, 0);
...
describe('ProceduralWalkController — diagonal stagger', () => {
  it('diagonal leg partners do not step simultaneously', () => {
    ...
    const DIAG_PAIRS: ['frontLegL', 'backLegR'][] = [['frontLegL', 'backLegR']];
    const DIAG_PAIRS2: ['frontLegR', 'backLegL'][] = [['frontLegR', 'backLegL']];
    ...
    for (let frame = 0; frame < 50; frame++) {
      ...
      for (const [a, b] of [...DIAG_PAIRS, ...DIAG_PAIRS2]) {
        ...
        if (feet[a].y > 0.38 && feet[b].y > 0.38) violations++;
      }
    }
...
describe('ProceduralWalkController — body height', () => {
  it('torso Y stays at natural height (no dynamic adjustment)', () => {
    ...
    for (let i = 0; i < 120; i++) ctrl.update(0.016, rig.root.position, 0);
