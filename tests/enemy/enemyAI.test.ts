...
import type { AggroListener } from '@/enemy/AggroSystem';
...
describe('PatrolBehavior — state transitions', () => {
  ...
  it('alert → chase after alertDuration expires', () => {
    ...
    for (let i = 0; i < 30; i++) fsm.tickAt(5, 0.016);
  ...
  it('shouldAttack fires on cooldown, not every frame', () => {
    ...
    let fired = false;
    for (let i = 0; i < 80; i++) {
      if (fsm.tickAt(0.5, 0.016).shouldAttack) { fired = true; break; }
    }
...
describe('StationaryShootBehavior — state transitions', () => {
  ...
  it('alert → aim after alert timer', () => {
    ...
    for (let i = 0; i < 40; i++) fsm.tick(8, 0.016);
  ...
  it('aim → shoot when in shootRange with timer expired', () => {
    ...
    let shot = false;
    for (let i = 0; i < 60; i++) {
      if (fsm.tick(7, 0.016).shouldShoot) { shot = true; break; }
    }
...
describe('AggroSystem — shout broadcast', () => {
  ...
  function makeListener(id: string, x: number, z: number): AggroListener {
    return {
      get worldPosition() { return new THREE.Vector3(x, 0, z); },
      onAggroShout(_shouter: AggroListener) { shouted.push(id); },
    };
  }

  beforeEach(() => {
    // Reset to fresh singleton state between tests
    (AggroSystem as unknown as { _instance: AggroSystem | null })._instance = null;
