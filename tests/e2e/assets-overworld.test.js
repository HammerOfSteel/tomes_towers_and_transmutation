...
async function waitForAssetTrees(page, timeoutMs = 30_000) {
    try {
        await page.waitForFunction(() => !!window.__game?.hasAssetTrees?.(), { timeout: timeoutMs });
        return true;
    }
    catch {
...
test.describe('Asset overworld — GLB tree upgrade', () => {
    ...
    test('captures before and after screenshots of tree upgrade', async ({ page }) => {
        ...
        await page.evaluate(() => window.__game.teleportPlayer(30, 2, 0));
    ...
    test('hasAssetTrees() returns true after upgrade', async ({ page }) => {
        ...
        const upgraded = await waitForAssetTrees(page, 30_000);
        if (upgraded) {
            const flag = await page.evaluate(() => window.__game.hasAssetTrees());
    ...
    test('player is still visible and positioned after asset upgrade', async ({ page }) => {
        ...
        const visible = await page.evaluate(() => window.__game.isPlayerVisible());
 ...
 */
async function waitForPhase1(page, timeoutMs = 45_000) {
    const wait = (fn) => page
        .waitForFunction((f) => !!window.__game?.[f]?.(), fn, { timeout: timeoutMs })
        .then(() => true)
        .catch(() => false);
...
test.describe('Phase 1 — Terrain decoration', () => {
    ...
    test('rock GLBs load and replace procedural dodecahedra', async ({ page }) => {
        ...
        await page.evaluate(() => window.__game.teleportPlayer(42, 2, 0));
        await page.waitForTimeout(300);
        ...
        if (rocks) {
            const flag = await page.evaluate(() => window.__game.hasAssetRocks());
            expect(flag).toBe(true);
        }
    ...
    test('ground clutter (grass, flowers, mushrooms) appears in forest', async ({ page }) => {
        ...
        await page.evaluate(() => window.__game.teleportPlayer(28, 2, 15));
        await page.waitForTimeout(300);
    ...
    test('castle-kit tower modules replace procedural octagonal tower', async ({ page }) => {
        ...
        await page.evaluate(() => window.__game.teleportPlayer(12, 2, 12));
        await page.waitForTimeout(300);
...
async function waitForSettlement(page, ms) {
    await page.waitForFunction(() => window.__game?.hasAssetSettlement?.() === true, { timeout: ms });
}
test.describe('Phase 2 — Settlement decoration', () => {
    ...
    test('settlement fountain and lanterns appear at town centre', async ({ page }) => {
        ...
        await page.evaluate(() => window.__game?.teleportPlayer(60, 0, 60));
        await page.waitForTimeout(400);
        ...
        const loaded = await page.evaluate(() => window.__game?.hasAssetSettlement?.());
    ...
    test('market stalls and carts appear near road tiles', async ({ page }) => {
        ...
        await waitForSettlement(page, 50_000);
        await page.evaluate(() => window.__game?.teleportPlayer(55, 0, 55));
        await page.waitForTimeout(800);
...
async function waitForDungeon(page, ms) {
    await page.waitForFunction(() => window.__game?.hasAssetDungeon?.() === true, { timeout: ms });
}
test.describe('Phase 3 — Dungeon entrance upgrade', () => {
    ...
    test('dungeon entrance GLBs replace procedural meshes', async ({ page }) => {
        ...
        const loaded = await page.evaluate(() => window.__game?.hasAssetDungeon?.());
    ...
    test('dungeon entrance trigger radius is preserved after upgrade', async ({ page }) => {
        ...
        await page.evaluate(() => window.__game?.teleportPlayer(48, 0, 48));
        await page.waitForTimeout(600);
