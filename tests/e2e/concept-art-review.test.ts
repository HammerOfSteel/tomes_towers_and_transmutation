...
async function openLab(page: Page): Promise<void> {
  ...
  await page.waitForFunction(() => !!(window as any).__lab, { timeout: 20_000 });
...
test.describe('Capture creature reference shots', () => {
  test('biped — front, iso, side (no outfit + full outfit)', async ({ page }) => {
    await openLab(page);
    // Default biped
    await page.evaluate(() => (window as any).__lab.showCreature('biped'));
    await page.evaluate(() => (window as any).__lab.setAnimState('idle'));
    await page.evaluate(() => (window as any).__lab.freezeAt(0));
    for (const [deg, label] of [[0,'front'],[45,'iso'],[90,'side']] as [number,string][]) {
      await page.evaluate((d: number) => (window as any).__lab.setAngle(d), deg);
    ...
    await page.evaluate(() => {
      ...
      (window as any).__lab.showFromJson(d, 'biped+skirt+tunic');
    });
    await page.evaluate(() => (window as any).__lab.freezeAt(0));
    for (const [deg, label] of [[0,'front'],[45,'iso'],[90,'side']] as [number,string][]) {
      await page.evaluate((d: number) => (window as any).__lab.setAngle(d), deg);
    ...
    await page.evaluate(() => {
      ...
      (window as any).__lab.showFromJson(d, 'biped+robe+hat');
    });
    await page.evaluate(() => (window as any).__lab.freezeAt(0));
    for (const [deg, label] of [[0,'front'],[45,'iso'],[90,'side']] as [number,string][]) {
      await page.evaluate((d: number) => (window as any).__lab.setAngle(d), deg);
    ...
    // Walk cycle
    await page.evaluate(() => (window as any).__lab.showCreature('biped'));
    await page.evaluate(() => (window as any).__lab.setAnimState('walk'));
    await page.evaluate(() => (window as any).__lab.setAngle(45));
    for (let f = 0; f < 6; f++) {
      await page.evaluate((t: number) => (window as any).__lab.freezeAt(t), f * 0.33);
      await page.waitForTimeout(80);
      await snap(page, `biped-walk-f${f}`);
    }
    await page.evaluate(() => (window as any).__lab.thawTime());
    expect(true).toBe(true);
  });

  test('all archetypes — front + iso + walk strip', async ({ page }) => {
    await openLab(page);
    for (const arch of ['biped','quadruped','avian','serpent','amoeba'] as const) {
      await page.evaluate((a: string) => (window as any).__lab.showCreature(a), arch);
      await page.evaluate(() => (window as any).__lab.setAnimState('idle'));
      await page.evaluate(() => (window as any).__lab.freezeAt(0.5));
      for (const [deg, label] of [[0,'front'],[45,'iso']] as [number,string][]) {
        await page.evaluate((d: number) => (window as any).__lab.setAngle(d), deg);
        await page.waitForTimeout(80);
      ...
      await page.evaluate(() => (window as any).__lab.setAngle(45));
      for (let f = 0; f < 4; f++) {
        await page.evaluate((t: number) => (window as any).__lab.freezeAt(t), f * 0.5);
        await page.waitForTimeout(80);
...
test.afterAll(async () => {
  ...
  const conceptImages: { name: string; src: string; file: string }[] = [];

  const addImg = (filePath: string, label: string) => {
    const src = imgToBase64(filePath);
    if (src) conceptImages.push({ name: label, src, file: path.basename(filePath) });
  };
  ...
  function shotCard(filename: string, title: string): string {
    const filepath = path.join(SHOTS_DIR, filename);
    const src = imgToBase64(filepath);
    if (!src) return `<div class="card missing">${title}<br/>(missing)</div>`;
