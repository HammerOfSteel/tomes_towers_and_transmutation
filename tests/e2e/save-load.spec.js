...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/save-load-${name}.png` });
// ── Tests ─────────────────────────────────────────────────────────────────────
test.describe('Save & Load', () => {
    ...
    test('game start writes autoSave to slot 0 on floor transition', async ({ page }) => {
        ...
        await page.evaluate(() => window.__game.onTeleportRoom?.('tower_floor_1_chamber'));
    ...
    test('reload page restores __game object', async ({ page }) => {
        ...
        const hasGame = await page.evaluate(() => typeof window.__game === 'object');
    ...
    test('floor state persists in localStorage after multiple transitions', async ({ page }) => {
        ...
        const saveCount = await page.evaluate(() => [...Array(localStorage.length)].filter((_, i) => (localStorage.key(i) ?? '').startsWith('ttt_save')).length);
