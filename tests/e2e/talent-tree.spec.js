...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/talent-${name}.png` });
...
test.describe('Talent Tree', () => {
    ...
    test('talent tree UI elements exist in DOM after game start', async ({ page }) => {
        ...
        const hasTalentEl = await page.evaluate(() => !!document.querySelector('[id*="talent"], [class*="talent"], [class*="tt-"]'));
    ...
    test('HUD ability bar shows Q/R/Z/X slots', async ({ page }) => {
        ...
        const hasAbilityBar = await page.evaluate(() => !!document.querySelector('[id*="ab-slot"], [class*="ab-slot"], [id*="hud-ab"]'));
    ...
    test('mana bar is visible during gameplay', async ({ page }) => {
        ...
        const hasMana = await page.evaluate(() => !!document.querySelector('[id*="mana"], [class*="mana"]'));
