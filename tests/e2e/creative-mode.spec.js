...
async function startGame(page) {
    ...
    await page.waitForFunction(() => !!window.__dev?.gameReady || document.querySelector('#hud-root'), { timeout: 15000 });
}
...
test.describe('Creative Mode', () => {
    ...
    test('Clicking an asset puts it in the hotbar and closes browser', async ({ page }) => {
        ...
        const state = await page.evaluate(() => window.__dev?.creativeModeState?.());
        if (state)
            expect(state.activeTool).toBe('place');
