...
import type { ModelStats }         from '../../src/model-review';
...
interface PackSpec {
  ...
  minClips:    number;   // 0 = static pack (skip animation test)
  hasTextures: boolean;  // false = solid-colour pack
}

const PACKS_TO_TEST: PackSpec[] = [
...
async function waitReady(page: Page, ms = 20_000) {
  await page.waitForFunction(() => (window as any).__modelReview?.ready === true, { timeout: ms });
}
async function waitLoaded(page: Page, ms = 30_000) {
  await page.waitForFunction(() => (window as any).__modelReview?.loaded === true, { timeout: ms });
}
async function getStats(page: Page): Promise<ModelStats> {
  return page.evaluate(() => (window as any).__modelReview.stats as ModelStats);
}
async function playAnim(page: Page, clipName: string) {
  await page.evaluate((c) => (window as any).__modelReview.playAnim(c), clipName);
}
async function setAngle(page: Page, angle: string) {
  await page.evaluate((a) => (window as any).__modelReview.setAngle(a), angle);
}
async function loadModelByPath(page: Page, path: string, animRig?: string): Promise<ModelStats> {
  return page.evaluate(
    ([p, r]) => (window as any).__modelReview.loadModel(p, r),
    [path, animRig ?? null] as [string, string | null],
  );
}
/** Disable the animated-only filter so all models are visible. */
async function showAll(page: Page) {
  await page.evaluate(() => { (window as any).__modelReview.animatedOnly = false; });
}

// ── Capture structured log lines from model-review ────────────────────────────
...
function parseModelLog(msg: string): ModelLog | null {
  if (!msg.startsWith('[model-review]')) return null;
  const id       = msg.match(/\] ([^\s|]+)/)?.[1] ?? '';
  const status   = (msg.match(/status:(\w+)/)?.[1] ?? 'ok') as ModelLog['status'];
  ...
  } catch { return { modelId: id, status, clips, textures, warnings: [], errors: [], raw: msg }; }
}
...
for (const pack of PACKS_TO_TEST) {
  test.describe(`[${pack.role.toUpperCase()}] ${pack.label} (${pack.packId})`, () => {
    ...
    test(`all ${pack.packId} models: visible, scale, no broken alpha`, async ({ page }) => {
      ...
      if (pack.minClips === 0) await showAll(page);

      const modelList: Array<{ id: string; name: string; path: string }> = await page.evaluate(
        () => (window as any).__modelReview.modelList,
      );
      ...
      consoleErrors = []; consoleWarnings = []; modelLogs = []; pageErrors = [];

      const issues: string[] = [];

      for (const model of packModels) {
        await page.evaluate((path) => (window as any).__modelReview.loadModel(path), model.path);
    ...
    test(`all ${pack.packId} models: animations play`, async ({ page }) => {
      ...
      const packModels = modelList.filter(m => m.id.startsWith(pack.packId + '/'));
      consoleErrors = []; modelLogs = []; pageErrors = [];
      ...
      for (const model of packModels) {
        ...
        const stats = await getStats(page);
        expect(
          stats.clipNames.length,
          `${model.id}: expected ≥ ${pack.minClips} clips`,
        ).toBeGreaterThanOrEqual(pack.minClips);
...
test.describe('Model deep inspection — 4-angle coverage', () => {
  ...
  for (const m of DEEP) {
    test(`${m.name}: renders + all angles captured`, async ({ page }) => {
      ...
      for (const angle of ['front', 'side', 'iso', 'top'] as const) {
...
test('Scale: all models normalise to 1.5–2.5 WU height', async ({ page }) => {
  ...
  const results: Array<{ name: string; rawH: number; displayH: number }> = [];
