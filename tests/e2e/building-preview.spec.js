...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/bldg-preview-${name}.png` });
 ...
 */
async function generatePlanJson(page, kind = 'inn', faction = 'human_town', seed = 42) {
    await loadPage(page);
    const json = await page.evaluate(([k, f, s]) => window.__game?.generateBuildingPreviewJson(k, f, s) ?? '', [kind, faction, seed]);
...
test.describe('Building Preview (ttt_building_preview localStorage flow)', () => {
    test('smoke: __game.generateBuildingPreviewJson() returns valid plan', async ({ page }) => {
        ...
        await loadPage(page);
        const json = await page.evaluate(() => window.__game?.generateBuildingPreviewJson('inn', 'human_town', 42));
    ...
    test('building plan is JSON-round-trip safe (no Maps or Sets)', async ({ page }) => {
        await loadPage(page);
        const json = await page.evaluate(() => window.__game?.generateBuildingPreviewJson('shop', 'dwarven', 99));
        const parsed = JSON.parse(json);
        for (const [id, bp] of Object.entries(parsed.rooms)) {
    ...
    test('localStorage key triggers auto-start: building loads, not tower', async ({ page }) => {
        ...
        const expectedRoomId = parsed.startRoomId;
        ...
        await page.waitForFunction(() => !!window.__buildingPreviewComplete || !!window.__buildingPreviewError, { timeout: 20_000 });
        await SS(page, '01-after-preview');
        // Step 6 — assert outcomes
        const previewError = await page.evaluate(() => window.__buildingPreviewError);
        expect(previewError, `previewBuilding threw an error: ${previewError}\n\nFull console:\n${console_.all.join('\n')}`).toBeUndefined();
        const currentRoomId = await page.evaluate(() => window.__game?.getCurrentRoomId());
        expect(currentRoomId, `Expected building room "${expectedRoomId}" but got "${currentRoomId}"\n\nFull console:\n${console_.all.join('\n')}`).toBe(expectedRoomId);
        // Confirm key debug log messages are present
        expect(console_.has('[buildingPreview] key found in localStorage'), `Missing "[buildingPreview] key found" log — auto-start may not have run.\nConsole:\n${console_.all.join('\n')}`).toBe(true);
        expect(console_.has('[previewBuilding] ✓ complete'), `Missing "[previewBuilding] ✓ complete" log.\nConsole:\n${console_.all.join('\n')}`).toBe(true);
        // No console errors
        expect(console_.errors, `Console errors during preview:\n${console_.all.join('\n')}`).toHaveLength(0);
    });
    test('player is in creative mode after preview loads', async ({ page }) => {
        ...
        await page.reload({ waitUntil: 'domcontentloaded' });
        await page.waitForFunction(() => !!window.__buildingPreviewComplete, { timeout: 30_000 });
        const isCreative = await page.evaluate(() => {
            const CreativeMode = window.__CreativeMode;
            // Fallback: check the game exposes it or check a known side-effect
            return window.__game?.isCreativeMode?.() ?? null;
        });
        // At minimum: no error during the preview
        const previewError = await page.evaluate(() => window.__buildingPreviewError);
        expect(previewError, `Preview error: ${previewError}\nConsole:\n${console_.all.join('\n')}`).toBeUndefined();
    ...
    test('missing localStorage key shows main menu, not auto-start', async ({ page }) => {
        ...
        expect(console_.has('[buildingPreview] key found'), 'Should NOT auto-start when key is absent').toBe(false);
        // __buildingPreviewComplete should not be set
        const complete = await page.evaluate(() => window.__buildingPreviewComplete);
    ...
    test('invalid plan JSON logs error, does not crash game', async ({ page }) => {
        ...
        await page.goto('/');
        await page.waitForFunction(() => !!window.__buildingPreviewComplete || !!window.__buildingPreviewError, { timeout: 30_000 });
        const previewError = await page.evaluate(() => window.__buildingPreviewError);
...
test.describe('previewBuilding() smoke (API call without reload)', () => {
    test('calling previewBuilding directly loads building room', async ({ page }) => {
        ...
        await page.evaluate((s) => window.__game?.startGame(s), 0xDEAD_BEEF);
        await page.waitForTimeout(800);
        const roomBefore = await page.evaluate(() => window.__game?.getCurrentRoomId());
        expect(roomBefore, 'should have a room after startGame').toBeTruthy();
        // Build a plan and call previewBuilding directly
        const planJson = await page.evaluate(() => window.__game?.generateBuildingPreviewJson('inn', 'human_town', 42));
        const expectedId = JSON.parse(planJson).startRoomId;
        await page.evaluate((json) => window.__game?.previewBuilding(json), planJson);
        await page.waitForFunction(() => !!window.__buildingPreviewComplete || !!window.__buildingPreviewError, { timeout: 10_000 });
        await SS(page, '05-direct-api-call');
        const previewError = await page.evaluate(() => window.__buildingPreviewError);
        expect(previewError, `previewBuilding error: ${previewError}\nConsole:\n${console_.all.join('\n')}`).toBeUndefined();
        const roomAfter = await page.evaluate(() => window.__game?.getCurrentRoomId());
        expect(roomAfter, `Room should be "${expectedId}" but got "${roomAfter}"\nConsole:\n${console_.all.join('\n')}`).toBe(expectedId);
        expect(console_.errors, `Console errors:\n${console_.all.join('\n')}`).toHaveLength(0);
    });
});
...
test.describe('Building room quality (no stray props)', () => {
    test('no [PropPlacer] logs for building rooms — isBuildingRoom guard works', async ({ page }) => {
        ...
        expect(propLogs, `PropPlacer should be skipped for building rooms but ran after previewBuilding:\n${propLogs.join('\n')}`).toHaveLength(0);
        // The skip log SHOULD be present
        const skipLog = afterLogs.find(l => l.includes('skipped KayKit props for building room'));
        expect(skipLog, `Expected "[onRoomLoaded] skipped KayKit props" log after previewBuilding.\nAfter-logs:\n${afterLogs.join('\n')}`).toBeTruthy();
        // No preview error
        const previewError = await page.evaluate(() => window.__buildingPreviewError);
