...
async function loadShowroom(page) {
    ...
    await page.waitForFunction(() => !!window.showroom, { timeout: 15_000 });
    ...
    await page.waitForFunction(() => {
        ...
        const s = window.__showroomScene;
...
test('building walkthrough — initial overview', async ({ page }) => {
    ...
    const hasPixels = await page.evaluate(() => {
        ...
        if (!ctx)
            return true; // WebGL canvas — assume OK
...
test('building walkthrough — NPC count in scene', async ({ page }) => {
    ...
    const npcCount = await page.evaluate(() => {
        ...
        let n = 0;
        s.traverse((o) => {
            if (o.userData?.npcName || o.userData?.species)
                n++;
        });
        return n;
    });
