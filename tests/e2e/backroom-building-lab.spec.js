...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/backroom-${name}.png`, fullPage: false });
...
const KINDS = ['cottage', 'inn', 'blacksmith', 'chapel', 'villa', 'apothecary'];
function colCentre(idx) {
    return { x: COLS[idx], z: INT_Z[KINDS[idx]] ?? -4 };
}
// ── Boot helper ───────────────────────────────────────────────────────────────
async function bootAndEnterLab(page) {
    await loadPage(page);
    // Start as foxling princess (deterministic)
    await page.evaluate(() => window.__game.quickPlayPrincess({ name: 'Foxley', species: 'foxling' }));
    await page.waitForFunction(() => window.__gameStarted === true, { timeout: 60_000 });
    // Enter the building lab backroom directly
    await page.evaluate(() => window.__game.enterBackroom('building_lab'));
    await page.waitForFunction(() => window.__backroomReady === true, { timeout: 20_000 });
...
test('building lab — overview: all 6 interiors visible', async ({ page }) => {
    ...
    const childCount = await page.evaluate(() => window.__showroomScene?.children.length ?? window.__game?.scene?.children.length ?? -1);
