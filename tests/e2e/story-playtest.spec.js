...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/story-${name}.png`, fullPage: false });
 ...
 */
async function walk(page, keys, durationMs, label, fallThreshold = -3) {
    ...
    while (Date.now() - start < durationMs) {
        ...
        if (pos.y < fallThreshold) {
            ...
            throw new Error(`FALL-THROUGH during "${label}" at (${pos.x.toFixed(1)}, ${pos.y.toFixed(1)}, ${pos.z.toFixed(1)}) — Y below ${fallThreshold}`);
        }
    }
    for (const key of keys)
        await page.keyboard.up(key).catch(() => { });
...
test('story playtest: vulperia → campfire → tower → walk exterior (real input)', async ({ page }) => {
    ...
    await page.evaluate(() => window.__game.quickPlayPrincess({ name: 'Maribel', species: 'foxling' }));
    await page.waitForFunction(() => window.__gameStarted === true, { timeout: 60_000 });
    await page.waitForTimeout(1500);
    ...
    expect(await page.evaluate(() => window.__game?.getCurrentFloor?.())).toBe(0);
    ...
    await page.evaluate(() => window.__game?.triggerExit?.());
    ...
    await page.evaluate(() => window.__game?.switchToInterior?.());
    await page.waitForTimeout(2000);
    await SS(page, '14-back-in-dungeon');
    const modeBack = await page.evaluate(() => window.__game?.getGameMode?.());
