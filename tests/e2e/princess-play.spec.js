...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/princess-${name}.png`, fullPage: false });
...
async function waitChoices(page, count) {
    await page.waitForFunction((n) => { const c = window.__campfire; return c && !c.speaking && c.choiceCount >= n; }, count, { timeout: 30_000 });
    await page.waitForTimeout(200);
}
async function waitForGameStart(page, logs) {
    await page.waitForFunction(() => !window.__campfire && !document.getElementById('plp-root'), { timeout: 60_000 });
    const started = await page.waitForFunction(() => window.__gameStarted === true, { timeout: 60_000 }).catch(() => null);
    if (!started) {
...
test('doc path 1: documentation → pick princess → game starts', async ({ page }) => {
    ...
    await loadPage(page);
    await page.evaluate(() => window.__game.triggerNewGame(99));
    await page.waitForFunction(() => !!window.__campfire, { timeout: 60_000 });
    await SS(page, '01-campfire');
    await waitChoices(page, 8);
    await SS(page, '02-eight-choices');
    // "I have documentation" = choice 7
    await page.evaluate(() => window.__campfire?.choose(7));
    ...
    const floor = await page.evaluate(() => window.__game?.getCurrentFloor?.());
    expect(floor).toBe(0);
...
test('doc path 2: quickPlayPrincess → game starts → floor 0', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        window.__game.quickPlayPrincess({ name: 'Isamilia', species: 'foxling' });
    });
    ...
    const floor = await page.evaluate(() => window.__game?.getCurrentFloor?.());
    expect(floor).toBe(0);
    const room = await page.evaluate(() => window.__game?.getCurrentRoom?.());
...
test('campfire human → game starts → floor 0', async ({ page }) => {
    ...
    await page.waitForFunction(() => !!window.__campfire, { timeout: 60_000 });
    await waitChoices(page, 8);
    await page.evaluate(() => window.__campfire?.choose(0)); // human
    await waitChoices(page, 4);
    await page.evaluate(() => window.__campfire?.choose(0)); // warrior
    await waitChoices(page, 4);
    await page.evaluate(() => window.__campfire?.choose(0)); // stat 1
    await waitChoices(page, 4);
    await page.evaluate(() => window.__campfire?.choose(0)); // stat 2
...
test('FULL: documentation → pick princess → game starts → go to basement', async ({ page }) => {
    ...
    const princessName = await page.evaluate(() => document.querySelector('.plp-card-name')?.textContent ?? 'unknown');
    ...
    const floor0 = await page.evaluate(() => window.__game?.getCurrentFloor?.());
    ...
    await page.evaluate(() => window.__game.goToFloor(-1));
    await page.waitForTimeout(3000); // let basement load + transition
    await SS(page, '47-basement-loading');
    const basement = await page.evaluate(() => window.__game?.getCurrentFloor?.());
