...
test.describe('Creature Lab — all archetypes overview', () => {
  test('all archetypes front + iso + side', async ({ page }) => {
    await openLab(page);
    await page.evaluate(() => (window as any).__lab.showAll());
    ...
    const info = await page.evaluate(() => (window as any).__lab.getRigInfo());
...
test.describe('Biped: top clothing', () => {
  test('all top options — front', async ({ page }) => {
    await openLab(page);
    await page.evaluate(() => (window as any).__lab.showTopOutfits());
    await page.waitForTimeout(300);
    await page.evaluate(() => (window as any).__lab.setAngle(0));
...
test.describe('Biped: over-clothing (capes, robes)', () => {
  test('all over options — iso view', async ({ page }) => {
    await openLab(page);
    await page.evaluate(() => (window as any).__lab.showOverOutfits());
    await page.waitForTimeout(300);
    await page.evaluate(() => (window as any).__lab.setAngle(45));
...
test.describe('Biped: body proportion morphs', () => {
  test('morph extremes — front', async ({ page }) => {
    await openLab(page);
    await page.evaluate(() => (window as any).__lab.showMorphs());
    await page.waitForTimeout(300);
    await page.evaluate(() => (window as any).__lab.setAngle(0));
...
test.describe('Animation states — all archetypes', () => {
  for (const state of ['idle', 'walk', 'run'] as const) {
    test(`all archetypes — ${state}`, async ({ page }) => {
      await openLab(page);
      await page.evaluate(() => (window as any).__lab.showAll());
      await page.evaluate((s: string) => (window as any).__lab.setAnimState(s), state);
      await page.evaluate(() => (window as any).__lab.setAngle(45));
