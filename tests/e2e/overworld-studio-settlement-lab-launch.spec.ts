...
test('Dev Rooms "Settlement Lab" button boots the game straight into the Settlement Lab', async ({ page, context }) => {
  const studioConsole = attachFullConsoleCapture(page);
  await openStudio(page);

  const [popup] = await Promise.all([
    context.waitForEvent('page'),
    page.click('#btn-devroom-settlement-lab'),
  ]);
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (state.mode === 'settlementlab') break;
