...
// ── Screenshot helper ─────────────────────────────────────────────────────────
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/startup-${name}.png` });
// ── Tests ─────────────────────────────────────────────────────────────────────
test.describe('Startup & Main Menu', () => {
    ...
    test('startGame API works and game enters playing state', async ({ page }) => {
        ...
        const mode = await page.evaluate(() => window.__game.getGameMode?.() ?? 'unknown');
    ...
    test('enterCreativeMode and exitCreativeMode APIs exist', async ({ page }) => {
        ...
        const hasCreative = await page.evaluate(() => typeof window.__game.enterCreativeMode === 'function' &&
            typeof window.__game.exitCreativeMode === 'function');
        expect(hasCreative).toBe(true);
    });
    test('setGameSpeed API exists (timeScale)', async ({ page }) => {
        await loadPage(page);
        const hasSpeed = await page.evaluate(() => typeof window.__game?.setGameSpeed === 'function');
