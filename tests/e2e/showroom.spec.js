...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/showroom-${name}.png`, fullPage: false });
async function loadShowroom(page) {
    await page.goto('/showroom.html');
    // Wait for the canvas + showroom API to be ready
    await page.waitForFunction(() => !!window.showroom && !!document.querySelector('canvas'));
...
test('showroom: building styles and kinds', async ({ page }) => {
    ...
    await page.evaluate(() => window.showroom.spawnAllBuildings());
...
test('showroom: enemy tier 1/2/3 comparison', async ({ page }) => {
    const log = logs(page);
    await loadShowroom(page);
    await page.evaluate(() => window.showroom.spawnEnemyTiers());
...
test('showroom: all 12 prop kinds', async ({ page }) => {
    ...
    for (const [kind, material] of props) {
        await page.evaluate(([k, m]) => window.showroom.spawnProp(k, m), [kind, material]);
...
test('showroom: full showcase — buildings + NPCs + enemies + props', async ({ page }) => {
    ...
    await page.evaluate(() => window.showroom.spawnEnemyTiers());
    // Props scatter
    for (const [k, m] of [['chest', 'wood'], ['lantern', 'iron'], ['cauldron', 'iron'], ['statue', 'stone']]) {
        await page.evaluate(([kind, mat]) => window.showroom.spawnProp(kind, mat), [k, m]);
    }
