...
// ── helpers ───────────────────────────────────────────────────────────────────
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/campfire-ng-${name}.png`, fullPage: false });
/** Launch the campfire for slot 99 (ephemeral — won't collide with real saves). */
async function launchCampfire(page) {
    const errors = [];
 ...
 */
async function waitForChoices(page, expectedCount, timeoutMs = 25_000) {
    await page.waitForFunction(({ count }) => {
        const c = window.__campfire;
...
test.describe('NewGameFlow Campfire', () => {
    ...
    test('campfire scene launches and wizard appears', async ({ page }) => {
        ...
        const hasCampfire = await page.evaluate(() => !!window.__campfire);
    ...
    test('all 8 species choices are available', async ({ page }) => {
        ...
        const choiceCount = await page.evaluate(() => window.__campfire?.choiceCount);
    ...
    for (const { idx, name, phase2Count } of SPECIES) {
        test(`${name} (choice ${idx}): wizard reacts and shows phase-2 choices`, async ({ page }) => {
            ...
            const count = await page.evaluate(() => window.__campfire?.choiceCount);
    ...
    test('princess choice (idx 7): wizard reacts and campfire ends', async ({ page }) => {
        ...
        await page.waitForFunction(() => {
            ...
            const campfireGone = !window.__campfire;
    ...
    test('full path: human warrior → game starts', async ({ page }) => {
        ...
        await page.waitForFunction(() => !window.__campfire, { timeout: 20_000 });
