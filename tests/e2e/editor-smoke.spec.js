...
async function openEditorTab(page) {
    ...
    await page.waitForFunction(() => !!window.__modelReview?.ready, { timeout: TIMEOUT });
    // Click the Editor tab
    await page.click('#tab-editor');
    await page.waitForFunction(() => !!window.__editorReview?.coreReady, { timeout: TIMEOUT });
}
...
test.describe('Editor Tab — foundation', () => {
    test('Editor tab is present and activates EditorCore', async ({ page }) => {
        ...
        const coreReady = await page.evaluate(() => window.__editorReview?.coreReady);
        expect(coreReady).toBe(true);
        await screenshot(page, 'L0-editor-tab');
    });
    test('Mode tabs are all present', async ({ page }) => {
        await openEditorTab(page);
        const tabs = await page.$$eval('.editor-sub', els => els.map(e => e.textContent?.trim()));
...
test.describe('Editor Tab — Environment + Scale calibration', () => {
    ...
    test('Scale slider updates displayed value', async ({ page }) => {
        ...
        await page.waitForTimeout(400);
        // Load first asset
        await page.evaluate(() => window.__envReview?.switchToEnv?.());
        await page.waitForTimeout(300);
        // Move slider to 3.0
