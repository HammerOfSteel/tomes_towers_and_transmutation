...
// ── Helpers ───────────────────────────────────────────────────────────────────
const shot = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/slime-im-${name}.png` });
...
async function spawnSlimes(page, count) {
    await page.evaluate((n) => window.__sandbox.slime.spawn(n), count);
    // Wait for the IM to be synced and at least one frame rendered
    await page.waitForTimeout(300);
    return page.evaluate(() => window.__sandbox.slime.getStats());
}
/**
...
async function canvasHasNonBlackPixels(page) {
    return page.evaluate(async () => {
        ...
        if (!sb?.canvasDataURL)
            return false;
        const dataURL = sb.canvasDataURL(); // renderer.render() + toDataURL
...
test.describe('Slime IM Lab — InstancedMesh rendering', () => {
    ...
    test('Spawn button in UI triggers InstancedMesh creation', async ({ page }) => {
        await loadSandbox(page);
        // Clear first
        await page.evaluate(() => window.__sandbox.slime.clear());
        await page.waitForTimeout(100);
        // Set count in input and click Spawn
        await page.fill('#slime-count', '15');
        await page.click('#btn-spawn-slimes');
        await page.waitForTimeout(300);
        const stats = await page.evaluate(() => window.__sandbox.slime.getStats());
    ...
    test('slimes render in three distinct tint groups (hostile/flee/recruit)', async ({ page }) => {
        ...
        await page.evaluate(() => window.__sandbox.slime.spawn(3));
        await page.waitForTimeout(300);
        // canvasDataURL forces a render — check the resulting image isn't monochrome
        const dataURL = await page.evaluate(() => window.__sandbox.canvasDataURL());
