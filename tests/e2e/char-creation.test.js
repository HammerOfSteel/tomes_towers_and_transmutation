...
// ── Helpers ───────────────────────────────────────────────────────────────────
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/char-creation-${name}.png` });
/** Open the character creation screen and wait for the overlay to be visible. */
async function openCharCreation(page, slotId = 0) {
    await page.evaluate((id) => window.__game.openCharCreation(id), slotId);
...
async function getState(page) {
    return page.evaluate(() => window.__game.getCharCreationState());
}
// ── Tests ─────────────────────────────────────────────────────────────────────
test.describe('CharacterCreationV2', () => {
    ...
    test('changing card selection changes the model', async ({ page }) => {
        ...
        if (count < 2)
            test.skip(); // need at least two models
        // Select first card
    ...
    test('begin button fires onComplete when model is selected', async ({ page }) => {
        ...
        await page.evaluate(() => { window.__ccv2_completed = false; });
