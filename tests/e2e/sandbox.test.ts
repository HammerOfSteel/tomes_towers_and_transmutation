...
const shot = (page: Page, name: string) =>
  page.screenshot({ path: `tests/e2e/screenshots/sandbox-${name}.png` });
...
async function loadSandbox(page: Page) {
  ...
  await page.waitForFunction(() => (window as any).__sandbox?.ready === true, { timeout: 45_000 });
...
async function clearAndSetCamera(page: Page, preset: 'iso' | 'top' | 'front' | 'side' = 'iso') {
  await page.evaluate(() => (window as any).__sandbox.clearAll());
  await page.evaluate((p) => (window as any).__sandbox.setCameraPreset(p), preset);
...
test.describe('Sandbox — interactive tile builder', () => {
  ...
  test('places a single wall tile and reports it in getTiles()', async ({ page }) => {
    ...
    await page.evaluate(() =>
      (window as any).__sandbox.placeTile('/assets/buildings/wall.glb', 0, 0, 0, 0),
    );
    await page.waitForTimeout(400);

    const tiles = await page.evaluate(() => (window as any).__sandbox.getTiles());
  ...
  test('places tiles on multiple floors (wall stack)', async ({ page }) => {
    ...
    for (let f = 0; f < 4; f++) {
      await page.evaluate(
        (floor) => (window as any).__sandbox.placeTile('/assets/buildings/wall.glb', 0, 0, floor),
        f,
      );
    }
  ...
  test('cottage building assembly', async ({ page }) => {
    ...
    await page.evaluate(() => (window as any).__sandbox.assembleBuilding('cottage', 1, 0, 0));
  ...
  test('inn building assembly (2 floors)', async ({ page }) => {
    ...
    await page.evaluate(() => (window as any).__sandbox.assembleBuilding('inn', 2, 0, 0));
    await page.waitForTimeout(700);
  ...
  test('six building types arranged in a row', async ({ page }) => {
    ...
    const types = ['cottage', 'inn', 'smithy', 'tavern', 'guard_tower', 'market_stall'] as const;

    for (let i = 0; i < types.length; i++) {
      await page.evaluate(
        ([type, col]) => (window as any).__sandbox.assembleBuilding(type, col + 1, col * 6 - 15, 0),
        [types[i], i] as [string, number],
      );
    }

    await page.evaluate(() =>
      (window as any).__sandbox.setCamera(0, 32, 48, 0, 0, 0),
    );
  ...
  test('castle tower 7-layer stack', async ({ page }) => {
    ...
    for (let i = 0; i < modules.length; i++) {
      await page.evaluate(
        ([path, floor]) => (window as any).__sandbox.placeTile(path, 0, 0, floor),
        [modules[i], i] as [string, number],
      );
    }

    // Tilt camera slightly for a better view of the tower
    await page.evaluate(() =>
      (window as any).__sandbox.setCamera(8, 14, 22, 0, 7, 0),
    );
  ...
  test('road intersection + surrounding paths', async ({ page }) => {
    ...
    for (let r = -1; r <= 1; r++) {
      for (let c = -1; c <= 1; c++) {
        await page.evaluate(
          ([col, row]) => (window as any).__sandbox.placeTile('/assets/town/road.glb', col, row, 0),
    ...
    for (let c = -3; c <= 3; c++) {
      await page.evaluate(
        ([col]) => (window as any).__sandbox.placeTile(
          '/assets/nature/ground_pathStraight.glb', col, 2, 0, 0,
        ),
        [c] as [number],
      );
      await page.evaluate(
  ...
  test('dungeon corridor layout', async ({ page }) => {
    ...
    for (const tile of layout) {
      await page.evaluate(
        ({ path, col, row }) => (window as any).__sandbox.placeTile(path, col, row, 0),
        tile,
      );
    }
    await page.waitForTimeout(600);
  ...
  test('clearAll removes all tiles from scene', async ({ page }) => {
    ...
    let tiles = await page.evaluate(() => (window as any).__sandbox.getTiles());
    expect(tiles.length).toBeGreaterThan(0);

    await page.evaluate(() => (window as any).__sandbox.clearAll());
    await page.waitForTimeout(100);

    tiles = await page.evaluate(() => (window as any).__sandbox.getTiles());
