...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/dungeon-${name}.png` });
...
test.describe('Dungeon Room', () => {
    test('game starts in interior mode (tower)', async ({ page }) => {
        ...
        const mode = await page.evaluate(() => window.__game.getGameMode?.());
    ...
    test('can teleport to different floors without errors', async ({ page }) => {
        ...
        for (const roomId of floors) {
            await page.evaluate((id) => window.__game.onTeleportRoom?.(id), roomId);
    ...
    test('getCurrentFloor changes after room transition', async ({ page }) => {
        ...
        await page.evaluate(() => window.__game.onTeleportRoom?.('tower_floor_2_chamber'));
        await page.waitForTimeout(800);
        const floor2 = await page.evaluate(() => window.__game.getCurrentFloor?.());
    ...
    test('HUD kill counter visible during gameplay', async ({ page }) => {
        ...
        const hasKillEl = await page.evaluate(() => !!document.querySelector('[id*="kill"], [class*="kill"], [id*="foe"]'));
    ...
    test('reward orb element can be created (SceneManager API)', async ({ page }) => {
        ...
        const hasSM = await page.evaluate(() => {
            ...
            return typeof window.__game?.getCurrentFloor === 'function';
        });
    ...
    test('teleport to basement and back without JS errors', async ({ page }) => {
        ...
        await startGame(page);
        await page.waitForTimeout(600);
        await page.evaluate(() => window.__game.onTeleportRoom?.('tower_floor_b1_chamber'));
        await page.waitForTimeout(800);
        await page.evaluate(() => window.__game.onTeleportRoom?.('tower_floor_0_chamber'));
        await page.waitForTimeout(800);
    ...
    test('player position changes within a room after movement input', async ({ page }) => {
        ...
        const dist = Math.sqrt(Math.pow(after.x - before.x, 2) +
    ...
    test('no errors on floor 9 (boss floor)', async ({ page }) => {
        ...
        await startGame(page);
        await page.waitForTimeout(600);
        await page.evaluate(() => window.__game.onTeleportRoom?.('tower_floor_9_chamber'));
