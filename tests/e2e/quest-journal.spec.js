...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/quest-journal-${name}.png` });
// ── Tests ─────────────────────────────────────────────────────────────────────
test.describe('Quest Journal', () => {
    ...
    test('quest log (Q key) still works alongside journal', async ({ page }) => {
        ...
        const panelExists = await page.evaluate(() => !!document.querySelector('[id*="ql"], [class*="quest-log"], [id*="quest"]'));
