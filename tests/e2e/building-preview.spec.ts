...
async function openBuildingModal(page: Page) {
  ...
  for (const point of points) {
    ...
    if (await page.locator('text=🎮 Play in 3D').first().isVisible()) return;
  }
...
test.describe('Building Preview (studio → isolated building-viewer handoff)', () => {
  test('Play in 3D opens building-viewer.html and stores a valid preview plan', async ({ page, context }) => {
    ...
    await popup.waitForFunction(() => !!(window as any).__buildingViewerReady || !!(window as any).__buildingViewerError, { timeout: 30_000 });
    ...
    const parsed = JSON.parse(storedPlan as string) as {
    ...
    const viewerError = await popup.evaluate(() => (window as any).__buildingViewerError);
  ...
  test('viewer popup reaches ready state for the handed-off plan', async ({ page, context }) => {
    ...
    await popup.waitForFunction(() => !!(window as any).__buildingViewerReady || !!(window as any).__buildingViewerError, {
      timeout: 30_000,
    });

    const ready = await popup.evaluate(() => !!(window as any).__buildingViewerReady);
    const roomId = await popup.evaluate(() => (window as any).__bvRoomId ?? null);
  ...
  test('viewer popup stays isolated from tower runtime logs', async ({ page, context }) => {
    ...
    expect(
      towerLogs,
      `Tower runtime logs must not appear in isolated building viewer:\n${towerLogs.join('\n')}\n\nFull console:\n${console_.all.join('\n')}`,
    ).toHaveLength(0);
