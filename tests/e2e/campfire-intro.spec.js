...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/campfire-${name}.png` });
...
async function openCharCreation(page) {
    await page.evaluate(() => window.__game.openCharCreation(0));
...
test.describe('Campfire Intro / Character Creation', () => {
    ...
    test('overlay has character grid', async ({ page }) => {
        await loadPage(page);
        await openCharCreation(page);
        const hasGrid = await page.evaluate(() => !!document.querySelector('.acb-grid, [class*="char-grid"], [class*="acb"]'));
    ...
    test('getCharCreationState returns valid structure', async ({ page }) => {
        await loadPage(page);
        await openCharCreation(page);
        const state = await page.evaluate(() => window.__game.getCharCreationState?.());
    ...
    test('boon cards are visible in char creation', async ({ page }) => {
        ...
        const boonCount = await page.evaluate(() => document.querySelectorAll('[data-boon], [class*="boon"]').length);
    ...
    test('name input field is present', async ({ page }) => {
        await loadPage(page);
        await openCharCreation(page);
        const hasName = await page.evaluate(() => !!document.querySelector('.ccv2-name-input, input[placeholder*="name"], input[type="text"]'));
    ...
    test('game starts correctly with deterministic seed', async ({ page }) => {
        const errors = captureErrors(page);
        await loadPage(page);
        await page.evaluate((s) => window.__game.startGame(s), 0xCAFE_BABE);
        await page.waitForTimeout(800);
