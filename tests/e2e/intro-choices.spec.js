...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/intro-${name}.png`, fullPage: false });
// ── log capture ───────────────────────────────────────────────────────────────
...
async function waitGameStart(page) {
...
test('princess animations: idle → walk → run → jump → idle', async ({ page }) => {
    ...
    const getState = () => page.evaluate(() => window.__game.getPrincessAnimState?.() ?? 'unknown');
    // 1. Idle at start
    ...
    if (!seenJump)
        console.warn('[test] jump_idle was NOT seen — physics may suppress short jumps in headless');
