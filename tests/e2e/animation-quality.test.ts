...
const ARCHETYPES = ['biped', 'quadruped', 'avian', 'serpent', 'amoeba'] as const;
    ...
    for (const state of STATES) {
      test(`${arch} — ${state}`, async ({ page }) => {
        ...
        for (let f = 0; f < FRAME_TIMES.length; f++) {
          ...
          await page.evaluate((time: number) => (window as any).__lab.freezeAt(time), t);
...
test.describe('Serpent — full orbit at walk', () => {
  test('12 angles, walk state', async ({ page }) => {
    await openLab(page);
    await page.evaluate(() => (window as any).__lab.showCreature('serpent'));
    await page.evaluate(() => (window as any).__lab.setAnimState('walk'));
    await page.evaluate(() => (window as any).__lab.freezeAt(0.5)); // mid-stride
...
test.describe('All archetypes — mid-stride comparison', () => {
  for (const state of STATES) {
    test(`all — ${state} mid-cycle (iso)`, async ({ page }) => {
      ...
      await page.evaluate((s: string) => (window as any).__lab.setAnimState(s), state);
      await page.evaluate(() => (window as any).__lab.setAngle(45));
      await page.evaluate(() => (window as any).__lab.freezeAt(0.4));
