...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/proc-${name}.png`, fullPage: false });
...
test('PROC-B1: spawn NPCs of all species', async ({ page }) => {
    ...
    for (const { species, role } of SPECIES_ROLES) {
        await page.evaluate(([s, r]) => window.__game.spawnTestNpc(s, r), [species, role]);
...
test('PROC-B2: spawn enemies tier 1/2/3', async ({ page }) => {
    ...
    for (const { species, role, tier } of ENEMY_VARIANTS) {
        await page.evaluate(([s, r, t]) => window.__game.spawnTestEnemy(s, r, t), [species, role, tier]);
...
test('PROC showcase: props + NPCs + enemies + buildings in one scene', async ({ page }) => {
    ...
    await startGame(page);
    // Spawn 2 NPCs
    await page.evaluate(() => window.__game.spawnTestNpc('elf', 'scholar'));
    await page.evaluate(() => window.__game.spawnTestNpc('vulperia', 'merchant'));
    // Spawn 2 enemies
    await page.evaluate(() => window.__game.spawnTestEnemy('undead', 'caster', 2));
    await page.evaluate(() => window.__game.spawnTestEnemy('draconic', 'melee', 3));
    // Spawn buildings of each style
    await page.evaluate(() => window.__game.spawnTestBuilding('house', 'thatched', 'small'));
    await page.evaluate(() => window.__game.spawnTestBuilding('shop', 'stone', 'medium'));
    await page.evaluate(() => window.__game.spawnTestBuilding('ruin', 'timber', 'small'));
