...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/exit-${name}.png`, fullPage: false });
...
async function startGameQuick(page) {
...
test('exit blocked without master key', async ({ page }) => {
    ...
    const hasKey = await page.evaluate(() => window.__game.hasMasterKey?.());
...
test('exit with master key → overworld loads without freeze', async ({ page }) => {
    ...
    const modeChanged = await page.waitForFunction(() => window.__game?.getGameMode?.() === 'exterior', { timeout: 30_000 }).then(() => true).catch(() => false);
    await SS(page, '12-after-exit');
    // Print all logs around the transition
    const exitLogs = logs.filter(l => l.includes('switchToExterior') || l.includes('onExitTrigger') ||
...
test('basement → floor 0 → exit to exterior', async ({ page }) => {
    ...
    await page.evaluate(() => window.__game.switchToInterior?.('floor_-1'));
    await page.waitForTimeout(2000);
    await SS(page, '21-basement');
    const basementFloor = await page.evaluate(() => window.__game.getCurrentFloor?.());
