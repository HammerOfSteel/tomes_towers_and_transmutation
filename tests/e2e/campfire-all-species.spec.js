...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/species-${name}.png`, fullPage: false });
// ── Page log capture ──────────────────────────────────────────────────────────
...
// ── Campfire helpers ──────────────────────────────────────────────────────────
async function startCampfire(page) {
    await page.evaluate(() => window.__game.triggerNewGame(99));
    await page.waitForFunction(() => !!window.__campfire, { timeout: 60_000 });
}
async function waitChoices(page, count, label) {
    await page.waitForFunction((n) => { const c = window.__campfire; return c && !c.speaking && c.choiceCount >= n; }, count, { timeout: 30_000 }).catch(() => {
        throw new Error(`Timed out waiting for ${count} choices at "${label}" — campfire may have crashed`);
    });
    await page.waitForTimeout(150);
}
async function choose(page, idx) {
    await page.evaluate((i) => window.__campfire?.choose(i), idx);
}
...
for (const sp of SPECIES_PATHS) {
    test(`campfire → ${sp.name}: completes + correct blueprint`, async ({ page }) => {
        ...
        if (crashed)
            throw new Error(`Campfire crashed for ${sp.name}: ${crashed}`);
        ...
        const info = await page.evaluate(() => window.__game?.getPrincessInfo?.());
...
test('campfire → princess (idx 7): gallery opens, no crash', async ({ page }) => {
    ...
    await page.waitForFunction(() => !!window.__campfire?.speaking || !!window.__campfire, { timeout: 10_000 }).catch(() => { }); // wizard may finish speaking fast
    await page.waitForTimeout(2000);
    await SS(page, 'princess-02-wizard-reacts');
    await page.waitForFunction(() => !!window.__campfire === false || !!document.getElementById('plp-root'), { timeout: 20_000 });
    ...
    const campfireGone = await page.evaluate(() => !window.__campfire);
