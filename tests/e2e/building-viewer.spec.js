...
const SS = (page, name) => page.screenshot({ path: `tests/e2e/screenshots/bv-${name}.png` });
// ── Helper: generate a valid plan JSON via the main game API ─────────────────
async function makePlanJson(kind = 'inn', faction = 'human_town', seed = 42) {
    // Use a throwaway page to call the game API
    const tmpPage = await test.info()._project?.use?.browser?.newPage?.() ?? null;
        ...
        rooms: {
            'inn_f0_r0': {
                ...
                tiles: [
                    ...
                    ...Array.from({ length: 10 }, (_, x) => ({ x, z: 0, type: 'wall' })),
                    ...Array.from({ length: 10 }, (_, x) => ({ x, z: 7, type: 'wall' })),
                    // Side walls
                    ...Array.from({ length: 6 }, (_, z) => ({ x: 0, z: z + 1, type: 'wall' })),
                    ...Array.from({ length: 6 }, (_, z) => ({ x: 9, z: z + 1, type: 'wall' })),
                ],
                doors: [{ x: 5, z: 7, facing: 'south', targetId: null }],
                staircases: [], spawns: [], interactables: [],
                floor: 0, floorType: 'wood',
            },
        },
...
test('page with valid plan in localStorage loads canvas without JS errors', async ({ page }) => {
    ...
    await page.waitForFunction(() => !!window.__buildingViewerReady || !!window.__buildingViewerError, { timeout: 30_000 });
    ...
    const statusDisplay = await page.locator('#bv-status').evaluate((el) => el.style.display);
    ...
    expect(viewerError, `BuildingViewer fatal error: ${viewerError}\n\nConsole:\n${console_.all.join('\n')}`).toBeUndefined();
    ...
    expect(codeErrors, `Code errors in building viewer:\n${console_.all.join('\n')}`).toHaveLength(0);
});
test('page without key shows "No plan loaded" message and does not crash', async ({ page }) => {
    ...
    const ready = await page.evaluate(() => !!window.__buildingViewerReady);
...
test('no tower system logs appear (zero coupling to main game)', async ({ page }) => {
    ...
    const towerLogs = console_.all.filter(l => l.includes('[PropPlacer]') ||
        l.includes('[StoryRunner]') ||
        l.includes('[tower]') ||
        l.includes('You wake in a tower') ||
        l.includes('[autoSave]') ||
        l.includes('[startGame]'));
    expect(towerLogs, `Tower system logs must not appear in isolated viewer:\n${towerLogs.join('\n')}\n\nFull console:\n${console_.all.join('\n')}`).toHaveLength(0);
