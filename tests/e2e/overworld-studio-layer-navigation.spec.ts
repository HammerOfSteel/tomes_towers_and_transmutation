...
test('realm view switches trigger transition flash instrumentation', async ({ page }) => {
  ...
  const before = await page.evaluate(() => (window as any).__owStudioTransitionCount ?? 0);

  await page.click('#realm-view-pills [data-view="planet"]');
  await page.waitForFunction(
    (prev) => ((window as any).__owStudioTransitionCount ?? 0) > prev,
    before,
  );
  ...
  const state = await page.evaluate(() => ({
    count: (window as any).__owStudioTransitionCount ?? 0,
    label: (window as any).__owStudioLastTransitionLabel ?? null,
  }));
...
test('realm drill-down triggers transition flash instrumentation', async ({ page }) => {
  ...
  const clickPoint = await page.evaluate(() => {
    ...
    const data = (window as any).__owStudioCurrentRealmData ?? null;
    if (!canvas || !data?.settlements?.length) return null;
