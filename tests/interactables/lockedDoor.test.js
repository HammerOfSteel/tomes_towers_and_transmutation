...
// ── Minimal stubs ─────────────────────────────────────────────────────────────
const makeBookReader = () => ({ isOpen: false, open: vi.fn(), close: vi.fn() });
const makeProgression = () => ({});
function makeDoor(id = 'door_exit', requiredKey = 'master_key', x = 0, z = 0) {
    return {
        id,
        type: 'locked_door',
...
describe('InteractableSystem — locked_door', () => {
    ...
    it('does not fire when door content is absent (defaults to master_key)', () => {
        ...
        const door = { id: 'door_nokey', type: 'locked_door',
            position: new THREE.Vector3(0, 0, 0), label: 'Old Door' };
        const cb = vi.fn();
        sys.onLockedDoor = cb;
